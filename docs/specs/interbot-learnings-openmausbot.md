...
(`internal/team`). Written to inform improvements to hivebot's interbot
experience. Nothing here is implemented yet.

## Where the two products stand today

| Axis | OpenMausBot | hivebot |
|---|---|---|
...
hivebot gap: a tag-wake between bots is visible only if you are looking
...
hivebot gap: one mechanism (tagged message) for both intents, and the
...
hivebot gap: bot-originated tags auto-promote like human ones
...
hivebot gap: per-slug queues handle the mechanics, but the channel shows
...
hivebot gap: CoS fans out work but nothing bounds an objective, and
...
hivebot gap: per-slug turn queues live in the Launcher process; a queued
...
by bot and thread). hivebot already behaves mostly this way; the value is
...
The current promo depicts hivebot's real model (open assignments and
