# hivebot-as-desktop-app via Pake — feasibility spike
...
- **Phase 0 (de-risk, now):** minimal Wails v2 window in `desktop/oswails/` that
  boots the broker in-process and displays the hivebot UI. Validate SSE +
...
- **(c) Phase 0 productionized** (verified on macOS): dynamic free **UI** port
  ...
  7973); **OS app-data runtime home** (`os.UserConfigDir()/hivebot/desktop`, not
...
- **(R) Robust dogfood — shared office + attach** (verified on macOS): the
  desktop shares the user's **active workspace** (no `HIVEX_RUNTIME_HOME`
  ...
  attaches to a live office (no second broker), and `hivex web` (CLI) opens the
  ...
  attach; `go build ./...` / vet / `internal/team` + `cmd/hivex` suites green.
  - **Triangulation review done (2026-06-14)** — 4 orthogonal lenses (security,
    ...
    - **Loopback-only** attach URL + **hivebot-identity probe** (GET `/` must be
      200 *and* contain a hivebot marker; redirects not followed) — a planted/
...
Yes — Pake can wrap hivebot into a native desktop app, and a working
...
`hivex web` already running*, not a double-clickable product.

Turning that into a real shippable app means adding a **process sidecar**
(spawn the `hivex` binary, wait for the port, load it) — which requires
...
**Phase 0a — Wails WKWebView renders hivebot over loopback (separate broker):**
...
- **Cross-platform: Linux WebKitGTK PROVEN (2026-06-14)** — the weak-link
  webview renders hivebot's full SPA correctly (real xvfb screenshot:
...
## What hivebot actually is (the constraint)

- Shipping hivebot is a **single Go binary** (`cmd/hivex`, 73 MB) that embeds
  `web/dist` (`embed.go`) and serves the React UI **and** the API over a
  loopback broker. `hivex web` → UI on `127.0.0.1:7891`; the SPA talks to it
  over loopback HTTP/SSE/WebSocket.
- So "desktop hivebot" is not "ship a static site" — the app is useless without
...
| `pake http://localhost:7951 --name hivebot --width 1400 --height 900` builds | ✅ Tauri release build, `2m28s` compile |
| Native macOS app produced | ✅ `hivebot.app`, **9.4 MB** (shell binary 9.2 MB) |
| Native window opens with real chrome | ✅ traffic-lights + 1400×866 window (`screenshot-native-shell.png`) |
| Webview loads hivebot from the broker | ✅ **`com.apple.WebKit.Networking.xpc` held an ESTABLISHED socket to `127.0.0.1:7951`** |
...
Repro: `hivex --web-port 7951 --no-open` (throwaway `HIVEX_RUNTIME_HOME`),
then `pake http://localhost:7951 --name hivebot …`, then `open hivebot.app`.
...
- **Plain `pake` app** = opens a window at a URL. If the URL is
  ...
  *separately* ran `hivex web`. Useless as a product, fine as a demo.
- A real app must spawn `hivex` itself.
...
1. Bundle `hivex` as a Tauri `externalBin` (named with the target triple,
   e.g. `hivex-aarch64-apple-darwin`).
2. On `setup()`, pick a free port, spawn
   `hivex web --web-port <port> --no-open` (set `HIVEX_RUNTIME_HOME` under
...
Electron shell forks a *TypeScript* `@hivex/broker` (a rewrite in progress),
...
|---|---|---|---|
| `apps/desktop/` (Electron 42) | Built shell: sandbox, contextIsolation, IPC allowlist, broker supervisor, ~25 tests, security-model docs | a **new TS** `@hivex/broker` (rewrite) | ~150–250 MB |
...
1. **Ship-it-fast demo:** Path A is genuinely useful *today* for a founder demo
   / "desktop hivebot exists" moment. ~10 min, no code. Keep it in this worktree.
...
- `hivebot.app` — the built 9.4 MB Pake app (hardcoded to `localhost:7951`).
- `screenshot-native-shell.png` — the native window chrome.
- `screenshot-wrapped-ui.png` — the hivebot UI the window wraps.
