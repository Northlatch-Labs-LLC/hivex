# Real "Demo workflow to Hive" call (operator S5/S6)
...
Hive watches the screen and listens, asks questions, and at the end **drafts (or
...
- **Vision**: frames are sampled from the screen-share `MediaStream` every few
  seconds and pushed as `input_image` conversation items, so Hive sees the screen
  while it talks. (Phase B swaps this for cua-driver screenshots + a11y tree.)
- **Handoff**: Hive is given one tool, `draft_workflow`, whose arguments map onto
...
`HIVEX_OPENAI_API_KEY` env > `OPENAI_API_KEY` env > config file
...
2. hivex-hosted voice (metered proxy) vs BYOK only — BYOK first; hosted is a
