...
This is the source of truth for hivebot's product analytics. It records what we
track, what we deliberately do **not** track, how consent works, and the
privacy guarantees we make to operators and end users.

## Goal

Understand how the **human operator** uses hivebot so we can drive product
...
- **Operator control.** A self-hosted operator can point at their own PostHog
  (`HIVEX_POSTHOG_KEY` / `HIVEX_POSTHOG_HOST`) or disable either channel at
...
- **Key + flag injection (self-hosted aware).** Build-time
  ...
  Hive-shipped builds. The loopback `/api-token` endpoint is extended to also
...
- [x] **P4 Dashboards** — seeded 439 tagged example events (`seed=true`, RevOps
      ...
      then built the **hivebot — Product Health** dashboard (id 1708570, pinned):
