...
> Scope: open-source self-hosted hivebot. Designed as a **shared kernel** the Hive
...
Cursor/Claude session, a vendor bot — participate fully in a hivebot office with
...
hivebot's compounding loop works because, when a hivebot-hosted bot spawns,
...
// a singleton. Hive cloud: one per tenant. The packer NEVER touches global state
...
const (
    ...
    BotHosted                    // hivebot-hosted agent (also gets push-side injection)
)
...
- Multi-tenant auth, network listener, credential issuance — that is **Hive cloud**.
