# hivex Operator — Prototype Brief

**This is the single, self-contained spec for building the hivex operator
...
**hivex** lets a non-technical operator build deterministic internal tools by
...
- The **app/brand is `hivex`** (lowercase). The sidebar shows the real hivex
  pixel-art logo (a pink/purple mark) plus the wordmark `hivex` and the eyebrow
  `OPERATOR`.
- The **in-product AI is named `Hive`.** Chat is "talking to Hive." The primary
  teach-by-voice call CTA reads **"Teach your workflow to Hive"** (this replaced
...
1. **Chats** — talk to Hive. Tune existing tools, or start a new one. Entry to the
...
5. **Settings** — workspace basics plus voice: bring your own OpenAI Realtime key
   or let hivex host voice. No key means the call is optional and chat authoring
...
which assembles itself step-by-step as Hive understands the description.** This is
...
1. **Enter the builder** from any "Build a tool" / "Describe a new tool" CTA
   ...
   placeholder ("Your workflow takes shape here"). Hive opens with: *"Tell me what
...
3. **Hive "thinks"** (a brief typing indicator), then narrates a reflect-back
...
5. **Hive asks exactly ONE sharp clarifying question** (the fit threshold, or
...
6. **The operator answers, and the matching step updates in place** (the decision
   node flashes and becomes, for example, `Is the score ≥ 70?`). Hive confirms.
7. **Hand-off:** Hive presents a draft-tool card in the conversation: the proposed
...
- **Edit with AI** opens a right-docked chat beside the tool (the tool stays
  visible). The operator describes a change, Hive shows a concrete proposed-change
...
- **Dark, always.** Use **shadcn `hive-dark` tokens** consumed at the shell root
...
- **Brand mark:** the real hivex pixel-art logo, rendered crisp
...
- React 19 + Vite + TypeScript, Bun for tooling, Tailwind v4 + shadcn
  (`hive-dark`). State is local component state for the prototype (no store
...
5. Settings shows the voice BYOK / hivex-hosted choice.
...
- **A new repo is authorized** if it is the cleaner start. This new product
  eventually replaces hivex entirely.
