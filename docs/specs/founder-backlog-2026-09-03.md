...
`~/.hivex-botrename`, based on `origin/main`).
...
- on-disk state: `~/.hivex/agents/<slug>/…`
...
- on-disk state `~/.hivex/agents/<slug>/`
- env var `HIVEX_AGENT_SLUG`, header `User-Agent`
...
`.claude/agents/`, `CLAUDE.md`, and the `hivex-agent` pi-mono build engine.
...
bar showed `running mcp__hivex-office…`.
...
**Root cause:** `hive-shell.css` and `hive-dark.css` both
`@import url("./hive.css")` as their base, and `hive.css` scopes its sidebar
rules with `[data-theme^="hive"]` — a **prefix** match, so those rules also
apply under `hive-shell` and `hive-dark`.
...
comment stated: "invisible on the modal's white card". Under `hive-shell` the
...
| Theme | Before | After |
|---|---|---|
| hive-shell | 1.26:1 | **12.00:1** |
| hive-dark | 1.26:1 | **15.60:1** |
| hive (light) | 14.57:1 | 14.57:1 (unchanged) |
...
2. `claude` spawns `hivex mcp-team` over **stdio** — a second full binary boot
...
2. **Open.** With that fixed, v0.237.1 reached `npm publish` and got:
   ...
   ```
   npm error code E404
   npm error 404 Not Found - PUT https://registry.npmjs.org/hivebot
   npm error 404  The requested resource 'hivebot@0.237.1' could not be found
                  or you do not have permission to access it.
   ```

   The package exists and `npm owner ls hivebot` is
   `northlatch-mohammad`, so this is not a missing package — npm returns 404
...
**What unblocks it** (npmjs.com → the `hivebot` package → Settings → Trusted
Publisher → GitHub Actions):

| field | value |
|---|---|
| Repository owner | `northlatch-mohammad` |
| Repository | `hivebot` |
...
published, not draft), npm ❌ (still 0.236.2). So `npx hivebot` installs
...
npm error code E403
npm error 403 Forbidden - PUT https://registry.npmjs.org/hivebot
...
| claim | value |
|---|---|
| repository | `northlatch-mohammad/hivebot` |
...
2. **Repository.** If it still names `Northlatch-Labs-LLC/hivex`, it is the pre-move path.
   The repo is `northlatch-mohammad/hivebot` now.
