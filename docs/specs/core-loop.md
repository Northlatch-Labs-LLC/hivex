...
4. **Spin up the team** — assign owner bot(s); create subtasks where needed.
5. **Bots execute with focused context** — bots continuously write to their **private knowledge graph** and use it to generate personal **notebooks**; notebooks hold pre-task research and post-task deliverables/learnings. Goal: stay focused without loading unnecessary context for everyone.
...
8. **Activate Skills & Policies for relevant bots** — auto-assigned to all relevant bots at skill/policy creation and during automatic bot creation by the CEO; the CEO can also assign a skill/policy to another bot during chat; assigned skills & policies are **always loaded** in that bot's system context.
...
**Memory tiers implied by step 5 + 7:** private per-bot knowledge graph → generates that bot's notebooks (personal, focused); team knowledge graph → generates the wiki (shared, curated). Both written deterministically by hooks, not ad-hoc LLM whim.
...
| R5 | **Bot skill proposals & request_skill_enable** | team_skill_create action=propose, skill-proposal interviews (the dead-Accept surface), skill nudges. Skills come ONLY from playbook compilation (B3). Keep + harden the SkillOpt mechanics (protected invariants, slow-update markers, bloat gates, consolidation/dedup) as the compiler's quality layer. |
...
| # | Decision | Why |
|---|---|---|
| C1 | Redactor removal preserves the PR-#684 approval-card sanitizer if separable | Different threat: confused-deputy via bot-controlled strings in approval payloads, not secret-masking |
