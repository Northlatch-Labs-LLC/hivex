...
> - **New repo authorized** if cleaner; this product eventually replaces hivex.
...
- Shows: the fit-score → route table (hero), the deterministic workflow + run +
  ...
  (primary activation), the **voice BYOK / Hive-hosted** settings (CEO #5).
...
- **Brand + copy locked:** real hivex pixel logo (`/favicon.svg`) in the
  sidebar; the in-product AI is named **Hive**; the call CTA reads **"Teach your
  workflow to Hive"** (was "Build on a call"). App = hivex, AI = Hive.
