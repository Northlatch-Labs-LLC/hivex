...
multiple chat sessions in our bot. Disable and Publish new version flows will
only belong to each workflow, not to the whole bot."

## The model
- A **routine** = `{ id, name, prompt, schedule, enabled, version, lastRun }`.
  It is nothing more than a prompt the bot runs in its own chat on a schedule.
  The chat already knows the bot's tools (create_tool / tool calls), so a
  routine's run IS a chat turn: the prompt goes in, the bot calls its tools,
  the outcome lands as messages (and artifacts).
- **Multiple chat sessions per bot.** Each routine run opens (or continues)
  its own session; the operator can browse every session and start new manual
  ones. The Ask Bot dock gains a session list.
- **Per-routine lifecycle.** `Disable` (pause the schedule) and `Publish new
  ...
  bot itself no longer has Disable/Publish in its header.
...
1. **FE mock (this slice).** The Workflow tab becomes **Routines**: a list of
   ...
   composer (prompt + schedule). The Ask Bot dock becomes session-aware:
   a sessions rail (routine sessions + manual sessions + New chat); each
   session holds its own transcript. Bot-header Disable / Publish buttons are
   removed. Mock state lives in a per-bot `routinesContext` +
   `sessionsContext` seam.
2. **Backend (DONE — reworked to NO custom engine, founder direction
   2026-07-01: "don't do custom; use what pi bot already provides").**
   ...
   (`scheduler_operator_routines.go`) and POSTs `HIVEX_AGENT_URL`
   `/routines/run {agent, slug, name, prompt}`; the bot service runs the
   ...
   fires within a tick). The bot service stores NO routine definitions and
