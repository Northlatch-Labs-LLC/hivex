...
Course-correction after live review. The v1 AUGMENT scope under-delivered: it kept hivebot's
...
- Gate: while `onboardingComplete === true` AND the office tour has not been seen
  (`localStorage: hivex.office-tour-done`), render `<OfficeTour>` as the full surface (no Shell
...
## B. Verified, guided provider picker (port the reference gem in hivebot style)
...
hivebot already has the backend primitives in `internal/onboarding/prereqs.go` (`CheckOne`,
...
- **Frontend:** uplift `PrePickScreen` so a selected runtime expands into the guided setup + a
  Verify button + classified result with a hint, in hivebot tokens/voice/themes. Keep the existing
  detection + copy-sign-in; add the guide + verify loop on top.

Everything in hivebot style: design tokens only, three themes, no em dashes, no contractions, Office
voice, "hivebot is not a CRM" framing intact. The RevOps reskin (steward/analyst, CRM hygiene) stays.

---

# Office Onboarding Uplift — bring the best of the reference onboarding into hivebot
...
Keep hivebot's CEO-chat phase wizard as the entire setup act, and add the three pieces hivebot is
genuinely missing:
...
**Rationale.** hivebot's wizard-as-conversation-with-the-CEO is load-bearing product identity, not
...
reference product's multi-step form wizard is strictly worse than this for hivebot's "tell, do not
...
| Reference surface | hivebot target | Action | Notes |
|---|---|---|---|
| Intro step (dictionary cold-open) | `OfficeTour` slide 1 | Adapt | Retell as "what an office is" in hivebot voice, serif `--font-logo` headline. |
| Welcome / "what is this" / room setup steps | CEO greet + blueprint + team phases | Drop | Already covered, far better, by the CEO chat. |
| Connect AI step (provider + verify) | `PrePickScreen` (exists) | Drop | hivebot runtime picker + copy-sign-in-command (#932) already exceeds this. |
...
| **Tour mockup-sidebar** | `TourMockupSidebar` | **Net-new** | Mock of hivebot's real sidebar (channels `#` + bots `@`), completion ticks. |
...
| **Seeded six-page getting-started wiki** | `team/getting-started/*.md` seeded at office seed | **Net-new content** | Authored fresh in hivebot voice. See section 5. |
...
| feedback-popup, data-dir-prompt, i18n/RTL, CLI verify, reference sidecar metadata | none | Drop | Not hivebot concerns; English-only; already covered; reference-specific. |

## 3. Net-new hivebot components
...
| `tour/useOfficeTour.ts` | Auto-open once per browser via `localStorage: hivex.office-tour-done`, `requestShowOfficeTour()` replay event, `markDone()`. Guards `typeof window`. | n/a |
| `tour/TourMockupSidebar.tsx` | Decorative mock of hivebot's real sidebar: workspace label + `#channel`/`@agent` rows + green completion ticks. | `--preview-row-*`, `--green`/`--green-bg`, `--font-mono`, `PixelAvatar` |
...
## 4. The guided tour, in hivebot style
...
the "you have left setup and entered your office" boundary hivebot deliberately draws
...
**Mock sidebar** (`TourMockupSidebar`): a faithful mock of hivebot's real left sidebar — workspace
...
optionally prefills an issue intent, and marks `hivex.office-tour-done`.

## 5. Seeded getting-started content

hivebot already seeds `team/about/{README,company,owner}.md` at the office seed boundary
(`company_seed.go`). Add a parallel `team/getting-started/` tree, authored fresh in hivebot voice,
...
| Reference topic | hivebot page (`team/getting-started/*.md`) | Teaches |
|---|---|---|
...
| rooms | `channels.md` | hivebot channels and DMs as the coordination surface. |
...
| Item github_star | "Star hivebot on GitHub (Michael would be proud, probably)" |
...
12. `resources/getting-started/*.md` — author the hivebot pages from section 5.
...
All three must pass and every visual component must render in `hive`, `hive-dark`, and `noir-gold`.
