...
> We have product-market signal. This document sharpens hivebot to the narrowest wedge
...
- **Context AND Knowledge backend → [gbrain](https://github.com/garrytan/gbrain), for it all.**
  ...
  `broker_entity_graph`/`facts`, and the Hive graph — and the karpathy-wiki branch built its own
...
9. **Activation + voice economics (CEO review).** The **call is the primary activation + ship
    ...
    Hive-cloud hosted/metered**; **no key → the call is optional and chat-authoring is the keyless
...
10. **Knowledge = a 5th surface.** **gbrain owns the entire Knowledge backend** (sources, compile,
   ...
   many workflows). gbrain's federation/OAuth-scoping is team-scale (Hive cloud), not needed for the
   single-operator MLP.
...
- Multi-tenant hosting (that's Hive cloud, not this OSS repo).
