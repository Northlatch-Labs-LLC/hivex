...
`wiki_index_bleve/sqlite`, `broker_entity_graph`/`facts`, Hive graph), does gbrain actually
...
- The shipping desktop shell is **Electron** (`apps/desktop`, `@hivex/desktop`, Electron 42 +
  electron-vite) — the "hivebot v1 desktop shell," an OS-level security boundary spawning the
...
4. gbrain's full federation/OAuth-scoping is **team-scale** (Hive cloud); the single-operator MLP uses
