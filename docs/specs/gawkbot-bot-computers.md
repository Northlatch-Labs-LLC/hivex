...
hivebot spawns `claude` and `codex` CLIs, so the bot must stay on the host and the
...
3. **Dev, runs hivebot headless on a Hetzner box.** Sets the SSH alias in
...
```
container name   hivebot-computer-<sha16>
workspace dir    $HIVEX_HOME/computers/<sha16>   (bind-mounted at /home/cua/workspace)
...
"computer": { "command": "docker", "args": ["exec", "-i", "<name>",
              "cua-driver", "mcp", "--socket", "/run/user/1000/hivebot-cua.sock"] }
```
