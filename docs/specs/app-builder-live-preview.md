...
App Builder edits src/  ──HMR──▶  bun run dev (:Pdev)
                                       │
runtime-home/.hivex/apps/<id>/src/     │  scrape "Local: http://localhost:Pdev"
                                       ▼
...
5. **Select-to-edit** + **iframe error capture** — SHIPPED (PR #1106). A dev-only
   Babel plugin stamps `data-hivex-source="file:line:col"` on host JSX (React 19
   ...
   click, and posts a display-only `hivex-select`/`hivex-error` over the existing
