...
Hive **writes a custom Tool** (a scripted function tailored to that workflow) that
...
1. **Score + route a lead.** Maya: "When a new lead comes in, score its fit and
   route hot ones to the right AE." → Hive writes `scoreAndRouteLead(lead)` →
...
2. **Weekly pipeline summary.** Maya: "Every Monday summarize last week's
   pipeline." → Hive writes `weeklyPipelineSummary()` → Tools. "run it" → the bot
   calls it → returns a glanceable summary.
3. **Draft a follow-up.** Maya: "Draft a follow-up email for a stalled deal." →
   Hive writes `draftFollowup(deal)` → Tools. "draft one for the Globex deal" → the
...
bot's `/tools/build` (vite proxy `/agent` → :8820, `HIVEX_AGENT_PORT`); the
...
`hive.ai.*` via `TOOL_RUNTIME_MODEL=1`, broker `integrations.call` +
`hive.browser` via `HIVEX_BROKER_URL`/`HIVEX_BROKER_TOKEN` — and deterministic
simulations otherwise, with every capability call recorded as an action trace.
**Send-gate:** gated capabilities (`crm.assign`, `hive.send`) default-deny —
...
integrations/browser via the broker, real `hive.ai.*` via a model, simulated
