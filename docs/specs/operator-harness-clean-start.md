# Operator Harness — Clean Start (deepbots-native)

**Status:** plan, pre-scaffold. **Decision date:** 2026-06-27 (founder picked the
clean deepbots-native rewrite). **Supersedes:** most of
`deepagents-migration-plan.md` (the LangGraph multi-bot stack) — salvage the seam only.
...
The backend is **one fast deepbots-native bot harness** that turns a conversation
(or a captured screen session) into a **compiled, deterministic workflow spec**, plus a
**deterministic executor** that runs that spec. No multi-bot office, no Go broker, no
channels/lifecycle/coordination.

## 2. Why a clean start

The Go broker existed to coordinate a multi-bot office. The product no longer has an
office — it has **one operator and one build bot**. Keeping the broker (office,
...
that were always about **one bot reaching tools and streaming to a UI**.
...
- **BUILD is agentic** (deepbots): a single deep bot that plans, drives discovery
...
### Why deepbots (LangChain) for BUILD
deepbots gives a prebuilt deep-bot loop on LangGraph: a planning tool, sub-bots, a
...
embeddable TypeScript bot SDK (the stack OpenClaw is built on). Why it won over the
...
| deepbots (LangChain) | ❌ BYOK only | ✅ | Python | demoted to BYOK option |
...
Python `harness/` fallback (CLI / deepbots / stub) has been REMOVED (founder decision,
...
- **Claude Bot SDK harness wiring** incl. the `allowed_tools` fix (wired MCP tools must
...
- Go **broker**, office, channels, 13-state lifecycle, coordination/sub-task kernel,
  multi-bot dispatch, notifier loops, re-hydrate.
...
The 2026-06-20 plan stripped hivex in place (build-queue Slice 3). The 2026-06-27 decision
added "new repo authorized if cleaner." Two real options:

**Option A — in-repo clean rebuild (RECOMMENDED).** Add the deepbots harness as a new
top-level package in hivex, keep the operator FE where it is, and **aggressively delete**
the broker/office (build-queue Slice 3). hivex *becomes* the operator product by
subtraction.
...
**Option B — brand-new repo.** Fresh repo with FE + deepbots harness + the seam, nothing
else.
...
deepbots product fastest and keeps the large, real salvage in place. Reserve Option B
...
- **S0 — Clean-start scaffold.** New `harness/` (Python: FastAPI app + deepbots bot
...
- **S2 — Chat-to-build (the spine).** Swap the FE's keyword `planWorkflow` compiler for the
  real deepbots build bot over `/build/stream`; it assembles a `WorkflowSpec` live and
...
`step` (one event per `WorkflowStep` as the bot assembles the spec) → `spec` (the
...
2. deepbots version pin + whether we wrap it or fork the loop (decide at S2).
