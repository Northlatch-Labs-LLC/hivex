# WIKI-OBSIDIAN-COMPATIBILITY.md — Contract for opening hivebot's wiki as an Obsidian vault

This document is the source of truth for **how hivebot's wiki interoperates with Obsidian**. It is a companion to `WIKI-SCHEMA.md`: that document defines the wiki contract; this one defines the additional invariants required to make the on-disk layout work as an Obsidian vault (read and write).

If a decision below conflicts with code, the contract wins. Fix the code.

This is the Obsidian-side counterpart to the substrate guarantee in `WIKI-SCHEMA.md` §1: "`rm -rf .hivex/index/` → restart broker → the wiki still works." The Obsidian extension is: open the wiki in Obsidian → the vault loads cleanly. Edit a brief in Obsidian → the next reconcile picks it up. Click a wikilink → it resolves to the right brief.
...
1. Point Obsidian at `~/.hivex/wiki/team/` and the vault loads cleanly. No `.jsonl` files in the file tree, no broken-link spam, link graph reflects the entity graph.
...
3. Use Obsidian-native primitives that have semantic meaning for hivebot — `[[wikilinks]]`, `![[image]]` embeds, callouts — and have them survive round-trips through synthesis without stomping user edits.
...
- **Wikilink resolution.** hivebot emits `[[people/nazz]]`. Obsidian's resolver looks for `<vault>/people/nazz.md`. With vault root at `team/`, that path matches `team/people/nazz.md` directly. With vault root at the wiki root, Obsidian would fall back to basename matching and silently mis-resolve collisions.
...
On `Repo.Init` and on every `ensureLayoutLocked`, hivebot idempotently writes a minimal `.obsidian/app.json` at the vault root. It is **committed** to the wiki git repo — a fresh clone gives the user a working configuration without further setup.
...
**Idempotency.** If `.obsidian/app.json` already exists, the bootstrap reads it, merges hivebot's required keys with any user customizations (user values win on conflict for non-required keys; hivebot values win for `useMarkdownLinks`, `newLinkFormat`, `userIgnoreFilters`), and rewrites the file. User customization of other keys (theme, hotkeys, plugins) is preserved.

---

## 5. Wikilink contract — kinded form is canonical

hivebot emits and stores wikilinks in their **kinded form**: `[[kind/slug]]` or `[[kind/slug|Display]]`. The `ExtractRefs` parser (`kindedWikilinkPattern` in `internal/team/entity_graph.go`) accepts `people|companies|customers` as kinds. This form is canonical because:
...
Obsidian-flavored callouts (`> [!note]`, `> [!info]`, `> [!warning]`, etc.) render in hivebot's web wiki surface. The renderer in `web/src/components/wiki/WikiArticle.tsx` recognizes the blockquote-with-bang-tag prefix and emits a styled box matching the design system in `DESIGN-WIKI.md`.
...
- **Dataview** queries — hivebot has its own query layer.
...
- **Bidirectional sync of arbitrary external Obsidian vaults** into hivebot. Importing an external vault is a separate spec.
- **Dark-mode parity** between Obsidian and hivebot's web wiki surface (`DESIGN-WIKI.md` §1.7 — light-only in v1).
