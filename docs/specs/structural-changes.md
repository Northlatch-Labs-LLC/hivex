# hivebot Structural Changes — Running Tracker

> Living doc for a series of deep structural changes to how hivebot works.
...
- **Worktree:** `/Users/admin/Documents/hivebot/.claude/worktrees/structural-changes`
...
- Repo commands (run from worktree root unless noted):
  - Go build: `go build -o hivex ./cmd/hivex`
...
- Model: `teamTask` struct `broker_types.go:183`; spec `IssueDraftSpec` (Goal/
  ...
  monolithic `brokerState` JSON blob at `~/.hivex/team/broker-state.json`
...
- Wiki promotion authority TODAY = CEO (reviewer). Demand pipeline
  ...
  state machine `promotion_state.go` (JSONL at `~/.hivex/wiki/.reviews/reviews.jsonl`).
...
- **D9 — Migration: MIGRATE EXISTING workspaces (for shipped USERS, not dev).**
  Refined 2026-06-02: the dev's own `~/.hivex-dev-home` is disposable (blast
  freely). The concern is real users who upgrade hivebot on their machines — their
  `~/.hivex` must come up clean with no data loss. So Phase 6 migration is
