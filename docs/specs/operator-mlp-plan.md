...
3. **gbrain for it all (Knowledge + context).** Stand up gbrain as an **MCP server** the broker
   ...
   `internal/embedding` + `wiki_index_*` + `broker_entity_graph`/`facts` + Hive graph, AND retire
...
1. **Electron desktop host.** The shipping shell is Electron (`apps/desktop`, `@hivex/desktop`),
...
- **Voice economics (#5):** **BYO OpenAI Realtime key** (Settings) **or Hive-cloud hosted/metered**;
