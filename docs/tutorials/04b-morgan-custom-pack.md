...
teammate run `npx hivex --pack agency-six` and land in the exact same
office.
...
```bash
mkdir -p ~/.hivex/packs/agency-six/agents
mkdir -p ~/.hivex/packs/agency-six/channels
```

### 2. Drop bot JSONs into the pack

Create `~/.hivex/packs/agency-six/agents/producer.json`,
...
Create `~/.hivex/packs/agency-six/channels/general.json` with
...
- The teammate's office has the same channels and the same bot
  roster.
- The bot JSONs match (compare `~/.hivex/agents/*.json`).

## What success looks like

The agency-six pack works on any machine after one `npx hivex --pack`
invocation. No code change. No private dependency. The pack is fully
described by JSON files in `~/.hivex/packs/agency-six/`. Morgan can
