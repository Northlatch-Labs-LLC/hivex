...
These fixtures model the first hivebot workflows where bot output should be HTML first:

1. Notebook exploration: a bot creates a dense, interactive HTML artifact next to its working notes.
2. Wiki promotion: the artifact is promoted into a durable wiki article with provenance.
3. Chat handoff: the bot references the artifact with a terse `visual-artifact:<id>` marker so the UI renders a preview card instead of a plain link.
...
- `source.md`: the notebook note the bot would write while exploring.
- `artifact.html`: the HTML artifact the bot should create from the note.
- `wiki.md`: the markdown summary used when promoting the artifact to the team wiki.
- `chat.txt`: the bot chat message that should become a rich artifact card in chat.

The `scenarios.json` manifest separates intended deployment paths from local fixture files. `sourceMarkdownPath` and `targetWikiPath` describe where the bot-created source note and promoted wiki article should live in a real workspace; `sourcePath`, `wikiPath`, `htmlPath`, and `chatPath` point at fixture files under this tutorial directory for tests and examples.
...
`STYLE.md` defines the hivebot paper-manual artifact style bots should use by
