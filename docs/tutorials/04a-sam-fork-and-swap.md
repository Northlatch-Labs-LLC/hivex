...
hivebot for a day. They are now testing the architectural-fit claim:
...
**Outcome they came for:** edit one JSON file, restart hivex, and see
...
```bash
cp ~/.hivex/agents/eng.json ~/.hivex/agents/research.json
```
...
```bash
rm ~/.hivex/agents/eng.json
```

#### Verify

- `~/.hivex/agents/` now lists CEO, DSG, CMO, Research.

### 4. Restart hivex

In the running CLI, Ctrl+C, then `npx hivex` again.
