...
fresh hivex install built from `origin/main` plus PR #876 (the inbox
...
- Runtime home: `~/.hivex-dev-home/.hivex`.
...
| 2 | Web bundle embedded and serves at `/` | PASS — `<title>hivebot - Slack for AI employees ...</title>`. |
...
- Steps 1, 2 (install + first look): **infra verified above**, looks
  correct as long as `npx hivex` resolves to the same binary that
  embeds the bundle.
...
- Manual gate. Requires the user to edit `~/.hivex/agents/*.json`.

### 04b Morgan custom pack
- Manual gate. Requires a `~/.hivex/packs/<name>/` directory and a
...
1. `npx hivex` (or rebuild this branch + `hivex-dev`).
