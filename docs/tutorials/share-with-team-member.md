# Share hivebot With a Team Member

This tutorial shows the safest way to use hivebot from more than one browser.

There are two different jobs:

- **Access your own hivebot remotely:** you are still the only user. Use the normal hivebot web UI through SSH, LAN, Tailscale, or WireGuard.
- **Invite a team member:** another person needs a scoped browser session. Create the invite from **Access & Health**.

Do not use `hivex share` as your personal remote-access tunnel. It is for team-member sessions.

## Example scenario

Maya runs hivebot on a small office server called `ops-box`. She uses it from her laptop during the day. Later, she wants Tara to join the same office for one planning session.

Maya needs two flows:

- From her own laptop, she opens the host hivebot UI through SSH forwarding.
- For Tara, she creates a one-use team-member invite over Tailscale from the hivebot UI.
...
## Access hivebot yourself

On the server, start hivebot:
...
You are still the host. This is the right path when you are one person accessing your own hivebot from another device.
...
In the hivebot sidebar, open **Access & Health**.
...
hivebot starts a scoped private-network listener and shows a one-use URL like:
...
The browser is redirected to the hivebot web UI in invite-acceptance mode. They see a short join screen and enter the display name teammates should see in messages and office activity.
...
After submitting, hivebot redirects them to:
