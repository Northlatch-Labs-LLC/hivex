...
A hivebot can have its own Linux desktop: a browser, a terminal, files, and a
...
Before every attach hivebot inspects the container and refuses one that
...
- **Sign in to ascii.dev.** hivebot installs the Box CLI (one static binary
  under `~/.ascii/bin`), opens ascii.dev's GitHub, Google, or email sign-in
  in your browser, mints an API key named `hivebot` on your account, checks
  it, and saves it. Nothing to copy.
- **Paste a key.** Create one at box.ascii.dev under API keys (they start
  with `box_`). hivebot checks the key with ascii.dev before saving it.
...
(onboarding, the Computer tab, Settings) hivebot reads the account's limits
...
`hivebot` key on your account, ends the CLI session, and forgets the key.
...
ascii.dev directly; hivebot never sees a bill. Sleep archives the box;
...
of the viewer, not hivebot, and stays folded unless you click it.
...
hivebot's REST proxy, where each action returns the resulting screenshot in
the same call.

## Where things live

```
~/.hivex/computers/<sha16>/     each bot's durable workspace
hivebot-computer-<sha16>        the container name (a digest of the slug)
localhost/hivebot/cua-computer  the shared image
```

Environment overrides: `HIVEX_COMPUTERS_DIR`, `HIVEX_BOX_API_KEY`,
`HIVEX_BOX_API`.
