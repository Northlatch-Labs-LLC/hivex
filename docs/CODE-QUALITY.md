...
**Reference:** PR #501, commit `546a462c`. Extracted ~3,000 LOC of pure rendering / projection logic out of `cmd/hivebot/channel*.go` into a new `cmd/hivebot/channelui/` package. 56 files, none over 600 LOC. The package depends on no network, no clock, no shared state.
...
Fix by pointing at a token that exists in all four themes (`hive`, `hive-dark`, `noir-gold`, `hive-shell` — see `web/src/lib/themes.ts`), not by defining one token per call site. The shared vocabulary is `web/src/styles/global.css :root`, which every theme inherits; theme files override only what differs. `scripts/css-phantom-token-allowlist.txt` holds sites that need a human design call plus the forward-only component baseline, each pinned to an exact occurrence count.
...
- **Theme files were in this state until recently.** `public/themes/` was outside the same commands, which is how a formatting error sat in `hive-shell.css` unnoticed. That one is now in scope.
...
- `cmd/hivebot` must not import `internal/team` internals — only its public types
- `cmd/hivebot/channelui` must not import network code (no `net/http`, no clients)
