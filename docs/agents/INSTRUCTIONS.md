...
These instructions apply to AI coding agents working in Hive repositories. Keep
...
Hive is a context graph platform for AI agents, not a CRM. Do not describe the
product as a CRM in code, comments, docs, or external copy.

Use the available Hive memory/context tools when they are installed:
...
## Hivex Agent Instructions

Use this profile for the Hivex public repo and its worktrees.
...
Hivex is built by a single person. There is no reviewer to wait for, so the
...
- Themes (`Hive Light`, `Hive Dark`, `Noir Gold`) switch from the
