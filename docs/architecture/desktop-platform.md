# hivebot — Deprecate TUI, Build a Cross-Platform Desktop Experience
...
> **moved to the `hive-crm/hive-local` repo** (a sibling to the web hivebot, not a
...
Hivex today is a Go single-binary that serves a React/Vite web UI from `web/dist` (embedded via `//go:embed all:web/dist` in `embed.go`) and exposes the broker over HTTP + SSE + WebSocket on `127.0.0.1:7891` (`internal/team/broker_web_proxy.go ServeWebUI`, `internal/team/broker_terminal.go`). The default user path is `npx hivex` → npm shim downloads the platform binary → binary launches the broker → browser auto-opens to the local web UI. A bubbletea TUI (`cmd/hivex/channel*.go` + `cmd/hivex/channelui/` ≈ 26k LOC) remains as `--tui` opt-in; tmux is also used as an *internal* per-bot process backend (`internal/team/pane*.go`, `tmux_runner.go` ≈ 3.7k LOC).
...
6. Fold in the highest-leverage deltas from `hive-crm/business-musings` bot comparison — citation-style memory UX, Sandcastle-style run records, Claude-Squad-style coding lanes, OpenHands-style credibility/evals — without expanding into a new product.
...
└──────────────────────────┬──────────────────────────────────┘
                ...
                ┌──────────────────────┐
                │ Browser → web/dist   │   `npx hivex` path
                └──────────────────────┘
```

**Two front doors, one broker, one UI bundle:**

- `cmd/hivex` — headless binary (today's binary, unchanged). Used by `npx hivex`, MCP, CI, `--cmd`. Continues to serve the web UI on `127.0.0.1:7891` for mode 1 (browser).
- `cmd/hivex-desktop` — new Wails v2 entry point. Embeds the broker package in-process, hosts the WebView pointing at `web/dist`, exposes OS verbs through Wails events.

**Same `web/dist` bundle in both modes.** App-data traffic is HTTP + SSE + WebSocket on loopback in both modes; the bot terminal already uses WebSocket through `web/src/lib/agentTerminalSocket.ts`. In mode 2 the WebView still talks to the in-process broker on a loopback port — no Wails bindings for app data. The discipline that prevents drift is **machine-enforced**: see "Lint enforcement" below.
...
1. **Go side** — `depguard` rule in `.golangci.yml`: `github.com/wailsapp/wails/v2/...` and `github.com/wailsapp/wails/v3/...` may be imported **only** by files under `desktop/oswails/`. All broker, bot, transport, and team code is forbidden from importing Wails. The `desktop/oswails/` package surface is small and reviewed line-by-line.
...
The existing `LaunchWeb` path blocks forever and owns browser-mode concerns: Hive onboarding, browser opening, stale broker cleanup, and PID files. That is wrong for a Wails shell. Add a sibling API:
...
Desktop releases use a separate `desktop-release` workflow with its own version cadence and update channels (`stable` / `beta`). Signing/notarization, updater manifests, and installer assets must not ride every `main` auto-release. The headless `cmd/hivex` binary continues on the existing npm + GitHub Releases path.
...
| v0.Y.0 (Phase 4, after desktop installers are green) | `--legacy-tui` exits non-zero unless `HIVEX_LEGACY_TUI_ACK=1` is set. CHANGELOG `Deprecated (hard)`. |
| v0.Z.0 (Phase 4, ~wk 11) | Delete `cmd/hivex/channel*.go` and `cmd/hivex/channelui/` (≈26k LOC). Delete `runChannelView` and `runTeam` from `cmd/hivex/main.go`. CHANGELOG `Removed`. |

**Keep** `internal/team/pane_*.go` and `tmux_runner.go`. These are tmux-as-process-backend (per-bot stdio capture), used optionally by web mode (`startPaneCaptureLoops`). On Windows the codepath is already dormant. Add `docs/PANE-BACKEND.md` documenting that this surface is *backend, not UI*, so future contributors don't conflate it with the deprecated TUI.

**Web parity gate before deletion** (Phase 3 audit, against TUI apps in `cmd/hivex/channelui/sidebar_apps.go` and slash commands in `internal/commands/slash.go`):
...
A new `cmd/hivexbench` Go binary (≈300 LOC) drives the SLOs below. Expand existing scaffolding (Playwright in `web/e2e/`, `bench/slice-1/` recall gate, `scripts/check-bundle-size.sh`, `scripts/benchmark.sh`) rather than introducing new frameworks.

| Metric | Tool | SLO (initial) |
|---|---|---|
| Cold start: binary launch → `/api/health` 200 | `cmd/hivexbench` polls, 20 iters | p95 < 1500 ms (warm: < 600 ms) |
...
| IPC latency: loopback `/api/health` | `cmd/hivexbench`, 1000 sequential | p99 < 8 ms |
...
| Eval pass rate (per bot class) | wire `evals/harness/` runner | ≥ 80% nightly on `main` |
| Install size (compressed installer) | CI step `du -sh dist/Hivex.*` | tracked, not gated (per user) |
...
- Phase 1: flip Windows smoke to a real `go test ./internal/team/... ./cmd/hivex/...` job. Fix POSIX-shell assumptions surfaced.
- Phase 4: add `macos-14` and `windows-2022` as full test runners. Use path-based filters (only on changes touching `internal/team/`, `cmd/hivex/`, `desktop/`) on PR; full matrix on `push: main` to control runner cost.
...
`cmd/hivex` headless binary continues to ship via npm shim and GitHub Releases tarballs (today's path, unchanged). Desktop app is a separate distribution: GitHub Releases + `hive.bot/download`, with Homebrew cask + winget submissions in Phase 4.

**Release strategy:** Desktop releases use a separate `desktop-release` workflow with their own cadence and `stable` / `beta` update channels. The headless `cmd/hivex` binary continues on the existing auto-release path via npm + GitHub Releases. Signing/notarization, updater manifests, and installer assets are excluded from routine `main` auto-releases so desktop release failures do not block normal headless releases.
...
| 5 | 2. Wails shell on broker | `cmd/hivex-desktop` calls `StartWeb()`, hosts WebView, deep-link `hivex://`, single-instance lock. |
| 6 | 2. Native OS verbs | Tray, native notifications via `desktop/oswails/` Wails API, dock badge, autostart. `internal/team/notifier_*` remains bot wake/routing machinery, not OS notification delivery. |
| 7 | 2. `cmd/hivexbench` advisory | SLOs land as advisory CI checks. Baselines collected on macOS/Windows/Linux. |
| 8 | 3. Web parity audit | Audit against `cmd/hivex/channelui/sidebar_apps.go` and `internal/commands/slash.go`. Fill real gaps; skip already-shipped items such as receipts, wiki/notebooks/reviews, and Cmd/Ctrl-K. |
...
| 11 | 4. Cut over | `--legacy-tui` hard-fails only after installers ship green. Delete `cmd/hivex/channel*.go` + `cmd/hivex/channelui/` (≈26k LOC). Ship desktop v1 on `hive.bot/download`. Submit Homebrew cask + winget. |
...
**Modified:**
- `cmd/hivex/main.go` — rename `--tui` → `--legacy-tui`; eventually delete `runChannelView`/`runTeam`
...
2. `cmd/hivexbench` reports SLOs within budget on all three OSes; CI gate flips from advisory to required only after macOS and Windows baselines exist.
3. `desktop-smoke` Playwright suite green on all three OSes (cold start + tray + notification + deep-link + auto-update self-test).
4. Manual: `npx hivex` (mode 1) and `Hivex.app/.msi/.AppImage` (mode 2) both serve the same UI and pass the same chat→receipt→wiki flow.
5. `--legacy-tui` exits non-zero without `HIVEX_LEGACY_TUI_ACK=1`. After deletion in Phase 4, `--legacy-tui` is unrecognized and `--tui` is gone.
6. `evals/harness/` runner reports per-bot-class pass rates ≥ 80% on `main` for 7 consecutive nightly runs.
...
8. Sign + notarize + install on a clean macOS 14, Windows 11, and Ubuntu 24.04 VM; first launch → first paint within SLO; tray icon visible; deep-link `hivex://task/test` opens the app and routes correctly.
...
3. **TUI deletion size is ~26k LOC, not ~17k.** `cmd/hivex/channel*.go` ≈ 17,046 LOC; `cmd/hivex/channelui/*.go` adds ≈ 9,056 LOC. Update CHANGELOG and PR descriptions accordingly.
4. **Parity sources were wrong.** `cmd/hivex/channelui/manifest.go` is roster/channel projection, not the TUI app manifest. Correct sources for the parity audit:
   - TUI apps: `cmd/hivex/channelui/sidebar_apps.go`
...
6. **`internal/team/notifier_*` is bot wake/routing machinery, not OS notifications.** Native OS notifications go through Wails' notification API in `desktop/oswails/` — they do not route through `notifier_*`.

### Highest-leverage requirements folded into the main body

1. **Define a real desktop launcher contract.** Today's `LaunchWeb` blocks forever, offers Hive onboarding, opens a browser, kills stale brokers, and writes PID files — none of that fits a Wails shell. Add a sibling: `StartWeb(ctx, opts) -> {webURL, brokerURL, token, shutdown func()}`. Wails calls `StartWeb`; the existing `LaunchWeb` keeps doing the npm-shim path. Both share a smaller `serve()` core.
...
| 7 | 2. `cmd/hivexbench` advisory | SLOs land as **advisory** CI checks. Baselines collected on macOS/Windows/Linux. |
| 8 | 3. Web parity audit (corrected sources) | Audit against `cmd/hivex/channelui/sidebar_apps.go` and `internal/commands/slash.go`. Fill real gaps; skip already-shipped items (receipts/wiki/notebooks/Cmd-K). |
| 9 | 3. Citation hover + run-record enrichment | citation/run-record deltas: hover-cards on cited claims; receipt schema additions for tools/approvals/files-changed. |
