...
hivebot should be easy for several bots to change without each bot rebuilding
...
| workspaces | workspace list/create/switch/pause/resume/shred/restore and admin pause | workspace orchestrator, launcher drain hooks, workspace state rows | `internal/team/broker_workspaces.go`, `cmd/hivex/workspace*.go`, `internal/workspace/*` | workspace rail and modals, settings danger zone, `web/src/api/workspaces.ts` | workspace commands and adapter | broker workspace tests, CLI workspace tests, Web workspace tests |
...
- TUI component files: a TUI bot owns `cmd/hivex/...` and
  `cmd/hivex/channelui/...` rendering and input tests.
...
Hive context and integrations are additive to local-first state. A context item
