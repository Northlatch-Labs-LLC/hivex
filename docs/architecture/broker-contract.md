...
same no matter which host starts it: today's `cmd/hivex` binary used by
`npx hivex`, and any external desktop host (the Electron shell lives in the
`hive-crm/hive-local` repo, which now owns the desktop broker package), must both
...
| Web UI listener | `7891` | `cmd/hivex/main.go` (`--web-port`) and `internal/workspaces/ports.go` (`MainWebPort`) |
...
2. Embedded filesystem from `hivex.WebFS()`.

   `embed.go` embeds `all:web/dist` into the top-level `hivex` package.
...
This order is part of the host contract. `cmd/hivex` and any external desktop
...
the Electron renderer in `hive-crm/hive-local`) must load the React bundle from
...
Multiple local Hivex instances coexist by using workspace-bound port pairs.
