...
The pane code is not the hivebot TUI. The files under
...
per-bot CLI sessions, currently the interactive Claude path, while the same
...
bots keep running without the live xterm stream.
...
The deprecated UI is the Bubble Tea "channel" interface under `cmd/hivex`.
It is user-facing terminal UI code. It renders office state, messages, tasks,
requests, composer state, sidebars, and app panels.

Current main wires this path as `--tui` in `cmd/hivex/main.go`. The desktop
...
|---|---|---|
| `cmd/hivex/channel*.go` | Bubble Tea program, input, render, routing, and tests in package `main` | about 17k LOC |
| `cmd/hivex/channelui/*` | Pure render/data-projection helpers for the channel TUI | about 9k LOC |
...
The process backend lives under `internal/team`. It exists to manage bot
...
| `internal/team/pane_lifecycle_spawn.go` | Visible and overflow bot pane spawn orchestration |
| `internal/team/pane_capture.go` | Polls `tmux capture-pane`, strips ANSI/control bytes, diffs lines, and pushes bot-stream data |
| `internal/team/pane_dispatch.go` | Serializes bot wake packets into live tmux panes with `send-keys` |
...
2. The pane backend starts or talks to subprocesses when a live pane target
   exists.
3. The broker publishes per-bot stream data.
...
starts one goroutine per pane-backed bot. Each goroutine polls the current
...
turn and tees stdout into the same bot stream buffer. Codex and Opencode are
...
checks `TmuxAvailable`, attempts to create the session and bot panes, and
records failed pane slugs. Failed or skipped targets route through headless
dispatch. The user loses live pane capture for those bots, but the bot turn
...
runtime and use the headless fallback until hivebot has a Windows-specific
pseudo-terminal backend.

That is the trade-off for the first desktop shell. The Wails desktop app on
Windows can still run the broker, web UI, bot dispatch, and headless provider
...
backend behind the same bot-stream contract, not a reason to keep or revive
the Bubble Tea channel UI.

## Contributor Rules

Do not delete `internal/team/pane_*.go` when removing the TUI. The TUI removal
target is `cmd/hivex/channel*.go` plus `cmd/hivex/channelui/*`. The pane backend
...
bot-stream writes. Rendering belongs to web components or, while it exists,
the legacy TUI.
...
| Term | Meaning |
|---|---|
| Legacy TUI | Bubble Tea user interface under `cmd/hivex/channel*.go` and `cmd/hivex/channelui/*` |
| Pane backend | `internal/team` process backend that may use tmux to host bot CLI processes |
| Bot terminal | Web xterm consumer of broker bot streams |

Do not conflate `internal/team/notifier_*` with native OS notifications either.
Those files own bot wake/routing: deciding which bot should receive a
...
now lives in the `hive-crm/hive-local` repo (Electron); the earlier Wails-based
...
`internal/team/notifier_*`, which is bot wake/routing machinery.
