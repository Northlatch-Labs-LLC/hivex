...
Pull a pure-function layer (rendering, projection, validation) out of a stateful god file into its own package. The new package does no IO and holds no state. **When to use:** the layer is genuinely pure. **Reference:** PR #501, `cmd/hivebot/channelui/`.
