...
`hivebot-mark.svg`. Two hollow eyes, gawking. Eleven by fifteen pixel grid,
...
```css
.hivebot-mark .hivebot-eyes {
  transform-box: fill-box; transform-origin: center;
  animation: hivebot-look 3.2s ease-in-out infinite alternate;
}
@keyframes hivebot-look { from { transform: scaleY(1); } to { transform: scaleY(0.42); } }
@media (prefers-reduced-motion: reduce) { .hivebot-mark .hivebot-eyes { animation: none; } }
```
...
`hivebot`. One word, always lowercase, including at the start of a sentence.
...
> hivebot is an independent, source-available project. It is a parody. It is
...
```
brand/
  hivebot-mark.svg              primary, scales to any size
  hivebot-mark-inverted.svg     for stamping on a filled ink field
  png/
    hivebot-mark-16.png           tab favicon
    hivebot-mark-32.png           standard favicon
    hivebot-mark-64.png
    hivebot-mark-128.png
    hivebot-mark-180.png          apple-touch-icon
    hivebot-mark-192.png          Android chrome icon
    hivebot-mark-256.png
    hivebot-mark-512.png          PWA splash
    hivebot-mark-1024.png         App Store and marketing hero
    hivebot-mark-inverted-*.png   same sizes
```
...
```bash
cd brand
EYES=$(sed -n 's/.*\(<g class="hivebot-eyes".*<\/g>\).*/\1/p' hivebot-mark.svg)
...
for size in 16 32 64 128 180 192 256 512 1024; do
  rsvg-convert -w $size -h $size -b '#F4F3EF' /tmp/sq-ink.svg \
    -o png/hivebot-mark-${size}.png
  rsvg-convert -w $size -h $size -b '#0E0E11' /tmp/sq-paper.svg \
    -o png/hivebot-mark-inverted-${size}.png
done
cp png/hivebot-mark-180.png ../website/apple-touch-icon.png
```
