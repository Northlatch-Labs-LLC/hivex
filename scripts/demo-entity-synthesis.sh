...
#
# Requirements: curl, python3, a running hivex instance with --memory-backend markdown
...
# Dev broker runs with HOME=~/.hivex-dev-home (port 7899).
# Prod runs with the real HOME (port 7890).
if [[ "$BROKER" =~ :7899 ]]; then
  HIVEX_HOME="${HOME}/.hivex-dev-home/.hivex"
else
  HIVEX_HOME="${HOME}/.hivex"
fi
WIKI_REPO="${HIVEX_REPO:-${HIVEX_HOME}/wiki}"
...
printf "\n%shivebot Entity Synthesis Demo%s\n" "$BOLD" "$RESET"
...
if [[ -z "$TOKEN" ]]; then
  ...
  Start the dev broker first:
    hivex-dev --broker-port 7899 --web-port 7900 --memory-backend markdown"
fi
ok "token acquired"
...
if [[ -d "${WIKI_REPO}/.git" ]]; then
  ...
  warn "Set HIVEX_REPO=/path/to/wiki to override"
fi
...
cat <<'EOF'
  ...
  1. demo-agent  recorded 5 facts → each landed as a git commit
     authored by demo-agent@hivex.local

  2. Fact #5 crossed the synthesis threshold (HIVEX_ENTITY_BRIEF_THRESHOLD=5)
