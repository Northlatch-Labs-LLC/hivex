...
BROKER_PORT="${HIVEX_PHASE5_PORT:-7893}"
...
for _ in $(seq 1 30); do
  if ! kill -0 "$HIVEX_PID" 2>/dev/null; then
    echo "[icp] FAIL: hivex exited during boot; see $HIVEX_LOG" >&2; exit 1
  fi
...
scenario_marcus() {
  ...
  before_llm="$(grep -cE 'ceo_draft:|llm.dispatch|provider.invoke' "$HIVEX_LOG" || true)"
