...
~/.hivex directory.  The guard prevents the familiar path-doubling bug that
occurs when the caller's shell already has HOME pointed at the dev-home
directory:

  BAD (doubles when HOME is already ~/.hivex-dev-home):
      home = os.path.join(os.getenv("HOME"), ".hivex-dev-home")
...
Usage:
  python3 scripts/skill_eval/eval.py \\
      ...
      --token-file /tmp/hivex-broker-token-7899 \\
...
_DEV_HOME_SUFFIX = "/.hivex-dev-home"


def _dev_home() -> str:
    """Return the dev-isolated HOME path without doubling.

    If HOME already ends with ``/.hivex-dev-home`` (e.g. the caller is
    running inside the hivex-dev shell alias that sets
    ``HOME=~/.hivex-dev-home``) we return it unchanged.  Otherwise we
    append the suffix so the broker subprocess writes into the isolated
    directory instead of the real ``~/.hivex``.
    """
...
def main() -> int:
    ...
    ap.add_argument("--token-file", default="/tmp/hivex-broker-token-7899")
    ap.add_argument(
        "--home",
        action="store_true",
        help=(
            "Isolate HOME to ~/.hivex-dev-home before running.  Safe to pass "
