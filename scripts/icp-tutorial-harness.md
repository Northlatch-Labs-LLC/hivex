...
`docs/tutorials/` against a freshly booted hivex instance and produces
a pass / fail / skipped report in `/tmp/icp-tutorial-results-<ts>/`.

## What it does

- Builds hivex from the current working tree (or uses `--binary=<path>`).
- Boots hivex on an isolated runtime home (`/tmp/icp-tutorial-results-<ts>/runtime`).
...
- Tears down the hivex process and (by default) wipes the runtime home.
...
bash scripts/icp-tutorial-harness.sh --binary=/usr/local/bin/hivex
