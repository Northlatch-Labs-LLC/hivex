...
This rig stands up a completely isolated hivebot instance so we can test the real
runtime path (HTTP → broker auto-promote → launcher targeting → headless
dispatch) without any contamination from the coworker's existing `~/.hivex`
state.
...
`~/.hivex` to the sandbox.

## Isolation

- Custom `HOME` at `/tmp/hivex-debug-tagging-home` — no touch to real state.
...
curl -s -H "Authorization: Bearer $(cat /tmp/hivex-broker-token-7899)" \
...
curl -s -H "Authorization: Bearer $(cat /tmp/hivex-broker-token)" \
