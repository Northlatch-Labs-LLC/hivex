...
BINARY="${BINARY:-$REPO_ROOT/.hivex-debug-tagging}"
SANDBOX_HOME="${SANDBOX_HOME:-/tmp/hivex-debug-tagging-home}"
...
LOG_DIR="${SANDBOX_HOME}/.hivex/logs"
...
if [ ! -x "$BINARY" ] || [ "${BUILD:-1}" = "1" ]; then
  log "build: compiling hivex to $BINARY"
  (cd "$REPO_ROOT" && go build -buildvcs=false -o "$BINARY" ./cmd/hivex)
fi
...
cat > "$SANDBOX_HOME/.hivex/config.json" <<JSON
{
  "blueprint": "$PACK",
  "pack": "$PACK",
...
# Isolate: custom HOME, custom PATH with fake bins first, no browser.
env \
  HOME="$SANDBOX_HOME" \
  PATH="$FAKE_BIN_DIR:$PATH" \
  HIVEX_BROKER_PORT="$BROKER_PORT" \
  "$BINARY" \
    --pack "$PACK" \
    --web-port "$WEB_PORT" \
    --no-open \
...
# non-default, so for BROKER_PORT=7899 it's /tmp/hivex-broker-token-7899.
...
LATENCY_LOG="${SANDBOX_HOME}/.hivex/logs/headless-codex-latency.log"
