...
REPORT_DIR="/tmp/hivex-benchmark-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$REPORT_DIR"
...
check_broker() {
  ...
  if [ "$status" != "ok" ]; then
    echo -e "${RED}Broker not running. Start with: hivex --pack starter${NC}"
    exit 1
  fi
}
...
test_single_turn() {
  ...
  # Comparison
  local hivex_billed
  hivex_billed=$(python3 -c "import json; print(json.load(open('$REPORT_DIR/single-total.json'))['total']['total_billed'])")
  echo ""
  ...
  echo -e "    ${GREEN}hivebot actual:           ${hivex_billed} tokens${NC}"
  if [ "$hivex_billed" -gt 0 ] 2>/dev/null; then
    python3 -c "
ratio = 372000 / $hivex_billed
...
# This is where hivebot's fresh-session architecture shines:
# baseline grows linearly, hivebot stays flat.
# ═══════════════════════════════════════════════════════════════
test_session() {
  ...
  echo -e "  ${DIM}hivebot: fresh session each turn (flat). baseline: --resume (linear growth).${NC}"
  echo ""
  ...
  echo -e "    hivebot 10-turn total:     $(printf "%'d" $total_billed) tokens"
  ...
  echo -e "  ${DIM}hivebot stays flat because each turn starts a fresh session.${NC}"
}

# ═══════════════════════════════════════════════════════════════
# Test 3: Idle burn
# Start the system, leave it idle for 2 minutes, measure tokens.
# hivebot: zero (push-driven, no polling).
...
test_idle() {
  ...
  echo -e "  ${DIM}hivebot: push-driven (zero idle cost). baseline: heartbeat every 30s.${NC}"
  echo ""
  ...
  echo -e "    hivebot idle tokens:       ${GREEN}${total_idle}${NC}"
  echo -e "    baseline estimate:      ${RED}$(printf "%'d" $baseline_idle)${NC}"
  echo ""
  if [ "$total_idle" -eq 0 ]; then
    echo -e "  ${GREEN}${BOLD}hivebot burned exactly zero tokens while idle.${NC}"
    echo -e "  ${DIM}baseline would burn ~${baseline_idle} tokens polling the LLM every 30s.${NC}"
  else
    echo -e "  ${YELLOW}hivebot burned ${total_idle} tokens during idle (unexpected — investigate).${NC}"
  fi
}
...
print(f'    hivebot:     only tagged agents wake ({total[\"total_billed\"]:,} tokens)')
...
print_report() {
  ...
  echo ""
  echo "  ┌────────────────────────┬──────────────────────┬──────────────────────┐"
  echo "  │ Dimension              │ hivebot                │ baseline            │"
  echo "  ├────────────────────────┼──────────────────────┼──────────────────────┤"
...
lines = ["# hivebot Token Benchmark Report\n"]
...
if single:
    ...
    if t['total_billed'] > 0:
        lines.append(f"- **hivebot is {372000/t['total_billed']:.1f}x more efficient**\n")
...
if session_lines:
    ...
    lines.append(f"\n- **hivebot total: {session_total:,}**")
    lines.append(f"- baseline estimate: {baseline_session:,}")
    if session_total > 0:
        lines.append(f"- **hivebot is {baseline_session/session_total:.1f}x more efficient over 10 turns**\n")
...
if idle_total == 0:
    lines.append("\n## Test 3: Idle burn\n")
    lines.append("- **hivebot: 0 tokens** (push-driven, zero idle cost)")
