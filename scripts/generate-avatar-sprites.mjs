...
const goOut = path.join(rootDir, 'internal/avatar/office_sprites.go')
...
const avatarRoleSpecs = {
  ...
  eng: {
    baseSpriteId: 'office20',
    aliases: ['eng', 'builder', 'founding-engineer', 'workflow-architect', 'automation-builder'],
    face: 'glassesSoft',
    outfit: 'shirtTie',
  },
  gtm: { baseSpriteId: 'office14', aliases: ['gtm', 'growth', 'growth-ops'], face: 'preserve', outfit: 'sales' },
  ...
  cro: { baseSpriteId: 'office10', aliases: ['cro', 'monetization', 'revenue', 'invoicing'], face: 'mouthOnly', outfit: 'suit' },
  jim: {
    baseSpriteId: 'office20',
    aliases: ['jim', 'halpert', 'jim-halpert'],
    face: 'jimOffice',
    hair: 'jimSheetSidePart',
    outfit: 'blueShirtTie',
    charm: 'officeJim',
  },
  pam: {
  ...
  hive: { baseSpriteId: 'office17', aliases: ['hive', 'operator', 'ops', 'operations'], face: 'preserve', outfit: 'ops' },
  generic: { baseSpriteId: 'office05', aliases: ['generic'], face: 'preserve', outfit: 'suit' },
  qa: { baseSpriteId: 'office03', aliases: ['qa', 'reviewer', 'review'], face: 'preserve', outfit: 'dwight' },
  ae: { baseSpriteId: 'office10', aliases: ['ae', 'client-success', 'customer-success'], face: 'mouthOnly', outfit: 'suit' },
  sdr: { baseSpriteId: 'office14', aliases: ['sdr'], face: 'preserve', outfit: 'sales' },
  research: { baseSpriteId: 'office16', aliases: ['research', 'research-lead', 'analyst', 'bookkeeper'], face: 'silver', outfit: 'suit' },
  content: {
    baseSpriteId: 'office25',
    aliases: ['content', 'scriptwriter', 'editor', 'packaging-lead', 'community-manager'],
    face: 'lightNoBrow',
    outfit: 'creative',
  },
}
...
function applyJimPalette(palette) {
  palette.length = 0
  palette.push(
    '#452a1d',
    '#683f2b',
...
function detectScale(components) {
  ...
  if (scale <= 0) {
    throw new Error('Could not infer avatar scale factor from pixel components')
  }
  return scale
}
...
function buildOfficeJimPortrait(_officePortrait, _spec) {
  return [
...
function addManualSprites(spriteCatalog) {
  ...
  const hybrids = Object.entries(avatarRoleSpecs).map(([role, spec]) => {
    ...
    } else if (spec.outfit === 'blueShirtTie') {
      applyJimPalette(palette)
    }
    ...
    } else if (spec.charm === 'officeJim') {
      portrait = buildOfficeJimPortrait(portrait, spec)
    } else {
...
function readSeedSpriteCatalog() {
  if (!fs.existsSync(seedCatalogPath)) {
    throw new Error(`Missing avatar seed catalog: ${seedCatalogPath}`)
  }

  const data = JSON.parse(fs.readFileSync(seedCatalogPath, 'utf8'))
...
function main() {
  const spriteCatalog = readSeedSpriteCatalog()
  ...
  console.log(`Generated ${spriteCatalogWithOverrides.length} avatar sprites`)
  console.log(webOut)
  console.log(goOut)
}
