#!/usr/bin/env bash
# Cross-compile hivex.exe for Windows on this Mac.
# Output: dist/windows/{amd64,arm64}/hivex.exe
#
...
# HIVEX_SKIP_WEB=1 only when you've just run `bun run build` and want to
# avoid the second invocation cost during a tight Go-only iteration loop.
if [[ "${HIVEX_SKIP_WEB:-0}" != "1" ]]; then
  echo "building web UI bundle..."
  ( cd web && bun install && bun run build )
fi

VERSION="${HIVEX_VERSION:-dev-$(git rev-parse --short HEAD 2>/dev/null || echo unknown)}"
TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

LDFLAGS=(
  -s -w
  "-X github.com/Northlatch-Labs-LLC/hivex/internal/buildinfo.Version=${VERSION}"
  "-X github.com/Northlatch-Labs-LLC/hivex/internal/buildinfo.BuildTimestamp=${TIMESTAMP}"
)

mkdir -p dist/windows/amd64 dist/windows/arm64

for arch in amd64 arm64; do
  out="dist/windows/${arch}/hivex.exe"
  echo "building ${out}..."
  CGO_ENABLED=0 GOOS=windows GOARCH="${arch}" \
    go build -trimpath -ldflags "${LDFLAGS[*]}" -o "${out}" ./cmd/hivex
  ls -lh "${out}"
done
