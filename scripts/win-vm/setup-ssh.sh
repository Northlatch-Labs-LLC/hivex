...
# Until then the autologin user is hivex with a blank password — sshd is
...
Windows, reboot, and try again. Or set HIVEX_VM_HOST=<ip> manually
...
cat <<EOF
About to install this public key into ${VM_USER}@${HIVEX_VM_HOST}:

  ${pubkey_file}

Windows will prompt for the ${VM_USER} password — enter the hivex account password.
EOF
...
# That's our case for `hivex`. We pipe a small PowerShell program over
# stdin so we don't have to escape the pubkey through nested quoting.
ssh -o StrictHostKeyChecking=accept-new \
    -o UserKnownHostsFile="$HOME/.ssh/known_hosts.hivex-vm" \
    "${VM_USER}@${HIVEX_VM_HOST}" \
...
echo "  ssh ${VM_USER}@${HIVEX_VM_HOST} hostname"
