#!/usr/bin/env bash
# Push the freshly-built hivex.exe into the running Win11 VM and run
...
#   HIVEX_VM_HOST=<ip>     skip utmctl ip-address probing
#   HIVEX_VM_USER=hivex
#   HIVEX_VM_BIN=dist/windows/arm64/hivex.exe

set -euo pipefail

cd "$(dirname "$0")/../.."

VM_UUID="${HIVEX_VM_UUID:-BD57713D-0B79-4B87-AD08-88A9BF7922CC}"
VM_USER="${HIVEX_VM_USER:-hivex}"
VM_BIN="${HIVEX_VM_BIN:-dist/windows/arm64/hivex.exe}"
...
if [[ -z "${HIVEX_VM_HOST:-}" ]]; then
  ...
  if [[ -z "${ip}" ]]; then
    echo "could not determine VM IP via utmctl. set HIVEX_VM_HOST=<ip> manually." >&2
    exit 1
  fi
  HIVEX_VM_HOST="${ip}"
fi

echo "VM host: ${HIVEX_VM_HOST}"
echo "binary:  ${VM_BIN}"
echo

ssh_target="${VM_USER}@${HIVEX_VM_HOST}"
ssh_opts=(-o StrictHostKeyChecking=accept-new -o UserKnownHostsFile="$HOME/.ssh/known_hosts.hivex-vm" -o ConnectTimeout=5)

remote_path='C:\hivex\hivex.exe'
...
set +e
fails=0
run "hivex.exe --version"   "${remote_path} --version" || ((fails++))
run "hivex.exe --help"      "${remote_path} --help"    || ((fails++))
run "hivex.exe doctor"      "${remote_path} doctor"    || ((fails++))
