...
Local cross-platform test harness for verifying `hivex` on Windows ARM64
...
| File | Purpose |
|---|---|
| `autounattend.xml` | Win11 ARM64 setup answer file. Zero-prompt install of a `hivex` local user with OpenSSH listening on :22, hostname `hivebot-DEV`. |
...
| `build.sh` | Cross-compile `hivex.exe` for windows/{amd64,arm64} on this Mac with the goreleaser ldflags. |
| `push-and-test.sh` | After `build.sh`, push the binary into the running UTM VM via SSH and run smoke tests. |

## VM identifiers

- UTM VM name: `Hivex-Win11-ARM`
- UTM UUID: `BD57713D-0B79-4B87-AD08-88A9BF7922CC`
- Hostname inside the VM: `hivebot-DEV`
- Local account: `hivex` (blank password — `autounattend.xml` ships an empty
...
/Applications/UTM.app/Contents/MacOS/utmctl file push <UUID> ./hivex.exe 'C:\hivex\hivex.exe'

# Or via SSH (preferred — works without UTM guest agent)
ssh hivex@<vm-ip> 'C:\hivex\hivex.exe --version'
...
2. `./scripts/win-vm/build.sh` — produces `dist/windows/{amd64,arm64}/hivex.exe`.
