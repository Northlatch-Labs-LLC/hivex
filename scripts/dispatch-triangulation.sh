...
lens_preamble() {
  case "$1" in
    ...
    conventions)
      ...
      ;;
    ipc)
      printf '%s\n' "The desktop contextBridge boundary now lives in the hive-crm/hive-local repo; in THIS repo the equivalent trust boundary is the broker's loopback HTTP/SSE/WebSocket surface. Did it gain new routes? Is the bind still loopback-only? Is bearer auth enforced on \`/api/*\`? Does any app data leave without auth, or any non-loopback origin get trusted? Read \`internal/team\` and \`packages/protocol\`."
      ;;
    coverage)
...
write_synthesis() {
...

  {
    ...
    if [[ -n "$marker_hits" ]]; then
      echo "$marker_hits" | awk '{print "- marker: " $0}'
      disagreement_count=$((disagreement_count + 1))
    fi
