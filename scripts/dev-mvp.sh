...
# user's real claude-code login. hivebot state lives in $HIVEX_HOME/.hivex;
...
#   2. hivex-mvp Go binary               — broker + embedded prompts
...
#
# Flags:
#   --reset       Wipe HIVEX_RUNTIME_HOME before starting (default keeps state).
#
# Env:
...
HIVEX_HOME="${HIVEX_HOME:-/tmp/hivex-mvp-home}"
BROKER_BIN="${BROKER_BIN:-./hivex-mvp}"
LOG="${LOG:-/tmp/hivex-mvp.log}"
...
if [[ "$RESET_STATE" == "1" ]]; then
  # Guard against accidental wipes when HIVEX_HOME is mis-set (empty, root,
  # the cwd, or a home directory). Refuse to recursively delete anything
  # that does not look like a hivex workspace — must either already contain
  # a .hivex/ subdir OR live at a path whose basename matches a hivex-*
  # pattern (covers fresh --reset runs against /tmp/hivex-mvp-home).
  case "$HIVEX_HOME" in
    ""|"/"|"."|"./"|"$HOME"|"$HOME/")
      log "refusing to wipe unsafe HIVEX_HOME='$HIVEX_HOME'"
      exit 1
      ;;
  esac
  base="$(basename -- "$HIVEX_HOME")"
  if [[ ! -d "$HIVEX_HOME/.hivex" && ! "$base" =~ ^hivex[-_].+ && ! "$base" =~ ^\.hivex ]]; then
    log "refusing to wipe '$HIVEX_HOME' — does not contain .hivex/ and basename does not match hivex-*"
    exit 1
  fi
  log "wiping $HIVEX_HOME (per --reset)"
  rm -rf -- "$HIVEX_HOME"
fi
...
if [[ "$SKIP_FE" != "1" ]]; then
  ...
  (cd web && bun run build) >/tmp/hivex-mvp-build.log 2>&1 || {
    log "FE build failed. tail:"
    tail -30 /tmp/hivex-mvp-build.log
    exit 1
  }
fi
...
if [[ "$SKIP_GO" != "1" || ! -x "$BROKER_BIN" ]]; then
  ...
  go build -o "$BROKER_BIN" ./cmd/hivex
fi
...
# 2026-09-03.
log "starting broker on web :$PORT_WEB / broker :$PORT_BROKER with HIVEX_RUNTIME_HOME=$HIVEX_HOME"
HIVEX_RUNTIME_HOME="$HIVEX_HOME" "$BROKER_BIN" --no-open \
...
echo
log "broker pid: $PID"
log "state dir:  $HIVEX_HOME/.hivex"
log "log:        $LOG"
