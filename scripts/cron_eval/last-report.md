...
| `cron-02-patch-happy-path` | PATCH hive-notifications interval_override=30 | PASS | interval_override=30 as set |
...
| `cron-06-disabled-skips-run` | Disabled cron skips run (hive-notifications) | PASS | last_run ''->'', log_lines mentioning hive-notifications: 0->0 |
| `cron-07a-persistence-snapshot` | Persistence snapshot (review-expiry) | PASS | snapshot written to /tmp/hivex-cron-eval-snapshot.json; restart broker, then run --verify-persistence |

## Sub-results

| scenario | sub | pass | detail |
|---|---|---|---|
| `cron-01-self-registration` | `hive-insights` | PASS |  |
| `cron-01-self-registration` | `hive-notifications` | PASS |  |
...
| `cron-03-floor-rejection` | `hive-insights below_floor (override=29, floor=30)` | PASS |  |
| `cron-03-floor-rejection` | `hive-insights at_floor (override=30)` | PASS |  |
| `cron-03-floor-rejection` | `hive-notifications below_floor (override=4, floor=5)` | PASS |  |
| `cron-03-floor-rejection` | `hive-notifications at_floor (override=5)` | PASS |  |
