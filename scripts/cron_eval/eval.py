...
Usage:
  python3 scripts/cron_eval/eval.py \\
      ...
      --broker-log /tmp/hivex-dev-eval.log \\
...
SYSTEM_CRONS: dict[str, dict[str, Any]] = {
    "hive-insights": {"min_floor": 30, "read_only": False},
    "hive-notifications": {"min_floor": 5, "read_only": False},
...
PERSISTENCE_SNAPSHOT_PATH = "/tmp/hivex-cron-eval-snapshot.json"
...
def scenario_patch_happy_path(broker: str, token: str) -> ScenarioResult:
    ...
    confirm GET reflects it. Uses hive-notifications (floor=5) at 30 min.
    """
    target_slug = "hive-notifications"
    want_override = 30
...
def scenario_disabled_skips_run(
    ...
    We disable hive-notifications (floor 5, real cron) and watch for ~12s.
...
def main() -> int:
    ...
    ap.add_argument(
        "--broker-log",
        default="/tmp/hivex-dev-eval.log",
