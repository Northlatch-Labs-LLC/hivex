...
if [ -z "$TOKEN" ]; then
  fail "broker not reachable at $BROKER (is hivex-dev running?)"
  fail "try:  ./hivex-dev --broker-port 7899 --web-port 7900 --memory-backend markdown"
  exit 1
fi
...
As of this week, new installs default to the markdown wiki instead of the Hive knowledge graph.

## Why

- Users can \`cat\` and \`git clone\` their team memory
- No API key required to try it
- File-over-app story matches hivebot's source-available/self-hosted posture

## What it means

- Existing Hive/GBrain users unaffected (backend switch is config-pinned)
- Four new MCP tools (\`team_wiki_read/search/list/write\`) replace \`team_memory_*\` when \`HIVEX_MEMORY_BACKEND=markdown\`
