...
# hivex instance via the broker REST API, scores each against its
...
#
# Flags:
#   --keep-runtime         Leave the hivex runtime home on disk for
#                          inspection. Default is to wipe.
#   --scenarios=<csv>      Run only the named scenarios; default = all 10.
#   --binary=<path>        Use this hivex binary instead of building.
...
RUNTIME_HOME=""
BROKER_PORT="${HIVEX_HARNESS_PORT:-7888}"
...
HIVEX_LOG="$REPORT_DIR/hivex.log"
echo "[]" > "$RESULTS_JSON"
...
build_binary() {
  if [ -n "$HIVEX_BINARY" ]; then
    ...
    # the caller intended.
    if [ ! -x "$HIVEX_BINARY" ]; then
      fail "--binary path is not executable: $HIVEX_BINARY"
      exit 2
    fi
    log "using provided binary: $HIVEX_BINARY"
    return
  fi
  HIVEX_BINARY="$REPORT_DIR/hivex"
  log "building hivex -> $HIVEX_BINARY"
...
boot_hivex() {
  ...
  # against a stale hivex and the harness would score scenarios
  ...
  log "booting hivex on broker=$BROKER_PORT web=$WEB_PORT runtime=$RUNTIME_HOME"
  HIVEX_RUNTIME_HOME="$RUNTIME_HOME" "$HIVEX_BINARY" \
    --broker-port "$BROKER_PORT" --web-port "$WEB_PORT" --no-open \
    > "$HIVEX_LOG" 2>&1 &
  HIVEX_PID=$!
  echo "$HIVEX_PID" > "$REPORT_DIR/hivex.pid"
  ...
  # positives if another hivex is already running on $BROKER_PORT).
  for _ in $(seq 1 30); do
    if ! kill -0 "$HIVEX_PID" 2>/dev/null; then
      fail "hivex process $HIVEX_PID exited during boot; see $HIVEX_LOG"
      exit 1
    fi
    if curl -fs --connect-timeout 2 --max-time 5 \
      ...
      if [ "$listener_pid" = "$HIVEX_PID" ]; then
      ...
      fail "port $BROKER_PORT listener pid=$listener_pid != harness hivex pid=$HIVEX_PID"
      exit 1
    fi
    sleep 0.5
  done
  fail "hivex did not come up within 15s; see $HIVEX_LOG"
  exit 1
}

teardown() {
  if [ -f "$REPORT_DIR/hivex.pid" ]; then
    local pid
    pid="$(cat "$REPORT_DIR/hivex.pid")"
    log "stopping hivex pid=$pid"
...
token() { cat "/tmp/hivex-broker-token-${BROKER_PORT}"; }
...
scenario_04a_sam_forkswap() {
  ...
  # file + restart hivex) is a CLI invocation. The harness can still
