...
BROKER_PORT="${HIVEX_BROKER_PORT:-7892}"
WEB_PORT="${HIVEX_WEB_PORT:-7893}"
VITE_PORT="${HIVEX_VITE_PORT:-5280}"
HOME_DIR="${HIVEX_RUNTIME_HOME:-/private/tmp/hivex-operator-home}"
COMPOSIO_USER_ID="${HIVEX_COMPOSIO_USER_ID:?set HIVEX_COMPOSIO_USER_ID to your Composio user id}"
LOG_DIR="${TMPDIR:-/tmp}"
...
if [[ "${1:-}" != "--no-build" ]]; then
  ...
  go build -o hivex-mvp ./cmd/hivex
fi
...
# (config.ResolveComposioUserID: HIVEX_COMPOSIO_USER_ID first, COMPOSIO_USER_ID
# as the compat fallback), so Composio is configured regardless of which name a
# given broker build reads.
env -u HIVEX_COMPOSIO_API_KEY -u OPENAI_API_KEY -u HIVEX_OPENAI_API_KEY \
  HIVEX_RUNTIME_HOME="$HOME_DIR" \
  HIVEX_COMPOSIO_USER_ID="$COMPOSIO_USER_ID" \
  COMPOSIO_USER_ID="$COMPOSIO_USER_ID" \
  ./hivex-mvp --no-open --broker-port "$BROKER_PORT" --web-port "$WEB_PORT" \
  >"$LOG_DIR/hivex-operator-broker.log" 2>&1 &
echo "  broker pid $! (log: $LOG_DIR/hivex-operator-broker.log)"

echo "==> starting vite on $VITE_PORT (proxying /api -> 127.0.0.1:$WEB_PORT)"
(cd web && HIVEX_WEB_PROXY_PORT="$WEB_PORT" bunx vite --port "$VITE_PORT" --strictPort \
  >"$LOG_DIR/hivex-operator-vite.log" 2>&1 &)
echo "  vite log: $LOG_DIR/hivex-operator-vite.log"
