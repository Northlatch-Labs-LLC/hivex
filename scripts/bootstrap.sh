#!/usr/bin/env bash
# bootstrap.sh — one-shot setup for a fresh hivex checkout.
#
...
# ---- bun run build in web/ (so cmd/hivex can embed web/dist) ----
# Without this, a fresh clone + `go build ./cmd/hivex` + `./hivex` lands on
# the "hivebot web UI assets are missing" fallback page. Fail loudly on build
