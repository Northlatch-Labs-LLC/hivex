#!/usr/bin/env bash
# fresh-office.sh — wipe a hivebot runtime home back to first-run state and
...
# apps directory alone runs to five figures of files). But .hivex/config.json
...
HOME_DIR="${HIVEX_RUNTIME_HOME:-$HOME/.hivex-demo-home}"
BIN="${HIVEX_BIN:-$HOME/.hivex-bin/hivex-dev}"
BROKER_PORT="${HIVEX_BROKER_PORT:-7899}"
WEB_PORT="${HIVEX_WEB_PORT:-7900}"
CONFIRM=0
RELAUNCH=0
HOME_EXPLICIT=0
# Did the caller actually choose these ports, or are they just the defaults?
PORTS_EXPLICIT=0
[ -n "${HIVEX_BROKER_PORT:-}${HIVEX_WEB_PORT:-}" ] && PORTS_EXPLICIT=1
...
# KEEP-LIST, not a wipe-list. Everything in .hivex goes EXCEPT these.
#
...
echo "preserving   : .hivex/config.json (API key, company name, provider choice)"
echo "removing     :"
...
if [ "$HOME_EXPLICIT" -eq 1 ] && [ "$PORTS_EXPLICIT" -eq 0 ]; then
  ...
  echo "      HIVEX_BROKER_PORT / HIVEX_WEB_PORT so this script knows which to stop."
else
...
if [ "$RELAUNCH" -eq 1 ]; then
  [ -x "$BIN" ] || { echo "no binary at $BIN — build first: go build -o $BIN ./cmd/hivex" >&2; exit 1; }
  HIVEX_RUNTIME_HOME="$HOME_DIR" HIVEX_BROKER_PORT="$BROKER_PORT" \
