# hivebot desktop shell (Wails)

A single Go binary that boots the **existing** hivebot broker + web UI
**in-process** (no sidecar) and attaches a native [Wails](https://wails.io) v2
window to it. The desktop app *is* the `hivex` process with a window bolted on.
...
workspace (the same office an unqualified `hivex web` uses). Before booting it
...
hivebot-identity-verified peer is live, **attaches** the window to it instead of
booting a second broker. The CLI's `hivex web` does the same — opens the running
...
- **Desktop+CLI simultaneous cold-start** has no lock between the attach-check
  ...
  the fix.
- **Identity probe** confirms a hivebot office via a marker in the served `/`; a
