...
// Command oswails is the hivebot desktop shell: a single Go binary that attaches a
// native Wails window to the user's office. If a broker is already serving the
// active workspace (a running CLI `hivex web` or another front-end), the window
...
func main() {
	...
	// (the same office an unqualified `hivex` CLI uses) — HIVEX_RUNTIME_HOME is
	...
	if err := wails.Run(&options.App{
		...
		SingleInstanceLock: &options.SingleInstanceLock{
			...
			UniqueId: "ai.hive.hivex.desktop",
