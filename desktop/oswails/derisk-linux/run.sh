...
cd /src || exit 90
go build -tags "desktop production webkit2_41" -ldflags "-w -s" -o /out/hivex-desktop ./desktop/oswails 2>/out/build.log
RC=$?
echo "build rc=$RC"
if [ $RC -ne 0 ]; then echo "BUILD FAILED:"; tail -40 /out/build.log; exit "$RC"; fi
ls -lh /out/hivex-desktop | awk '{print "linux binary:",$5}'

cat > /tmp/inner.sh <<'INNER'
#!/usr/bin/env bash
# xvfb-run -a sets DISPLAY for us; don't override it.
/out/hivex-desktop >/out/app.log 2>&1 &
APP=$!
