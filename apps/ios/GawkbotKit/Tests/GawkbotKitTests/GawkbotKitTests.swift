...
final class PairingTests: XCTestCase {
    func testParsesPairLink() {
        let p = Pairing.parse("hivebot://pair?url=http%3A%2F%2F100.64.0.5%3A7890&token=abc123")
        XCTAssertEqual(p?.brokerURL.absoluteString, "http://100.64.0.5:7890")
        XCTAssertEqual(p?.token, "abc123")
        XCTAssertEqual(p?.link, "hivebot://pair?url=http://100.64.0.5:7890&token=abc123")
    }
    ...
    func testRejectsGarbage() {
        ...
        XCTAssertNil(Pairing.parse("hivebot://pair?url=http://x"))
    ...
    func testCleartextOnlyForPrivateHosts() {
        ...
        XCTAssertNil(Pairing.parse("hivebot://pair?url=http%3A%2F%2Fevil.example.com&token=x"))
    }
