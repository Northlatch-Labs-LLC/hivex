...
// publish.sh wrapper passes via $HIVEX_SCREENSHOTS_OUT.
//
// Why a hand-rolled harness instead of `playwright test`:
//   1. We point at vite dev (`http://localhost:5273`) and intercept /api/*
//      with route mocks — no real hivex broker needed. The existing
//      tests/ harness boots an actual hivex binary; that's overkill for
