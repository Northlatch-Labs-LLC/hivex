...
#      it to be ready.
#   3. Runs the spec with HIVEX_SCREENSHOTS_OUT=/tmp/hivex-screenshots-<feature>.
#   4. Creates an orphan worktree at /tmp/hivex-screenshots-branch-<pr>,
...
#   An orphan branch under Northlatch-Labs-LLC/hivex is the simplest public-API
...
out_dir="/tmp/hivex-screenshots-${feature}"
mkdir -p "$out_dir"
...
(
  ...
  HIVEX_SCREENSHOTS_OUT="$out_dir" BASE_URL="$base_url" \
    node "$spec"
)
...
worktree_dir="/tmp/hivex-screenshots-branch-${pr_number}"
