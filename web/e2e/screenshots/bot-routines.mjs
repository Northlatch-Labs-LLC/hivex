...
const OUT = process.env.HIVEX_SCREENSHOTS_OUT;
if (!OUT) {
  console.error("HIVEX_SCREENSHOTS_OUT is not set; run via publish.sh / driver");
  process.exit(2);
}
...
const TRANSCRIPTS = {
  sess_manual01: [
    ...
    { from: "hive", body: "Made scoreAndRouteLead(lead) callable — I will use it whenever you hand me a lead.", at: hrsAgo(30) },
  ],
  sess_recap01: [
    ...
    { from: "hive", body: "3 stage moves — Globex to Negotiation, Acme to Evaluation, Initech to Discovery. 2 new leads (Hooli, Stark). Umbrella has been stalled 21 days: owner Priya, last touch was the pricing call.", at: hrsAgo(2) },
  ],
  sess_hygiene1: [
    ...
    { from: "hive", body: "2 deals need attention. Drafted nudges for Priya (Umbrella, no next step) and Sam (Globex, close date slipped 9 days) — drafts only, nothing sent.", at: hrsAgo(25) },
  ],
};
