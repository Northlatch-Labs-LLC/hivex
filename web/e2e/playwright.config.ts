...
// The hivex backend serves the built web UI directly at :7891. CI boots the
...
// it sandboxes HIVEX_RUNTIME_HOME, seeds onboarded.json for the shell phase,
// and runs both wizard and smoke specs against alternate ports so the
// developer's real ~/.hivex state is never touched.
export default defineConfig({
  ...
  use: {
    baseURL: process.env.HIVEX_E2E_BASE_URL ?? "http://localhost:7891",
