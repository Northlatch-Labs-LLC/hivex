...
// Shared helpers for the hivex web e2e suite. Kept under `tests/` with a
...
export function collectReactErrors(page: Page): () => string[] {
  ...
  page.on("console", (msg) => {
    if (msg.type() === "error") {
      const text = msg.text();
      // The boundary's own log line is `[hivebot ErrorBoundary]` — see
      // web/src/App.tsx:69 (`console.error("[hivebot ErrorBoundary]", ...)`).
      ...
      if (
        ...
        text.includes("hivebot ErrorBoundary")
      ) {
        errors.push(text);
      }
    }
...
// hivex process, so without an explicit reset earlier messages bleed into
