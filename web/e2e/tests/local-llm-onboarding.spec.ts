...
test.describe("Onboarding → Run a local model", () => {
  ...
  test("Hive API key panel hides unless memory backend needs it", async ({
    page,
  }) => {
    // The Hive API key panel only matters when memory_backend === "hive".
    // Team wiki (default) doesn't need a Hive key; surfacing the input
    // would suggest a missing config piece when there isn't one.
    stubStatusEndpoint(page);
    await advanceToSetupStep(page);

    // Default backend is Markdown (Team wiki) → no Hive panel.
    await expect(page.getByTestId("wizard-hive-api-key-panel")).toHaveCount(0);

    // Click the Hive memory tile → panel appears.
    await page
      .locator(".wizard-panel")
      .filter({ hasText: "Organizational memory" })
      .getByRole("button", { name: /^Hive/ })
      .click();
    await expect(page.getByTestId("wizard-hive-api-key-panel")).toBeVisible();
