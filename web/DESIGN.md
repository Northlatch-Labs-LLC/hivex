...
browser (`localStorage: hivex.office-tour-done`), replayable from Help via
the `hivex:show-office-tour` event. Files: `components/onboarding/tour/*`,
...
Token-only, verified in `hive`, `hive-dark`, and `noir-gold`. The overlay
