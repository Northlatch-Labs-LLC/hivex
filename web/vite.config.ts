...
// Set HIVEX_WEB_PROXY_TARGET (e.g. http://127.0.0.1:7893) or just
// HIVEX_WEB_PROXY_PORT (e.g. 7893).
const proxyTarget =
  process.env.HIVEX_WEB_PROXY_TARGET ||
  `http://127.0.0.1:${process.env.HIVEX_WEB_PROXY_PORT || "7891"}`;
...
const botTarget =
  process.env.HIVEX_AGENT_TARGET ||
  `http://127.0.0.1:${process.env.HIVEX_AGENT_PORT || "8820"}`;
const botEntry = {
  target: botTarget,
  changeOrigin: true,
