...
export type JoinInviteErrorCode =
  ...
  // wire — see cmd/hivebot/share_join_guard.go).
  | "passcode_required"
...
// Keep this set in sync with cmd/hivebot/share.go writeShareJoinError. Codes
    ...
    return {
      ...
      message:
        err instanceof Error && err.message
          ...
          : "Could not reach hivebot. Check your network and try again.",
    };
  }
...
async function interpretJoinResponse(
  ...
  if (response.ok && body === null) {
    return {
      ...
      code: "unknown",
      message:
        "hivebot accepted the invite but the response was unreadable. Reload — you may already be in.",
    };
  }
  ...
  return {
    ok: false,
    code: "unknown",
    message: `hivebot returned an unexpected response (${response.status}). Ask the host for a new invite.`,
  };
}
