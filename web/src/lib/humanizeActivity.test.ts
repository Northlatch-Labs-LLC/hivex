...
describe("looksLikeRawToolPayload / humanizeActivity", () => {
  it("collapses raw tool-call JSON to Working…", () => {
    const raw =
      '[{"tool_name":"mcp__hivex-office__team_task","type":"tool_reference"}]';
  ...
  it("collapses MCP ids and snake_case tool names", () => {
    expect(humanizeActivity("running mcp__hivex_wiki_lookup")).toBe("Working…");
    expect(humanizeActivity("hivex_office_members")).toBe("Working…");
  });
...
describe("looksLikeRawToolPayload — runner exhaust", () => {
  it.each([
    ...
    "/Users/someone/Documents/hive/hivex/web/src/App.tsx",
    "App.css App.tsx assets index.css main.tsx",
    "running mcp__hivex-office__register_app",
  ])("collapses %j", (raw) => {
    expect(looksLikeRawToolPayload(raw)).toBe(true);
    expect(humanizeActivity(raw)).toBe("Working…");
  });
