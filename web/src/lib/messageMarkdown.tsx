 ...
 *   `data-hivex-mention="true"` hProperty as the discriminator. The anchor
 *   renderer detects that data attribute and emits a styled
 *   <span class="mention"> chip — never an <a> with a clickable href.
 *   (Note: an earlier draft used a `hivex-mention:` URL scheme, but
 ...
 * link nodes carrying a `data-hivex-mention="true"` hProperty (the URL is
...
function buildAppRefReplacements(value: string): MdAnyNode[] {
  ...
  for (const m of matches) {
    ...
    out.push({
      ...
      data: { hProperties: { "data-hivex-app": "true", "data-app-id": id } },
    });
...
function buildTaskRefReplacements(value: string): MdAnyNode[] {
  ...
  for (const m of matches) {
    ...
    out.push({
      ...
      url: "#",
      children: [{ type: "text", value: id }],
      data: { hProperties: { "data-hivex-task": "true", "data-task-id": id } },
    });
    cursor = start + id.length;
  }
  if (cursor === 0) return [];
...
function buildMentionReplacements(value: string): MdAnyNode[] {
  ...
  for (const m of matches) {
    ...
    // anchor renderer below checks data-hivex-mention to swap to a chip.
    out.push({
      type: "link",
      url: "#",
...
export const messageMarkdownComponents: Partial<Components> = {
  a: (props: AnchorProps): ReactElement => {
    ...
    if (record["data-hivex-mention"] === "true") {
      // Mention chip — never a navigable link.
      return <span className="mention">{children}</span>;
    }
    if (record["data-hivex-app"] === "true") {
    ...
    if (record["data-hivex-task"] === "true") {
