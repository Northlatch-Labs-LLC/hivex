...
> {
  ...
  componentDidCatch(error: Error, info: { componentStack?: string | null }) {
    ...
    console.error("[hivebot ErrorBoundary]", error, info);
    ...
    if (isChunkError && typeof window !== "undefined") {
      const key = "hivex:chunk-reload-attempted";
  ...
  return (
    ...
    >
      <strong style={{ fontSize: 16, color: "var(--text)" }}>
        hivebot can&rsquo;t reach the office broker — retrying…
      </strong>
    ...
    initApi()
      ...
      .catch((err: unknown) => {
        ...
        if (isBrokerUnreachableError(err)) {
          ...
          // eslint-disable-next-line no-console
          console.warn(
            `[hivebot boot] broker unreachable (attempt ${bootAttempt + 1})`,
            err,
          );
          unreachable = true;
          setBootError(true);
