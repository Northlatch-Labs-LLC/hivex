...
import { agentName, reloadBotNames, setBotName } from "./botNames";
...
describe("agentNames", () => {
  ...
  it("falls back to the given name with no override", () => {
    expect(agentName("app_x", "Pipeline Bot")).toBe("Pipeline Bot");
  });

  it("persists a rename and applies it", () => {
    setBotName("app_x", "Revenue Radar");
    expect(agentName("app_x", "Pipeline Bot")).toBe("Revenue Radar");
  });
  ...
  it("keeps the rename in memory when localStorage.setItem throws", () => {
    ...
    expect(localStorage.getItem("hivex.operator.agentNames")).toBeNull();
    // …but only the cross-reload copy is lost — the in-memory rename applies.
    expect(agentName("app_x", "Pipeline Bot")).toBe("Revenue Radar");
  });

  // Real bots (app_ ids) ALSO persist the rename to the broker,
  ...
  describe("broker persistence", () => {
    ...
    it("does not PATCH for a mock bot id", () => {
      const fetchMock = vi.fn();
      vi.stubGlobal("fetch", fetchMock);
      setBotName("inbound-routing", "Revenue Radar");
