// botStateClient — typed client for the bot's persisted state, split
...
//     bot service, reached via the /bot vite proxy.
//
// House pattern: every bot-service call
...
// the FE keeps working offline. All bot POST bodies carry schema_version.
...
// ── Wire shapes (mirror the bot service) ──────────────────────────────────
...
interface BotFetchInit {
  method?: "GET" | "POST" | "PATCH";
  body?: Record<string, unknown>;
  timeoutMs: number;
}

async function botFetch<T>(path: string, init: BotFetchInit): Promise<T> {
...
export async function listBotTools(agent: string): Promise<WireTool[]> {
  const data = await botFetch<{ tools: WireTool[] }>(`/tools?${q(agent)}`, {
 ...
 * against the bot service within one tick (~20s). The outcome lands in the
...
export async function listSessions(agent: string): Promise<WireSession[]> {
  const data = await botFetch<{ sessions: WireSession[] }>(
    `/sessions?${q(agent)}`,
    { timeoutMs: READ_TIMEOUT_MS },
  );
...
export async function getSession(
  ...
  const data = await botFetch<SessionDetail>(
...
export async function createSession(
  ...
  const data = await botFetch<{ session: WireSession }>("/sessions", {
    method: "POST",
    body,
...
export async function listArtifacts(agent: string): Promise<WireArtifact[]> {
  const data = await botFetch<{ artifacts: WireArtifact[] }>(
    `/artifacts?${q(agent)}`,
    { timeoutMs: READ_TIMEOUT_MS },
  );
...
export function tryListBotTools(agent: string): Promise<WireTool[] | null> {
  return orNull(listBotTools(agent));
}
