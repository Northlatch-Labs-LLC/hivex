// Bot display names. Operators can RENAME a bot; overrides live
// client-side (localStorage) as the optimistic cache and apply everywhere a
// name renders (detail header, sidebar rail, lists). For a REAL bot (app_…
...
const KEY = "hivex.operator.agentNames";
...
export function setBotName(id: string, name: string): void {
  ...
  cache = next;
  notify();
  // Real bots also persist the rename on the broker. Fire-and-forget: the
...
export function reloadBotNames(): void {
  cache = read();
  notify();
}
...
/** Reactive display name for a bot: the operator's rename, else the given. */
export function useBotName(id: string, fallback: string): string {
...
/** The whole reactive override map (for lists that name many bots). */
export function useBotNames(): Readonly<Record<string, string>> {
