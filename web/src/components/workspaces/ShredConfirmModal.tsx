...
export function ShredConfirmModal({
  ...
  // the legacy ~/.hivex/ location after the symmetric multi-workspace
  // migration moves the main workspace under ~/.hivex-spaces/main/.hivex.
  const mainPath = workspace.runtime_home
    ? `${workspace.runtime_home}/.hivex`
    : "~/.hivex-spaces/main/.hivex";
  const introCopy = isMain ? (
    ...
    <>
      ...
      <code>hivex workspace restore</code> or the undo toast that appears next.
    </>
  );
