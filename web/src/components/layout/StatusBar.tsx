...
export function StatusBar() {
  ...
  const versionTitle = (() => {
    ...
    switch (versionStatus.kind) {
      case "ok":
        return `hivebot ${versionLabel} — up to date`;
      case "outdated":
        return latest
          ? `hivebot ${versionLabel} — update available (${latest})`
          : `hivebot ${versionLabel} — update available`;
      case "dev":
        return `hivebot ${versionLabel} — dev build`;
      case "error":
        return `hivebot ${versionLabel} — version check failed`;
      case "loading":
        return `hivebot ${versionLabel} — checking for updates…`;
      default:
        return `hivebot ${versionLabel}`;
    }
  })();
