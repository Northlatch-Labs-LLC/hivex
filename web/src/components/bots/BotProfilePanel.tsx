...
interface BotProfilePanelProps {
   ...
   * e.g. inside BotSubspace's Config tab.
   */
  headless?: boolean;
}
...
/**
 * Statuses where a bot should be making progress but may have silently
...
function RecentArtifactsSection({ agentSlug, tasks }: RecentTasksSectionProps) {
  ...
  const resumeMutation = useMutation({
    ...
    onError: (err) => {
      showNotice(
        err instanceof Error ? err.message : "Failed to resume the bot.",
        "error",
      );
    },
  });
...
// RuntimeSection is the per-bot runtime picker — the surface the user task
// description calls out as "enable [provider selection] in the bot's
// settings in the UI." Three rendering paths:
//
//  1. Gateway-bound bot (kind ∈ {openclaw, openclaw-http, hermes-bot}):
...
//     the bot from its imported session. Editing a gateway-bound bot's
...
//     because the dispatch resolver will ignore the per-bot binding anyway.
...
function RuntimeSection({
  ...
  // Re-sync draft state when the bot prop swaps (e.g. user picks a
  // different bot in the sidebar without unmounting the panel). useState
  // initialisers run only on mount, so without this the picker would show
  // the previous bot's kind/model until a manual reset.
  useEffect(() => {
    setDraftKind(
      (binding.kind && llmKinds.includes(binding.kind as LLMRuntimeKind)
        ? (binding.kind as LLMRuntimeKind)
  ...
  // Lead bots (CEO, other built-ins) can have their runtime changed from
  ...
  // Gateway-bound bots are the only non-editable case: their gateway
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (draftKind === "") {
        // Empty kind means "clear per-bot binding, inherit default".
...
// anyway). Applies to every bot including the lead — the broker's
// built-in gate fires only on remove, not on update.
// Exported so BotSubspace's shell header can render it outside the panel.
export function EditableName({ agent }: { agent: OfficeMember }) {
  ...
  // component on a slug switch (key prop on BotPanelView), so we only
  // need to track name changes within the same bot. Use useEffect
