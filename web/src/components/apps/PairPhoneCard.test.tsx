...
vi.mock("../../api/client", () => ({
  pairingLink: () =>
    "hivebot://pair?url=http%3A%2F%2F100.64.0.5%3A7890&token=tok",
...
// The hivebot iOS app pairs by scanning this card. The code carries the
...
describe("PairPhoneCard", () => {
  it("renders a scannable code and a copyable link for the host", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByAltText("Pairing code for the hivebot iOS app"),
  ...
  it("does not show the code to a team member", () => {
    ...
    expect(
      screen.queryByAltText("Pairing code for the hivebot iOS app"),
    ).toBeNull();
  });
});
