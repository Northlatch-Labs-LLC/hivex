...
describe("routeInboundMessage (the load-bearing ordering invariant)", () => {
  ...
  it("routes a hivex-select to the callback and NEVER touches the broker", () => {
    const { frame, win } = makeFrame();
    const onSelect = vi.fn();
    routeInboundMessage(
      event(win, {
  ...
  it("routes a hivex-error to the callback and NEVER touches the broker", () => {
    const { frame, win } = makeFrame();
    const onErr = vi.fn();
    routeInboundMessage(
      event(win, {
...
describe("routeInboundMessage — Bridge v2 forwarding", () => {
  ...
  it("rejects an empty ai prompt WITHOUT touching the broker", () => {
    const { frame, win } = makeFrame();
    routeInboundMessage(
      event(win, { source: "hivex-app", type: "ai", id: 4, prompt: "  " }),
      frame,
      "*",
      { current: vi.fn() },
      { current: vi.fn() },
  ...
  it("forwards the host app_id on ai() and integration calls for per-app budgeting", async () => {
    const { frame, win } = makeFrame();
    routeInboundMessage(
      event(win, { source: "hivex-app", type: "ai", id: 5, prompt: "go" }),
      frame,
      "*",
      { current: vi.fn() },
      { current: vi.fn() },
...
describe("parseDownloadPayload (host-brokered download validation)", () => {
  const msg = (over: Record<string, unknown>) =>
    ({ source: "hivex-app", type: "download", id: 1, ...over }) as never;
