...
describe("RoutinesApp", () => {
  ...
  it("renders the list view by default and hides system routines", async () => {
    ...
    // System routine (hive-insights) is hidden behind the opt-in toggle.
    expect(
      screen.queryByTestId("routine-row-hive-insights"),
  ...
  it("never surfaces one-shot watchdog jobs, even with system routines shown", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("routine-row-hive-insights"),
      ).toBeInTheDocument(),
    );
  ...
  it("switches to calendar view and projects routine chips into the month grid", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getAllByTestId("routine-chip-hive-insights").length,
      ).toBeGreaterThan(0);
    });
  });
