...
// team rather than to back a hivebot-created bot's turns.
...
function LocalProviderCard({
  ...
  return (
    ...
    >
      ...
      <Field
        label="Base URL"
        hint={`HIVEX_${meta.kind.toUpperCase().replace(/-/g, "_")}_BASE_URL`}
      >
        <input
          style={styles.input}
      ...
      <Field
        label="Model"
        hint={`HIVEX_${meta.kind.toUpperCase().replace(/-/g, "_")}_MODEL`}
      >
        <input
          style={styles.input}
...
function LocalLLMsSection({ cfg, save }: SectionProps) {
  ...
  return (
    <div>
      ...
      <p style={styles.sectionDesc}>
        Run hivebot agents through a model on your own machine — no cloud key
  ...
  return (
    <div>
      ...
      <p style={styles.sectionDesc}>
        ...
        your local config file and never transmitted to hivebot servers. Enter a
...
const ENV_VARS: [string, string][] = [
  ["HIVEX_LLM_PROVIDER", "LLM provider override"],
  ["HIVEX_BROKER_PORT", "Broker port"],
  ["HIVEX_CONFIG_PATH", "Config file path override"],
  ["HIVEX_RUNTIME_HOME", "Runtime state directory"],
  ["HIVEX_START_FROM_SCRATCH", "Start without blueprint (1)"],
  ["HIVEX_ONE_ON_ONE", "Enable 1:1 mode (1)"],
  ["HIVEX_HEADLESS_PROVIDER", "Headless provider override"],
  ["HIVEX_INSIGHTS_INTERVAL_MINUTES", "Insights poll interval"],
  ["HIVEX_TASK_FOLLOWUP_MINUTES", "Task follow-up interval"],
  ["HIVEX_TASK_REMINDER_MINUTES", "Task reminder interval"],
  ["HIVEX_TASK_RECHECK_MINUTES", "Task recheck interval"],
];
...
function FlagsSection() {
  return (
    <div>
      ...
      <p style={styles.sectionDesc}>
        All flags available when launching hivebot from the terminal. These are
    ...
    <div>
      ...
      <div style={dangerStyles.card("warn")}>
        ...
        <ul style={dangerStyles.list}>
          <li>
            Broker runtime state (<code>~/.hivex/team/broker-state.json</code>)
          </li>
