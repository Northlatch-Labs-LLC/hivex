...
describe("<WikiAudit>", () => {
  ...
  it("renders every entry with author, message, and SHA", async () => {
    ...
    expect(screen.getByText("hivex: init wiki")).toBeInTheDocument();
  ...
  it("filters to bots only via the author dropdown", async () => {
    ...
    expect(screen.queryByText("hivex: init wiki")).not.toBeInTheDocument();
  });
