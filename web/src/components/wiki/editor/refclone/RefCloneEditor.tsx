...
/**
 * RefCloneEditor — hivebot host wrapper around the ported reference KB editor.
 *
 ...
 * API — every hivebot-owned concern (content + persistence, file upload, wiki-link
...
export default function RefCloneEditor({
  ...
  // ancestor/own class. hivebot drives theming via `data-theme` on the document;
  // mirror that here so the editor matches the host's light/dark surface.
  const theme = useAppStore((s) => s.theme);
  const isDark = theme !== "hive";
  ...
  // Upload via hivebot's multipart endpoint, then resolve to a servable URL.
