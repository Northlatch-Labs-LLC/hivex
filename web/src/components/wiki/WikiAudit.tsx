...
function isSystemSlug(slug: string): boolean {
  return (
    slug === "system" || slug === "hivex-bootstrap" || slug === "hivex-recovery"
  );
}

function authorTag(slug: string): string | null {
  if (slug === "hivex-bootstrap") return "bootstrap";
  if (slug === "hivex-recovery") return "recovery";
  if (slug === "system") return "system";
  return null;
}

function rowClass(slug: string): string {
  if (slug === "hivex-bootstrap") return "is-bootstrap";
  if (slug === "hivex-recovery") return "is-recovery";
...
function downloadCSV(entries: WikiAuditEntry[]): void {
  ...
  a.href = url;
  a.download = `hivex-wiki-audit-${new Date().toISOString().slice(0, 10)}.csv`;
