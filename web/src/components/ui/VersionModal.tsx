...
function deriveInstallCommand(check: UpgradeCheckResponse | undefined): string {
  ...
  return check?.upgrade_command ?? "npm install -g hivebot@latest";
}
      ...
      >
        ...
        <div className="help-body version-modal-body">
          ...
          <ActionsSection
            ...
            latestForCopy={formatVersion(check?.latest, "hivebot@latest")}
...
function errorHeadline(result: UpgradeRunResult): string {
  ...
  if (result.install_method === "unknown") {
    return "Couldn't detect how hivex was installed — run the command below from a terminal:";
  }
