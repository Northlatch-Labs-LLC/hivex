...
const _storedTheme = ((): Theme => {
  try {
    const v = localStorage.getItem("hivex-theme");
...
const SIDEBAR_SECTIONS_KEY = "hivex-sidebar-sections";
...
export interface AppStore {
  ...
  // Bot computers (SSE `computer` events, docs/specs/hivebot-bot-computers.md).
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  setTheme: (t) => {
    ...
    try {
      localStorage.setItem("hivex-theme", t);
    } catch (err) {
      console.warn(
