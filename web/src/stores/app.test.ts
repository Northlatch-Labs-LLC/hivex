...
describe("setTheme", () => {
  it("updates DOM + store even when localStorage.setItem throws", () => {
    ...
    try {
      useAppStore.getState().setTheme("hive-dark");

      expect(setItemSpy).toHaveBeenCalledWith("hivex-theme", "hive-dark");
      expect(useAppStore.getState().theme).toBe("hive-dark");
      ...
      document.documentElement.setAttribute("data-theme", "hive");
      useAppStore.setState({ theme: "hive" });
    }
  });
});
