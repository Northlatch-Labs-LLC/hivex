...
describe("archiveName", () => {
  test("Windows release name uses .zip + windows/amd64", () => {
    setPlatform("win32", "x64");
    expect(archiveName("0.9.0")).toBe("hivebot_0.9.0_windows_amd64.zip");
  });

  test("Windows arm64 release name uses .zip + windows/arm64", () => {
    setPlatform("win32", "arm64");
    expect(archiveName("1.2.3")).toBe("hivebot_1.2.3_windows_arm64.zip");
  });

  test("darwin release name uses .tar.gz", () => {
    setPlatform("darwin", "arm64");
    expect(archiveName("0.9.0")).toBe("hivebot_0.9.0_darwin_arm64.tar.gz");
  });

  test("linux release name uses .tar.gz", () => {
    setPlatform("linux", "x64");
    expect(archiveName("0.9.0")).toBe("hivebot_0.9.0_linux_amd64.tar.gz");
  });
});

describe("binaryFilename", () => {
  test("returns hivebot.exe on Windows (CreateProcess requires the .exe suffix)", () => {
    setPlatform("win32", "x64");
    expect(binaryFilename()).toBe("hivebot.exe");
  });

  test("returns bare hivebot on Unix", () => {
    setPlatform("darwin", "arm64");
    expect(binaryFilename()).toBe("hivebot");
    setPlatform("linux", "x64");
    expect(binaryFilename()).toBe("hivebot");
  });
});
