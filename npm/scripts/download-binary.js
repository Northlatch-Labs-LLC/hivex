...
// Downloads the hivebot binary that matches the current package version
// from the corresponding GitHub release and extracts it into bin/.
// GoReleaser archive name: hivebot_<version>_<os>_<arch>.tar.gz
...
//
// Verification flow:
//   1. Download the per-platform archive (hivebot_<ver>_<os>_<arch>.tar.gz).
...
// token replaces the tarball, the mismatch causes `npm install hivebot` to fail
...
const REPO = "Northlatch-Labs-LLC/hivebot";
...
function detectPlatform() {
  ...
  if (!osMap[platform]) {
    throw new Error(
      `Unsupported platform: ${platform}. hivebot supports darwin, linux, and windows.`,
    );
  }
  if (!archMap[arch]) {
    throw new Error(
      `Unsupported architecture: ${arch}. hivebot supports x64 (amd64) and arm64.`,
    );
  }
...
function archiveName(version) {
  ...
  return `hivebot_${version}_${goOs}_${goArch}.${archiveExtension()}`;
}

// Go names Windows binaries with a .exe suffix; the rest get the bare name.
function binaryFilename() {
  return process.platform === "win32" ? "hivebot.exe" : "hivebot";
}
...
async function verifyArchive({ version, archivePath, archiveBasename, silent }) {
  ...
  if (!silent) {
    process.stderr.write(`hivebot: verifying ${archiveBasename} against ${CHECKSUMS_FILENAME}\n`);
  }
...
//                 recorded in package.json. Used by bin/hivex.js to fetch a
...
//                 bin/hivex inside this package. The out-of-tree cache uses
...
async function downloadBinary({ silent = false, version, targetPath } = {}) {
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "hivex-"));
  const archivePath = path.join(tmpDir, archiveBasename);

  try {
    if (!silent) {
      process.stderr.write(`hivex: downloading ${url}\n`);
    }
