...
// Queries npm for the published `latest` hivex version, with a 24h on-disk
// cache so we don't hammer the registry on every CLI invocation.
//
// Why this exists: `npm install -g hivex` does not auto-upgrade. A user who
// installed weeks ago runs their old binary forever unless they manually
// re-install. The shim in bin/hivex.js uses this to transparently serve the
// *latest* release from a verified cache while pointing the user at a real
// fix (`npm install -g hivex@latest`). See bin/hivex.js for how the result
...
const REGISTRY_URL = "https://registry.npmjs.org/hivebot/latest";
...
function cacheDir() {
  // Sits under the active hivebot runtime home so workspace/dev overrides get
  // separate caches from prod.
  const runtimeHome = process.env.HIVEX_RUNTIME_HOME?.trim() || os.homedir();
  return path.join(runtimeHome, ".hivex", "cache");
}
