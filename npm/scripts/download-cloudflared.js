...
//      current platform isn't listed, exit 0 silently — hivex core install
...
async function downloadCloudflared({ silent = false } = {}) {
  ...
  if (!manifestKey) {
    if (!silent) {
      process.stderr.write(
        `hivex: cloudflared not bundled for ${process.platform}-${process.arch}; ` +
  ...
  if (!entry) {
    if (!silent) {
      process.stderr.write(
        `hivex: no cloudflared asset pinned for ${manifestKey}; ` +
          `the Public Tunnel feature will report it missing at runtime.\n`,
      );
    }
    return null;
  ...
  const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "hivex-cf-"));
  const downloadPath = path.join(tmpDir, asset);

  try {
    if (!silent) {
      process.stderr.write(
        `hivex: downloading cloudflared ${manifest.version} (${asset})\n`,
      );
    }
    ...
    if (!silent) {
      process.stderr.write(
        `hivex: cloudflared ${manifest.version} installed at ${target}\n`,
      );
    }
    return target;
  } finally {
