...
// Thin shim that spawns the native hivebot binary.
//
...
//
// Escape hatches:
//   HIVEX_BINARY=/path/to/hivex         — use a specific binary.
//   HIVEX_SKIP_VERSION_CHECK=1          — never query npm, always run the
...
const binaryName = process.platform === "win32" ? "hivebot.exe" : "hivebot";
...
function cachedBinaryPath(version) {
  ...
  const runtimeHome = process.env.HIVEX_RUNTIME_HOME?.trim() || os.homedir();
  return path.join(
    runtimeHome,
    ".hivex",
    "cache",
    "binaries",
    `hivebot-${version}${suffix}`,
  );
}
...
async function ensureBinary() {
  if (process.env.HIVEX_BINARY && fs.existsSync(process.env.HIVEX_BINARY)) {
    return process.env.HIVEX_BINARY;
  }

  const installed = await resolveInstalledBinary();
  if (process.env.HIVEX_SKIP_VERSION_CHECK === "1") return installed;
  ...
  if (!fs.existsSync(cachedPath)) {
    try {
      ...
      process.stderr.write(
        `hivebot: self-heal download of v${latestVersion} failed: ${err.message}\n` +
          `hivebot: running installed v${installedVersion}. ` +
          `Run \`npm install -g hivebot@latest\` to upgrade.\n`,
      );
      return installed;
    }
  }
  ...
  process.stderr.write(
    `hivebot: serving cached v${latestVersion} (installed is v${installedVersion}). ` +
      `Run \`npm install -g hivebot@latest\` to upgrade permanently, ` +
      `or set HIVEX_SKIP_VERSION_CHECK=1 to disable this check.\n`,
  );
  return cachedPath;
}
...
function run(resolvedPath) {
  ...
  child.on("error", (err) => {
    process.stderr.write(`hivebot: failed to launch binary: ${err.message}\n`);
    process.exit(1);
  });
}
...
ensureBinary()
  .then(run)
  .catch((err) => {
    process.stderr.write(`hivebot: ${err.message}\n`);
    process.exit(1);
  });
