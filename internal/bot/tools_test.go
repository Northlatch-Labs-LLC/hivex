...
func makeTestTool(name string) bot.BotTool {
	return bot.BotTool{
		Name:        name,
...
func builtinTool(t *testing.T, name string) bot.BotTool {
	t.Helper()
	for _, tool := range bot.CreateBuiltinTools() {
		if tool.Name == name {
			return tool
		}
	}
...
func TestSendMessageToolWritesOutbox(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(home, ".hivex", "office", "messages", "outbox.jsonl"))
	if err != nil {
