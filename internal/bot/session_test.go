...
func newTempStore(t *testing.T) *bot.SessionStore {
	t.Helper()
	dir, err := os.MkdirTemp("", "hive-session-*")
	if err != nil {
		t.Fatalf("create temp dir: %v", err)
	}
	t.Cleanup(func() { os.RemoveAll(dir) })
	return bot.NewSessionStoreAt(dir)
}
...
func TestSessionCreateAppendRead(t *testing.T) {
	...
	e1, err := store.Append(id, bot.SessionEntry{Type: "user", Content: "hello"})
	if err != nil {
		t.Fatalf("Append user: %v", err)
	}
	e2, err := store.Append(id, bot.SessionEntry{Type: "assistant", Content: "world"})
	if err != nil {
...
func TestSessionGetHistoryLimit(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		store.Append(id, bot.SessionEntry{Type: "user", Content: "msg"})
	}
...
func TestSessionGetHistoryFromID(t *testing.T) {
	...
	e1, _ := store.Append(id, bot.SessionEntry{Type: "user", Content: "a"})
	store.Append(id, bot.SessionEntry{Type: "assistant", Content: "b"})
	store.Append(id, bot.SessionEntry{Type: "user", Content: "c"})
