...
// SessionStore manages JSONL-based session files for bots.
type SessionStore struct {
	baseDir string
}

// NewSessionStore creates a store rooted at ~/.hivex/sessions by default.
func NewSessionStore() (*SessionStore, error) {
	home := config.RuntimeHomeDir()
	if home == "" {
		return nil, fmt.Errorf("get home dir: cannot resolve runtime home directory")
	}
	return &SessionStore{baseDir: filepath.Join(home, ".hivex", "sessions")}, nil
}
...
// Session ID format: "<botSlug>_<uuid>"
...
// slugFromID extracts the bot slug from a session ID.
...
func (s *SessionStore) Create(botSlug string) (string, error) {
	sessionID := botSlug + "_" + uuid.NewString()
	dir := filepath.Join(s.baseDir, botSlug)
...
// ListSessions returns all session IDs for the given bot slug.
func (s *SessionStore) ListSessions(botSlug string) ([]string, error) {
	dir := filepath.Join(s.baseDir, botSlug)
