...
func TestBuildManifest_PreservesFrontmatter(t *testing.T) {
	...
	got, err := BuildManifest(fm, body, "hivex", at)
	if err != nil {
	...
	if got.Source != "hivex-hivex-deploy-frontend" {
		t.Fatalf("source: got %q want %q", got.Source, "hivex-hivex-deploy-frontend")
	}
...
func TestBuildManifest_DefaultsLicenseAndVersion(t *testing.T) {
	...
	if got.Source != "hivex-x" {
		t.Fatalf("source without repo: got %q want hivex-x", got.Source)
	}
	if got.PublishedAt == "" {
...
func TestBuildManifest_RequiresNameAndDescription(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if _, err := BuildManifest(tc.fm, "body", "hivex", time.Now()); err == nil {
...
func TestHubURL_GithubScheme(t *testing.T) {
	t.Parallel()
	got, err := HubURL("github:Northlatch-Labs-LLC/hivex-skills", "review-pr")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestHubURL_GithubScheme_BranchOverride(t *testing.T) {
	t.Parallel()
	got, err := HubURL("github:Northlatch-Labs-LLC/hivex-skills@master", "review-pr")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPublishBranchName(t *testing.T) {
	...
	want := "hivex-publish-deploy-frontend-1777377600"
	if got != want {
