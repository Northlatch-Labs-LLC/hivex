// Package skillpublish carries the pure logic for turning a compiled hivebot
...
// out to `gh` from the cmd/hivebot layer so we never roll our own GitHub API
// client.
...
// Manifest is the wire-shape hivebot emits per published skill. Frontmatter
...
// of the form `hivex-{repo}-{slug}`; PublishedAt is RFC3339 UTC.
//
// JSON tags are kept stable: the manifest is the durable contract a hub
// indexer can rely on if/when hivebot adds a hub-side companion service.
...
//
// Source format `hivex-{repo}-{slug}` is the provenance string a hub indexer
// can use to dedupe — passing repo="" yields `hivex-{slug}` so the prefix is
...
//
// The returned URL is suitable for `hivex skills install` — drop in `name`
...
// Format: `hivex-publish-{slug}-{unix-timestamp}`. The unix timestamp keeps
...
func PublishBranchName(slug string, t time.Time) string {
	...
	return fmt.Sprintf("hivex-publish-%s-%d", slug, t.UTC().Unix())
}
