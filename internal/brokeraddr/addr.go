...
// frozen — downstream packages (internal/team, internal/teammcp, cmd/hivebot)
...
func defaultTokenFile() string {
	if runtime.GOOS == "windows" {
		return filepath.Join(os.TempDir(), "hivex-broker-token")
	}
	return "/tmp/hivex-broker-token"
}

func tokenFileForPort(port int) string {
	if runtime.GOOS == "windows" {
		return filepath.Join(os.TempDir(), fmt.Sprintf("hivex-broker-token-%d", port))
	}
	return fmt.Sprintf("/tmp/hivex-broker-token-%d", port)
}
...
func ResolvePort() int {
	for _, key := range []string{"HIVEX_BROKER_PORT"} {
...
func ResolveTokenFile() string {
	for _, key := range []string{"HIVEX_BROKER_TOKEN_FILE"} {
