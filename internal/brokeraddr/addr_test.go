...
// On Unix the historical /tmp/hivex-broker-token path is the contract — keep
...
func TestResolveTokenFile_RespectsEnvOverride(t *testing.T) {
	t.Setenv("HIVEX_BROKER_TOKEN_FILE", filepath.Join(t.TempDir(), "custom-token"))
	t.Setenv("HIVEX_BROKER_PORT", "")
	t.Setenv("HIVEX_BROKER_BASE_URL", "")
	t.Setenv("HIVEX_TEAM_BROKER_URL", "")
	want := os.Getenv("HIVEX_BROKER_TOKEN_FILE")
...
func TestResolveTokenFile_NonDefaultPort_UsesPortVariant(t *testing.T) {
	...
	t.Setenv("HIVEX_BROKER_PORT", "8000")
	t.Setenv("HIVEX_BROKER_BASE_URL", "")
	t.Setenv("HIVEX_TEAM_BROKER_URL", "")
