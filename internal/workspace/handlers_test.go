...
func TestShredHandlerReportsRemovedPaths(t *testing.T) {
	...
	// we do require that the hivex home surfaces somewhere in the list.
	if len(removed) == 0 {
		t.Fatalf("expected non-empty removed list")
	}
	wantSubstring := filepath.Join(dir, ".hivex")
	var found bool
...
func TestShredHandlerOnEmptyHomeIsOK(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(dir, ".hivex"), 0o700); err != nil {
