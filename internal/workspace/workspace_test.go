...
// seedWorkspace builds a realistic ~/.hivex tree under dir and returns the
...
// pointing HIVEX_RUNTIME_HOME at a t.TempDir().
...
func TestClearRuntimeMatchesResetAt(t *testing.T) {
	...
	resB, err := ResetAt(filepath.Join(dir2, ".hivex"))
	if err != nil {
...
func TestShredMatchesShredAt(t *testing.T) {
	...
	// no env-override paths are pointing outside the hivexHome tree.
	dir1 := withRuntimeHome(t)
	seedWorkspace(t, dir1)
	resA, err := Shred()
	if err != nil {
	...
	dir2 := withRuntimeHome(t)
	seedWorkspace(t, dir2)
	resB, err := ShredAt(filepath.Join(dir2, ".hivex"))
	if err != nil {
