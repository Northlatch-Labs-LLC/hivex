...
// launcher-set HIVEX_AGENT_SLUG env is the trusted identity.

func TestResolveSlug_RejectsImpersonationViaArg(t *testing.T) {
	t.Setenv("HIVEX_AGENT_SLUG", "")
	for _, bad := range []string{
		"human", "you", "system", "broker", "librarian",
...
func TestResolveSlug_PrivilegedSlugOnlyFromTrustedEnv(t *testing.T) {
	// The launcher stamps HIVEX_AGENT_SLUG for the real librarian process; a
	// my_slug arg matching the trusted env slug is allowed, and env alone
	// (no arg) resolves to it.
	t.Setenv("HIVEX_AGENT_SLUG", "librarian")
	...
	t.Setenv("HIVEX_AGENT_SLUG", "executor")
