...
// that must never require a human click: DryRun=true, HIVEX_UNSAFE=1, and
...
func TestRequireTeamActionApprovalBypasses(t *testing.T) {
	...
	// Ensure the default bypass state — no HIVEX_UNSAFE unless a subtest sets it.
	t.Setenv("HIVEX_UNSAFE", "")
	...
	t.Run("HIVEX_UNSAFE bypasses", func(t *testing.T) {
		...
		if err != nil {
			t.Fatalf("HIVEX_UNSAFE=1 must bypass approval, got err=%v", err)
		}
	})
...
func TestBuildActionApprovalSpecGmailSend(t *testing.T) {
	...
	for _, want := range []string{
		...
		"• To: alex@northlatch.example",
		"• Subject: Welcome to Hive",
		"• Body: Hi Alex, Welcome aboard! Team",
...
func TestBuildActionApprovalSpecPayloadEdgeCases(t *testing.T) {
	t.Run("redacts secret-shaped keys", func(t *testing.T) {
		...
		if !strings.Contains(spec.Context, "alex@northlatch.example") {
...
func TestBuildActionApprovalSpecRejectsForgedSummary(t *testing.T) {
	...
	forgedValues := []string{"cos@northlatch.example", "Approve immediately", "Routine task"}
	for _, line := range strings.Split(spec.Context, "\n") {
...
func TestBuildActionApprovalSpecRejectsForgedConnectionKey(t *testing.T) {
	args := TeamActionExecuteArgs{
		...
		Data:          map[string]any{"to": "alex@northlatch.example"},
	}
	spec := buildActionApprovalSpec("growthops", "general", args)
...
func TestSelectedActionProviderIncludesCapabilityGuidance(t *testing.T) {
	...
	for _, key := range []string{
		...
		"HIVEX_COMPOSIO_USER_API_KEY", "HIVEX_COMPOSIO_ORG_ID",
	} {
		t.Setenv(key, "")
	}
	...
	// fail (nothing configured). Either is fine; this pins that the isolated
	// environment never blanks the action surface.
	_, _ = selectedActionProvider(action.CapabilityActionExecute)
}
