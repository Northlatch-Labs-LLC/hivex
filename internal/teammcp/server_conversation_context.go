...
func resolveChannelHint(input string) string {
	...
	if channel == "" {
		channel = normalizeChannelInput(os.Getenv("HIVEX_CHANNEL"))
	}
	return channel
}
...
// bot: the launcher stamps its identity into HIVEX_AGENT_SLUG, so the
...
//
// resolveChannel can only consult HIVEX_AGENT_SLUG, which the launcher sets on
