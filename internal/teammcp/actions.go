...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/action"
...
//  2. HIVEX_UNSAFE=1 bypasses the gate. The --unsafe launch flag sets this.
...
type TeamActionExecuteArgs struct {
	...
	MySlug          string         `json:"my_slug,omitempty" jsonschema:"Bot slug performing the action. Defaults to HIVEX_AGENT_SLUG."`
...
type TeamActionWorkflowCreateArgs struct {
	...
	DefinitionJSON   string   `json:"definition_json" jsonschema:"Full hivebot workflow JSON definition as a string"`
	Channel          string   `json:"channel,omitempty" jsonschema:"Optional office channel for logging"`
	MySlug           string   `json:"my_slug,omitempty" jsonschema:"Bot slug creating the workflow. Defaults to HIVEX_AGENT_SLUG."`
	Summary          string   `json:"summary,omitempty" jsonschema:"Optional short office log summary"`
	SkillName        string   `json:"skill_name,omitempty" jsonschema:"Optional hivebot skill name. Defaults to the workflow key."`
...
type TeamActionWorkflowExecuteArgs struct {
	...
	Channel   string         `json:"channel,omitempty" jsonschema:"Optional office channel for logging"`
	MySlug    string         `json:"my_slug,omitempty" jsonschema:"Bot slug executing the workflow. Defaults to HIVEX_AGENT_SLUG."`
	Summary   string         `json:"summary,omitempty" jsonschema:"Optional short office log summary"`
}
...
type TeamActionWorkflowScheduleArgs struct {
	...
	Channel    string         `json:"channel,omitempty" jsonschema:"Optional office channel for logging"`
	MySlug     string         `json:"my_slug,omitempty" jsonschema:"Bot slug scheduling the workflow. Defaults to HIVEX_AGENT_SLUG."`
	Summary    string         `json:"summary,omitempty" jsonschema:"Optional short office log summary"`
	SkillName  string         `json:"skill_name,omitempty" jsonschema:"Optional existing or new hivebot skill name to mirror this workflow"`
...
type TeamActionRelayCreateArgs struct {
	...
	Channel       string   `json:"channel,omitempty" jsonschema:"Optional office channel for logging"`
	MySlug        string   `json:"my_slug,omitempty" jsonschema:"Bot slug creating the relay. Defaults to HIVEX_AGENT_SLUG."`
	Summary       string   `json:"summary,omitempty" jsonschema:"Optional short office log summary"`
}
...
type TeamActionRelayActivateArgs struct {
	...
	Channel       string `json:"channel,omitempty" jsonschema:"Optional office channel for logging"`
	MySlug        string `json:"my_slug,omitempty" jsonschema:"Bot slug activating the relay. Defaults to HIVEX_AGENT_SLUG."`
	Summary       string `json:"summary,omitempty" jsonschema:"Optional short office log summary"`
}
...
func registerActionTools(server *mcp.Server) {
	...
	mcp.AddTool(server, officeWriteTool(
		...
		"Save a reusable external workflow from a full hivebot workflow JSON definition.",
	...
	mcp.AddTool(server, officeWriteTool(
		...
		"Schedule a saved external workflow on a hivebot-native cadence so it shows up in Calendar and runs through the team scheduler. Set run_now when the human also asked for an immediate first run.",
...
func handleTeamActionExecute(ctx context.Context, _ *mcp.CallToolRequest, args TeamActionExecuteArgs) (*mcp.CallToolResult, any, error) {
	...
	// needs to cover side-effecting actions. DryRun and HIVEX_UNSAFE bypass —
	...
	if !args.DryRun && os.Getenv("HIVEX_UNSAFE") != "1" && !actionIsReadOnly(args.ActionID) {
...
func workflowSkillContent(spec workflowSkillSpec) string {
	...
	lines := []string{
		fmt.Sprintf("hivebot workflow skill (%s): %s", label, humanizeWorkflowKey(fallbackString(spec.WorkflowKey, spec.Name))),
		"Use team_action_workflow_execute to run it through hivebot.",
	}
