...
type TeamBroadcastArgs struct {
	...
	MySlug    string   `json:"my_slug,omitempty" jsonschema:"Bot slug sending the message. Defaults to HIVEX_AGENT_SLUG."`
...
type TeamReactArgs struct {
	...
	MySlug    string `json:"my_slug,omitempty" jsonschema:"Bot slug. Defaults to HIVEX_AGENT_SLUG."`
}
...
type TeamPollArgs struct {
	...
	MySlug  string `json:"my_slug,omitempty" jsonschema:"Your bot slug so tagged_count can be computed. Defaults to HIVEX_AGENT_SLUG."`
...
type TeamStatusArgs struct {
	...
	MySlug  string `json:"my_slug,omitempty" jsonschema:"Bot slug sending the status. Defaults to HIVEX_AGENT_SLUG."`
}
...
type HumanInterviewArgs struct {
	...
	MySlug              string                 `json:"my_slug,omitempty" jsonschema:"Bot slug asking the question. Defaults to HIVEX_AGENT_SLUG."`
...
type HumanMessageArgs struct {
	...
	MySlug    string `json:"my_slug,omitempty" jsonschema:"Bot slug speaking to the human. Defaults to HIVEX_AGENT_SLUG."`
...
type TeamRequestsArgs struct {
	...
	MySlug          string `json:"my_slug,omitempty" jsonschema:"Your bot slug. Defaults to HIVEX_AGENT_SLUG."`
}
...
type TeamTaskArgs struct {
	...
	Owner                string   `json:"owner,omitempty" jsonschema:"Owner slug for assign. Ignored by claim, which always takes the task for you."`
...
type TeamBridgeArgs struct {
	...
	MySlug        string   `json:"my_slug,omitempty" jsonschema:"Bot slug performing the bridge. Defaults to HIVEX_AGENT_SLUG."`
...
type TeamMemberArgs struct {
	...
	OpenclawSessionKey string `json:"openclaw_session_key,omitempty" jsonschema:"Optional: attach to an existing OpenClaw session key (e.g. after hivebot reinstall). Leave empty to auto-create a new session."`
...
type TeamWikiWriteArgs struct {
	MySlug       string `json:"my_slug,omitempty" jsonschema:"Your bot slug. Defaults to HIVEX_AGENT_SLUG env."`
	...
	HumanRequest string `json:"human_request,omitempty" jsonschema:"Broker message ID for the recent human-authored request explicitly authorizing this direct wiki write. Required unless HIVEX_ENABLE_AGENT_WIKI_WRITE=true. For bot-authored knowledge not explicitly requested by the human, use notebook_write then notebook_promote instead."`
}
...
// TeamWikiLookupArgs is the contract for hivex_wiki_lookup.
