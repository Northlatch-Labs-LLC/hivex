...
// learning_tools.go defines the hivebot team learning MCP tools:
//
//   team_learning_record  — append one typed, scoped learning
//   team_learning_search  — retrieve prior learnings for the current task
//
// Registered only when HIVEX_MEMORY_BACKEND=markdown because the source of
...
type TeamLearningRecordArgs struct {
	...
	TaskID       string   `json:"task_id,omitempty" jsonschema:"Optional hivebot task ID this learning came from."`
