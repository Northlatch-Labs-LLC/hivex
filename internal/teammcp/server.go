...
func Run(ctx context.Context) error {
	...
	configureServerTools(server, resolveSlugOptional(""), strings.TrimSpace(os.Getenv("HIVEX_CHANNEL")), isOneOnOneMode())
	return server.Run(ctx, &mcp.StdioTransport{})
}

func adminDirectWikiWriteBypassEnabled() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("HIVEX_ENABLE_AGENT_WIKI_WRITE"))) {
	case "1", "true", "yes", "on":
		return true
	default:
		return false
