...
type ContextLookupArgs struct {
	...
	MySlug         string `json:"my_slug,omitempty" jsonschema:"Bot slug for task workflow attribution. Defaults to HIVEX_AGENT_SLUG."`
}
...
type ContextCaptureArgs struct {
	...
	MySlug       string `json:"my_slug,omitempty" jsonschema:"Bot slug writing the notebook entry. Defaults to HIVEX_AGENT_SLUG."`
...
type ContextPromoteArgs struct {
	...
	MySlug         string `json:"my_slug,omitempty" jsonschema:"Bot slug submitting the promotion. Defaults to HIVEX_AGENT_SLUG."`
...
func registerContextTools(server *mcp.Server) {
	mcp.AddTool(server, readOnlyTool(
		"context_lookup",
		"Backend-neutral context lookup. Returns typed citations plus a readable summary from the active hivebot context backend.",
	), handleContextLookup)
