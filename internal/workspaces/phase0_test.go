...
// Each entry must have a "user-global; intentionally NOT under HIVEX_RUNTIME_HOME"
...
var allowedFiles = map[string]string{
	...
	// session there too; hivebot only locates the binary, so the runtime home
	// is the wrong root. HIVEX_BOX_CLI_DIR overrides it for tests.
	...
	// npm install detection — walks up from user real HOME, not hivebot state.
	...
	"cmd/hivex-oc-probe/main.go": "OpenClaw identity is device-bound credentials, not workspace state",

	// Cross-workspace registry root — ~/.hivex-spaces/ is the SHARED orchestration
	// directory (registry.json, tokens/, <name>/.hivex/, .backups/). It must live
	// at the user's REAL home, not under any single workspace's HIVEX_RUNTIME_HOME,
	// or sibling brokers cannot find each other.
	"internal/workspaces/registry.go":    "spacesDir — ~/.hivex-spaces is shared cross-workspace, lives at real HOME",
	"internal/workspaces/migration.go":   "MigrateToSymmetric operates on legacy ~/.hivex at real HOME, not HIVEX_RUNTIME_HOME",
	...
	for scanner.Scan() {
		...
		if !matched {
			t.Errorf("PHASE0 AUDIT FAILURE: unexpected os.UserHomeDir()/os.Getenv(\"HOME\") call in %q\n"+
				...
				"  'user-global; intentionally NOT under HIVEX_RUNTIME_HOME' comment\n"+
