...
//	symlink:missing                       — recreate ~/.hivex compatibility symlink
...
// values are produced by translateDoctorReport in cmd/hivebot/workspaces_adapter.go
...
func FixDoctorIssue(ctx context.Context, fixID string) error {
	...
	case "migration":
		if rest == "partial" {
			return fmt.Errorf("%w: ~/.hivex is a regular directory while ~/.hivex-spaces/main exists; "+
...
// fixOrphanTreeRegister registers an existing tree under ~/.hivex-spaces/<name>
...
func fixOrphanTreeRegister(treePath string) error {
	...
	// Refuse to register paths outside ~/.hivex-spaces — this is the only
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			...
			if err != nil {
				...
				return fmt.Errorf("workspaces: orphan-register %q: registry and backup both unreadable — run 'hivex workspace doctor --dry-run' for manual recovery: %w", name, err)
			}
		}
	}
...
// fixOrphanTreeDelete moves the orphan tree to ~/.hivex-spaces/.backups/ as
...
// fixSymlinkMissing creates the ~/.hivex → ~/.hivex-spaces/main/.hivex
...
func fixSymlinkMissing() error {
	...
	if lerr == nil {
		if info.Mode()&os.ModeSymlink == 0 {
			...
			return fmt.Errorf("%w: ~/.hivex is a regular path, not a symlink — see docs/multi-workspace.md",
...
// symlinkPaths resolves the user's REAL home (NOT HIVEX_RUNTIME_HOME), the
// ~/.hivex symlink path, and the expected target ~/.hivex-spaces/main/.hivex.
//
// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — the compatibility
// symlink lives at the user's real home alongside ~/.hivex-spaces/, which is
...
// HIVEX_RUNTIME_HOME is set (tests, dev isolation, workspace overrides).
func symlinkPaths() (home, symlinkPath, expectedTarget string, err error) {
	...
	return home, filepath.Join(home, ".hivex"), filepath.Join(sd, "main", ".hivex"), nil
}
