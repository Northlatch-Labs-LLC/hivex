...
// real hivex binary (must be pre-built as ./hivex-test-bin relative to repo
// root, or the test skips) with HIVEX_RUNTIME_HOME pointing at a temp
// directory, waits for the broker health endpoint to respond, then tears
// down and asserts that no files under $HOME/.hivex/ were created or modified
// during the run.
//
// To run locally:
//
//	go build -o hivex-test-bin ./cmd/hivebot && \
//	  go test ./internal/workspaces/ -run TestRuntimeHomeIsolation -v -timeout 60s
//
// The test is skipped in normal `go test ./...` runs unless HIVEX_TEST_BIN is
// set (pointing at the pre-built binary) or the file ./hivex-test-bin exists
// at the repo root.
//
...
//     the assumption is that real hivebot state writes are small registry /
...
// hivexTestBinary returns the path to the hivex binary for integration tests,
// or "" if neither HIVEX_TEST_BIN nor ./hivex-test-bin exist.
func hivexTestBinary(root string) string {
	if v := strings.TrimSpace(os.Getenv("HIVEX_TEST_BIN")); v != "" {
		return v
	}
	candidate := filepath.Join(root, "hivex-test-bin")
...
//
// It spawns a real hivex binary with HIVEX_RUNTIME_HOME=<tmpdir>, waits for
// the broker health endpoint, tears down, and asserts that no files under
// $HOME/.hivex/ were created or modified during the run.
func TestRuntimeHomeIsolation(t *testing.T) {
	...
	bin := hivexTestBinary(root)
	if bin == "" {
		t.Skip("hivex binary not found — build with: go build -o hivex-test-bin ./cmd/hivebot\n" +
			"or set HIVEX_TEST_BIN=/path/to/hivex")
	}
	...
	realHivexDir := filepath.Join(realHome, ".hivex")

	// Snapshot the real ~/.hivex BEFORE starting the binary.
	before := snapshotDir(t, realHivexDir)
	...
	// real credentials (safest for a test binary run). HIVEX_RUNTIME_HOME
	// carries all hivebot state; HIVEX_GLOBAL_HOME is kept as real HOME so
	// headlessCodexGlobalHomeDir still resolves correctly in code paths that
	// need it — but no hivebot state should write there.
	fakeHome := filepath.Join(runtimeHome, "fake-home")
	if err := os.MkdirAll(fakeHome, 0o755); err != nil {
		t.Fatalf("mkdir fake home: %v", err)
	}
	cmd.Env = append(os.Environ(),
		"HIVEX_RUNTIME_HOME="+runtimeHome,
		"HOME="+fakeHome,
		"HIVEX_GLOBAL_HOME="+realHome,
	)
	...
	if err := cmd.Start(); err != nil {
		t.Fatalf("start hivex binary: %v", err)
	}
	...
	// Snapshot AFTER.
	after := snapshotDir(t, realHivexDir)

	// Assert no leak.
	diff := diffSnapshots(before, after)
	if diff != "" {
		t.Errorf("PHASE0 LEAK DETECTED: $HOME/.hivex/ was modified during isolated run.\n"+
			"HIVEX_RUNTIME_HOME was %q but state leaked to real home %q.\n\n"+
			"Changed files:\n%s\n"+
			"Process output:\n%s",
			runtimeHome, realHivexDir, diff, outBuf.String())
	}

	// Assert runtime home was actually written to (sanity check that the
	// binary is actually using HIVEX_RUNTIME_HOME and not falling back).
	if _, err := os.Stat(filepath.Join(runtimeHome, ".hivex")); os.IsNotExist(err) {
		t.Logf("WARNING: no .hivex dir found under runtimeHome %q — binary may not have fully initialized (check process output)", runtimeHome)
