...
func TestEnsureCacheBackupExclusionsDoesNotCreateMissingCaches(t *testing.T) {
	...
	cacheDir := filepath.Join(home, ".hivex-spaces", "main", ".hivex", "cache")
	reg := &Registry{
		Version:    Version,
		CLICurrent: "main",
		Workspaces: []*Workspace{{
			Name:        "main",
			RuntimeHome: filepath.Join(home, ".hivex-spaces", "main"),
...
func TestEnsureCacheBackupExclusionsAcceptsExistingCaches(t *testing.T) {
	...
	runtimeHome := filepath.Join(home, ".hivex-spaces", "main")
	cacheDir := filepath.Join(runtimeHome, ".hivex", "cache")
