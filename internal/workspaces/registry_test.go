...
// withSpacesDir sets HOME (and HIVEX_RUNTIME_HOME for any callers that hit
...
// ~/.hivex-spaces is shared cross-workspace and lives at the real user
// HOME, not under any single workspace's HIVEX_RUNTIME_HOME.
func withSpacesDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	t.Setenv("HOME", dir)
	t.Setenv("HIVEX_RUNTIME_HOME", dir)
	return filepath.Join(dir, ".hivex-spaces")
}
...
func TestUpdatePreservesOtherEntries(t *testing.T) {
	...
	if main.CompanyName != "Updated Hive" {
		t.Errorf("main.CompanyName: want %q, got %q", "Updated Hive", main.CompanyName)
	}
	if demo == nil {
