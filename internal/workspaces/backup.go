...
// under ~/.hivex-spaces/.backups/. The directory name encodes both the
...
//	  wiki/           — mirror of <hivexHome>/wiki/ (includes wiki/team/skills/)
//	  skills/         — duplicate of <hivexHome>/wiki/team/skills/ for quick access
//	  chats/          — mirror of <hivexHome>/sessions/
//	  context/        — remaining state, preserves relative paths within hivexHome
//
...
// categorySkipNames is the set of top-level hivexDir entries that get their
...
// hivexDir is moved into context/ preserving its relative layout, which keeps
...
// writeCategorizedBackup moves the contents of <runtimeHome>/.hivex/ into a
// fresh ~/.hivex-spaces/.backups/<name>-<unix-ts>/ tree, split across the
...
// Anything under hivexDir that is not wiki/ or sessions/ is moved into
...
func writeCategorizedBackup(target *Workspace) (backupRootResult string, returnErr error) {
	...
	hivexDir := filepath.Join(target.RuntimeHome, ".hivex")
	if _, err := os.Stat(hivexDir); err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return "", fmt.Errorf("workspaces: backup %q: no .hivex at %s", target.Name, hivexDir)
		}
	...
	wikiSrc := filepath.Join(hivexDir, "wiki")
	...
	sessionsSrc := filepath.Join(hivexDir, "sessions")
	...
	// context -> remaining hivexDir entries, layout preserved.
	// We enumerate hivexDir dynamically (rather than hard-coding paths) so the
	...
	entries, err := os.ReadDir(hivexDir)
	if err != nil {
		return backupRoot, fmt.Errorf("workspaces: backup %q: read hivex dir: %w", target.Name, err)
	}
	for _, entry := range entries {
		...
		src := filepath.Join(hivexDir, name)
...
// restoreCategorizedBackup reconstructs a fresh hivexHome at dstRuntimeHome
// from a categorized backup directory. Falls back to the legacy "flat tree
// with a .hivex/ child" layout that Doctor's orphan-cleanup produces.
//
...
func restoreCategorizedBackup(backupDir, dstRuntimeHome string) (returnErr error) {
	dstHivex := filepath.Join(dstRuntimeHome, ".hivex")

	// Legacy fallback: doctor's orphan-cleanup writes the whole runtime tree
	// flat (no manifest, .hivex/ as a direct child). Detect and rename the
	...
	if _, err := os.Stat(filepath.Join(backupDir, backupManifestFile)); errors.Is(err, os.ErrNotExist) {
		if _, err := os.Stat(filepath.Join(backupDir, ".hivex")); err == nil {
			return os.Rename(backupDir, dstRuntimeHome)
		}
	}

	if err := os.MkdirAll(dstHivex, 0o700); err != nil {
		return fmt.Errorf("workspaces: restore: mkdir hivex: %w", err)
	}
	...
	// wiki -> hivexHome/wiki. Treat genuine ENOENT as "nothing to restore";
	...
	case err == nil:
		if err := restoreMove(wikiBackup, filepath.Join(dstHivex, "wiki"), "wiki"); err != nil {
			return err
		}
	case errors.Is(err, os.ErrNotExist):
	...
	// chats -> hivexHome/sessions
	chatsBackup := filepath.Join(backupDir, backupChatsDir)
	switch _, err := os.Stat(chatsBackup); {
	case err == nil:
		if err := restoreMove(chatsBackup, filepath.Join(dstHivex, "sessions"), "chats"); err != nil {
			return err
		}
	case errors.Is(err, os.ErrNotExist):
		// no chats captured
	...
	// context/* -> hivexHome/*. context/ now mirrors the hivexDir top-level
	...
	case err == nil:
		for _, entry := range entries {
			...
			dst := filepath.Join(dstHivex, name)
