// Package workspaces manages the set of hivebot workspaces — the registry,
...
// Registry is the top-level structure stored in ~/.hivex-spaces/registry.json.
...
// hivebot instance with its own runtime home, ports, and lifecycle state.
...
// spacesDir returns ~/.hivex-spaces at the user's REAL home dir. This is the
...
// config.RuntimeHomeDir() here, a broker running with HIVEX_RUNTIME_HOME set
// would resolve spacesDir under its OWN runtime tree and never see siblings.
//
// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — the spaces
...
func spacesDir() (string, error) {
	...
	return filepath.Join(home, ".hivex-spaces"), nil
}
