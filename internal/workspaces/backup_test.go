...
// seedWorkspaceTree populates a fake .hivex/ tree under runtimeHome with one
// representative file per category so categorized backup + restore can be
// exercised end-to-end. Returns the hivexHome path.
func seedWorkspaceTree(t *testing.T, runtimeHome string) string {
	t.Helper()
	hivexHome := filepath.Join(runtimeHome, ".hivex")
	mustWrite := func(rel, body string) {
		t.Helper()
		full := filepath.Join(hivexHome, rel)
...
func TestShredCreatesCategorizedBackup(t *testing.T) {
	...
	hivexHome := seedWorkspaceTree(t, runtimeHome)
	...
	if _, err := os.Stat(hivexHome); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("hivexHome should be removed after shred; stat err=%v", err)
	}
	...
	// context/ preserves hivexHome-relative layout for everything else.
...
func TestRestoreFromCategorizedBackup(t *testing.T) {
	...
	// Reconstructed hivexHome should have the original layout back in place.
	restoredHivex := filepath.Join(sd, "rt-restore", ".hivex")
	expect := func(rel, wantBody string) {
		t.Helper()
		got, err := os.ReadFile(filepath.Join(restoredHivex, rel))
		if err != nil {
			t.Fatalf("read %s: %v", rel, err)
		}
		if string(got) != wantBody {
...
func TestShredRollbackOnPartialBackupFailure(t *testing.T) {
	...
	for _, rel := range []string{
		...
		if _, err := os.Stat(filepath.Join(hivexHome, rel)); err != nil {
