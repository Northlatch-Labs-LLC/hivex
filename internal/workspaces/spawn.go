...
// redirected to <runtimeHome>/.hivex/logs/broker.log.
//
...
func defaultSpawn(name string, runtimeHome string, brokerPort, webPort int) error {
	...
	logDir := filepath.Join(runtimeHome, ".hivex", "logs")
	...
	env := os.Environ()
	env = appendOrReplace(env, "HIVEX_RUNTIME_HOME", runtimeHome)
	env = appendOrReplace(env, "HIVEX_BROKER_PORT", strconv.Itoa(brokerPort))
	cmd.Env = env
...
// resolveBinary returns the path to the running hivex binary using
// os.Executable, falling back to runtimebin.LookPath("hivex").
func resolveBinary() (string, error) {
	if exe, err := os.Executable(); err == nil && exe != "" {
		return exe, nil
	}
	return runtimebin.LookPath("hivex")
}
