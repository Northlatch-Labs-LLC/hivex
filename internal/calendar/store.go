...
// CalendarStore persists schedules and events to ~/.hivex/calendar.json.
...
// If path is empty, defaults to ~/.hivex/calendar.json.
func NewCalendarStore(path string) *CalendarStore {
	if path == "" {
		home := config.RuntimeHomeDir()
		path = filepath.Join(home, ".hivex", "calendar.json")
	}
