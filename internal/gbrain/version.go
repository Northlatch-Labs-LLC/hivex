...
//
// Below it hivebot still works — the compensating code activates — but the
...
func VersionAdvisory(ctx context.Context) string {
	...
	return fmt.Sprintf(
		"gbrain %s predates %s: writes to a previously deleted page can be silently lost, "+
			"and page listing cannot paginate. hivebot compensates, at the cost of an extra call per write. "+
