...
// pins HIVEX_GBRAIN_COMMAND at the freshly installed binary so the rest of the
...
func EnsureInstalled(ctx context.Context, progress func(string)) error {
	...
	// HIVEX_GBRAIN_COMMAND first), so we do not depend on a PATH restart.
	if dir := bunBinDir(); dir != "" {
		gb := filepath.Join(dir, "gbrain")
		if st, err := os.Stat(gb); err == nil && !st.IsDir() {
			_ = os.Setenv("HIVEX_GBRAIN_COMMAND", gb)
		}
	}
	if !isInstalledFn() {
