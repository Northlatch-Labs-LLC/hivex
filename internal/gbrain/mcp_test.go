...
func TestClient_GetLinksMapsSlugFields(t *testing.T) {
	...
	c := startFakeClient(t, fakeServerConfig{
		getLinksText: `[{"from_slug":"chat-pricing","to_slug":"pricing-strategy","link_type":"related","link_source":"hivex-capture"}]`,
	})
	...
	l := links[0]
	if l.From != "chat-pricing" || l.To != "pricing-strategy" || l.Type != "related" || l.Source != "hivex-capture" {
...
func TestClient_AddLinkRequiresEndpoints(t *testing.T) {
	c := startFakeClient(t, fakeServerConfig{})
	if err := c.AddLink(context.Background(), "a", "b", "related", "hivex-capture", ""); err != nil {
...
// unless HIVEX_GBRAIN_IT=1. Requires an initialised brain on the host.
func TestIntegration_Identity(t *testing.T) {
	if os.Getenv("HIVEX_GBRAIN_IT") != "1" {
		t.Skip("set HIVEX_GBRAIN_IT=1 to run the live gbrain integration test")
	}
	if BinaryPath() == "" {
