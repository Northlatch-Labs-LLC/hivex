...
func TestBinaryPathExplicitCommandResolves(t *testing.T) {
	...
	t.Setenv("HIVEX_GBRAIN_COMMAND", wrapper)
...
func TestBinaryPathExplicitCommandMissingDoesNotFallBack(t *testing.T) {
	...
	t.Setenv("HIVEX_GBRAIN_COMMAND", filepath.Join(dir, "missing-wrapper.sh"))
	if got := BinaryPath(); got != "" {
		t.Fatalf("BinaryPath() = %q, want empty: explicit HIVEX_GBRAIN_COMMAND that does not resolve must not fall back to PATH", got)
	}
	if IsInstalled() {
...
func TestBinaryPathExplicitBareNameResolvesViaPath(t *testing.T) {
	...
	t.Setenv("HIVEX_GBRAIN_COMMAND", "gbrain-wrapper")
	if got := BinaryPath(); got != wrapper {
