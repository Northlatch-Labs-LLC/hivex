...
// (GBrain) and into the hivebot markdown wiki at ~/.hivex/wiki/team/.
//
// Why this exists
// ===============
//
// Existing GBrain installs store their team knowledge in those legacy
// backends. Moving to hivebot's markdown wiki without a migration path means
// walking away from that prior investment. This package closes the gap:
// `hivex memory migrate --from gbrain` walks the source, converts each
...
//   - The writer owns identity ("migrate" slug → `migrate <migrate@hivex.local>`
...
type MigrationRecord struct {
	...
	Content string
	// Source identifies the originating backend ("gbrain"). Used
