...
// State mirrors the full contents of ~/.hivex/onboarded.json.
type State struct {
...
// StatePath returns the absolute path to ~/.hivex/onboarded.json.
...
func StatePath() string {
	...
	if home == "" {
		return filepath.Join(".hivex", "onboarded.json")
	}
	return filepath.Join(home, ".hivex", "onboarded.json")
}

// Load reads and parses ~/.hivex/onboarded.json.
...
// Save atomically writes s to ~/.hivex/onboarded.json by first writing to a
