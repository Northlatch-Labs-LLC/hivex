...
func TestVerifyRuntimeNotInstalled(t *testing.T) {
	res := VerifyRuntime(context.Background(), "nonexistent-runtime-xyz-hivex")
	if res.Status != VerifyStatusNotInstalled {
		t.Fatalf("Status: got %q, want %q", res.Status, VerifyStatusNotInstalled)
	}
	if res.Runtime != "nonexistent-runtime-xyz-hivex" {
...
func TestHandleVerifyClassifiesBogusRuntime(t *testing.T) {
	tests := []struct {
		...
		{name: "json body", body: `{"runtime":"nonexistent-runtime-xyz-hivex"}`},
		{name: "query param", query: "runtime=nonexistent-runtime-xyz-hivex"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
