...
func TestCheckOneNonexistentBinary(t *testing.T) {
	r := CheckOne(context.Background(), "nonexistent-binary-xyz-hivex")
	if r.Found {
	...
	if r.Name != "nonexistent-binary-xyz-hivex" {
		t.Errorf("Name: got %q, want %q", r.Name, "nonexistent-binary-xyz-hivex")
	}
}
