...
// the duration of f, keeping test state isolated from the real ~/.hivex.
	...
	withTempHome(t, func(home string) {
		...
		for _, e := range entries {
			if e.Name() != "onboarded.json" {
				t.Errorf("unexpected file in .hivex dir after Save: %s", e.Name())
			}
		}
	})
}
