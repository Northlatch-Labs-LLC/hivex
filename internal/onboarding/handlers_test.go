...
// hivex package (which wires the embed FS) never runs in onboarding
...
func TestHandleAnswerPersistsOwnerEmail(t *testing.T) {
	withTempHome(t, func(_ string) {
		req := httptest.NewRequest(
			http.MethodPost,
			"/onboarding/answer",
			bytes.NewReader([]byte(`{"field":"owner_email","value":"  maya@northlatch.example  "}`)),
		)
		w := httptest.NewRecorder()
		...
		if s.FormAnswers.OwnerEmail != "maya@northlatch.example" {
			t.Fatalf("owner email = %q, want %q", s.FormAnswers.OwnerEmail, "maya@northlatch.example")
		}
	})
}
...
func TestHandleAnswerRejectsNonStringOwnerEmail(t *testing.T) {
	withTempHome(t, func(_ string) {
		...
		if s.FormAnswers.OwnerEmail != "maya@northlatch.example" {
			t.Fatalf("owner email was clobbered: got %q, want %q", s.FormAnswers.OwnerEmail, "maya@northlatch.example")
		}
	})
}
...
func TestHandleCompletePersistsAnalyticsConsent(t *testing.T) {
	withTempHome(t, func(home string) {
		// Isolate config writes from any process-leaked HIVEX_RUNTIME_HOME.
		t.Setenv("HIVEX_CONFIG_PATH", filepath.Join(home, ".hivex", "config.json"))
...
func TestHandleCompleteReturns500OnCompleteFnError(t *testing.T) {
	withTempHome(t, func(_ string) {
		const secretDetail = "secret-path-/etc/hivex/state.yaml"
...
func TestHandleBlueprintsMarksLeadBuiltIn(t *testing.T) {
	withTempHome(t, func(_ string) {
		...
		// niche-crm's blueprint yaml names `cos` as type: lead with
		...
		for _, tpl := range resp.Templates {
			...
			for _, a := range tpl.Bots {
				if a.BuiltIn {
					leadCount++
					if a.Slug != "cos" {
						t.Errorf("niche-crm lead should be cos, got %q (built_in=true)", a.Slug)
					}
				}
			}
			if leadCount == 0 {
