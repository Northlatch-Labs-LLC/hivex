...
// headless_workspace.go — working-directory resolution for headless bot
// turns (ICP-eval v3 fix family #3, V3-N5).
//
// The v3 live run had bots executing in the broker process's launch cwd:
...
//     per-bot scratch directory under the runtime home,
//     <HIVEX_RUNTIME_HOME>/.hivex/bot-scratch/<slug>/, created on demand.
//
...
// human launched hivex in their own directory on purpose.
...
func botScratchDir(slug string) string {
	...
	if home := config.RuntimeHomeDir(); home != "" {
		root = filepath.Join(home, ".hivex", "agent-scratch")
		migrateLegacyLeadSlugDir(root)
	} else {
		root = filepath.Join(os.TempDir(), "hivex-agent-scratch")
	}
	dir := filepath.Join(root, token)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		fallback := filepath.Join(os.TempDir(), "hivex-agent-scratch", token)
		if fallback != dir {
...
// the bot's scratch dir. The second return reports whether the directory
// is a real task worktree (callers use it to decide whether to advertise
// HIVEX_WORKTREE_PATH to the child process).
func (l *Launcher) headlessTurnWorkspace(slug, taskID string) (string, bool) {
	...
	return normalizeHeadlessWorkspaceDir(botScratchDir(slug)), false
}
