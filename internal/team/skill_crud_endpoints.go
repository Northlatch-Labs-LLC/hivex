...
// skill_crud_endpoints.go owns the HTTP handlers for the full hivebot skill CRUD
...
func (b *Broker) handleSkillPatch(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil {
		if _, _, err := wikiWorker.Enqueue(r.Context(), skCopy.Name, wikiPath, string(mdBytes), "replace", "hivex: patch skill "+skCopy.Name); err != nil {
...
func (b *Broker) handleSkillEdit(w http.ResponseWriter, r *http.Request, name string) {
	...
	if t := strings.TrimSpace(fm.Metadata.Hivex.Title); t != "" {
		sk.Title = t
	}
	...
	if wikiWorker != nil {
		if _, _, err := wikiWorker.Enqueue(r.Context(), skCopy.Name, wikiPath, string(mdBytes), "replace", "hivex: edit skill "+skCopy.Name); err != nil {
			http.Error(w, "wiki enqueue: "+err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
// hard-deletes; the SKILL.md is rewritten with metadata.hivex.status updated.
func (b *Broker) handleSkillArchive(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil {
		commitMsg := "hivex: archive skill " + skCopy.Name
...
func (b *Broker) handleSkillDisable(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil && renderErr == nil {
		if _, _, err := wikiWorker.Enqueue(context.Background(), skCopy.Name, wikiPath, string(mdBytes), "replace", "hivex: disable skill "+skCopy.Name); err != nil {
...
func (b *Broker) handleSkillEnable(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil && renderErr == nil {
		if _, _, err := wikiWorker.Enqueue(context.Background(), skCopy.Name, wikiPath, string(mdBytes), "replace", "hivex: enable skill "+skCopy.Name); err != nil {
...
func (b *Broker) handleSkillRestore(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil && renderErr == nil {
		if _, _, err := wikiWorker.Enqueue(context.Background(), skCopy.Name, wikiPath, string(mdBytes), "replace", "hivex: restore skill "+skCopy.Name); err != nil {
...
func (b *Broker) handleSkillWriteFile(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil {
		if _, _, err := wikiWorker.Enqueue(r.Context(), skName, wikiPath, body.FileContent, "replace", "hivex: write file "+cleanFile+" for skill "+skName); err != nil {
			http.Error(w, "wiki enqueue: "+err.Error(), http.StatusInternalServerError)
			return
		}
	}
...
func (b *Broker) handleSkillApprove(w http.ResponseWriter, r *http.Request, name string) {
	...
	if wikiWorker != nil && renderErr == nil {
		if _, _, err := wikiWorker.Enqueue(r.Context(), skCopy.Name, wikiPath, string(mdBytes), "replace", "hivex: approve skill "+skCopy.Name); err != nil {
...
func (b *Broker) reconcileSkillStatusFromDisk() {
	...
	for i := range b.skills {
		...
		diskStatus := strings.TrimSpace(fm.Metadata.Hivex.Status)
		if diskStatus == "" {
			continue
		}
		diskDisabledFromStatus := strings.TrimSpace(fm.Metadata.Hivex.DisabledFromStatus)
...
func (b *Broker) backfillSkillFilesFromState(ctx context.Context) {
	...
	for _, name := range candidates {
		...
		commitMsg := "hivex: backfill skill " + skCopy.Name + " from broker state"
...
func resolveSkillSourceArticle(b *Broker, name string) string {
	...
	if len(fm.Metadata.Hivex.SourceArticles) > 0 {
		if s := strings.TrimSpace(fm.Metadata.Hivex.SourceArticles[0]); s != "" {
			return s
		}
	}
	if len(fm.Metadata.Hivex.SourceSignals) > 0 {
		if s := strings.TrimSpace(fm.Metadata.Hivex.SourceSignals[0]); s != "" {
			return s
		}
	}
	return ""
...
func rewriteSkillTombstoneLocked(b *Broker, entries []SkillTombstoneEntry, slug string) {
	...
	_, _, enqErr := wikiWorker.Enqueue(
		...
		raw,
		"replace",
		"hivex: undo reject — restore skill "+slug,
	)
	b.mu.Lock()
	if enqErr != nil {
...
func (b *Broker) handleSkillEnableForBot(w http.ResponseWriter, r *http.Request, name string) {
	...
	if changed && wikiWorker != nil {
		if err := enqueueSkillWikiWrite(enqueueCtx, wikiWorker, out, wikiPath, "hivex: enable skill "+out.Name+" for @"+bot); err != nil {
...
func (b *Broker) handleSkillDisableForBot(w http.ResponseWriter, r *http.Request, name string) {
	...
	// wiki preview path.
	if changed && wikiWorker != nil {
		if err := enqueueSkillWikiWrite(enqueueCtx, wikiWorker, out, wikiPath, "hivex: disable skill "+out.Name+" for @"+bot); err != nil {
