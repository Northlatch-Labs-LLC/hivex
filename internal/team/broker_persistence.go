...
// non-trivial entity to ~/.hivex/team/broker-state.json (overridable
// via HIVEX_BROKER_STATE_PATH for tests). On restart it loads the
...
func defaultBrokerStatePath() string {
	...
	// the user's real ~/.hivex/team/ dir without needing to remap HOME (which
