...
func TestActionGrantExactMatchOnly(t *testing.T) {
	...
	b.addActionGrant(actionGrant{BotSlug: "cos", Platform: "gmail", ActionScope: "GMAIL_SEND_EMAIL", GrantedBy: "human"})
	...
	for _, c := range []struct{ bot, platform, action, label string }{
		{"cos", "gmail", "GMAIL_DELETE_EMAIL", "different action"},
		{"sales", "gmail", "GMAIL_SEND_EMAIL", "different bot"},
		{"cos", "slack", "GMAIL_SEND_EMAIL", "different platform"},
	} {
...
func TestActionGrantExpiryAndRevoke(t *testing.T) {
	...
	expired := b.addActionGrant(actionGrant{
		BotSlug: "cos", Platform: "gmail", ActionScope: "GMAIL_SEND_EMAIL",
	...
	live := b.addActionGrant(actionGrant{BotSlug: "cos", Platform: "slack", ActionScope: "SLACK_SEND_MESSAGE"})
	if !b.hasActiveActionGrant("cos", "slack", "SLACK_SEND_MESSAGE", now) {
...
func TestActionGrantIdempotent(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	b := newBrokerWithTeamRoom(filepath.Join(t.TempDir(), "state.json"))
	g1 := b.addActionGrant(actionGrant{BotSlug: "cos", Platform: "gmail", ActionScope: "GMAIL_SEND_EMAIL"})
	g2 := b.addActionGrant(actionGrant{BotSlug: "cos", Platform: "gmail", ActionScope: "GMAIL_SEND_EMAIL"})
	if g1.ID != g2.ID {
...
func TestActionGrantCappedToMaxTTL(t *testing.T) {
	...
	g := b.addActionGrant(actionGrant{BotSlug: "cos", Platform: "gmail", ActionScope: "GMAIL_SEND_EMAIL"})
	if g.ExpiresAt == "" {
	...
	if b.hasActiveActionGrant("cos", "gmail", "GMAIL_SEND_EMAIL", now.Add(maxGrantTTL+time.Hour)) {
	...
	g2 := b.addActionGrant(actionGrant{BotSlug: "cos", Platform: "slack", ActionScope: "SLACK_SEND_MESSAGE", ExpiresAt: far})
...
func TestResolveProceedsWithGrant(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	t.Setenv("HIVEX_RUNTIME_HOME", tmp)
	t.Setenv("HIVEX_COMPOSIO_API_KEY", "cmp_test")
	t.Setenv("HIVEX_COMPOSIO_USER_ID", "cos@example.com")
	...
	t.Setenv("HIVEX_COMPOSIO_BASE_URL", composioServer.URL)
	...
	body, _ := json.Marshal(integrationResolveRequest{
		Platform: "gmail", ActionID: "GMAIL_SEND_EMAIL", Bot: "cos",
...
func TestActionGrantEndpointLifecycle(t *testing.T) {
	...
	wild, _ := json.Marshal(map[string]any{"agent_slug": "cos", "platform": "gmail", "action_scope": "*"})
	...
	create, _ := json.Marshal(map[string]any{
		"agent_slug": "cos", "platform": "gmail", "action_scope": "GMAIL_SEND_EMAIL", "channel": "team",
	})
	...
	if !b.hasActiveActionGrant("cos", "gmail", "GMAIL_SEND_EMAIL", time.Now().UTC()) {
	...
	if b2.hasActiveActionGrant("cos", "gmail", "GMAIL_SEND_EMAIL", time.Now().UTC()) {
