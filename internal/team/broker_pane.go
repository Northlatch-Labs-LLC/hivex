...
// the pane at the conventional position (hivex-team:team.<i>); in
...
func (b *Broker) capturePaneActivity(slugOverride string) map[string]string {
	...
	if slugOverride != "" {
		...
		for i, bot := range manifest.Members {
			checks = append(checks, paneCheck{
				slug:   bot.Slug,
				target: fmt.Sprintf("hivex-team:team.%d", i+1),
			})
		}
	}
	...
	for _, check := range checks {
		...
		paneOut, err := exec.CommandContext(ctx, "tmux", "-L", "hivex", "capture-pane",
			"-p", "-J",
