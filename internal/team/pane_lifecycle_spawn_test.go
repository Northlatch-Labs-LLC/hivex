...
func TestPaneLifecycle_SplitFirstAgentArgs(t *testing.T) {
	...
	out, err := newPaneLifecycle("hivex-team").SplitFirstBot("/repo", "claude --print")
	if err != nil {
	...
	want := []string{"split-window", "-h", "-t", "hivex-team:team", "-p", "65", "-c", "/repo", "claude --print"}
...
func TestPaneLifecycle_SplitAdditionalAgentArgs(t *testing.T) {
	...
	if _, err := newPaneLifecycle("hivex-team").SplitAdditionalBot("/repo", "claude"); err != nil {
	...
	want := []string{"split-window", "-t", "hivex-team:team.1", "-c", "/repo", "claude"}
	if !equalStrings(calls[0], want) {
...
func TestPaneLifecycle_NewOverflowWindowArgs(t *testing.T) {
	...
	if _, err := newPaneLifecycle("hivex-team").NewOverflowWindow("agent-fe", "/repo", "claude"); err != nil {
	...
	want := []string{"new-window", "-d", "-t", "hivex-team", "-n", "agent-fe", "-c", "/repo", "claude"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_NewSessionArgs(t *testing.T) {
	...
	if err := newPaneLifecycle("hivex-team").NewSession("/repo", "sleep infinity"); err != nil {
	...
	want := []string{"new-session", "-d", "-s", "hivex-team", "-n", "team", "-c", "/repo", "sleep infinity"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_SetSessionOptionArgs(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").SetSessionOption("mouse", "off")
	calls := fake.callsFor("set-option")
	if len(calls) != 1 {
		t.Fatalf("set-option calls = %d, want 1", len(calls))
	}
	want := []string{"set-option", "-t", "hivex-team", "mouse", "off"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_SetTeamWindowOptionArgs(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").SetTeamWindowOption("remain-on-exit", "on")
	calls := fake.callsFor("set-window-option")
	if len(calls) != 1 {
		t.Fatalf("set-window-option calls = %d, want 1", len(calls))
	}
	want := []string{"set-window-option", "-t", "hivex-team:team", "remain-on-exit", "on"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_ApplyMainVerticalLayoutArgs(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").ApplyMainVerticalLayout()
	calls := fake.callsFor("select-layout")
	if len(calls) != 1 {
		t.Fatalf("select-layout calls = %d, want 1", len(calls))
	}
	want := []string{"select-layout", "-t", "hivex-team:team", "main-vertical"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_SetPaneTitleArgs(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").SetPaneTitle("hivex-team:team.2", "🤖 cos (@cos)")
	calls := fake.callsFor("select-pane")
	if len(calls) != 1 {
		t.Fatalf("select-pane calls = %d, want 1", len(calls))
	}
	want := []string{"select-pane", "-t", "hivex-team:team.2", "-T", "🤖 cos (@cos)"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_SelectTeamWindowAndFocusPane(t *testing.T) {
	...
	pl := newPaneLifecycle("hivex-team")
	pl.SelectTeamWindow()
	pl.FocusPane("hivex-team:team.0")

	wins := fake.callsFor("select-window")
	if len(wins) != 1 {
		t.Fatalf("select-window calls = %d, want 1", len(wins))
	}
	if !equalStrings(wins[0], []string{"select-window", "-t", "hivex-team:team"}) {
	...
	if !equalStrings(panes[0], []string{"select-pane", "-t", "hivex-team:team.0"}) {
...
func TestPaneLifecycle_IsPaneDeadParsesOutput(t *testing.T) {
	...
	dead, err := newPaneLifecycle("hivex-team").IsPaneDead("hivex-team:team.2")
	if err != nil {
	...
	want := []string{"display-message", "-t", "hivex-team:team.2", "-p", "#{pane_dead}"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_CapturePaneHistoryArgs(t *testing.T) {
	...
	out, err := newPaneLifecycle("hivex-team").CapturePaneHistory("hivex-team:team.2", 200)
	if err != nil {
	...
	want := []string{"capture-pane", "-t", "hivex-team:team.2", "-p", "-J", "-S", "-200"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
...
func TestPaneLifecycle_SendEnterArgs(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").SendEnter("hivex-team:team.2")

	calls := fake.callsFor("send-keys")
	if len(calls) != 1 {
		t.Fatalf("send-keys calls = %d, want 1", len(calls))
	}
	want := []string{"send-keys", "-t", "hivex-team:team.2", "Enter"}
	if !equalStrings(calls[0], want) {
		t.Errorf("args = %v, want %v", calls[0], want)
	}
}
