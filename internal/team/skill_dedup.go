...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/embedding"
)
...
// skillDedupEnabled returns false when HIVEX_SKILL_DEDUP_ENABLED=0.
func skillDedupEnabled() bool {
	raw := strings.ToLower(strings.TrimSpace(os.Getenv("HIVEX_SKILL_DEDUP_ENABLED")))
	switch raw {
	case "0", "false", "no", "off":
		return false
	}
...
func skillDedupSlugThreshold() float64 {
	if v, err := strconv.ParseFloat(os.Getenv("HIVEX_SKILL_DEDUP_SLUG_THRESHOLD"), 64); err == nil && v > 0 && v <= 1 {
		return v
	}
...
func skillDedupDescThreshold() float64 {
	if v, err := strconv.ParseFloat(os.Getenv("HIVEX_SKILL_DEDUP_DESC_THRESHOLD"), 64); err == nil && v > 0 && v <= 1 {
		return v
	}
...
func skillDedupEmbedThreshold() float32 {
	if v, err := strconv.ParseFloat(os.Getenv("HIVEX_SKILL_DEDUP_EMBED_THRESHOLD"), 64); err == nil && v > 0 && v <= 1 {
		return float32(v)
	}
