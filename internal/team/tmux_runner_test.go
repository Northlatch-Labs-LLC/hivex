...
func TestPaneLifecycle_HasLiveSession(t *testing.T) {
	...
	want := []string{"has-session", "-t", "hivex-team"}
...
func TestPaneLifecycle_ListTeamPanesParsesOutput(t *testing.T) {
	...
	got, err := newPaneLifecycle("hivex-team").ListTeamPanes()
	if err != nil {
	...
	wantArgs := []string{"list-panes", "-t", "hivex-team:team", "-F", "#{pane_index} #{pane_title}"}
...
func TestPaneLifecycle_ListTeamPanesMissingSessionIsNilNil(t *testing.T) {
	...
	fake.outputs["list-panes"] = []byte("no server running on /tmp/tmux-1000/hivex")
...
func TestPaneLifecycle_ChannelPaneStatusTrimsOutput(t *testing.T) {
	...
	got, err := newPaneLifecycle("hivex-team").ChannelPaneStatus()
	if err != nil {
...
func TestPaneLifecycle_CapturePaneContentBuildsTarget(t *testing.T) {
	...
	got, err := newPaneLifecycle("hivex-team").CapturePaneContent(2)
	if err != nil {
	...
	wantArgs := []string{"capture-pane", "-p", "-J", "-t", "hivex-team:team.2"}
	if !equalStrings(calls[0], wantArgs) {
