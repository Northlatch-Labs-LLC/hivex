...
// in (HIVEX_GOVERNOR_ENABLED). They are overridable via env (loadGovernorConfig).
const (
	...
	// automatic checkpoint with HIVEX_GOVERNOR_ENABLED=1.
...
func loadGovernorConfig() governorConfig {
	...
	if v := strings.TrimSpace(os.Getenv("HIVEX_BUDGET_MAX_TOKENS")); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n >= 0 {
			cfg.MaxSessionTokens = n
		}
	}
	if v := strings.TrimSpace(os.Getenv("HIVEX_BUDGET_MAX_COST_USD")); v != "" {
		if f, err := strconv.ParseFloat(v, 64); err == nil && f >= 0 {
			cfg.MaxSessionCostUsd = f
		}
	}
	if v := strings.TrimSpace(os.Getenv("HIVEX_CHECKPOINT_EVERY_TURNS")); v != "" {
		if n, err := strconv.Atoi(v); err == nil && n >= 0 {
			cfg.MaxTurnsPerGate = n
		}
	}
	// Automatic pausing is off by default (defaultGovernorDisabled). Opt back in
	// with HIVEX_GOVERNOR_ENABLED; HIVEX_GOVERNOR_DISABLED still forces it off and
	// wins if both are set.
	if envTruthy(os.Getenv("HIVEX_GOVERNOR_ENABLED")) {
		cfg.Disabled = false
	}
	if envTruthy(os.Getenv("HIVEX_GOVERNOR_DISABLED")) {
		cfg.Disabled = true
	}
	return cfg
}
