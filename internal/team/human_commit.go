...
//     `human` slug, yielding commit author `Human <human@hivex.local>`.
...
// / `user.email` on this machine). Yields `human <human@hivex.local>`
// via runGitLocked's identity derivation. Distinct from every bot slug
// and from the other synthetic identities (archivist, hivex-bootstrap,
// hivex-recovery, system) so audit views can colour human edits
// distinctly.
//
...
// zero value, the fallback `human <human@hivex.local>` identity is used
