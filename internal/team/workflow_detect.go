...
// MCP server prefix (mcp__hivex-office__) is tolerated.
...
// workflow work. The office's own coordination MCP (mcp__hivex-office__*) is
...
func isOrchestrationTool(name string) bool {
	n := strings.ToLower(strings.TrimSpace(name))
	if strings.HasPrefix(n, "mcp__hivex-office__") {
		return true
	}
