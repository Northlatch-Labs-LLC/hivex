...
func respawnBotPane(slug string) {
	...
	for i, bot := range manifest.Members {
		if bot.Slug == slug {
			...
			target := fmt.Sprintf("hivex-team:team.%d", paneIdx)
			...
			defer cancel()
			_ = exec.CommandContext(ctx, "tmux", "-L", "hivex", "send-keys", "-t", target, "C-c", "").Run()
			time.Sleep(500 * time.Millisecond)
			_ = exec.CommandContext(ctx, "tmux", "-L", "hivex", "send-keys", "-t", target, "C-c", "").Run()
			time.Sleep(500 * time.Millisecond)
			// Respawn the pane with a fresh claude session
			_ = exec.CommandContext(ctx, "tmux", "-L", "hivex", "respawn-pane", "-k", "-t", target).Run()
			return
		}
	}
}
