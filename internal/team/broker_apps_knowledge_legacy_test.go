...
// seedLegacyWiki writes a small office-era wiki tree under home/.hivex/wiki.
func seedLegacyWiki(t *testing.T, home string) {
	t.Helper()
	write := func(rel, content string) {
		t.Helper()
		path := filepath.Join(home, ".hivex", "wiki", rel)
...
func TestLoadLegacyKnowledgePages(t *testing.T) {
	...
	pages := loadLegacyKnowledgePages(filepath.Join(home, ".hivex", "wiki"))
...
// previous product's machine-metadata comment (the hivex:entity-article
...
func TestLegacyPageStripsHTMLComments(t *testing.T) {
	page, ok := writeLegacyPage(t, "final-rag.md", strings.Join([]string{
		"Before the marker.",
		"",
		"<!-- hivex:entity-article — generated from the team knowledge graph",
	...
	for _, leaked := range []string{"<!--", "-->", "hivex:entity-article", "machine note"} {
