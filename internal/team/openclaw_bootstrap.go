...
func BuildOpenclawBridgeFromConfig(broker *Broker) (*OpenclawBridge, error) {
	...
	cfg, _ := config.Load()
	gatewayConfigured := strings.TrimSpace(cfg.OpenclawGatewayURL) != "" || strings.TrimSpace(os.Getenv("HIVEX_OPENCLAW_GATEWAY_URL")) != ""
...
// behavior production hivebot runs via launcher.go. The goroutine exits when
...
// without a hivebot restart. OpenClaw rejects token-only clients with zero scopes,
