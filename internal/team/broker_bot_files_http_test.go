...
// postBotFile is a small helper that POSTs to /bot-files/write and returns
// the recorder. expectedSha == "" means "create".
func postBotFile(t *testing.T, b *Broker, path, content, expectedSha string) *httptest.ResponseRecorder {
	t.Helper()
	...
	b.handleBotFileWrite(rec, req)
	return rec
}

func getBotFile(t *testing.T, b *Broker, path string) (*httptest.ResponseRecorder, botFileReadResponse) {
	t.Helper()
	req := httptest.NewRequest(http.MethodGet, "/agent-files/read?path="+path, nil)
	rec := httptest.NewRecorder()
	b.handleBotFileRead(rec, req)
	var out botFileReadResponse
...
// TestBotFileWriteThenRead covers the core editor round-trip: create a file
// via the write endpoint, then read it back with content + a real SHA + exists.
func TestBotFileWriteThenRead(t *testing.T) {
...
// TestBotFileReadMissSeedsOfficeUser verifies that reading a not-yet-written
...
func TestBotFileReadMissSeedsOfficeUser(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)

	rec, got := getBotFile(t, b, officeUserFileRel)
	if rec.Code != http.StatusOK {
...
// TestBotFileReadMissSeedsBotFromRoster verifies the bots/{slug}/*.md
...
func TestBotFileReadMissSeedsBotFromRoster(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)
	...
	rec, got := getBotFile(t, b, "agents/growth/SOUL.md")
	if rec.Code != http.StatusOK {
		t.Fatalf("read miss: want 200, got %d (%s)", rec.Code, rec.Body.String())
	}
	if got.Exists {
...
// TestBotFileWrite409OnStaleSHA verifies optimistic concurrency: a replace
// against a stale sha is rejected with 409 + the current sha and bytes.
func TestBotFileWrite409OnStaleSHA(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)
...
// TestBotFileWriteRejectsNonBotPath ensures the strict validator blocks any
// attempt to use this endpoint to write outside the bot-file allowlist.
func TestBotFileWriteRejectsNonBotPath(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)
...
// TestBotFileReadRejectsNonBotPath mirrors the write guard for reads.
func TestBotFileReadRejectsNonBotPath(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)
...
// TestBotFileWriteRequiresHumanActor verifies the write handler rejects a
// non-human (e.g. broker-token) actor — a bot must never rewrite an
// instruction file, since those feed the system prompt.
func TestBotFileWriteRequiresHumanActor(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)
...
// TestHumanRouteAllowedBotFiles locks the human/web token's access to the new
// endpoints: GET read and POST write are allowed; other methods are not.
func TestHumanRouteAllowedBotFiles(t *testing.T) {
	cases := []struct {
		method, path string
		want         bool
	}{
...
func postGenerate(t *testing.T, b *Broker, path string, human bool) *httptest.ResponseRecorder {
	...
	rec := httptest.NewRecorder()
	b.handleBotFileGenerate(rec, req)
	return rec
}

// TestBotFileGenerateEndpoint covers the draft-authoring endpoint: a human
...
func TestBotFileGenerateEndpoint(t *testing.T) {
	worker, _, _, teardown := newStartedWorker(t)
	defer teardown()
	b := brokerForTest(t, worker)
	b.SetGenerateBotFileFn(func(_ context.Context, relPath, _ string) (string, error) {
	...
	if data, _ := worker.BotFileRead("agents/growth/SOUL.md"); len(data) != 0 {
	...
	b.SetGenerateBotFileFn(func(_ context.Context, _, _ string) (string, error) {
...
// TestGenerateBotFileGating verifies the prose-only allowlist (SOUL/OPERATIONS
// + office USER) and the test stub short-circuit, without needing a real model.
func TestGenerateBotFileGating(t *testing.T) {
	t.Setenv("HIVEX_AGENT_FILE_STUB", "# stub content")
	l := &Launcher{}
	ctx := context.Background()

	for _, ok := range []string{"agents/x/SOUL.md", "agents/x/OPERATIONS.md", officeUserFileRel} {
		got, err := l.GenerateBotFileFromContext(ctx, ok, "")
	...
	for _, bad := range []string{"agents/x/IDENTITY.md", "agents/x/TOOLS.md"} {
		if _, err := l.GenerateBotFileFromContext(ctx, bad, ""); err == nil {
			t.Errorf("path %q: expected gating error, got nil", bad)
		}
	}
	// An invalid path is rejected before anything else.
	if _, err := l.GenerateBotFileFromContext(ctx, "team/people/x.md", ""); err == nil {
...
// TestCommitBotFileHumanRoundTrip exercises the repo-level human-write path:
// create, replace with the correct sha, and a stale-sha replace that must be
// rejected with ErrWikiSHAMismatch.
func TestCommitBotFileHumanRoundTrip(t *testing.T) {
	repo := newTestRepo(t)
	ctx := context.Background()
