...
func (b *Broker) materializeScratchWikiStubs(s *onboarding.State) {
	...
	if home == "" {
		log.Printf("onboarding: materializeScratchWikiStubs: HIVEX_RUNTIME_HOME unset")
		return
	}
	wikiRoot := filepath.Join(home, ".hivex", "wiki")
	...
	sha, err := repo.CommitBootstrap(ctx, "hivex: materialize scratch wiki about/ skeleton")
	if err != nil {
...
func (b *Broker) materializeGettingStarted() {
	home := config.RuntimeHomeDir()
	if home == "" {
		log.Printf("onboarding: materializeGettingStarted: HIVEX_RUNTIME_HOME unset")
		return
	}
	wikiRoot := filepath.Join(home, ".hivex", "wiki")
	...
	sha, err := repo.CommitBootstrap(ctx, "hivex: materialize getting-started wiki pages")
	if err != nil {
...
//
// See Northlatch-Labs-LLC/hivex#941.
