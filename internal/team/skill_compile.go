...
func (b *Broker) startSkillCompileCron(ctx context.Context) {
	...
	if interval <= 0 {
		slog.Info("skill_compile cron: disabled (set HIVEX_SKILL_COMPILE_INTERVAL to enable)")
		return
	}
	go func() {
...
func skillCompileIntervalFromEnv() time.Duration {
	raw := strings.TrimSpace(os.Getenv("HIVEX_SKILL_COMPILE_INTERVAL"))
	if raw == "" {
	...
	if err != nil {
		slog.Warn("skill_compile: invalid HIVEX_SKILL_COMPILE_INTERVAL, falling back to default",
			"value", raw, "err", err)
		return defaultSkillCompileInterval
	}
	return d
...
func skillCompileCooldownFromEnv() time.Duration {
	raw := strings.TrimSpace(os.Getenv("HIVEX_SKILL_COMPILE_COOLDOWN"))
	if raw == "" {
		return defaultSkillCompileCooldown
	}
	d, err := time.ParseDuration(raw)
	if err != nil {
		slog.Warn("skill_compile: invalid HIVEX_SKILL_COMPILE_COOLDOWN, falling back to default",
			"value", raw, "err", err)
		return defaultSkillCompileCooldown
	}
	return d
...
func skillCompileBudgetFromEnv() int {
	raw := strings.TrimSpace(os.Getenv("HIVEX_SKILL_COMPILE_LLM_BUDGET"))
	if raw == "" {
