...
// HIVEX_GBRAIN_TEST=1 plus a GBRAIN_HOME pointing at a brain that is safe to
// mutate. Everything they do is destructive inside the hivex namespaces, so
...
// test harness points HIVEX_RUNTIME_HOME at a temp dir for isolation, so
...
func gbrainTestEnabled() bool {
	return strings.TrimSpace(os.Getenv("HIVEX_GBRAIN_TEST")) == "1"
}

// newGBrainTestStore connects to the configured brain and purges every page in
// the hivex namespaces so each test starts clean.
//
...
func newGBrainTestStore(t *testing.T) FactStore {
	...
	if !gbrainTestEnabled() {
		t.Skip("live gbrain contract tests are opt-in: set HIVEX_GBRAIN_TEST=1 and GBRAIN_HOME")
	}
...
// purgeGBrainNamespaces deletes every page hivebot owns in the brain.
//
