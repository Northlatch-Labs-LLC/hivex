...
// causes those articles to land under $HOME/.hivex/wiki/ after
...
func TestOnboardingCompleteMaterializesWiki(t *testing.T) {
	...
	// tempdir instead of ~/.hivex. Set both HOME (for legacy test-isolation
	// patterns) AND HIVEX_RUNTIME_HOME (post-Phase-0, materializeBlueprintWiki
	// resolves through config.RuntimeHomeDir which checks HIVEX_RUNTIME_HOME first).
	tmpHome := t.TempDir()
	...
	wikiRoot := filepath.Join(tmpHome, ".hivex", "wiki")
	...
	repo := NewRepoAt(wikiRoot, filepath.Join(tmpHome, ".hivex", "wiki.bak"))
...
func TestOnboardingCompleteWikiIsIdempotent(t *testing.T) {
	...
	// where a CLI restart reads ~/.hivex/team/broker-state.json).
...
func TestOnboardingCompleteSynthesizedBlueprintSeedsGettingStarted(t *testing.T) {
	...
	gsIndex := filepath.Join(tmpHome, ".hivex", "wiki", "team", "getting-started", "index.md")
