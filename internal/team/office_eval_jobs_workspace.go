...
func evalJobWorkspaceIsolation(fx *officeEvalFixture, r *OfficeEvalReport) error {
	...
	// knob the live team launches with (HIVEX_RUNTIME_HOME) — so bot
	...
	if err := os.Setenv("HIVEX_RUNTIME_HOME", fx.scratchDir); err != nil {
		return err
	}
	defer func() {
		if hadHome {
	...
	wantScratch := normalizeHeadlessWorkspaceDir(filepath.Join(fx.scratchDir, ".hivex", "agent-scratch", "eng"))
