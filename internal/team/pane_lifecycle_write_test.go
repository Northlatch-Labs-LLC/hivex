...
func TestPaneLifecycle_ClearAgentPanesKillsHigherIndicesFirst(t *testing.T) {
	...
	if err := newPaneLifecycle("hivex-team").ClearBotPanes(); err != nil {
	...
	wantTargets := []string{"hivex-team:team.3", "hivex-team:team.2", "hivex-team:team.1"}
...
func TestPaneLifecycle_ClearOverflowAgentWindowsFiltersByPrefix(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").ClearOverflowBotWindows()

	kills := fake.callsFor("kill-window")
	if len(kills) != 2 {
		t.Fatalf("kill-window calls = %d, want 2 (bot-fe, bot-be)", len(kills))
	}
	wantTargets := []string{"hivex-team:agent-fe", "hivex-team:agent-be"}
	for i, call := range kills {
		if len(call) < 3 || call[2] != wantTargets[i] {
...
func TestPaneLifecycle_RespawnAgentPaneSurfacesTmuxOutput(t *testing.T) {
	...
	out, err := newPaneLifecycle("hivex-team").RespawnBotPane(2, "/tmp/cwd", "claude --print")
	if err == nil {
	...
	want := []string{"respawn-pane", "-k", "-t", "hivex-team:team.2", "-c", "/tmp/cwd", "claude --print"}
	if !equalStrings(calls[0], want) {
...
func TestPaneLifecycle_RespawnChannelPaneIssuesBothCommands(t *testing.T) {
	...
	newPaneLifecycle("hivex-team").RespawnChannelPane("hivex channel", "/tmp/cwd")

	respawn := fake.callsFor("respawn-pane")
	if len(respawn) != 1 {
		t.Fatalf("respawn-pane calls = %d, want 1", len(respawn))
	}
	wantRespawn := []string{"respawn-pane", "-k", "-t", "hivex-team:team.0", "-c", "/tmp/cwd", "hivex channel"}
	...
	if selectPane[0][2] != "hivex-team:team.0" {
		t.Errorf("select-pane target = %q, want hivex-team:team.0", selectPane[0][2])
	}
...
func TestPaneLifecycle_CaptureDeadChannelPaneWritesSnapshot(t *testing.T) {
	...
	if err := newPaneLifecycle("hivex-team").CaptureDeadChannelPane("1 0 claude"); err != nil {
		t.Fatalf("CaptureDeadChannelPane err = %v", err)
	}

	path := filepath.Join(tmp, ".hivex", "logs", "channel-pane.log")
