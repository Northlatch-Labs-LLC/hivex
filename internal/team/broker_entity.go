...
func resolveThresholdFromEnv() int {
	raw := strings.TrimSpace(os.Getenv("HIVEX_ENTITY_BRIEF_THRESHOLD"))
	if raw == "" {
...
func resolveTimeoutFromEnv() time.Duration {
	raw := strings.TrimSpace(os.Getenv("HIVEX_ENTITY_BRIEF_TIMEOUT"))
	if raw == "" {
...
func resolveSynthesisModeFromEnv() SynthesisMode {
	if strings.TrimSpace(os.Getenv("HIVEX_ENTITY_SYNTHESIS_MODE")) == "demand" {
...
func (b *Broker) handleEntityFact(w http.ResponseWriter, r *http.Request) {
	...
	if recordedBy == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "recorded_by or X-HIVEX-Agent header is required"})
		return
	}
