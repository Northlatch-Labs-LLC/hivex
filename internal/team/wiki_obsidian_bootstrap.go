...
// obsidianRequiredAppKeys are the keys hivebot owns inside .obsidian/app.json.
...
// merged so hivebot-owned keys cannot drift while user customisations (theme,
...
// writeObsidianAppJSON reads any existing app.json, merges hivebot's required
// keys over the top (hivebot wins on required keys, user wins on everything
