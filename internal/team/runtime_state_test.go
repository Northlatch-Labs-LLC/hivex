...
func TestDetectRuntimeCapabilities(t *testing.T) {
	...
	t.Setenv("HIVEX_LLM_PROVIDER", "claude-code")
	...
	// Under --no-hive the new ResolveMemoryBackend default lands on markdown
	...
	// skip Hive; they didn't ask to lose shared memory.
	if memEntry, ok := got.Registry.Entry(CapabilityKeyMemory); !ok || memEntry.Lifecycle != CapabilityLifecycleReady {
		t.Fatalf("expected memory backend to be ready (markdown) under --no-hive, got %+v", memEntry)
	}
}
...
func TestDetectRuntimeCapabilitiesWhenTmuxServerIsMissing(t *testing.T) {
	...
	commandCombinedOutputFn = func(name string, args ...string) ([]byte, error) {
		...
		if len(args) == 5 && args[0] == "-L" && args[1] == tmuxSocketName && args[2] == "list-sessions" && args[3] == "-F" {
			return []byte("no server running on /tmp/tmux-1000/hivex\n"), errors.New("exit status 1")
		}
		return nil, errors.New("unexpected tmux probe")
	}
	actionProviderForCapabilityFn = func(action.Capability) (action.Provider, error) {
...
func TestBuildRuntimeSnapshotFormatsRecoveryAndCapabilities(t *testing.T) {
	snapshot := BuildRuntimeSnapshot(RuntimeSnapshotInput{
		...
		Artifacts: []RuntimeArtifact{
			{
				...
				Path:          "/tmp/hivex-task-1/output.log",
				Worktree:      "/tmp/hivex-task-1",
				ResumeHint:    "Resume in /tmp/hivex-task-1 or reopen the task thread.",
		...
		Capabilities: RuntimeCapabilities{
			...
			Items: []CapabilityStatus{{
				...
				Detail: "tmux 3.4a on socket hivex is running with session hivex-team (2 attached, 4 windows).",
			}},
		},
	...
	for _, want := range []string{
		...
		"Use working_directory /tmp/hivex-task-1",
		"Execution artifacts:",
		...
		"Version: tmux 3.4a",
		"Inside tmux: yes",
		"hivebot session: running (2 attached, 4 windows)",
