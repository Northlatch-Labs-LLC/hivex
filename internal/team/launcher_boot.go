...
func (l *Launcher) Launch() error {
	...
	// Resolve hivex binary path for the channel view
	hivexBinary, _ := os.Executable()
	...
	channelEnv := []string{
		fmt.Sprintf("HIVEX_BROKER_TOKEN=%s", l.broker.Token()),
		fmt.Sprintf("HIVEX_BROKER_BASE_URL=%s", l.BrokerBaseURL()),
	}
	if l.isOneOnOne() {
		channelEnv = append(channelEnv,
			"HIVEX_ONE_ON_ONE=1",
			fmt.Sprintf("HIVEX_ONE_ON_ONE_AGENT=%s", l.oneOnOneBot()),
		)
	}
	channelCmd := fmt.Sprintf("%s %s --channel-view 2>>%s", strings.Join(channelEnv, " "), hivexBinary, shellQuote(channelStderrLogPath()))
	...
	// hivebot is keyboard-first; we don't want the TUI or tmux to steal mouse events.
