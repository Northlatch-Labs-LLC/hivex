...
func TestSessionMemorySnapshotRestorationContextCollectsResumeHandles(t *testing.T) {
	snapshot := SessionMemorySnapshot{
		...
		Tasks: []SessionMemoryTaskSummary{
			{ID: "task-1", Status: "in_progress", WorktreePath: "/tmp/hivex-task-1", ThreadID: "msg-9"},
			{ID: "task-2", Status: "review", WorktreePath: "/tmp/hivex-task-1", ThreadID: "msg-10"},
		},
	...
	if len(ctx.WorkingDirectories) != 1 || ctx.WorkingDirectories[0] != "/tmp/hivex-task-1" {
...
func TestBuildSessionMemorySnapshotFromOfficeStateReconstructsContext(t *testing.T) {
	...
	if len(restore.WorkingDirectories) != 1 || restore.WorkingDirectories[0] != "/tmp/hivex-task-7" {
