...
// slack_spawned_agents.go is the OUTBOUND half of spawned Slack bots
// (broker_slack_spawn.go): when an office message authored by a spawned bot
// goes out to a bridged Slack channel, it is posted with the BOT'S OWN bot
// token — so it appears in Slack as that bot, with its own name and avatar —
// instead of the main hivex bot token.
//
...
func (t *SlackTransport) spawnedSenderParticipant(from string) transport.Participant {
	...
	if slug == "" || t.Broker.SpawnedSlackBotTokenEnv(slug) == "" {
		return transport.Participant{}
	}
	return transport.Participant{
		AdapterName: "slack",
...
// bot's own client when the outbound carries a spawned-sender participant
...
func (t *SlackTransport) postClientFor(p transport.Participant) slackAPI {
	...
	if c := t.spawnedBotClient(slug); c != nil {
		return c
	}
	return t.api
}

// spawnedBotClient returns (and caches) a Web API client authed as the
// spawned bot's own bot, or nil when slug is not a spawned bot or its
// token env var is unset (Send then degrades to the main bot token, so the
// message still lands — just attributed to the hivex bot). The cache holds
// the miss too: env vars are process-lifetime, so a re-probe per message
// would only repeat the same lookup and warning.
func (t *SlackTransport) spawnedBotClient(slug string) slackAPI {
	...
	var client slackAPI
	if t.Broker != nil {
		if env := t.Broker.SpawnedSlackBotTokenEnv(slug); env != "" {
			if token := strings.TrimSpace(os.Getenv(env)); token != "" {
				client = &slackClient{api: slack.New(token)}
			} else {
				log.Printf("[slack] spawned bot %q: env %s is unset — posting with the main bot token", slug, env)
			}
		}
	}
