...
const (
	...
	injectionEscapePrefix = "[HIVEX-ESCAPED] "
)
...
//
// Idempotency: the replacement prefix "[HIVEX-ESCAPED] " does not match any
...
// "[HIVEX-ESCAPED] " to their payload and smuggle the raw injection past
// the escaper.
func escapeInjectionPatterns(s string) string {
	...
	for _, pat := range injectionPatterns {
		...
		for searchFrom < len(lower) {
			...
			// sentinel could prepend "[hivebot-ESCAPED] " to their payload
...
// Combined with the visible "[hivebot-ESCAPED] " prefix emitted by the caller,
