...
func (b *notificationContextBuilder) ResponseInstructionForTarget(msg channelMessage, slug string) string {
	...
	if slug == lead {
		...
		isFromHuman := isHumanMessageSender(from) || from == "hive"
		if !isFromHuman {
	...
	if slug == b.targeter.LeadSlug() {
		...
		if b.channelMessages != nil {
			...
			for _, tm := range b.channelMessages(channelSlug) {
				...
				if !isHumanMessageSender(tm.From) && tm.From != "hive" && tm.From != slug {
...
func (b *notificationContextBuilder) BuildTaskExecutionPacketWithContext(slug string, action officeActionLog, task teamTask, content string) (string, []string) {
	...
	lines = append(lines, "Runtime rule: never launch another hivebot office, copied hivex binary, browser instance, or local web server/--web-port process from inside this turn. The team is already running; use the existing repo, broker state, and assigned worktree instead.")
