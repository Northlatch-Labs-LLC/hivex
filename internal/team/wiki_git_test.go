...
func TestRepoRecoverDirtyTree(t *testing.T) {
	...
	if refs[0].Author != "hivex-recovery" {
		t.Fatalf("expected hivex-recovery author, got %q", refs[0].Author)
	}
}
...
func TestRepoCommitBootstrapAttributesToBootstrapAuthor(t *testing.T) {
	...
	// attribute them to hivex-bootstrap (NOT hivex-recovery, NOT system).
	...
	sha, err := repo.CommitBootstrap(ctx, "hivex: materialize test blueprint")
	if err != nil {
	...
	// The most recent commit for each skeleton must be authored by hivex-bootstrap.
	for rel := range skeletons {
		...
		if refs[0].Author != "hivex-bootstrap" {
			t.Fatalf("%s: expected hivex-bootstrap author, got %q", rel, refs[0].Author)
		}
...
func TestRepoAuditLogCoversAllAuthors(t *testing.T) {
	...
	for _, want := range []string{"system", "hivex-bootstrap", "operator", "planner"} {
		if !authors[want] {
...
// hivex invoked from inside a git hook (which exports GIT_DIR pointing at the
// outer repo) silently commits wiki state onto the user's real branch. The
// symptom was thousands of `hivex: init wiki` commits in the reflog of real
...
func TestRepoInitIgnoresInheritedGitDir(t *testing.T) {
	...
	if got := strings.TrimSpace(string(msg)); got != "hivex: init wiki" {
