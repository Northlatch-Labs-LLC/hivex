...
// ~/.hivex/tasks/<task_id>/decision_packet.json.
//
...
//     <runtime-home>/.hivex/tasks/<taskID>/decision_packet.json with
...
// rootDir is bound at construction (typically <runtime-home>/.hivex) so
...
// runtime home (env-overridable via HIVEX_RUNTIME_HOME). When the
// runtime home cannot be resolved, falls back to a process-relative
// .hivex to mirror defaultBrokerStatePath's behaviour.
func newOSDecisionPacketStore() *osDecisionPacketStore {
	home := config.RuntimeHomeDir()
	if home == "" {
		return &osDecisionPacketStore{rootDir: ".hivex"}
	}
	return &osDecisionPacketStore{rootDir: filepath.Join(home, ".hivex")}
}

// taskDir returns the per-task directory ~/.hivex/tasks/<id>/.
...
// state. mkdir-all keeps the broker resilient to a fresh ~/.hivex
...
func renderWikiPromotion(packet DecisionPacket) string {
	...
	sb.WriteString("## Decision\n\nApproved via hivebot Decision Inbox.\n")
	return sb.String()
}
