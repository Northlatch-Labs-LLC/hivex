...
func chatDigestIntervalFromEnv() time.Duration {
	return chatDigestDurationFromEnv("HIVEX_CHAT_DIGEST_INTERVAL", defaultChatDigestInterval)
}

func chatDigestWindowFromEnv() time.Duration {
	d := chatDigestDurationFromEnv("HIVEX_CHAT_DIGEST_WINDOW", defaultChatDigestWindow)
	if d <= 0 {
...
// (HIVEX_CHAT_DIGEST_INTERVAL=0/disabled) leaves the loop unstarted.
