...
var localProviderSpecs = []localProviderSpec{
	...
	},
	{
		...
		notes: []string{
			"Hermes Bot is reached through its OpenAI-compatible API server on port 8642. If you set API_SERVER_KEY for Hermes, set the same value as HIVEX_HERMES_AGENT_API_KEY before starting hivebot.",
		},
	},
	{
		...
		notes: []string{
			"OpenClaw Gateway's OpenAI-compatible endpoint is disabled by default. Enable gateway.http.endpoints.chatCompletions.enabled, then use gateway.auth.token as HIVEX_OPENCLAW_HTTP_API_KEY, OPENCLAW_GATEWAY_TOKEN, or HIVEX_OPENCLAW_TOKEN.",
		},
	},
}
...
func computeOneStatus(ctx context.Context, spec localProviderSpec, ov localProvidersStatusOverrides) LocalProviderStatus {
	...
	if ov.goos == "windows" {
		st.WindowsNote = "Local LLM runtimes don't have native Windows support today. Run hivex inside WSL2 (Ubuntu) and install the runtime there — the broker will then detect it from inside WSL."
	}
