...
// the hivex-office server entry, and the small slug + path helpers
...
// writeHeadlessOpencodeMCPConfig merges hivebot's MCP server definition into an
...
// with different HIVEX_AGENT_SLUG values. Preserves other top-level user keys
// (theme, provider preferences, user-configured MCP servers) and only touches
// the hivex-office entry under `mcp`. Secrets live in the MCP subprocess's
...
func (l *Launcher) writeHeadlessOpencodeMCPConfig(slug string) (string, error) {
	hivexBinary, err := headlessOpencodeExecutablePath()
	if err != nil {
		return "", fmt.Errorf("resolve hivex binary: %w", err)
	}
	// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — the base opencode
	...
	if runtimeHome == "" {
		return "", fmt.Errorf("resolve runtime home for opencode config: HIVEX_RUNTIME_HOME unset and os.UserHomeDir failed")
	}
	...
	if raw, err := os.ReadFile(baseConfigPath); err == nil && len(raw) > 0 {
		...
		// writing a minimal overlay so hivex keeps booting — but surface the
	...
	entry := l.buildHeadlessOpencodeMCPEntry(hivexBinary, slug)
...
// buildHeadlessOpencodeMCPEntry constructs the `mcp.hivex-office` block for
// opencode.json. The hivebot-managed secrets (broker token, identity, Hive API
...
func (l *Launcher) buildHeadlessOpencodeMCPEntry(hivexBinary string, slug string) map[string]any {
	entry := map[string]any{
		"type":    "local",
		"command": []string{hivexBinary, "mcp-team"},
		"enabled": true,
	}
	envMap := map[string]string{
		"HIVEX_AGENT_SLUG":      slug,
		"HIVEX_BROKER_BASE_URL": l.BrokerBaseURL(),
	}
	if l != nil && l.broker != nil {
		envMap["HIVEX_BROKER_TOKEN"] = l.broker.Token()
	}
	if l != nil && l.isOneOnOne() {
		envMap["HIVEX_ONE_ON_ONE"] = "1"
		if v := strings.TrimSpace(l.oneOnOneBot()); v != "" {
			envMap["HIVEX_ONE_ON_ONE_AGENT"] = v
		}
	}
