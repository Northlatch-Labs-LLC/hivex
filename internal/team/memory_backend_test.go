...
func TestGBrainBackendWriteSharedSendsSlugAndContent(t *testing.T) {
	...
	if !strings.HasPrefix(slug, "hivex-shared--pm--") {
...
func TestResolveMemoryBackendStatusDefaultFallsBackToMarkdown(t *testing.T) {
	// No explicit backend pick lands the user on the markdown wiki — the
	// default shared memory that needs no external service. Silent 'none'
	// would be a worse default: an unset pick must not lose all shared
	// memory.
...
func TestResolveMemoryBackendStatusGBrainReady(t *testing.T) {
	...
	if status.SelectedKind != config.MemoryBackendGBrain || status.ActiveKind != config.MemoryBackendGBrain {
		t.Fatalf("expected gbrain to stay active with no legacy override, got %+v", status)
	}
}
...
func TestResolveMemoryBackendStatusGBrainAnthropicOnlyIsNotSemantic(t *testing.T) {
	...
	t.Setenv("HIVEX_MEMORY_BACKEND", config.MemoryBackendGBrain)
	t.Setenv("HIVEX_ANTHROPIC_API_KEY", "sk-ant-test-anthropic")
	t.Setenv("PATH", t.TempDir())
	t.Setenv("HIVEX_GBRAIN_COMMAND", "")
	t.Setenv("HIVEX_OPENAI_API_KEY", "")
	t.Setenv("OPENAI_API_KEY", "")
...
func TestInferSharedMemoryOwnerFromGBrainSlug(t *testing.T) {
	owner := inferSharedMemoryOwner("hivex-shared--pm--launch-brief--20260416-120000", "")
	if owner != "pm" {
