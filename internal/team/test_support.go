...
// (<root>/.hivex/task-worktrees/<repoToken>/hivex-task-<id>) so test
...
// apart — one stub passing "contains .hivex/task-worktrees/" while the
...
func stubTaskWorktreePath(taskID string) (string, string) {
	...
	return filepath.Join(root, "hivex-task-"+id), "hivex-" + id
}
...
// `git worktree add` against the developer's hivex repo, nor load stale
// state from the user's real ~/.hivex/.
//
...
func DisableRealTaskWorktreeForTests() {
	...
	// If the caller's test package hasn't set HIVEX_RUNTIME_HOME, point
	// it at a process-lifetime leaked tempdir so any implicit
	// ~/.hivex/... write falls through to /tmp instead of the user's
	...
	if os.Getenv("HIVEX_RUNTIME_HOME") == "" {
		if dir, err := os.MkdirTemp("", "hivex-disable-real-worktree-home-*"); err == nil {
			_ = os.Setenv("HIVEX_RUNTIME_HOME", dir)
		}
	}
}
