...
func NewMemoryWorkflowReconciler(worker *WikiWorker, now func() time.Time) *MemoryWorkflowReconciler {
	if now == nil {
		now = time.Now
	}
	return &MemoryWorkflowReconciler{worker: worker, now: now}
}
...
func (r *MemoryWorkflowReconciler) ReconcileTasks(ctx context.Context, tasks []teamTask) ([]teamTask, MemoryWorkflowReconcileReport, error) {
	...
	for i, task := range tasks {
		...
		r.repairTaskWorkflow(&task, &taskResult, timestamp)
		report.Checked++
...
func (r *MemoryWorkflowReconciler) repairTaskWorkflow(task *teamTask, result *MemoryWorkflowReconcileTaskResult, timestamp string) {
...
func (b *Broker) ReconcileMemoryWorkflows(ctx context.Context) (MemoryWorkflowReconcileReport, error) {
	...
	reconciler := NewMemoryWorkflowReconciler(worker, time.Now)
