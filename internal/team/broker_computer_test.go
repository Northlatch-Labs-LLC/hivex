...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/computer"
...
func newComputerTestBroker(t *testing.T, run computer.Runner) *Broker {
	t.Helper()
	t.Setenv("HIVEX_COMPUTERS_DIR", filepath.Join(t.TempDir(), "computers"))
...
func TestMountShapesForRunners(t *testing.T) {
	...
	launch := computer.ContainerMCPLaunch("/usr/local/bin/hivebot", computer.RuntimeDocker, target, computer.ControlEndpoint{URL: "http://127.0.0.1:1/computer-control/cos", Token: "tok"})
	m := &computerMount{slug: "cos", turnID: "t", target: target, Launch: launch}
	servers := m.mcpServers()
	entry, _ := servers["computer"].(map[string]any)
	if entry["command"] != "/usr/local/bin/hivebot" {
	...
	if env["HIVEX_COMPUTER_CONTROL_TOKEN"] != "tok" {
	...
	pairs := m.envPairs()
	if len(pairs) != 2 || !strings.HasPrefix(pairs[0], "HIVEX_COMPUTER_CONTROL_TOKEN=") {
		t.Fatalf("env pairs must be sorted KEY=VALUE: %v", pairs)
	}
	overrides := m.codexOverrides()
	joined := strings.Join(overrides, "\n")
	for _, want := range []string{`mcp_servers.computer.command="/usr/local/bin/hivebot"`, `mcp_servers.computer.args=["computer-mcp","docker","` + target.ContainerName + `","` + computer.CuaSocket + `"]`, `mcp_servers.computer.env_vars=` + tomlStringArray([]string{"HIVEX_COMPUTER_CONTROL_TOKEN", "HIVEX_COMPUTER_CONTROL_URL"})} {
		if !strings.Contains(joined, want) {
			t.Fatalf("codex overrides missing %q in\n%s", want, joined)
		}
	}
	if !isComputerTool("mcp__computer__click") || isComputerTool("mcp__hivex-office__team_broadcast") {
...
func TestConfigRefusesABadBoxKeyBeforeSaving(t *testing.T) {
	...
	defer fake.Close()
	t.Setenv("HIVEX_BOX_API", fake.URL)
	b := newComputerTestBroker(t, noRuntime)
	mux := http.NewServeMux()
...
func writeFakeBoxCLI(t *testing.T, dir string) string {
	...
	script := "#!/bin/sh\n" +
		...
		"  api-key) case \"$2\" in list) echo '{\"apiKeys\":[{\"id\":\"k1\",\"name\":\"hivebot\"},{\"id\":\"k2\",\"name\":\"other\"}]}' ;; revoke) echo \"$3\" >> " + filepath.Join(dir, "revoked") + " ;; *) echo '{\"id\":\"k1\",\"secret\":\"box_minted\"}' ;; esac ;;\n" +
...
func TestBoxSigninFlowMintsAndStoresAKey(t *testing.T) {
	t.Setenv("HIVEX_CONFIG_PATH", filepath.Join(t.TempDir(), "config.json"))
	t.Setenv("HIVEX_BOX_CLI", writeFakeBoxCLI(t, t.TempDir()))
	fake := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// Sign out: revokes only the hivebot key, ends the session, forgets the key.
	...
	revoked, _ := os.ReadFile(filepath.Join(filepath.Dir(os.Getenv("HIVEX_BOX_CLI")), "revoked"))
	if strings.TrimSpace(string(revoked)) != "k1" {
		t.Fatalf("only the hivebot key must be revoked, got %q", revoked)
	}
	_, account = authedJSON(t, srv, b.Token(), http.MethodGet, "/computer/box/account?fresh=1", nil)
...
func TestBoxSigninReportsMissingCLIWhenInstallFails(t *testing.T) {
	t.Setenv("HIVEX_CONFIG_PATH", filepath.Join(t.TempDir(), "config.json"))
	t.Setenv("HIVEX_BOX_CLI", filepath.Join(t.TempDir(), "absent"))
	prev := boxInstaller
