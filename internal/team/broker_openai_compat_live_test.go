...
func liveShimEnabled() bool {
	return strings.TrimSpace(os.Getenv("HIVEX_SHIM_LIVE_TEST")) == "1"
}
...
func TestOpenAIChatCompletionsLive_SuccessPath(t *testing.T) {
	if !liveShimEnabled() {
		t.Skip("spends provider tokens: set HIVEX_SHIM_LIVE_TEST=1")
	}
...
func TestGBrainConsumesTheShim(t *testing.T) {
	...
	cfg["expansion_model"] = "litellm:hivex-configured-provider"
