...
// Operator bot-routines: a scheduler job whose TargetID is a CUSTOM APP id
// (app_<16hex>) fires against the operator bot service (the pi-mono bot/
...
// THIS registry — the previous product avatar's proven engine. The bot
...
// operatorBotBaseURL resolves the operator bot service. HIVEX_AGENT_URL
// overrides; the default matches the bot/ package's dev port.
func operatorBotBaseURL() string {
	if v := strings.TrimSpace(os.Getenv("HIVEX_AGENT_URL")); v != "" {
		return strings.TrimRight(v, "/")
	}
	return "http://127.0.0.1:8820"
}

// isOperatorBotTarget reports whether a routine's owner is a custom app —
// the discriminator between operator dispatch and office channel-post
// dispatch inside processBotJob.
func isOperatorBotTarget(id string) bool {
	return validateCustomAppID(strings.TrimSpace(id)) == nil
}

// operatorRoutineRunResult mirrors bot/src/service.ts POST /routines/run.
...
// postOperatorRoutineRun fires one routine at the bot service.
func postOperatorRoutineRun(baseURL, appID, slug, name, prompt string) (operatorRoutineRunResult, error) {
	...
	if err != nil {
		return operatorRoutineRunResult{}, fmt.Errorf("bot service unreachable: %w", err)
	}
	defer resp.Body.Close()
	raw, err := io.ReadAll(io.LimitReader(resp.Body, 1<<20))
	if err != nil {
		return operatorRoutineRunResult{}, fmt.Errorf("read bot response: %w", err)
	}
	var out operatorRoutineRunResult
	if err := json.Unmarshal(raw, &out); err != nil {
		return operatorRoutineRunResult{}, fmt.Errorf("decode bot response (%d): %w", resp.StatusCode, err)
	}
	if resp.StatusCode != http.StatusOK {
		...
		return operatorRoutineRunResult{}, fmt.Errorf("bot service %d: %s", resp.StatusCode, msg)
	}
	return out, nil
}
...
// goroutine when the bot service answers.
func (w *watchdogScheduler) processOperatorRoutineJob(job schedulerJob) {
	...
	go func() {
		...
		case res.Status == "error":
			...
			run.Events = append(events, "Bot reported an error outcome", "Session "+res.SessionID)
