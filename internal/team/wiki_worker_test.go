...
func TestWikiSearchReturnsHits(t *testing.T) {
	...
	if _, _, err := worker.Enqueue(ctx, "cos", "team/people/nazz.md", "# Nazz\n\nFounded HIVEX swarm in 2026.\n", "create", "m"); err != nil {
		t.Fatalf("enqueue: %v", err)
	}

	// Act
	hits, err := searchArticles(worker.Repo(), "HIVEX swarm")
...
func TestBrokerWikiAuditReturnsFullLineage(t *testing.T) {
	...
	// three distinct authors (system, hivex-bootstrap, operator).
	...
	for _, want := range []string{"system", "hivex-bootstrap", "operator"} {
		if !seen[want] {
...
func TestWikiRootAndBackupDirsHonorRuntimeHome(t *testing.T) {
	...
	if got := WikiRootDir(); got != filepath.Join(dir, ".hivex", "wiki") {
		t.Fatalf("unexpected root: %s", got)
	}
	if got := WikiBackupDir(); got != filepath.Join(dir, ".hivex", "wiki.bak") {
		t.Fatalf("unexpected backup: %s", got)
	}
	r := NewRepo()
	if r.Root() != filepath.Join(dir, ".hivex", "wiki") {
