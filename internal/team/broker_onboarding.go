...
func (b *Broker) onboardingCompleteFn(task string, skipTask bool, blueprintID string, selectedBots []string, companyName string) error {
	...
	// owns the git repo at ~/.hivex/wiki; we write the skeleton files, commit
	// them under the reserved `hivex-bootstrap` author, then regenerate the
	...
	if companyName != "" {
		...
		// has a real company name (e.g. "Hive" → NEX-1). Any Issues minted
...
// materializeBlueprintWiki resolves ~/.hivex/wiki, runs the skeleton
// materializer, commits any newly-written skeletons as `hivex-bootstrap`,
...
func (b *Broker) materializeBlueprintWiki(bp operations.Blueprint) {
	...
	// RecoverDirtyTree on the next launch to fold it into a `hivex-recovery`
	...
	bootstrapMsg := fmt.Sprintf("hivex: materialize %s blueprint skeletons", bp.ID)
...
// default for a brand-new hivebot office: it covers the four functions a
...
func blankSlateOfficeMembersFromBlueprint(blueprint operations.Blueprint, selectedBots []string) []officeMember {
	...
	now := time.Now().UTC().Format(time.RFC3339)
	return []officeMember{
		{Slug: "cos", Name: company.ChiefOfStaffName(), Role: company.ChiefOfStaffRole(), BuiltIn: true, CreatedBy: "hivex", CreatedAt: now},
	}
}
