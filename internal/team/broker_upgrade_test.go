...
func TestHandleUpgradeRun_SuccessReturnsOKWithOutput(t *testing.T) {
	// Happy path: the fake npm exits 0 with characteristic + hivex@…
	...
	if body.Command != "npm install -g hivex@latest" {
		t.Errorf("command=%q drift", body.Command)
	}
	if !strings.Contains(body.Output, "hivex@99.0.0") {
...
func TestHandleUpgradeCheck_IncludesInstallMethod(t *testing.T) {
	...
	upgradeCheckCache.Store(&upgradeCheckCacheEntry{
		res: upgradecheck.Result{
			...
			UpgradeCommand:   "npm install -g hivex@latest",
		},
		storeAt: time.Now(),
	})
	...
	if body.InstallCommand != "npm install hivex@latest" {
	...
	if body.UpgradeCommand != "npm install -g hivex@latest" {
		t.Errorf("upgrade_command should keep its canonical value, got %q", body.UpgradeCommand)
	}
}

func TestPkgDeclaresHivex(t *testing.T) {
	cases := []struct {
		name string
		json string
		want bool
	}{
		{
			...
			json: `{"name":"app","dependencies":{"hivex":"^0.83.0","react":"19"}}`,
			want: true,
		},
		{
			name: "declared in devDependencies",
			json: `{"name":"app","devDependencies":{"hivex":"^0.83.0"}}`,
			want: true,
		},
		{
		...
		},
		{
			...
			json: `{"name":"app","peerDependencies":{"hivex":"^0.83.0"}}`,
			want: false,
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if got := pkgDeclaresHivex([]byte(c.json)); got != c.want {
				t.Errorf("pkgDeclaresHivex(%q)=%v want %v", c.json, got, c.want)
			}
		})
	}
}
...
func TestDetectLocalInstall(t *testing.T) {
	...
	declaresHivex := `{"dependencies":{"hivex":"^0.83.0"}}`
	noHivex := `{"name":"sub"}`
	hivexPkg := `{"name":"hivex","version":"0.83.0"}`

	t.Run("monorepo: subpackage doesn't declare, root does — walks past sub", func(t *testing.T) {
		root := t.TempDir()
		writeFile(t, filepath.Join(root, "package.json"), declaresHivex)
		writeFile(t, filepath.Join(root, "node_modules", "hivex", "package.json"), hivexPkg)
		writeFile(t, filepath.Join(root, "packages", "sub", "package.json"), noHivex)
	...
	t.Run("declares + materialised wins immediately — does not keep walking", func(t *testing.T) {
		// If the cwd's package.json BOTH declares hivex AND has it
		// installed, we accept it — even if a parent project also
		// declares hivex. The nearest match is the user's intent.
		root := t.TempDir()
		writeFile(t, filepath.Join(root, "package.json"), declaresHivex)
		writeFile(t, filepath.Join(root, "node_modules", "hivex", "package.json"), hivexPkg)
		sub := filepath.Join(root, "packages", "sub")
		writeFile(t, filepath.Join(sub, "package.json"), declaresHivex)
		writeFile(t, filepath.Join(sub, "node_modules", "hivex", "package.json"), hivexPkg)
	...
	t.Run("declared but not installed — bails to unknown without walking past", func(t *testing.T) {
		// Fresh-clone case: package.json declares hivex but
		// node_modules/hivex isn't materialised yet. Walking past to
		// a parent project's hivex would silently upgrade the WRONG
		// project. Expect "unknown" so the click-to-run UI surfaces
		// its explicit fallback copy.
		root := t.TempDir()
		// Parent has a fully-installed hivex — tempting to walk to
		// it, but we must not, because the sub-package's intent is
		// clear: install ITS hivex.
		writeFile(t, filepath.Join(root, "package.json"), declaresHivex)
		writeFile(t, filepath.Join(root, "node_modules", "hivex", "package.json"), hivexPkg)
		sub := filepath.Join(root, "packages", "sub")
		writeFile(t, filepath.Join(sub, "package.json"), declaresHivex)
	...
	t.Run("no ancestor declares hivex — unknown", func(t *testing.T) {
		root := t.TempDir()
		writeFile(t, filepath.Join(root, "package.json"), noHivex)
		writeFile(t, filepath.Join(root, "packages", "sub", "package.json"), noHivex)
		got := detectLocalInstall(filepath.Join(root, "packages", "sub"), "")
		if got.Method != "unknown" {
	...
	t.Run("$HOME stops the walk before inspecting its package.json", func(t *testing.T) {
		// Even if a stray ~/package.json declared hivex and a phantom
		// ~/node_modules/hivex existed (some exotic Volta/nvm setup),
		// running `npm install hivex@latest` in $HOME would mutate
		// the user's home directory. The $HOME guard must stop the
		// walk BEFORE the package.json check.
		home := t.TempDir()
		writeFile(t, filepath.Join(home, "package.json"), declaresHivex)
		writeFile(t, filepath.Join(home, "node_modules", "hivex", "package.json"), hivexPkg)
