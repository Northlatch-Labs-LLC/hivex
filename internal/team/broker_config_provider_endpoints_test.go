...
// withHivexHomeDir redirects ~/.hivex to a temp dir and returns the
// matching ~/.hivex path so tests can read the file the broker just
// wrote. Mirrors the pattern used in internal/config/config_test.go.
func withHivexHomeDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	t.Setenv("HIVEX_RUNTIME_HOME", dir)
	return filepath.Join(dir, ".hivex")
}
...
func TestHandleConfig_ProviderEndpointsRoundTrip(t *testing.T) {
	...
	raw, err := os.ReadFile(filepath.Join(hivexDir, "config.json"))
	if err != nil {
