...
// MCP client carries it so gbrain can attribute these pages to hivebot's
// bot-authored shared memory rather than human-curated wiki content.
const gbrainSharedMemorySourceKind = "hivex_shared_memory"
...
func (b gbrainMemoryBackend) WriteShared(ctx context.Context, note SharedMemoryWrite) (string, error) {
	...
	slug = fmt.Sprintf("hivex-shared--%s--%s--%s", actor, slug, time.Now().UTC().Format("20060102-150405"))
...
func ResolveMemoryBackendStatus() MemoryBackendStatus {
	...
	case config.MemoryBackendMarkdown:
		...
		// ~/.hivex/wiki. No API keys, no external service. Always "ready"
		...
		status.Detail = "Markdown-backed team wiki at ~/.hivex/wiki is configured. Every edit commits to git with per-bot authorship."
	case config.MemoryBackendMemanto:
		memantoBackend := memantoMemoryBackend{}
		if !memantoBackend.Ready() {
			status.Detail = "Memanto backend selected, but the instance is not answering."
			status.NextStep = "Set HIVEX_MEMANTO_URL and HIVEX_MEMANTO_API_KEY to a running Memanto instance (see third_party/memanto/DEPLOY.md)."
			return status
		}
...
func gbrainMCPEnv() map[string]string {
	env := map[string]string{}
	// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — gbrain is a
...
func parseGBrainSharedMemoryOwner(identifier string) string {
	...
	if strings.TrimSpace(parts[0]) != "hivex-shared" {
		return ""
	}
