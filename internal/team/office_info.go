...
func officeInfoPath() string {
	...
	if home == "" {
		return filepath.Join(".hivex", "team", "office.json")
	}
	return filepath.Join(home, ".hivex", "team", "office.json")
}
...
// workspace, if one is alive, on loopback, and identifiably hivebot. Front-ends
...
// officeIsHivex returns true if a live hivebot web UI answers at webURL. It is the
// liveness signal (cross-platform, no OS process syscalls) AND an identity check
// — a non-hivebot service that grabbed a recycled loopback port must not be
// attached to. Redirects are not followed (a real broker serves its UI at "/").
func officeIsHivex(webURL string) bool {
	...
	return strings.Contains(strings.ToLower(string(body)), "hivex")
}
