...
// broker_slack_agents.go registers FOREIGN Slack bots — bots that live in a
...
// outbound mention map: an office message that @mentions a registered bot's
// slug is rendered with a real <@U…> ping so the foreign bot actually wakes.
//
//	POST /slack/bots { user_id, name, slug? } → { slug, name, user_id, created }
//	GET  /slack/bots → { bots: [ { slug, name, user_id } ] }
...
type slackBotRegisterRequest struct {
...
// handleSlackBots serves the foreign-bot registry: POST registers (or
// idempotently re-registers) a foreign Slack bot as a bridged member; GET lists
// the current registrations.
func (b *Broker) handleSlackBots(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		writeJSON(w, http.StatusOK, map[string]any{"agents": b.slackBots()})
	case http.MethodPost:
		var body slackBotRegisterRequest
		...
		created, err := b.RegisterSlackBot(slug, name, userID)
		if err != nil {
...
// RegisterSlackBot registers a foreign Slack bot as a bridged office member
...
func (b *Broker) RegisterSlackBot(slug, name, userID string) (created bool, err error) {
	...
	if conflictErr := b.slackBotConflict(slug, userID); conflictErr != nil {
		return false, conflictErr
	}
	existed := b.memberExists(slug)
	if err := b.EnsureBridgedMember(slug, name, "slack"); err != nil {
		return false, fmt.Errorf("register slack bot %q: %w", slug, err)
	}
	if err := b.SetMemberProvider(slug, provider.ProviderBinding{
		...
		return false, fmt.Errorf("bind slack bot %q: %w", slug, err)
	}
	// Make the bot visible in every bridged room, not just #general (which
...
// slackBotConflict checks the one-to-one invariants between office slugs and
// Slack user ids under the broker lock. A nil return means slug+userID is free
// or already bound to exactly this pair. The user-id space is shared between
// FOREIGN registrations (Kind == slack) and SPAWNED office bots
// (broker_slack_spawn.go — own runtime + Slack identity): one Slack user id
// maps to exactly one office member either way, so attribution and the
// ingress/echo-guard decision stay unambiguous.
func (b *Broker) slackBotConflict(slug, userID string) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	for i := range b.members {
		...
		if m.Slug == slug {
			if boundID == userID {
				if m.Provider.Kind != provider.KindSlack {
					// A SPAWNED bot already owns this slug + user id.
					// Re-registering it as FOREIGN would flip its runtime to
					// the gateway kind and stop its turns from dispatching.
					return fmt.Errorf("slug %q is a spawned office bot, not a foreign one", slug)
				}
...
// carrying a "slack" surface, so bots registered after a channel was bridged
...
// slackBots returns the registered foreign bots (members whose provider
// binding is Kind == slack), for the GET listing.
func (b *Broker) slackBots() []slackBotView {
	b.mu.Lock()
	defer b.mu.Unlock()
	out := []slackBotView{}
	for i := range b.members {
		...
		out = append(out, slackBotView{Slug: m.Slug, Name: m.Name, UserID: m.Provider.Slack.UserID})
	}
	return out
}

// ChannelHasSurface reports whether the channel carries a surface bound to
// the given provider (e.g. "slack" for a bridged Slack channel).
func (b *Broker) ChannelHasSurface(slug, provider string) bool {
	b.mu.Lock()
	defer b.mu.Unlock()
	ch := b.findChannelLocked(normalizeChannelSlug(slug))
	return ch != nil && ch.Surface != nil && ch.Surface.Provider == provider
}

// OfficeLeadSlug returns the slug of the office lead (the coordinator whose
// voice the Slack bot speaks with), or "" when no members exist.
func (b *Broker) OfficeLeadSlug() string {
	b.mu.Lock()
	defer b.mu.Unlock()
	return officeLeadSlugFrom(b.members)
}
...
// SlackBotSlugByUserID resolves a Slack user id to the office slug of a
// registered foreign bot, or "" when the id is not registered. This is the
// transport's inbound allowlist lookup — an empty return means "drop".
func (b *Broker) SlackBotSlugByUserID(userID string) string {
...
// SlackBotUserIDBySlug resolves an office slug to its registered Slack user
// id, or "" when slug is not a registered foreign bot. This is the outbound
// mention map — the ONLY source a real <@U…> ping may be built from (never
// message text).
func (b *Broker) SlackBotUserIDBySlug(slug string) string {
...
// via GET /slack/bots, and embedded in outbound <@…> mentions.
