...
// slack_home.go renders the hivex Slack app's App Home tab: a native Block
...
const slackHomeLinksBlockID = "hivex_home_links"
...
func buildSlackHomeBlocks(b *Broker) []slack.Block {
	...
	if base != "" {
		...
		blocks = append(blocks, slack.NewActionBlock(slackHomeLinksBlockID,
			button("hivex_home_tasks", "📋 Task board", base+"/tasks"),
			button("hivex_home_wiki", "📚 Wiki", base+"/wiki"),
			button("hivex_home_inbox", "📥 Inbox", base+"/inbox"),
			button("hivex_home_agents", "🤖 Bots", base+"/agents"),
		))
