...
func TestBuildMCPServerMap_GBrainCredentialsFlowThroughOffice(t *testing.T) {
	...
	entry, ok := servers["hivex-office"]
	if !ok {
		t.Fatalf("'hivex-office' server must be present when GBrain is selected, got servers: %v", mapKeys(servers))
	}
	server, ok := entry.(map[string]any)
	if !ok {
		t.Fatalf("expected hivex-office entry to be an object, got %T", entry)
	}
...
// TestBuildMCPServerMap_UnknownMemoryBackendKeepsOfficeServer pins the
// resilience rule: a leftover unrecognized HIVEX_MEMORY_BACKEND value (saved
// by an older build) neither breaks server-map building nor mounts a stray
// server — the office server stays mounted.
func TestBuildMCPServerMap_UnknownMemoryBackendKeepsOfficeServer(t *testing.T) {
	t.Setenv("HIVEX_CONFIG_PATH", filepath.Join(t.TempDir(), "config.json"))
	t.Setenv("HIVEX_MEMORY_BACKEND", "legacy-retired")
	...
	if _, ok := servers["legacy-retired"]; ok {
		t.Fatalf("an unrecognized backend must never mount a named server, got servers: %v", mapKeys(servers))
	}
	if _, ok := servers["hivex-office"]; !ok {
		t.Fatalf("'hivex-office' server must always be present, got servers: %v", mapKeys(servers))
	}
}
