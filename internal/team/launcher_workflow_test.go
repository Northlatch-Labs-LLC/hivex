...
func TestProcessDueWorkflowJobUsesComposioProvider(t *testing.T) {
	...
	t.Setenv("HIVEX_ACTION_PROVIDER", "composio")
	t.Setenv("HIVEX_API_KEY", "hive-test-key")
	t.Setenv("HIVEX_COMPOSIO_API_KEY", "cmp-test-key")
	t.Setenv("HIVEX_COMPOSIO_USER_ID", "founder@northlatch.example")
	...
	t.Setenv("HIVEX_DEV_URL", server.URL)
	t.Setenv("HIVEX_COMPOSIO_BASE_URL", server.URL)
	...
	definition, _ := json.Marshal(map[string]any{
		...
		"inputs": map[string]any{
			...
			"recipient_email": "founder@northlatch.example",
