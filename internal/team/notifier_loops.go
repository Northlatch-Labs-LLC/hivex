...
// ~/.hivex/logs/panics.log which the user often shares verbatim
...
// ~/.hivex/logs/panics.log so the broker stays up even if a specific action
...
func recoverPanicTo(site, extra string) {
	...
	if home := config.RuntimeHomeDir(); home != "" {
		...
		// `rm -rf ~/.hivex`) the logs directory does not yet
		...
		// systems where hivex runs under a service account whose
