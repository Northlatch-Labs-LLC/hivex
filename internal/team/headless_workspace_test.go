...
// layout: <HIVEX_RUNTIME_HOME>/.hivex/bot-scratch/<slug>, created on
...
func TestAgentScratchDir_UnderRuntimeHomeCreatedOnDemand(t *testing.T) {
	...
	want := filepath.Join(home, ".hivex", "agent-scratch", "eng")
	if got != want {
	...
	if got := botScratchDir(""); !strings.HasPrefix(got, filepath.Join(home, ".hivex", "agent-scratch")) {
...
func TestHeadlessTurnWorkspace_WorktreeElseScratch(t *testing.T) {
	...
	wantScratch := filepath.Join(home, ".hivex", "agent-scratch", "cos")
...
func TestRunHeadlessClaudeTurn_NoWorktreeRunsInAgentScratch(t *testing.T) {
	...
	for _, kv := range captured.Env {
		if strings.HasPrefix(kv, "HIVEX_WORKTREE_PATH=") {
			t.Fatalf("scratch-dir turn must not advertise HIVEX_WORKTREE_PATH; env %q", kv)
		}
	}
}
