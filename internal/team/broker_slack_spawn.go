...
// broker_slack_spawn.go spawns hivebot-runtime office bots that carry their
// OWN Slack identity — the INVERSE of broker_slack_agents.go's foreign
// registry. A foreign bot runs outside hivebot and its Slack posts are
// ALLOWED inbound; a spawned bot runs on hivebot's own runtime, posts to
...
//	  → reads the bot token from env var HIVEX_SLACK_AGENT_<SLUG>_TOKEN
...
// flows through the main hivex bot's Socket Mode connection), so there is no
...
// must live in: HIVEX_SLACK_AGENT_<SLUG>_TOKEN with non-alphanumerics mapped
...
func slackSpawnTokenEnv(slug string) string {
	...
	return "HIVEX_SLACK_AGENT_" + mapped + "_TOKEN"
}
...
func slackSpawnGuide(name, tokenEnv, slug string) []string {
	return []string{
		...
		fmt.Sprintf("5. Set the token in the hivebot broker's environment as %s (never paste the raw token into a request body) and restart hivebot so the env var is visible.", tokenEnv),
		fmt.Sprintf(`6. Finish with: POST /slack/agents/spawn/complete {"slug":%q} — hivebot verifies the token, discovers the bot's Slack user id, and creates the team agent.`, slug),
	}
}
...
// SPAWNED office bot — a member running on hivebot's own runtime (Kind !=
