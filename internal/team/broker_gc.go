...
// Defaults to 500; overridable via HIVEX_MAX_MESSAGES.
func maxMessagesFromEnv() int {
	if v, err := strconv.Atoi(os.Getenv("HIVEX_MAX_MESSAGES")); err == nil && v > 0 {
		return v
	}
...
// Overridable via HIVEX_TASK_RETENTION_DAYS.
func taskRetentionFromEnv() time.Duration {
	if v, err := strconv.Atoi(os.Getenv("HIVEX_TASK_RETENTION_DAYS")); err == nil && v > 0 {
