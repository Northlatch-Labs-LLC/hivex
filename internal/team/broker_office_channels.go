...
// to ~/.hivex/config.json so a locally-authenticated client can't
...
//
// Loopback hosts are allowed — hivex's whole point is local-LLM
...
// handleConfig exposes GET/POST over ~/.hivex/config.json for the web UI
...
func (b *Broker) handleConfig(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		envLLMProvider := strings.TrimSpace(os.Getenv("HIVEX_LLM_PROVIDER"))
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"hiveapi_key_set":      config.ResolveHiveAPIAPIKey() != "",
	...
	case http.MethodPost:
		...
		var body struct {
			...
			HiveAPIKey      *string `json:"hiveapi_api_key,omitempty"`
		...
		if body.TelegramToken != nil {
			...
			if body.HiveAPIKey != nil {
				cfg.HiveAPIAPIKey = strings.TrimSpace(*body.HiveAPIKey)
			}
			changed = true
		}
		...
		if body.ProviderEndpoints != nil {
			...
			for kind, ep := range *body.ProviderEndpoints {
				...
				// anyone with write access to ~/.hivex/config.json) must
