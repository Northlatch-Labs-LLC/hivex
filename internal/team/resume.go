...
func isHumanOrSystemSender(from string) bool {
	...
	return isHumanMessageSender(f) || f == "system"
}
...
// at least one BOT message (not human/system) in allMessages has ReplyTo
...
func findUnansweredMessages(humanMsgs, allMessages []channelMessage) []channelMessage {
	...
	// Skip replies from human/hive/system senders — only bot replies count.
