...
func (b *Broker) handleUpgradeCheck(w http.ResponseWriter, r *http.Request) {
	...
	// this, the chip showed "npm install -g hivex@latest" universally
	// even when the broker would actually run `npm install hivex@latest`
...
// running hivex was installed:
//
//   • global (npm install -g hivex)  → `npm install -g hivex@latest`
//   • local  (project devDep)        → `npm install hivex@latest` in that dir
//   • source build / standalone      → 400 with install_method="unknown"
//
// The detection runs `npm root -g` and walks up from the caller's cwd
// looking for `node_modules/hivex/package.json`, so we never lie about which
...
type upgradeRunResult struct {
	...
	Command       string `json:"command,omitempty"`     // human-readable rendered cmd (e.g. "npm install -g hivex@latest")
...
type upgradeInstallPlan struct {
	...
	Args       []string // e.g. ["install", "-g", "hivex@latest"]; empty when Method=="unknown"
...
func runUpgradeCmd(ctx context.Context, plan upgradeInstallPlan) ([]byte, bool, error) {
	...
	// real error / `+ hivex@x.y.z` line lives). Setting the same
...
// detectUpgradeInstall figures out how hivex is installed on this machine.
...
// that BOTH declares hivex in dependencies/devDependencies AND has
// `node_modules/hivex/package.json` materialised. A non-declaring ancestor
...
// wrong project's hivex, and "unknown" plus the fallback copy is safer.
//
// The walk is bounded by:
//   - 32-iteration cap (belt-and-suspenders for runaway symlink loops),
//   - filesystem root (parent == dir),
//   - $HOME — even if a `package.json` at $HOME declared hivex, running
...
// because the hivex CLI starts the broker and inherits cwd. A future
...
func detectUpgradeInstall(ctx context.Context) upgradeInstallPlan {
	...
	if out, err := exec.CommandContext(rootCtx, "npm", "root", "-g").Output(); err == nil {
		...
		if globalRoot != "" {
			if _, err := os.Stat(filepath.Join(globalRoot, "hivex", "package.json")); err == nil {
				return upgradeInstallPlan{
					Method:  "global",
					Args:    []string{"install", "-g", "hivex@latest"},
					Command: "npm install -g hivex@latest",
				}
			}
		}
	}
	// Local install: walk up from cwd, skipping non-declaring ancestors so
	// monorepos with hivex at the workspace root still detect cleanly when
	// cwd is inside a sub-package that doesn't declare it directly.
	// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — detectLocalInstall
	// walks the npm dependency tree from the real user HOME, not hivebot state.
...
// that declares hivex in package.json AND has node_modules/hivex installed.
// Skips ancestors that don't declare hivex (monorepo sub-packages); stops
...
func detectLocalInstall(cwd, homeDir string) upgradeInstallPlan {
	...
	for i := 0; i < 32; i++ {
		...
		// hivex can't redirect the install into the user's home.
		if homeDir != "" && dir == homeDir {
			break
		}
		projectPkg := filepath.Join(dir, "package.json")
		if data, err := os.ReadFile(projectPkg); err == nil {
			if pkgDeclaresHivex(data) {
				candidate := filepath.Join(dir, "node_modules", "hivex", "package.json")
				if _, err := os.Stat(candidate); err == nil {
					return upgradeInstallPlan{
						Method:     "local",
						Args:       []string{"install", "hivex@latest"},
						WorkingDir: dir,
						Command:    "npm install hivex@latest",
					}
				}
				// Declares hivex but the dep isn't materialised
				// (fresh clone, skipped `npm install`). Don't keep
				// walking — a parent project's hivex is NOT this
				// project's hivex, and silently upgrading it would
			...
			// package.json that doesn't declare hivex: a monorepo
...
// pkgDeclaresHivex reports whether the given package.json bytes declare
// hivex in dependencies or devDependencies. Robust to extra unknown fields
...
func pkgDeclaresHivex(data []byte) bool {
	var pkg struct {
	...
	if _, ok := pkg.Dependencies["hivex"]; ok {
		return true
	}
	if _, ok := pkg.DevDependencies["hivex"]; ok {
		return true
	}
	return false
}
...
func (b *Broker) handleUpgradeRun(w http.ResponseWriter, r *http.Request) {
	...
	if plan.Method == "unknown" {
		...
		_ = json.NewEncoder(w).Encode(upgradeRunResult{
			OK:            false,
			InstallMethod: plan.Method,
			Error:         "Could not detect how hivex was installed (no npm, or hivex isn't under a global/local node_modules). Try `npm install -g hivex@latest` from a terminal.",
		})
		return
	}
	...
	// an in-flight install. Without this, `hivex restart` clicked right
	...
	// progress hits stderr, the final `+ hivex@x.y.z` line hits stdout,
