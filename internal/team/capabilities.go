...
func DetectRuntimeCapabilitiesWithOptions(opts CapabilityProbeOptions) RuntimeCapabilities {
	...
	claudeStatus := probeBinaryCapability("claude", "Install claude so hivebot can start teammate runtime sessions.")
	codexStatus := probeBinaryCapability("codex", "Install Codex CLI and run `codex login` so hivebot can start the headless Codex office runtime.")
	opencodeStatus := probeBinaryCapability("opencode", "Install Opencode CLI (https://opencode.ai) and configure your provider credentials so hivebot can start the headless Opencode office runtime.")
...
func probeTmuxCapability() (CapabilityStatus, TmuxCapability) {
	...
	if err != nil {
		return CapabilityStatus{
			...
			NextStep: "Install tmux so hivebot can manage the team session.",
		}, capability
	}
...
func (t TmuxCapability) summaryDetail() string {
	...
	if !t.ServerRunning {
		return fmt.Sprintf("%s is installed, but the hivebot tmux server on socket %s is not running yet.", version, t.SocketName)
	}
...
func (t TmuxCapability) nextStep() string {
	if t.BinaryPath == "" {
		return "Install tmux so hivebot can manage the team session."
	}
	if !t.ServerRunning {
		return "Launch hivebot to create the tmux office session."
	}
	if _, ok := t.targetSession(); !ok {
		return "Restart hivebot to recreate the missing office session."
	}
	return ""
}
...
func (t TmuxCapability) FormatLines() []string {
	...
	if !t.ServerRunning {
		lines = append(lines, fmt.Sprintf("- hivebot session: not running yet (%s)", t.SessionName))
	} else if session, ok := t.targetSession(); ok {
		lines = append(lines, fmt.Sprintf("- hivebot session: running (%d attached, %d windows)", session.Attached, session.Windows))
	} else {
		lines = append(lines, fmt.Sprintf("- hivebot session: missing from socket %s", t.SocketName))
	}
	if len(t.Sessions) > 0 {
