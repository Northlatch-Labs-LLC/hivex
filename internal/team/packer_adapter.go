...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/packer"
)
...
// PlanStep renders the structured plan step from the task's R4 Definition
// (goal / deliverables / success criteria) — the CEO/human-authored intake
// contract that replaced IssueDraftSpec in the core-loop rebuild (#1062). It
// deliberately does NOT fall back to the raw teamTask.Details: that free-form
// field is the untrusted task body the egress policy denies, never the
// defined scope. A task with no Definition yields an empty plan step.
...
func renderPlanStep(task *teamTask) string {
	...
	writePlanSection(&sb, "Goal", def.Goal)
	deliverables := make([]string, 0, len(def.Deliverables))
	for _, d := range def.Deliverables {
		entry := strings.TrimSpace(d.Name)
		if entry == "" {
			continue
		}
		...
		deliverables = append(deliverables, entry)
	}
	writePlanSection(&sb, "Deliverables", strings.Join(deliverables, "; "))
	writePlanSection(&sb, "Success criteria", strings.Join(def.SuccessCriteria, "; "))
