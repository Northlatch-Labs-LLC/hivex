...
//
// The office-era hivex kept a files-on-disk knowledge base under
// <runtime home>/.hivex/wiki: team articles in team/<category>/*.md and each
...
func (b *Broker) legacyKnowledgePages() []appKnowledgePage {
	b.legacyKnowledgeOnce.Do(func() {
		root := filepath.Join(config.RuntimeHomeDir(), ".hivex", "wiki")
...
func (b *Broker) handleLegacyKnowledgeArtifact(w http.ResponseWriter, r *http.Request) {
	...
	path := filepath.Join(config.RuntimeHomeDir(), ".hivex", "wiki", legacyArtifactsRelDir, name)
...
// "<!-- hivex:entity-article ... -->" marker on generated entity articles);
