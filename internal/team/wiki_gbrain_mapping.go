...
// wiki_gbrain_mapping.go — the hivebot ⇄ gbrain data mapping.
//
// gbrain models a brain as pages + typed links + tags. hivebot models the wiki as
...
// gbrainLinkSource marks every edge hivebot writes, so a brain shared with a
...
const (
	gbrainLinkRedirect       = "hivex_redirect"
	gbrainLinkCategory       = "hivex_category"
	gbrainLinkParentCategory = "hivex_parent_category"
)

// gbrainFactBlobKey is the frontmatter key holding the base64 JSON TypedFact.
const gbrainFactBlobKey = "hivex_fact"

// gbrainEntityBlobKey is the frontmatter key holding the base64 JSON IndexEntity.
const gbrainEntityBlobKey = "hivex_entity"
...
// entityKindToPageType maps a hivebot entity kind onto gbrain's page-type
...
// entitySlug returns the gbrain slug for a hivebot entity slug. Entity slugs are
...
// slug it references.
//
// hivebot triplet objects come in two shapes (§4.2): a bare slug ("carol-mei") or
