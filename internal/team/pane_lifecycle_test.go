...
func TestIsMissingTmuxSession_RecognizesCommonOutputs(t *testing.T) {
	for _, s := range []string{
		"no server running on /tmp/tmux-501/hivex",
		"can't find session: hivex-team",
...
func TestChannelStderrLogPath_NonEmpty(t *testing.T) {
	...
	if !strings.HasSuffix(got, "channel-stderr.log") && !strings.HasSuffix(got, ".hivex-channel-stderr.log") {
...
func TestChannelPaneSnapshotPath_NonEmpty(t *testing.T) {
	...
	if !strings.HasSuffix(got, "channel-pane.log") && !strings.HasSuffix(got, ".hivex-channel-pane.log") {
