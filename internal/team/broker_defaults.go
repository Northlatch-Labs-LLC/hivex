...
func defaultOfficeMembers() []officeMember {
	...
	for _, cfg := range manifest.Members {
		...
		members = append(members, memberFromSpec(cfg, "hivex", now, builtIn))
	}
	return members
}
...
func defaultTeamChannels() []teamChannel {
	...
	for _, channel := range manifest.Channels {
		...
		// broker_pane.go, broker_misc_handlers.go, cmd/hivebot/channel_splash.go,
		// and two in cmd/hivebot/channelui/manifest.go — plus LoadRuntimeManifest
