...
//	HIVEX_GBRAIN_IT=1 HOME=/tmp/gbrain-demo go test ./internal/team/ \
...
func TestGBrainCompoundingCycle(t *testing.T) {
	if os.Getenv("HIVEX_GBRAIN_IT") != "1" {
		t.Skip("set HIVEX_GBRAIN_IT=1 (and HOME at an Ollama-embedded gbrain brain) to run the live compounding-cycle test")
	}
	...
	for k, want := range map[string]string{"kind": "chat", "origin": "pricing-2026-06-26", "source": "hivex-office"} {
	...
	if !strings.Contains(page.Content, "Captured from hivebot office") {
