...
import (
	...
	teamTransport "github.com/Northlatch-Labs-LLC/hivex/internal/team/transport"
)
...
// hivebot presents as ONE coordinating bot in Slack. Internal bots carry no
...
func TestSlackRenderOfficeTagsForRealSlackReaders(t *testing.T) {
	...
	if !strings.Contains(out.Text, "Approved hivex's request") {
...
// Tagging the hivex bot in Slack is how a human talks to the office: the
// inbound <@BOT> mention must translate to "@<lead>" so the lead wakes, and
// the human sender must be classified as HUMAN (the "human:<id>" actor) so the
// response gets human priority over whatever the office is doing.
func TestSlackInboundHumanMentionOfBotWakesLead(t *testing.T) {
	api := newFakeSlackAPI()
	...
	if err := tr.routeInbound(context.Background(), host, &slackevents.MessageEvent{
		User: "U7", Channel: "C0123", TimeStamp: "9.1",
		Text: "<@UBOT> what is the status? loop in <@U777> and <@U7|naj> later",
	}); err != nil {
	...
	m := msgs[0]
	if m.From != "human:u7" {
		t.Fatalf("human sender must post as the human:<id> actor, got %q", m.From)
	}
