...
// hivexServer is an httptest server that looks like a hivebot web UI (loopback +
// "hivex" in the body) so officeIsHivex accepts it.
func hivexServer(t *testing.T) *httptest.Server {
	t.Helper()
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte("<title>hivebot — the hivex office</title>"))
	}))
}
...
func TestRunningOfficeURL(t *testing.T) {
	...
	t.Run("live reachable hivebot peer on loopback is attachable", func(t *testing.T) {
		t.Setenv("HIVEX_RUNTIME_HOME", t.TempDir())
		srv := hivexServer(t)
		defer srv.Close()
	...
	t.Run("a non-hivebot service on the recorded port is not attached", func(t *testing.T) {
		t.Setenv("HIVEX_RUNTIME_HOME", t.TempDir())
		...
		if url, ok := RunningOfficeURL(); ok {
			t.Errorf("RunningOfficeURL() = %q, true; want \"\", false (foreign service is not a hivebot office)", url)
		}
	})
