...
func TestCleanupPersistedTaskWorktreesRemovesUniqueTrackedWorktrees(t *testing.T) {
	...
	state := struct {
		...
		Tasks: []teamTask{
			{ID: "task-1", WorktreePath: "/tmp/hivex-task-1", WorktreeBranch: "hivex-task-1"},
			{ID: "task-2", WorktreePath: "/tmp/hivex-task-1", WorktreeBranch: "hivex-task-1"},
			{ID: "task-3", WorktreePath: "/tmp/hivex-task-3", WorktreeBranch: "hivex-task-3"},
		},
	}
...
func TestDefaultPrepareTaskWorktreeOverlaysDirtyWorkspace(t *testing.T) {
	...
	run("git", "init", "-b", "main")
	run("git", "config", "user.name", "hivebot Test")
	run("git", "config", "user.email", "hivex@example.com")
	...
	skippedCachePath := filepath.Join(repoDir, ".hivex", "cache", "go-build", "cos", "trim.txt")
	...
	skippedStatePath := filepath.Join(repoDir, ".hivex", "browser-home-test", ".hivex", "team", "broker-state.json")
	...
	if _, err := os.Stat(filepath.Join(path, ".hivex", "cache", "go-build", "cos", "trim.txt")); !os.IsNotExist(err) {
		t.Fatalf("expected generated cache file to be skipped, stat err=%v", err)
	}
	if _, err := os.Stat(filepath.Join(path, ".hivex", "browser-home-test", ".hivex", "team", "broker-state.json")); !os.IsNotExist(err) {
		t.Fatalf("expected runtime state under .hivex to be skipped, stat err=%v", err)
	}
...
func TestDefaultPrepareTaskWorktreeOverlaysCompletedSiblingTaskWorkspace(t *testing.T) {
	...
	run(repoDir, "git", "config", "user.name", "hivebot Test")
	run(repoDir, "git", "config", "user.email", "hivex@example.com")
	if err := os.WriteFile(filepath.Join(repoDir, "tracked.txt"), []byte("base\n"), 0o644); err != nil {
		t.Fatalf("write tracked baseline: %v", err)
	}
	...
	state := struct {
		...
		Tasks: []teamTask{
			...
			},
			{
				...
				WorktreeBranch: "hivex-missing-task-12",
			},
		},
	}
	raw, err := json.Marshal(state)
...
func TestWorktreePathLooksSafeAllowsManagedAndLegacyRoots(t *testing.T) {
	...
	managed := filepath.Join(taskWorktreeRootDir("repo"), "hivex-task-task-1")
	if !worktreePathLooksSafe(managed) {
		t.Fatalf("expected managed worktree path to be safe: %q", managed)
	}

	legacy := filepath.Join(os.TempDir(), "hivex-task-task-legacy")
