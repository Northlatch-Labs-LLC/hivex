...
func TestNewLauncherFromScratchUsesGenericOffice(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	// Pair HOME with HIVEX_RUNTIME_HOME so NewLauncher reads from this
	...
	t.Setenv("HIVEX_START_FROM_SCRATCH", "1")

	l, err := NewLauncher("from-scratch")
	if err != nil {
		t.Fatalf("NewLauncher(from-scratch): %v", err)
	}
	if got := l.PackName(); got != "HIVEX swarm" {
		t.Fatalf("PackName: got %q, want %q", got, "HIVEX swarm")
	}
...
func TestOfficeMembersSnapshotPrefersPersistedStateOverPack(t *testing.T) {
	...
	t.Setenv("HIVEX_BROKER_STATE_PATH", statePath)
...
func TestLoadRunningSessionModePrefersLiveBrokerState(t *testing.T) {
	...
	t.Setenv("HIVEX_BROKER_TOKEN", "test-token")
	t.Setenv("HIVEX_BROKER_BASE_URL", server.URL)
...
func TestEnsureMCPConfigUsesLocalGoTeamServer(t *testing.T) {
	...
	server, ok := cfg.MCPServers["hivex-office"]
	if !ok {
		t.Fatal("expected hivex-office MCP server entry")
	}
...
func TestChannelPaneNeedsRespawn(t *testing.T) {
	tests := []struct {
		...
		{name: "healthy channel", status: "0 0 hivex", want: false},
...
func TestChannelPaneLogPaths(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	if got := channelStderrLogPath(); !strings.Contains(got, ".hivex/logs/channel-stderr.log") {
		t.Fatalf("unexpected stderr log path: %q", got)
	}
	if got := channelPaneSnapshotPath(); !strings.Contains(got, ".hivex/logs/channel-pane.log") {
...
func TestNotificationTargetsCEOWakesOnlyOnMention(t *testing.T) {
	...
	for _, focus := range []bool{true, false} {
		...
		// Automation traffic is inbound external work (scheduled triggers,
		// external notifications), not internal bot chatter — it routes to the
		// CEO to triage even when untagged. The router's wake predicate is
		// Kind=="automation"; cover the production sender shape.
		for _, inbound := range []struct {
			...
			{name: "automation ingest", msg: channelMessage{From: "hive", Kind: "automation", Content: "inbound: new lead assigned to the workspace"}},
		} {
			inbound := inbound
...
func TestTaskNotificationContentIncludesWorktreeDetails(t *testing.T) {
	...
	got := l.taskNotificationContent(officeActionLog{
		...
		WorktreeBranch: "hivex-task-10",
		WorktreePath:   "/tmp/hivex-task-task-10",
	})
	if !strings.Contains(got, "execution local_worktree") {
		t.Fatalf("expected execution mode in content: %q", got)
	}
	if !strings.Contains(got, "branch hivex-task-10") || !strings.Contains(got, "path /tmp/hivex-task-task-10") {
		t.Fatalf("expected worktree details in content: %q", got)
	}
	if !strings.Contains(got, `working_directory="/tmp/hivex-task-task-10"`) {
...
func TestBuildTaskExecutionPacketLocalWorktreeForbidsNestedOffice(t *testing.T) {
	...
	got := l.buildTaskExecutionPacket("eng", officeActionLog{
		...
		Details:       "Wire `docs/youtube-factory/default-channel-pack.yaml` into `cmd/hivebot/main.go` first.",
		Owner:         "eng",
		status:        "in_progress",
		ExecutionMode: "local_worktree",
		WorktreePath:  "/tmp/hivex-task-task-11",
	...
	if !strings.Contains(got, "never launch another hivebot office") {
	...
	if !strings.Contains(got, "Named file targets: docs/youtube-factory/default-channel-pack.yaml, cmd/hivebot/main.go") {
...
func TestBuildPromptIncludesTaskStatusAndWorktreeGuidance(t *testing.T) {
	...
	if !strings.Contains(specialist, "Never launch another hivebot office") {
