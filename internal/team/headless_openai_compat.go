...
// bot bound to an OpenAI-compatible runtime (mlx-lm, ollama, exo, Hermes,
// or OpenClaw Gateway HTTP).
//
// Flow:
//  1. Spawn `hivex mcp-team` and connect an MCP client → list of tools.
//  2. Stream the model with the bot prompt, the user notification, and
...
//
// If the MCP bridge fails to start (e.g. hivex binary path is unresolvable)
// the runner falls back to text-only chat so the user gets a visible reply
// rather than a silent failure. This is intentionally noisy in the bot
...
func (l *Launcher) runHeadlessOpenAICompatTurn(ctx context.Context, slug string, notification string, channel ...string) error {
	...
	// per-task provider, not just the bot binding.
	...
	// Broker.BotStream is keyed by slug and returns the same buffer on
	...
	// We push the failure into botStream too so it shows up in the live
	...
	if bridgeErr != nil {
		...
		if botStream != nil {
			botStream.PushTask(activeTaskID, fmt.Sprintf("[bridge unavailable: %v — replying without tools]", bridgeErr))
		}
		tools = nil
	}

	// Stable tool order for the prompt. Slightly more cache-friendly across
	// turns and easier for humans reading bot logs.
	sort.Slice(tools, func(i, j int) bool { return tools[i].Name < tools[j].Name })
	toolByName := make(map[string]bot.BotTool, len(tools))
	for _, t := range tools {
		toolByName[t.Name] = t
	}

	// Some OpenAI-compat upstreams (Hermes api_server, OpenClaw HTTP) ignore
	// the caller's tools[] field and run their own bot loop. We can still
	// make tool calls work by prompting the model to emit
	// `<tools>{"name":"X","arguments":{...}}</tools>` in its text — hivebot's
	...
	if kind == provider.KindHermesBot || kind == provider.KindOpenclawHTTP {
		if len(tools) > 0 {
	...
	msgs := make([]bot.Message, 0, 4)
	if systemPrompt != "" {
		msgs = append(msgs, bot.Message{Role: "system", Content: systemPrompt})
	}
	msgs = append(msgs, bot.Message{Role: "user", Content: userPrompt})
	...
	state := newOpenAICompatTurnState(&runtimeTurnSinks{
		...
		slug:   slug,
		taskID: activeTaskID,
		// botStream may be nil during edge-case launcher setups; the
		// sink's pushBotStream short-circuits on nil so the state
	...
	// bot's turn into its own row. Format mirrors bot.nextTaskID.
	...
	loop := openAICompatToolLoop{
		...
		taskLogRoot: bot.DefaultTaskLogRoot(),
		taskID:      taskID,
		botSlug:     slug,
		onText: func(chunk string) {
			state.onText(chunk)
			turnTextLen += len(chunk)
			emitHeadlessText(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, chunk, kind+".text")
		},
		onToolUse: func(name, rawInput string) {
			...
			emitHeadlessToolUse(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, name, rawInput, kind+".tool_use")
		},
		...
		onToolResult: func(name, result string, err error) {
			...
			emitHeadlessToolResult(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, name, text, kind+".tool_result")
		},
		onFirstEvent: func() {
	...
	if (turnUsage.InputTokens > 0 || turnUsage.OutputTokens > 0) && l.broker != nil {
		...
		l.broker.StampTurnUsage(headlessEngineTurnID(ctx), turnUsage)
	}
	if err != nil {
		...
		emitHeadlessTerminalWithTurn(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, "", err.Error(), metrics, claudeUsageToTokenUsage(turnUsage))
		emitHeadlessManifest(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, err.Error(), turnToolNames, turnTextLen, metrics, claudeUsageToTokenUsage(turnUsage))
		return err
	}
	if streamErr != "" {
		metrics.TotalMs = time.Since(startedAt).Milliseconds()
		...
		emitHeadlessTerminalWithTurn(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, "", streamErr, metrics, claudeUsageToTokenUsage(turnUsage))
		emitHeadlessManifest(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, streamErr, turnToolNames, turnTextLen, metrics, claudeUsageToTokenUsage(turnUsage))
	...
	// channel as the bot's message — replace it with a short hint
	...
	if looksUnparsedToolCall(text) {
		...
		text = "(local model emitted a tool-call shape we couldn't parse — see the bot log for the raw output and try a different model or prompt)"
	}
	...
	emitHeadlessTerminalWithTurn(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, summary, "", metrics, claudeUsageToTokenUsage(turnUsage))
	emitHeadlessManifest(botStream, turnID, HeadlessProviderOpenAICompat, slug, activeTaskID, "", turnToolNames, turnTextLen, metrics, claudeUsageToTokenUsage(turnUsage))

	state.flushLiveChat()
	if text != "" && state.shouldPostFinalText() {
		// Suppress when the bot already posted via team_broadcast /
		// reply / etc this turn. Posting finalText here would
		// double-up the user-visible reply AND fan out through the
		// broker's channel-dispatch back to the bot itself.
...
// the bot invokes one of these in a turn, the post-loop final-text
...
// the bot through the broker's channel-dispatch path.
...
// have its raw JSON posted to the channel as if it were the bot's
...
// that its reply text will be posted to the channel verbatim by hivebot. We do
...
func openAICompatTextOnlyPrompt(slug, originalPrompt string) string {
	...
	header.WriteString("This API session does NOT support tool calls. Tools like team_broadcast, team_task, team_skill_run, human_message, mcp__hivex-office__*, query_context, etc. are NOT callable here, even if the section below mentions them.\n")
	header.WriteString("Reply with plain assistant text ONLY. hivebot will automatically post that text to the channel for you — no tool invocation is needed or possible.\n")
...
// invocations as `<tools>{...}</tools>` blocks. hivebot's existing
...
// tools[] field (Hermes api_server runs its own internal bot loop;
...
func openAICompatPromptedToolsPrompt(slug, originalPrompt string, tools []bot.BotTool) string {
	header := strings.Builder{}
	header.WriteString("== TOOL CALLING (PROMPT-ENCODED) ==\n")
	header.WriteString("This API session does NOT deliver tools through the OpenAI `tool_calls` field. Instead, you invoke tools by emitting a `<tools>{...}</tools>` block in your reply text. hivebot parses these blocks, executes the tool, and feeds the result back into the conversation as the next user turn.\n\n")
	...
	header.WriteString("- If you do NOT need to call a tool this turn, reply with plain prose — hivebot will post it to the channel automatically.\n")
