...
// agent_path_validation.go holds the bot-scoped path validators shared by
// the rich-artifact and bot-file surfaces. They were previously colocated
// with the (now removed) notebook worker; the validators themselves are
// generic guards over bots/{slug}/... paths and are kept here so the
// surviving callers keep compiling.
//
// Note on the "notebook" name: the user/bot-facing surface is now called
// "visual artifact" (tools visual_artifact_*, routes /visual-artifacts), but
// the on-disk storage dir is deliberately still bots/{slug}/notebook/ and
...
// under the caller's own bots/{my_slug}/notebook/ subtree.
var ErrNotebookPathNotAuthorOwned = errors.New("notebook_path_not_author_owned: write path must live under bots/{my_slug}/notebook/")

// validateNotebookWritePath enforces that the path lives under
// bots/{slug}/notebook/ where slug is the caller's own bot slug.
...
// validateNotebookPath allows any bots/{slug}/notebook/{file}.md path,
...
func validateNotebookPath(relPath string) error {
	...
	if !strings.HasPrefix(clean, "agents/") {
		return fmt.Errorf("notebook: path must be within bots/; got %q", relPath)
	}
	parts := strings.Split(clean, "/")
	if len(parts) < 4 || parts[2] != "notebook" {
		return fmt.Errorf("notebook: path must match bots/{slug}/notebook/...; got %q", relPath)
	}
...
// paths or be mistaken for directory traversal. Bot slugs are kebab-case
// alphanumerics in hivebot.
