...
// into the sealed bundle — by reading the on-disk hivex-bridge.ts and embedding
...
func stubBuildBundle(srcDir string) ([]byte, error) {
	bridge, err := os.ReadFile(filepath.Join(srcDir, "src", "hivex-bridge.ts"))
	if err != nil {
...
// regression: a bot that rewrites the protected hivex-bridge.ts (here,
...
func TestPublishOverwritesTamperedBridgeWithCanonical(t *testing.T) {
	...
	app, err := store.Save(CustomAppWriteRequest{
		...
		Files: map[string]string{
			...
			"src/hivex-bridge.ts":    tamperedBridge, // the tamper
			"src/hivex-inspector.ts": "// agent inspector tamper",
	...
	canonical, err := canonicalProtectedFile("src/hivex-bridge.ts")
	if err != nil {
		t.Fatalf("canonical bridge: %v", err)
	}
	if src["src/hivex-bridge.ts"] != string(canonical) {
		t.Fatalf("persisted bridge is not canonical (bot tamper survived)")
	}
	if strings.Contains(src["src/hivex-bridge.ts"], tamperMarker) {
		t.Fatalf("persisted bridge still contains the bot tamper marker")
	}
	// The other protected files were overwritten too.
	for _, p := range []string{"src/hivex-inspector.ts", "vite.config.ts"} {
...
func TestOverwriteProtectedFilesIsPure(t *testing.T) {
	in := map[string]string{
		...
		"src/hivex-bridge.ts": "AGENT TAMPER",
	}
	...
	// Input is untouched.
	if in["src/hivex-bridge.ts"] != "AGENT TAMPER" {
