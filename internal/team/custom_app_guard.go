...
//     `hivex-allow:` marker on the offending line (or the line directly above),
...
const (
	...
	// hivexAllowMarker is the deterministic opt-in. Placed on the offending line or
	// the line directly above, it records that the human asked for this cadence and
	// suppresses the violation, e.g.
	//   // hivex-allow: poll — user asked for a 10s live ticker
	hivexAllowMarker = "hivex-allow"
)
...
const (
	appGuardFocusMsg = "re-runs work when the browser tab regains focus. A visibilitychange / window focus|blur|pageshow listener fires every time the user switches tabs, so the app re-fetches data and re-runs ai() — burning integration rate limits and LLM tokens. Load once on mount and refresh only on an explicit user action (a button) or a deliberate schedule (a timer with a computed delay). If the human explicitly asked for focus-triggered refresh, put `// hivex-allow: focus-refresh — <why>` on the listener line."
	appGuardPollMsg  = "polls faster than 30s with setInterval. In a sealed internal tool a tight poll hammers integration rate limits and LLM tokens. Use a longer cadence, a computed-delay timer, or refresh on user action. If the human asked for a fast live refresh, put `// hivex-allow: poll — <why>` on the setInterval line."
	...
	appGuardCardPileMsg = "renders a list as a pile of <Card>s (a .map(...) producing <Card>). A homogeneous list belongs in a <Table> or divider-separated rows; reserve <Card> for a few standalone objects (see DESIGN.md §4). If this really is a small set of distinct objects, put `// hivex-allow: card-pile — <why>` on the .map line."
)
...
// `hivex-allow: no-mantine` opt-out. Returns nil for an html-only app (no
// scannable source).
func checkAppStackConformance(files map[string]string) []appGuardViolation {
	...
	for rel, content := range files {
		...
		if strings.Contains(content, hivexAllowNoMantine) {
			optedOut = true
		}
...
func checkAppThemeDepth(files map[string]string) []appGuardViolation {
	...
	for rel, content := range files {
		...
		if strings.Contains(content, hivexAllowNoMantine) || strings.Contains(content, hivexAllowDefaultTheme) {
			return nil
		}
		code := appCommentStripped(content)
		if reMantineProvider.MatchString(code) {
...
// body produces a `<Card>`. Honors a line-level `hivex-allow: card-pile` opt-out
...
// to honor the hivex-allow opt-in marker, which lives in a comment.
...
// appGuardSuppressed reports whether the hivex-allow opt-in marker appears IN A
// COMMENT on the violating line or the nearest non-blank line above it. Requiring
// a comment (not just any text) means a `hivexAllowList` identifier or a string
...
func markerInComment(rawLines, codeLines []string, idx int) bool {
	...
	if !strings.Contains(rawLines[idx], hivexAllowMarker) {
		return false
	}
	if idx < len(codeLines) && strings.Contains(codeLines[idx], hivexAllowMarker) {
