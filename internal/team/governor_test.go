...
func TestLoadGovernorConfigEnv(t *testing.T) {
	t.Setenv("HIVEX_BUDGET_MAX_TOKENS", "42000")
	t.Setenv("HIVEX_BUDGET_MAX_COST_USD", "9.5")
	t.Setenv("HIVEX_CHECKPOINT_EVERY_TURNS", "7")
	t.Setenv("HIVEX_GOVERNOR_DISABLED", "true")
...
// HIVEX_GOVERNOR_ENABLED opts auto-pausing back in; HIVEX_GOVERNOR_DISABLED
// still wins when both are set.
func TestLoadGovernorConfigEnabledOptIn(t *testing.T) {
	t.Setenv("HIVEX_GOVERNOR_ENABLED", "1")
	if loadGovernorConfig().Disabled {
		t.Fatalf("HIVEX_GOVERNOR_ENABLED should re-enable automatic pausing")
	}
	t.Setenv("HIVEX_GOVERNOR_DISABLED", "1")
	if !loadGovernorConfig().Disabled {
		t.Fatalf("HIVEX_GOVERNOR_DISABLED must win over ENABLED")
	}
}
