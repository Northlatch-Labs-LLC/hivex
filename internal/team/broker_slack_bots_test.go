...
func TestRegisterSlackAgent_CreatesBridgedMemberWithBinding(t *testing.T) {
	...
	created, err := b.RegisterSlackBot("claude-bot", "Claude Bot", "U777")
	if err != nil {
	...
	if member == nil {
		t.Fatal("registered bot should be a team member")
	}
	if member.CreatedBy != "slack" || member.Role != "Bridged bot" {
		t.Fatalf("member = %+v, want CreatedBy=slack Role=Bridged bot", member)
	}
	...
	if got := b.SlackBotSlugByUserID("U777"); got != "claude-bot" {
		t.Fatalf("SlackBotSlugByUserID = %q, want claude-bot", got)
	}
	if got := b.SlackBotUserIDBySlug("claude-bot"); got != "U777" {
		t.Fatalf("SlackBotUserIDBySlug = %q, want U777", got)
	}

	// The bot joined the bridged room (not just #general).
	b.mu.Lock()
	defer b.mu.Unlock()
	ch := b.findChannelLocked("slack-general")
	if ch == nil || !containsString(ch.Members, "claude-bot") {
		t.Fatalf("bot should be a member of the bridged slack channel, got %+v", ch)
	}
}
...
func TestRegisterSlackAgent_IdempotentAndConflicts(t *testing.T) {
	...
	if _, err := b.RegisterSlackBot("claude-bot", "Impostor", "U888"); err == nil {
	...
	if _, err := b.RegisterSlackBot("other-bot", "Other", "U777"); err == nil {
