...
// are appended manually via the CLI (`hivex task review --invite <slug>`)
...
func (b *Broker) ResolveReviewers(taskID string) ([]string, error) {
	...
	// Run the unlocked git diff first so a slow filesystem (or a git
	// process holding a repo lock) cannot pin b.mu for the full diff
	// timeout. The snapshot-then-diff-then-resolve pattern keeps the
	// broker responsive at the cost of two short b.mu acquisitions
	...
	b.mu.Unlock()
	files := taskWorktreeDiff(worktreePath, worktreeBranch, taskID)

	b.mu.Lock()
	defer b.mu.Unlock()
	return b.resolveReviewersLocked(taskID, files)
}

func (b *Broker) resolveReviewersLocked(taskID string, prefetchedFiles []string) ([]string, error) {
	taskID = strings.TrimSpace(taskID)
	if taskID == "" {
		return nil, fmt.Errorf("resolve reviewers: task id required")
	}
	...
	// prefetchedFiles == nil means the caller is already holding b.mu
	// and could not release it (e.g. the lifecycle-transition cascade);
	// fall back to the legacy in-lock diff so behaviour is unchanged
	// for those paths. ResolveReviewers, which CAN release the lock,
	// passes a non-nil slice (possibly empty) computed outside b.mu.
	var signals ReviewerRoutingSignals
	if prefetchedFiles == nil {
		signals = b.extractRoutingSignalsLocked(task)
	} else {
		signals = b.extractRoutingSignalsLockedWithFiles(task, prefetchedFiles)
	}
...
func (b *Broker) extractRoutingSignalsLocked(task *teamTask) ReviewerRoutingSignals {
	...
	// Legacy path: callers that have not snapshot-released-reacquired
	// still pay the in-lock diff cost. New callers go through
	// extractRoutingSignalsLockedWithFiles after running the diff
	// outside the lock.
	files := taskWorktreeDiff(strings.TrimSpace(task.WorktreePath), strings.TrimSpace(task.WorktreeBranch), task.ID)
	return b.extractRoutingSignalsLockedWithFiles(task, files)
}
...
// taskWorktreeDiff runs `git diff --name-only` against the task's
// worktree and parent branch. Returns nil on any error; logging is
// best-effort only. Hard 5s timeout — this used to run under b.mu on
// the lifecycle hot path, so the budget is tight to keep the broker
// responsive even on slow filesystems. ResolveReviewers now calls this
// outside b.mu, so the timeout no longer stalls the whole broker.
//
// Free function (not a method) so it cannot accidentally take b.mu.
func taskWorktreeDiff(worktreePath, worktreeBranch, taskID string) []string {
	worktreePath = strings.TrimSpace(worktreePath)
	if worktreePath == "" {
		return nil
	}
	parent := strings.TrimSpace(worktreeBranch)
	if parent == "" {
		parent = "HEAD"
	}
	...
	if err != nil {
		log.Printf("broker: reviewer routing: diff failed for task %q (worktree=%q parent=%q): %v",
			taskID, worktreePath, parent, err)
		return nil
	}
