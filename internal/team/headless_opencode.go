...
// headlessOpencodeSecretEnvVars lists hivebot-managed secrets that must NOT flow
...
// trust. These secrets are still available to the hivebot MCP subprocess via
// opencode.json's per-server `environment` block, where they are scoped to the
// hivex-office MCP server and never reach the model backend.
var headlessOpencodeSecretEnvVars = []string{
	"HIVEX_BROKER_TOKEN",
	"HIVEX_API_KEY",
	"HIVEX_OPENAI_API_KEY",
	"ONE_SECRET",
}
...
func (l *Launcher) runHeadlessOpencodeTurn(ctx context.Context, slug string, notification string, channel ...string) error {
	...
	// should never reach the third-party opencode process, overlay hivebot's MCP
	...
	env = setEnvValue(env, "HIVEX_HEADLESS_PROVIDER", "opencode")
	...
	if isTaskWorktree {
		env = append(env, "HIVEX_WORKTREE_PATH="+workspaceDir)
	}
	...
	if err != nil {
		...
		// without the hivex-office tools. Log so the user can debug.
...
// hivex can see tool-use / tool-result / error events that opencode otherwise
