...
func TestMain(m *testing.M) {
	...
	// 1) Broker token file: default path is /tmp/hivex-broker-token which
	...
	dir, err := os.MkdirTemp("", "hivex-broker-test-*")
	if err != nil {
	...
	//    stable temp dir so they don't pollute the real ~/.hivex/logs and
	//    so per-test t.TempDir cleanups don't race appender writes. Set
	//    via the in-process hivexLogDirOverride hook (the HIVEX_LOG_DIR
	//    env var was retired — env leaks into spawned codex/claude
	//    subprocesses, which is not what tests want).
	logDir, err := os.MkdirTemp("", "hivex-team-test-logs-*")
	if err != nil {
	...
	// Intentionally do NOT clear hivexLogDirOverride on cleanup: the process
	...
	// keep writing into the isolated temp dir, never the real ~/.hivex/logs.
	...
	// Goroutine-leak detection: opt-in via HIVEX_GOLEAK=1 until existing
	...
	// "hivex_test_isolation_breakdown") makes a regression detector
	// load-bearing.
	if rc == 0 && os.Getenv("HIVEX_GOLEAK") != "" {
		if err := goleak.Find(
