...
func (l *Launcher) GenerateMemberTemplateFromPrompt(request string) (generatedMemberTemplate, error) {
	...
	if stub := strings.TrimSpace(os.Getenv("HIVEX_AGENT_TEMPLATE_STUB")); stub != "" {
		return parseGeneratedMemberTemplate(stub)
	}
	systemPrompt := l.buildPrompt(l.targeter().LeadSlug()) + `

You are designing a NEW office teammate template for hivebot.
...
// HIVEX_AGENT_FILE_STUB env var short-circuits the model for tests.
func (l *Launcher) GenerateBotFileFromContext(ctx context.Context, relPath, hint string) (string, error) {
	...
	if stub := strings.TrimSpace(os.Getenv("HIVEX_AGENT_FILE_STUB")); stub != "" {
		return stub, nil
	}
...
You are authoring the %s.md instruction file for a hivebot office bot. This
...
func (l *Launcher) GenerateChannelTemplateFromPromptCtx(ctx context.Context, request string) (generatedChannelTemplate, error) {
	...
	if stub := strings.TrimSpace(os.Getenv("HIVEX_CHANNEL_TEMPLATE_STUB")); stub != "" {
		return parseGeneratedChannelTemplate(stub)
	}
	systemPrompt := l.buildPrompt(l.targeter().LeadSlug()) + `

You are designing a NEW office channel for hivebot.
Return exactly one JSON object and nothing else.
Do not wrap it in markdown fences.
Do not explain your reasoning.
