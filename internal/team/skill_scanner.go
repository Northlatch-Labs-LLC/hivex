...
// supply a budget. 50 covers the design's "Cap HIVEX_SKILL_COMPILE_TICK_BUDGET
...
func (s *SkillScanner) Scan(ctx context.Context, scopePath string, dryRun bool, trigger string) (ScanResult, error) {
	...
	for _, c := range candidates {
		...
		fm.Metadata.Hivex.SourceArticles = appendUnique(fm.Metadata.Hivex.SourceArticles, c.relPath)
		fm.Metadata.Hivex.CreatedBy = LibrarianSlug
...
func specToTeamSkill(fm SkillFrontmatter, body, sourceArticle string) teamSkill {
	...
	if src == "" && len(hivex.SourceArticles) > 0 {
		src = strings.TrimSpace(hivex.SourceArticles[0])
	}
	status := strings.TrimSpace(hivex.Status)
	if status == "" {
		status = "active"
	}
	return teamSkill{
		...
		Content:            body,
		CreatedBy:          stringOr(hivex.CreatedBy, LibrarianSlug),
		SourceArticle:      src,
		Channel:            "general",
		Tags:               append([]string(nil), hivex.Tags...),
		Trigger:            hivex.Trigger,
		WorkflowProvider:   hivex.WorkflowProvider,
		WorkflowKey:        hivex.WorkflowKey,
		WorkflowDefinition: hivex.WorkflowDefinition,
		WorkflowSchedule:   hivex.WorkflowSchedule,
		RelayID:            hivex.RelayID,
		RelayPlatform:      hivex.RelayPlatform,
		RelayEventTypes:    append([]string(nil), hivex.RelayEventTypes...),
		Status:             status,
		DisabledFromStatus: strings.TrimSpace(hivex.DisabledFromStatus),
	}
}
...
// classification. Override via HIVEX_SKILL_LLM_TIMEOUT (seconds).
...
func skillLLMTimeout() time.Duration {
	if v := strings.TrimSpace(os.Getenv("HIVEX_SKILL_LLM_TIMEOUT")); v != "" {
