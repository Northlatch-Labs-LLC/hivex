...
// Logs land under ~/.hivex/logs by default; tests redirect via the
// hivexLogDirOverride atomic.Pointer (set in TestMain) so suites
// don't pollute the developer's real log directory. The previous
// HIVEX_LOG_DIR env var was retired because env leaks into spawned
...
// hivexLogDirOverride is a test hook for redirecting headless log writes to
...
// log files don't pollute the user's real ~/.hivex/logs while the suite
// runs. The previous HIVEX_LOG_DIR environment variable was retired in
// favour of this in-process hook — env vars leak into spawned codex/claude
// subprocesses, which is not what tests want.
var hivexLogDirOverride atomic.Pointer[string]

func hivexLogDir() string {
	if p := hivexLogDirOverride.Load(); p != nil {
		override := strings.TrimSpace(*p)
		if override == "" {
			return ""
		}
		if err := os.MkdirAll(override, 0o700); err != nil {
			fmt.Fprintf(os.Stderr, "hivex: log dir override %q unwritable: %v — headless logging disabled\n", override, err)
			return ""
		}
		return override
	}
	if home := config.RuntimeHomeDir(); home != "" {
		dir := filepath.Join(home, ".hivex", "logs")
