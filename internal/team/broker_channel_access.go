...
func (b *Broker) canAccessChannelLocked(slug, channel string) bool {
	...
	// The human sees everything in their own workspace. "system" is a
	// synthetic sender rather than a bot — broker-authored notices —
	// and shares the human bypass.
	//
	...
	if isHumanMessageSender(slug) || slug == "system" {
		return true
	}
