...
func TestNewBrokerSeedsDefaultOfficeRosterOnFreshState(t *testing.T) {
	t.Setenv("HOME", t.TempDir()) // isolate from ~/.hivex company.json (e.g. RevOps pack)
...
func TestNewBrokerSeedsBlueprintBackedOfficeRosterOnFreshState(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	// Pair HOME with HIVEX_RUNTIME_HOME so config.RuntimeHomeDir (which
	...
	manifestPath := filepath.Join(home, ".hivex", "company.json")
...
func TestChannelMembersRejectDisableOrRemoveOfLead(t *testing.T) {
	...
	b.members = []officeMember{
		{Slug: "operator", Name: "Operator", Role: "Operator", BuiltIn: true, CreatedBy: "hivex", CreatedAt: now},
		{Slug: "builder", Name: "Builder", Role: "Builder", CreatedBy: "hivex", CreatedAt: now},
	}
	b.channels = []teamChannel{
		{Slug: "general", Name: "general", Members: []string{"operator", "builder"}, CreatedBy: "hivex", CreatedAt: now, UpdatedAt: now},
	}
	b.mu.Unlock()
	if err := b.StartOnPort(0); err != nil {
		t.Fatalf("failed to start broker: %v", err)
...
// shape: canAccessChannelLocked treats a small set of slugs ("system",
...
func TestChannelCreateRejectsReservedSlugs(t *testing.T) {
	...
	for _, reserved := range []string{"system", "you", "human", "cos", LibrarianSlug, appBuilderSlug} {
...
func TestLoadDoesNotAppendDefaultsAfterBlueprintSeed(t *testing.T) {
	...
	b.members = []officeMember{
		{Slug: "operator", Name: "Operator", Role: "Operator", BuiltIn: true, CreatedBy: "hivex", CreatedAt: now},
		{Slug: "planner", Name: "Planner", Role: "Planner", CreatedBy: "hivex", CreatedAt: now},
		{Slug: "builder", Name: "Builder", Role: "Builder", CreatedBy: "hivex", CreatedAt: now},
		{Slug: "growth", Name: "Growth", Role: "Growth", CreatedBy: "hivex", CreatedAt: now},
		{Slug: "reviewer", Name: "Reviewer", Role: "Reviewer", CreatedBy: "hivex", CreatedAt: now},
	}
	// Seed a task so saveLocked doesn't short-circuit on default state.
	b.tasks = []teamTask{{ID: "niche-crm-1", Channel: "general", Title: "Choose the niche", status: "open", CreatedBy: "hivex", CreatedAt: now, UpdatedAt: now}}
	if err := b.saveLocked(); err != nil {
		b.mu.Unlock()
		t.Fatalf("saveLocked failed: %v", err)
	}
