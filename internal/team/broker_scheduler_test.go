...
func TestRunSchedulerJob_TriggersJobAndUpdatesLastRun(t *testing.T) {
	...
	if err := b.SetSchedulerJob(schedulerJob{
		Slug:            "hive-notifications",
		Label:           "Hive notifications",
		IntervalMinutes: 10,
		NextRun:         nextRun,
		Status:          "sleeping",
		SystemManaged:   true,
	...
	req, _ := http.NewRequest(http.MethodPost, base+"/scheduler/hive-notifications/run", nil)
	...
	if body.Slug != "hive-notifications" {
		t.Errorf("expected slug=hive-notifications, got %q", body.Slug)
	}
	if body.At == "" {
	...
	for i := range b.scheduler {
		if b.scheduler[i].Slug == "hive-notifications" {
			found = &b.scheduler[i]
			break
		}
	}
...
func TestRunSchedulerJob_WrongMethodReturns405(t *testing.T) {
	...
	for _, method := range []string{http.MethodGet, http.MethodPatch} {
		req, _ := http.NewRequest(method, base+"/scheduler/hive-notifications/run", nil)
		req.Header.Set("Authorization", "Bearer "+b.Token())
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
