...
func TestMarkdownKnowledgeToolBlock_ListsCanonicalTools(t *testing.T) {
	...
	for _, tool := range []string{
		...
		"team_wiki_read", "team_wiki_write", "hivex_wiki_lookup",
...
func TestPromptBuilder_OneOnOneBranch(t *testing.T) {
	...
	got := pb.Build("cos")
	if !strings.Contains(got, "direct one-on-one hivebot session with the human") {
	...
	if strings.Contains(got, "Hive") {
		t.Fatalf("prompt must not mention Hive:\n%s", got)
	}
}
...
func TestPromptBuilder_MarkdownMemoryPromptsNaturalHTMLArtifactsDuringWork(t *testing.T) {
	pb := &promptBuilder{
		isOneOnOne:  func() bool { return false },
		isFocusMode: func() bool { return false },
		packName:    func() string { return "HIVEX swarm" },
		leadSlug:    func() string { return "cos" },
		members: func() []officeMember {
			return []officeMember{
				{Slug: "cos", Name: "CEO"},
...
func TestPromptBuilder_NoAgentOwnsTheWikiPromotionNeedsAHuman(t *testing.T) {
	...
	for _, shared := range []string{
		"Use hivex_wiki_lookup or team_wiki_search when prior knowledge matters",
