...
//
//  1. HIVEX_BROKER_STATE_PATH env override still wins over defaults.
...
func TestDefaultBrokerStatePath_EnvOverrideWins(t *testing.T) {
	// HIVEX_BROKER_STATE_PATH takes precedence over HIVEX_RUNTIME_HOME
	...
	t.Setenv("HIVEX_BROKER_STATE_PATH", override)
	t.Setenv("HIVEX_RUNTIME_HOME", "/tmp/should-be-ignored")
...
func TestDefaultBrokerStatePath_RuntimeHomeFallback(t *testing.T) {
	// No env override, HIVEX_RUNTIME_HOME set → <home>/.hivex/team/
	// broker-state.json. This is the happy path prod and tests both
	// take (tests via worktree_guard_test.go pinning HIVEX_RUNTIME_HOME
	// to a leaked tempdir).
	t.Setenv("HIVEX_BROKER_STATE_PATH", "")
	home := t.TempDir()
	t.Setenv("HIVEX_RUNTIME_HOME", home)

	got := defaultBrokerStatePath()
	want := filepath.Join(home, ".hivex", "team", "broker-state.json")
	if got != want {
...
func TestDefaultBrokerStatePath_RelativeFallbackWhenHomeMissing(t *testing.T) {
	...
	// than panicking.
	t.Setenv("HIVEX_BROKER_STATE_PATH", "")
	t.Setenv("HIVEX_RUNTIME_HOME", "")
	t.Setenv("HOME", "")

	got := defaultBrokerStatePath()
	want := filepath.Join(".hivex", "team", "broker-state.json")
	if got != want {
