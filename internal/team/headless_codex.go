...
func defaultHeadlessCodexRunTurn(l *Launcher, ctx context.Context, slug, notification string, channel ...string) error {
	...
	// finds the record at dispatch and closes it honestly as interrupted.
	turnCh := ""
	if len(channel) > 0 {
		turnCh = channel[0]
	}
	...
	l.broker.TurnTransition(turnID, TurnVerifying, "runner returned; completion-path verification applies")
	...
	if err != nil {
		l.broker.TurnFail(turnID, err.Error())
	} else {
		l.broker.TurnSettle(turnID, "turn settled; outcome journalled in the task ledger")
	}
	return err
}
...
// https://github.com/Northlatch-Labs-LLC/hivex/issues/313.
...
var (
	...
	headlessCodexTurnTimeout = headlessCodexTurnTimeoutEnv("HIVEX_TURN_TIMEOUT", 15*time.Minute)
	...
	// turns already run 2.5–3.4m, so the old 4m default (HIVEX_TURN_TIMEOUT)
	...
	// turns their own generous budget, operator-tunable via HIVEX_OFFICE_TIMEOUT.
	headlessCodexOfficeTurnTimeout = headlessCodexTurnTimeoutEnv("HIVEX_OFFICE_TIMEOUT", 10*time.Minute)
	...
	headlessCodexAppBuildTurnTimeout      = headlessCodexTurnTimeoutEnv("HIVEX_BUILD_TIMEOUT", 25*time.Minute)
	headlessCodexOfficeLaunchTurnTimeout  = headlessCodexTurnTimeoutEnv("HIVEX_OFFICE_LAUNCH_TIMEOUT", 10*time.Minute)
	headlessCodexLocalWorktreeTurnTimeout = headlessCodexTurnTimeoutEnv("HIVEX_WORKTREE_TIMEOUT", 12*time.Minute)
...
func (l *Launcher) resolveHeadlessConcurrencyCaps() {
	...
	// set, so an explicit HIVEX_MAX_CONCURRENT_TURNS=0 disables the cap (unlimited
	// — see headlessConcurrencyCaps); an unset value falls back to the default.
	l.headless.maxConcurrent = envIntDefault("HIVEX_MAX_CONCURRENT_TURNS", global)
	l.headless.maxConcurrentPerBot = envIntDefault("HIVEX_MAX_CONCURRENT_PER_AGENT", 3)
}
