...
func TestHandleSlackAgentsSpawn_ManifestAndGuide(t *testing.T) {
	...
	if resp.TokenEnv != "HIVEX_SLACK_AGENT_RESEARCHER_TOKEN" {
	...
	for _, want := range []string{
		"api.slack.com/apps",
		"HIVEX_SLACK_AGENT_RESEARCHER_TOKEN",
...
func TestSlackSpawnPendingSurvivesRestart(t *testing.T) {
	...
	if len(got) != 1 || got[0].Slug != "researcher" || got[0].TokenEnv != "HIVEX_SLACK_AGENT_RESEARCHER_TOKEN" {
...
func TestCompleteSlackAgentSpawn_CreatesRealOfficeAgent(t *testing.T) {
	...
	t.Setenv("HIVEX_SLACK_AGENT_RESEARCHER_TOKEN", "bot-token-test-555")
	...
	if member.Provider.Slack == nil || member.Provider.Slack.UserID != "U555" ||
		member.Provider.Slack.BotTokenEnv != "HIVEX_SLACK_AGENT_RESEARCHER_TOKEN" {
	...
	if got := b.SpawnedSlackBotTokenEnv("researcher"); got != "HIVEX_SLACK_AGENT_RESEARCHER_TOKEN" {
	...
	if got := b2.SpawnedSlackBotTokenEnv("researcher"); got != "HIVEX_SLACK_AGENT_RESEARCHER_TOKEN" {
...
func TestCompleteSlackAgentSpawn_Errors(t *testing.T) {
	...
	w := spawnComplete(t, b, `{"slug":"researcher"}`)
	if w.Code != http.StatusConflict || !strings.Contains(w.Body.String(), "HIVEX_SLACK_AGENT_RESEARCHER_TOKEN") {
	...
	t.Setenv("HIVEX_SLACK_AGENT_RESEARCHER_TOKEN", "xoxb-bad")
