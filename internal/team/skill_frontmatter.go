...
// without re-formatting. hivebot-specific provenance lives under metadata.hivex.
...
type SkillFrontmatter struct {
	...
	// Metadata contains hivebot-specific provenance under the hivex namespace.
	Metadata SkillMetadata `yaml:"metadata,omitempty"`
}

// SkillMetadata is the top-level metadata namespace. Only the hivex sub-key
// is defined here; other tools may add their own namespaces alongside it.
type SkillMetadata struct {
	Hivex SkillHivexMeta `yaml:"hivex,omitempty"`
}

// SkillHivexMeta carries all hivebot-specific provenance for a compiled skill.
type SkillHivexMeta struct {
...
// metadata.hivex.source_articles[0] so the on-disk SKILL.md preserves the
...
// metadata.hivex.source_signals instead, populated at the synth layer.
