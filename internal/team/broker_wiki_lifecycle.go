...
func (b *Broker) initWikiWorker() {
	...
	if idxErr != nil {
		// Only reachable when the operator pinned HIVEX_WIKI_BACKEND=gbrain and
	...
	// off with HIVEX_OBSIDIAN_WATCHER=0. Non-fatal on failure.
	...
	// Daily lint cron. The schedule is controlled by HIVEX_LINT_CRON (default
	...
	// candidate skills. Cron runs at HIVEX_SKILL_COMPILE_INTERVAL (default
	// 30m); cooldown gates back-to-back ticks via HIVEX_SKILL_COMPILE_COOLDOWN
