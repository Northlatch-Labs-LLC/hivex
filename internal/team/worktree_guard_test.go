...
func init() {
	...
	// Pin HIVEX_RUNTIME_HOME into a process-lifetime leaked tempdir so
	// any test that constructs a Broker without its own isolation setup
	// falls back to /tmp instead of the developer's real ~/.hivex.
	...
	runtimeHome, err := os.MkdirTemp("", "hivex-test-runtime-home-*")
	if err != nil {
		panic(fmt.Sprintf("worktree_guard_test init: mktemp runtime home: %v", err))
	}
	if err := os.Setenv("HIVEX_RUNTIME_HOME", runtimeHome); err != nil {
		panic(fmt.Sprintf("worktree_guard_test init: setenv HIVEX_RUNTIME_HOME: %v", err))
	}
}
...
func stubPrepareTaskWorktree(taskID string) (string, string, error) {
	...
	// both stubs emit the same `<root>/.hivex/task-worktrees/<repoToken>/hivex-task-<id>`
