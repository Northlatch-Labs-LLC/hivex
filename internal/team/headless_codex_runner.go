...
func (l *Launcher) runHeadlessCodexTurn(ctx context.Context, slug string, notification string, channel ...string) error {
	...
	if isTaskWorktree {
		cmd.Env = append(cmd.Env, "HIVEX_WORKTREE_PATH="+workspaceDir)
	}
...
func (l *Launcher) buildHeadlessCodexEnv(slug string, workspaceDir string, channel string) []string {
	...
	// sandbox. If hivex inherited GIT_DIR / GIT_WORK_TREE /
	// GIT_CONFIG_PARAMETERS from a parent (git hook, nested hivex call)
	...
	if codexHome := prepareHeadlessCodexHome(); codexHome != "" {
		...
		// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — HOME passthrough
	...
	env = setEnvValue(env, "HIVEX_AGENT_SLUG", slug)
	if channel = strings.TrimSpace(channel); channel != "" {
		env = setEnvValue(env, "HIVEX_CHANNEL", channel)
	}
	env = setEnvValue(env, "HIVEX_BROKER_TOKEN", l.broker.Token())
	env = setEnvValue(env, "HIVEX_BROKER_BASE_URL", l.BrokerBaseURL())
	env = setEnvValue(env, "HIVEX_HEADLESS_PROVIDER", "codex")
	if l.isOneOnOne() {
		env = setEnvValue(env, "HIVEX_ONE_ON_ONE", "1")
		env = setEnvValue(env, "HIVEX_ONE_ON_ONE_AGENT", l.oneOnOneBot())
	}
	...
	if openAIKey := strings.TrimSpace(config.ResolveOpenAIAPIKey()); openAIKey != "" {
		env = setEnvValue(env, "HIVEX_OPENAI_API_KEY", openAIKey)
...
func headlessCodexHomeDir() string {
	...
	// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — ~/.codex is the
...
func headlessCodexGlobalHomeDir() string {
	if raw := strings.TrimSpace(os.Getenv("HIVEX_GLOBAL_HOME")); raw != "" {
		if abs, err := filepath.Abs(raw); err == nil && strings.TrimSpace(abs) != "" {
			return abs
		}
		return raw
	}
	// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — resolves the
...
func headlessCodexRuntimeHomeDir() string {
	if home := config.RuntimeHomeDir(); home != "" {
		return filepath.Join(home, ".hivex", "codex-headless")
	}
	return ""
}
...
func (l *Launcher) headlessCodexWorkspaceCacheDir(workspaceDir string) string {
	...
	return filepath.Join(base, ".hivex", "cache")
}
...
func (l *Launcher) buildCodexOfficeConfigOverrides(slug string) ([]string, error) {
	hivexBinary, err := headlessCodexExecutablePath()
	if err != nil {
		return nil, err
	}
	hivexEnvVars := []string{
		"HIVEX_AGENT_SLUG",
		"HIVEX_BROKER_TOKEN",
		"HIVEX_BROKER_BASE_URL",
	}
	if l.isOneOnOne() {
		hivexEnvVars = append(hivexEnvVars,
			"HIVEX_ONE_ON_ONE",
			"HIVEX_ONE_ON_ONE_AGENT",
		)
	}
	if secret := strings.TrimSpace(config.ResolveOneSecret()); secret != "" {
		hivexEnvVars = append(hivexEnvVars, "ONE_SECRET")
	}
	if identity := strings.TrimSpace(config.ResolveOneIdentity()); identity != "" {
		hivexEnvVars = append(hivexEnvVars, "ONE_IDENTITY")
		if identityType := strings.TrimSpace(config.ResolveOneIdentityType()); identityType != "" {
			hivexEnvVars = append(hivexEnvVars, "ONE_IDENTITY_TYPE")
		}
	}

	overrides := []string{
		fmt.Sprintf(`mcp_servers.hivex-office.command=%s`, tomlQuote(hivexBinary)),
		`mcp_servers.hivex-office.args=["mcp-team"]`,
		fmt.Sprintf(`mcp_servers.hivex-office.env_vars=%s`, tomlStringArray(hivexEnvVars)),
	}
...
// for ~10s and hivebot's error log reads "exit status 1: exit status 1" — totally
...
func (l *Launcher) preflightHeadlessCodexAuth(slug string, channel string) error {
	// Check the source codex creds that hivebot will later copy into the isolated
	...
	// (~/.hivex/codex-headless) would find auth there — but codex
...
// shell script in $HIVEX_DEBUG_CODEX_DUMP when that env var is set. Off by
// default. Used to reproduce a failing turn outside hivebot in a few seconds.
func dumpHeadlessCodexInvocation(slug, workspaceDir string, args []string, env []string, stdinText string) {
	dumpDir := strings.TrimSpace(os.Getenv("HIVEX_DEBUG_CODEX_DUMP"))
	if dumpDir == "" {
		return
	}
	...
	fmt.Fprintf(&sh, "# Reproduces the exact codex invocation hivebot builds for bot=%s\n", slug)
