...
func RunOfficeEvals(dir string) (*OfficeEvalReport, error) {
	...
	// real ~/.hivex. Restored on return; in `go test` the package init
	// already pins this var to a leaked tempdir, so the override nests.
	priorHome, hadHome := os.LookupEnv("HIVEX_RUNTIME_HOME")
	if err := os.Setenv("HIVEX_RUNTIME_HOME", dir); err != nil {
		return nil, fmt.Errorf("office eval: pin runtime home: %w", err)
	}
	defer func() {
		if hadHome {
			_ = os.Setenv("HIVEX_RUNTIME_HOME", priorHome)
		} else {
			_ = os.Unsetenv("HIVEX_RUNTIME_HOME")
		}
	}()
	jobs := []struct {
		name string
...
// evalJobTurnEngine: the Turn Engine v2 state machine holds its contract on
// a real broker: a driven turn leaves a terminal record with the canonical
// state trail, terminal history is immutable, a mid-flight record is closed
// honestly by the restart-resume path, the default-deny policy gate answers
// correctly without any grants, and the settle meter fires exactly once per
// turn. No LLM calls — the driver drives the engine the way the launcher
// wrapper does in production.
func evalJobTurnEngine(fx *officeEvalFixture, r *OfficeEvalReport) error {
	b := fx.broker
	job := "turn-engine"

	// 1. A driven turn settles with the full state trail.
	id := b.TurnBegin("eng", "task-eval-1", "general")
	for _, step := range []struct {
		state  TurnState
		detail string
	}{
		{TurnPolicyGate, "default-deny gate armed"},
		{TurnContextAssembly, "packet assembled"},
		{TurnDispatch, "runner invoked"},
		{TurnVerifying, "runner returned"},
	} {
		b.TurnTransition(id, step.state, step.detail)
	}
	b.TurnSettle(id, "settled")
	records := b.TurnRecords()
	if len(records) != 1 {
		r.add(job, "turn settles with a full state trail", false,
			fmt.Sprintf("expected one record, got %d", len(records)), "")
		return nil
	}
	rec := records[0]
	trailHasCanonicalStates := true
	seen := map[TurnState]bool{}
	for _, t := range rec.Transitions {
		seen[t.State] = true
	}
	for _, want := range []TurnState{TurnIntake, TurnPolicyGate, TurnContextAssembly, TurnDispatch, TurnVerifying, TurnSettled} {
		if !seen[want] {
			trailHasCanonicalStates = false
		}
	}
	pass := rec.State == TurnSettled && rec.Bot == "eng" && rec.TaskID == "task-eval-1" && trailHasCanonicalStates
	r.add(job, "turn settles with a full state trail", pass,
		fmt.Sprintf("state=%s trail=%d", rec.State, len(rec.Transitions)), "")

	// 2. Terminal history is immutable — a late transition must not reopen it.
	b.TurnTransition(id, TurnDispatch, "late attempt")
	after := b.TurnRecords()[0]
	r.add(job, "terminal turn records are immutable",
		after.State == TurnSettled && len(after.Transitions) == len(rec.Transitions),
		"", "")

	// 3. The crash story: a mid-flight record closes honestly on resume.
	interrupted := b.TurnBegin("eng", "task-eval-2", "general")
	b.TurnTransition(interrupted, TurnDispatch, "runner invoked")
	resumed := turnResumeInterruptedLocked(b.TurnRecords())
	closedHonestly := false
	for _, rr := range resumed {
		if rr.ID == interrupted && rr.State == TurnFailed {
			last := rr.Transitions[len(rr.Transitions)-1]
			closedHonestly = strings.Contains(last.Detail, "interrupted by broker restart while dispatch")
		}
	}
	r.add(job, "interrupted turns close honestly on restart", closedHonestly, "", "")

	// 4. The default-deny policy gate on the live broker: no grants minted,
	// a mutating capability is approve (human gate), and an irreversible
	// capability is approve even when granted.
	d := b.EvaluateCapability("eng", "memory.write", "")
	r.add(job, "policy gate default-deny without grants",
		d.Decision == "approve" && strings.Contains(d.Reason, "no active grant"),
		d.Reason, "")
	irreversible := b.EvaluateCapability("eng", "spend.authorize", "")
	r.add(job, "policy gate irreversible needs a human",
		irreversible.Decision == "approve" && strings.Contains(irreversible.Reason, "irreversible"),
		irreversible.Reason, "")
...
// meterFunc adapts a function to TurnMeter for eval scenarios.
