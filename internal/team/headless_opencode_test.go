...
func TestWriteHeadlessOpencodeMCPConfigConcurrent(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	// Pair HIVEX_RUNTIME_HOME with HOME so the post-Phase-0 opencode race fix
	// (per-bot configs under <runtime_home>/.hivex/opencode-configs/) lands
	...
	// the merge (theme key is untouched by hivebot).
	...
	// construct the MCP entry without needing the real hivex binary.
	orig := headlessOpencodeExecutablePath
	headlessOpencodeExecutablePath = func() (string, error) { return "/usr/local/bin/hivex", nil }
	...
	if _, ok := base["mcp"]; ok {
		t.Fatal("base opencode.json should not be rewritten with hivebot's bot-scoped MCP entry")
	}
	...
	for _, slug := range slugs {
		...
		hivexOffice, _ := mcp["hivex-office"].(map[string]any)
		if hivexOffice == nil {
			t.Fatalf("mcp.hivex-office missing from %s config", slug)
		}
		env, _ := hivexOffice["environment"].(map[string]any)
		if env == nil {
			t.Fatalf("mcp.hivex-office.environment missing from %s config", slug)
		}
		if env["HIVEX_AGENT_SLUG"] != slug {
			t.Fatalf("%s config has HIVEX_AGENT_SLUG=%#v", slug, env["HIVEX_AGENT_SLUG"])
		}
	}
}
...
func TestWriteHeadlessOpencodeMCPConfigLogsBaseConfigParseFailure(t *testing.T) {
	...
	t.Setenv("HOME", home)
	logDir := t.TempDir()
	prevLogDir := hivexLogDirOverride.Load()
	hivexLogDirOverride.Store(&logDir)
	t.Cleanup(func() { hivexLogDirOverride.Store(prevLogDir) })
