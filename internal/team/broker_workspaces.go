...
// and launcherDrainer — set on the Broker by callers (cmd/hivebot wires the
...
// shape Lane C's tests assert against. At merge, cmd/hivebot adapts Lane B's
...
// adapter in cmd/hivebot maps between the two.
...
// auth (`~/.hivex-spaces/tokens/`). The directory is owned by the
...
// to avoid touching the real ~/.hivex-spaces.
...
func workspaceTokenDir() string {
	...
	return filepath.Join(home, ".hivex-spaces", "tokens")
}
...
// breaking compile-time consumers (cmd/hivebot/main.go) until they are
...
// ~/.hivex-spaces/.backups/<trash_id>/ for restore-within-30-days and the
...
//
// Returns: {"trash": [...]}. Lists the contents of ~/.hivex-spaces/.backups/
