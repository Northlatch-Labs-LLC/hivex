...
const (
	...
	appKnowledgeTag = "hivex-app-knowledge"
	// appKnowledgeSourceKind / appKnowledgeIngestedVia are the gbrain provenance
	// stamps for pages this surface writes.
	appKnowledgeSourceKind  = "hivex_app_knowledge"
	appKnowledgeIngestedVia = "hivex-app-knowledge"
)
...
var knowledgeB64Re = regexp.MustCompile(`<!--hivex-knowledge-b64:([A-Za-z0-9+/=]+)-->`)
...
func appKnowledgeScopeTag(appID string) string {
	return "hivex-app-" + strings.TrimPrefix(appID, "app_")
}
...
// appIDFromScopeTag reverses appKnowledgeScopeTag: "hivex-app-<idhex>" -> app id.
// Returns false for the shared "hivex-app-knowledge" tag or any non-app tag.
func appIDFromScopeTag(tag string) (string, bool) {
	s := strings.TrimPrefix(tag, "hivex-app-")
	if s == tag || s == "" {
		return "", false
	}
	id := "app_" + s
...
// appScopeTagsOf returns the app-scope tags (hivex-app-<idhex>) present on a page,
// deduped, preserving order. The shared hivex-app-knowledge tag is not included.
...
func renderKnowledgePageForGBrain(page appKnowledgePage, scopeTags []string) (string, error) {
	...
	buf.WriteString("<!--hivex-knowledge-b64:")
