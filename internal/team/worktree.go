...
// against the developer's `hivex` repo. Tests that legitimately need the real
...
func defaultPrepareTaskWorktree(taskID string) (string, string, error) {
	...
	if strings.TrimSpace(root) == "" {
		root = filepath.Join(os.TempDir(), "hivex-task-worktrees")
	}
	if err := os.MkdirAll(root, 0o755); err != nil {
		return "", "", fmt.Errorf("prepare task worktree root: %w", err)
	}
	path := filepath.Join(root, "hivex-task-"+sanitizeWorktreeToken(taskID))
...
func defaultVerifyTaskWorktreeWritable(path string) error {
	...
	probe, err := os.CreateTemp(path, ".hivex-write-probe-*")
	if err != nil {
...
func defaultTaskWorktreeRootDir(repoRoot string) string {
	...
	if home := config.RuntimeHomeDir(); home != "" {
		return filepath.Join(home, ".hivex", "task-worktrees", repoToken)
	}
	return filepath.Join(os.TempDir(), "hivex-task-worktrees", repoToken)
}
...
func worktreeBranchName(taskID string) string {
	repoRoot, err := gitRepoRoot()
	if err != nil {
		return "hivex-" + sanitizeWorktreeToken(taskID)
	}
...
func worktreeBranchNameForRepo(taskID string, repoRoot string) string {
	...
	if namespace := worktreeNamespaceToken(repoRoot); namespace != "" {
		return "hivex-" + namespace + "-" + taskToken
	}
	return "hivex-" + taskToken
}
...
func worktreePathLooksSafe(path string) bool {
	...
	if !strings.Contains(filepath.Base(path), "hivex-task-") {
		return false
	}
...
func managedWorktreeRoots() []string {
	...
	if home := config.RuntimeHomeDir(); home != "" {
		roots = append(roots, filepath.Join(home, ".hivex", "task-worktrees"))
	}
	roots = append(roots, filepath.Join(os.TempDir(), "hivex-task-worktrees"))
	return roots
}
