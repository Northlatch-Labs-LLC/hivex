...
// connectOpenAICompatMCPBridge spawns `hivex mcp-team` as a subprocess in the
...
// (mlx-lm, ollama, exo) actually usable as a hivex bot: the model sees the
...
func (l *Launcher) connectOpenAICompatMCPBridge(
	...
	hivexBin, err := headlessOpenAICompatExecutablePath()
	if err != nil {
		return nil, nil, fmt.Errorf("resolve hivex binary: %w", err)
	}
	...
	client := mcp.NewClient(&mcp.Implementation{
		Name:    "hivex-openai-compat-bridge",
		Version: "0.1.0",
	}, nil)
...
// map[string]any hivex's openai-compat layer expects to forward as
