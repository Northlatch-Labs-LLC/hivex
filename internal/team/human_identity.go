...
// disk at
//
//	~/.hivex/humans/{sha256(email)[:16]}.json
//
...
// back to the v1.4 synthetic identity (`human`/`human@hivex.local`). That
...
// ~/.hivex/humans/{sha256(email)[:16]}.json and is the source of truth
...
// {RuntimeHomeDir}/.hivex/humans. The directory is created lazily on
...
// humansDir mirrors WikiRootDir's layout so dev runs (HIVEX_RUNTIME_HOME
// override) stay isolated from a user's prod ~/.hivex.
func humansDir() string {
	home := strings.TrimSpace(config.RuntimeHomeDir())
	if home == "" {
		return filepath.Join(".hivex", "humans")
	}
	return filepath.Join(home, ".hivex", "humans")
}
...
func runGitConfig(key string) string {
	...
	//
	// gitexec.CleanEnv strips GIT_CONFIG_PARAMETERS and friends: when hivex
