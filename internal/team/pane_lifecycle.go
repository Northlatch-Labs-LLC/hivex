...
// HasLiveSession returns true when a hivex-team tmux session is running.
...
// tmux pane target (e.g. "hivex-team:team.0") with capture-pane's -p -J
...
// KillSession terminates the entire hivex-team tmux session. Used by
...
func channelStderrLogPath() string {
	...
	if home == "" {
		return ".hivex-channel-stderr.log"
	}
	return filepath.Join(home, ".hivex", "logs", "channel-stderr.log")
}
...
func channelPaneSnapshotPath() string {
	home := config.RuntimeHomeDir()
	if home == "" {
		return ".hivex-channel-pane.log"
	}
	return filepath.Join(home, ".hivex", "logs", "channel-pane.log")
}
...
func paneFallbackMessages(tmuxInstalled bool, detail string) (stderrMsg, brokerMsg string) {
	...
	stderrMsg = fmt.Sprintf(
		"  Bots:  pane-backed fallback attempted but unavailable (%s). %s tmux IS installed but rejected the launch command; please file a bug with the detail above at https://github.com/Northlatch-Labs-LLC/hivex/issues.\n",
		detail, headlessBlurb,
	)
	brokerMsg = fmt.Sprintf(
...
// HasLiveTmuxSession returns true if a hivex-team tmux session is
