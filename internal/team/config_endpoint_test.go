...
func TestConfigEndpointAndHealth(t *testing.T) {
	...
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	// Pair HOME with HIVEX_RUNTIME_HOME so config.RuntimeHomeDir resolves
	...
	if err := os.MkdirAll(filepath.Join(tmp, ".hivex"), 0o700); err != nil {
		t.Fatal(err)
	}
	// Seed config with claude-code, then POST codex.
	initial := `{"llm_provider":"claude-code"}`
	if err := os.WriteFile(filepath.Join(tmp, ".hivex", "config.json"), []byte(initial), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	disk, _ := os.ReadFile(filepath.Join(tmp, ".hivex", "config.json"))
