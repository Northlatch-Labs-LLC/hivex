...
func (b *Broker) handleTelegramVerify(w http.ResponseWriter, r *http.Request) {
	...
	//      HIVEX_TELEGRAM_BOT_TOKEN, which would mean a developer who
...
func (b *Broker) handleTelegramConnect(w http.ResponseWriter, r *http.Request) {
	...
	if chatID != 0 {
		if sendErr := SendTelegramMessage(token, chatID,
			"Connected to HIVEX swarm. Messages here will be visible to the team."); sendErr != nil {
...
// seeds in cmd/hivebot/channel_integration.go's connectTelegramGroup.
...
func resolveTelegramTokenFromBody(token string) string {
	...
	if env := strings.TrimSpace(os.Getenv("HIVEX_TELEGRAM_BOT_TOKEN")); env != "" {
		return env
	}
