...
func TestRenderAndParseSkillMarkdown_RoundTrip(t *testing.T) {
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// hivex metadata.
			w := tc.fm.Metadata.Hivex
			gw := got.Metadata.Hivex
			if w.Title != "" && gw.Title != w.Title {
				t.Errorf("Hivex.Title: got %q, want %q", gw.Title, w.Title)
			}
			if w.Trigger != "" && gw.Trigger != w.Trigger {
				t.Errorf("Hivex.Trigger: got %q, want %q", gw.Trigger, w.Trigger)
			}
			if w.CreatedBy != "" && gw.CreatedBy != w.CreatedBy {
				t.Errorf("Hivex.CreatedBy: got %q, want %q", gw.CreatedBy, w.CreatedBy)
			}
			if w.Status != "" && gw.Status != w.Status {
				t.Errorf("Hivex.Status: got %q, want %q", gw.Status, w.Status)
			}
			if w.DisabledFromStatus != "" && gw.DisabledFromStatus != w.DisabledFromStatus {
				t.Errorf("Hivex.DisabledFromStatus: got %q, want %q", gw.DisabledFromStatus, w.DisabledFromStatus)
			}
			if w.LastSynthesizedSHA != "" && gw.LastSynthesizedSHA != w.LastSynthesizedSHA {
				t.Errorf("Hivex.LastSynthesizedSHA: got %q, want %q", gw.LastSynthesizedSHA, w.LastSynthesizedSHA)
			}
			if w.FactCountAtSynthesis != 0 && gw.FactCountAtSynthesis != w.FactCountAtSynthesis {
				t.Errorf("Hivex.FactCountAtSynthesis: got %d, want %d", gw.FactCountAtSynthesis, w.FactCountAtSynthesis)
			}
			if w.WorkflowProvider != "" && gw.WorkflowProvider != w.WorkflowProvider {
				t.Errorf("Hivex.WorkflowProvider: got %q, want %q", gw.WorkflowProvider, w.WorkflowProvider)
			}
			if w.RelayPlatform != "" && gw.RelayPlatform != w.RelayPlatform {
				t.Errorf("Hivex.RelayPlatform: got %q, want %q", gw.RelayPlatform, w.RelayPlatform)
			}
			if w.SafetyScan != nil {
...
func TestTeamSkillToFrontmatter(t *testing.T) {
	...
	w := fm.Metadata.Hivex
	if w.Title != sk.Title {
		t.Errorf("Hivex.Title: got %q, want %q", w.Title, sk.Title)
	}
	if w.Trigger != sk.Trigger {
		t.Errorf("Hivex.Trigger: got %q, want %q", w.Trigger, sk.Trigger)
	}
	if w.CreatedBy != sk.CreatedBy {
		t.Errorf("Hivex.CreatedBy: got %q, want %q", w.CreatedBy, sk.CreatedBy)
	}
	if w.Status != sk.Status {
		t.Errorf("Hivex.Status: got %q, want %q", w.Status, sk.Status)
	}
	if w.DisabledFromStatus != sk.DisabledFromStatus {
		t.Errorf("Hivex.DisabledFromStatus: got %q, want %q", w.DisabledFromStatus, sk.DisabledFromStatus)
	}
	if w.WorkflowProvider != sk.WorkflowProvider {
		t.Errorf("Hivex.WorkflowProvider: got %q, want %q", w.WorkflowProvider, sk.WorkflowProvider)
	}
	if w.RelayPlatform != sk.RelayPlatform {
		t.Errorf("Hivex.RelayPlatform: got %q, want %q", w.RelayPlatform, sk.RelayPlatform)
	}
	if len(w.Tags) != len(sk.Tags) {
		t.Errorf("Hivex.Tags len: got %d, want %d", len(w.Tags), len(sk.Tags))
	}
	if len(w.RelayEventTypes) != len(sk.RelayEventTypes) {
		t.Errorf("Hivex.RelayEventTypes len: got %d, want %d", len(w.RelayEventTypes), len(sk.RelayEventTypes))
	}
	if len(w.OwnerBots) != len(sk.OwnerBots) {
		t.Fatalf("Hivex.OwnerBots len: got %d, want %d", len(w.OwnerBots), len(sk.OwnerBots))
	}
	for i, slug := range sk.OwnerBots {
		if w.OwnerBots[i] != slug {
			t.Errorf("Hivex.OwnerBots[%d]: got %q, want %q", i, w.OwnerBots[i], slug)
		}
	}
}
...
func TestSkillFrontmatterOwnerBotsRoundTrip(t *testing.T) {
	...
	got := out.Metadata.Hivex.OwnerBots
	want := in.Metadata.Hivex.OwnerBots
	if len(got) != len(want) {
