...
// X-hivebot-Bot identity header. The operator's "remove a failed app" action
// sends X-hivebot-Bot: app-builder; the app-writer gate (appWriterAllowed) only
...
// arbitrary bot slug. The proxy already attaches the broker token, so a
...
func TestWebUIProxyBotHeaderForwarding(t *testing.T) {
	var gotBot, gotAuth string
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotBot = r.Header.Get(botRateLimitHeader)
	...
	call := func(bot, fetchSite string) string {
		gotBot = "sentinel"
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodDelete, "/api/apps/app_x", nil)
		if bot != "" {
			req.Header.Set(botRateLimitHeader, bot)
		}
		if fetchSite != "" {
