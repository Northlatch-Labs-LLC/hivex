...
const (
	...
	// captured page.
	gbrainCaptureSource = "hivex-office"
	...
	// captured task lands as source_kind "hivex_office_task", etc.
	gbrainCaptureSourceKindPrefix = "hivex_office_"
	// gbrainCaptureIngestedVia is the put_page ingested_via provenance.
	gbrainCaptureIngestedVia = "hivex-capture"
	...
	captureLinkSource         = "hivex-capture"
)
...
// tagged with the link_source "hivex-capture" so they are distinguishable from
...
func renderGBrainSourcePage(job SourceCaptureJob) (string, error) {
	...
	// the reader shows it.
	buf.WriteString("> Captured from hivebot office · ")
