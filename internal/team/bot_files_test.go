...
// TestRepoCommitAndReadBotFile exercises the storage layer end to end against
...
func TestRepoCommitAndReadBotFile(t *testing.T) {
	repo := newTestRepo(t)
...
func TestValidateBotFilePath(t *testing.T) {
	...
	for _, p := range ok {
		if err := validateBotFilePath(p); err != nil {
...
func TestRenderBotFilesDeterministic(t *testing.T) {
	member := officeMember{
		Slug:         "growth",
		Name:         "Growth Lead",
		Role:         "growth lead",
	...
	soul := renderBotSoul(member, false)
	...
	if leadSoul := renderBotSoul(member, true); !strings.Contains(leadSoul, "You are the lead") {
		t.Errorf("lead SOUL must carry the lead boundary:\n%s", leadSoul)
	}

	identity := renderBotIdentity(member)
	...
	ops := renderBotOperations(member, false)
	...
	if leadOps := renderBotOperations(member, true); !strings.Contains(leadOps, "decompose it into owned sub-tasks") {
		t.Errorf("lead OPERATIONS must describe decomposition:\n%s", leadOps)
	}

	tools := renderBotTools(member)
	...
	if fallback := renderBotTools(officeMember{Slug: "x"}); !strings.Contains(fallback, "default office toolset") {
	...
	if fallback := renderBotTools(officeMember{Slug: "x", AllowedTools: []string{" ", "\t"}}); !strings.Contains(fallback, "default office toolset") {
...
func TestBotFilesPromptBlock(t *testing.T) {
	...
	if strings.Index(got, "lead persona") > strings.Index(got, "the human") {
		t.Error("bot files should precede the team USER file in the block")
	}
}
