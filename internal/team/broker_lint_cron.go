...
//   HIVEX_LINT_CRON   — "HH:MM" (24-hour, local time) to enable a daily run
...
//
// The cron is opt-in. Set HIVEX_LINT_CRON=HH:MM to enable a daily run.
...
func (b *Broker) startLintCron(ctx context.Context, idx *WikiIndex, worker *WikiWorker) {
	schedule := strings.TrimSpace(os.Getenv("HIVEX_LINT_CRON"))
	if schedule == "" || schedule == "disabled" {
		log.Printf("wiki lint cron: disabled (set HIVEX_LINT_CRON=HH:MM to enable a daily run)")
		return
	}

	hour, minute, err := parseLintCronTime(schedule)
	if err != nil {
		log.Printf("wiki lint cron: invalid HIVEX_LINT_CRON %q: %v — cron disabled", schedule, err)
		return
	}
