...
// Distinct from hivex-bootstrap / hivex-recovery / system so audit tools
...
// ScannerWikiRoot returns the wiki root honouring HIVEX_RUNTIME_HOME so
// dev runs stay isolated from the prod ~/.hivex.
func ScannerWikiRoot() string {
	home := strings.TrimSpace(config.RuntimeHomeDir())
	if home == "" {
		return filepath.Join(".hivex", "wiki")
	}
	return filepath.Join(home, ".hivex", "wiki")
}
