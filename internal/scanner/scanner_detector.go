...
// detector as file ingestion so Hivex has one secret catalog.
...
// HIVEX_SCAN_EXTENSIONS (comma-separated, leading "." optional).
...
func LoadScanExtensions(override []string) map[string]struct{} {
	...
	case len(override) > 0:
		list = override
	default:
		if env := strings.TrimSpace(os.Getenv("HIVEX_SCAN_EXTENSIONS")); env != "" {
