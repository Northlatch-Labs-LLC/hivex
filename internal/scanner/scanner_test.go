...
func TestScannerWalkDirSkipsSymlinks(t *testing.T) {
	...
	// Regression anchor for the eng-review decision to diverge from hive-cli.
	if len(got) != 2 {
...
func TestScannerLoadScanExtensions(t *testing.T) {
	t.Setenv("HIVEX_SCAN_EXTENSIONS", "")
	...
	t.Setenv("HIVEX_SCAN_EXTENSIONS", "go, md , txt")
...
func TestScannerScanFixtureOnlyIngestsInnocentMarkdown(t *testing.T) {
	...
	t.Setenv("HIVEX_SCAN_EXTENSIONS", "md,pem,env")
