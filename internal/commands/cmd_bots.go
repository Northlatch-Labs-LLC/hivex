...
func resolveBrokerBaseURL() string {
	if v := strings.TrimSpace(os.Getenv("HIVEX_TEAM_BROKER_URL")); v != "" {
...
func resolveBrokerToken() string {
	if v := strings.TrimSpace(os.Getenv("HIVEX_BROKER_TOKEN")); v != "" {
		return v
	}
	if path := strings.TrimSpace(os.Getenv("HIVEX_BROKER_TOKEN_FILE")); path != "" {
