...
func TestCmdAgentCreate_RejectsInvalidProvider(t *testing.T) {
	...
	if err := cmdBotCreate(ctx, "pm --provider gemini"); err != nil {
		t.Fatalf("cmdBotCreate: %v", err)
	}
...
func TestCmdAgentCreate_MissingSlug(t *testing.T) {
	ctx, out := captureMessages()
	if err := cmdBotCreate(ctx, "--provider codex"); err != nil {
		t.Fatalf("cmdBotCreate: %v", err)
	}
...
func TestCmdAgentCreate_PostsToBroker(t *testing.T) {
	...
	t.Setenv("HIVEX_TEAM_BROKER_URL", ts.URL)
	t.Setenv("HIVEX_BROKER_TOKEN", "test-token")
	t.Setenv("HIVEX_BROKER_TOKEN_FILE", "")

	ctx, out := captureMessages()
	if err := cmdBotCreate(ctx, "pm-bot --name 'PM Bot' --provider codex --model gpt-5.4 --role 'Product Manager'"); err != nil {
		t.Fatalf("cmdBotCreate: %v", err)
	}
...
func TestCmdAgentPrompt_GeneratesAndCreatesMember(t *testing.T) {
	...
	ctx, out := captureMessages()
	if err := cmdBotPrompt(ctx, "deep research teammate"); err != nil {
		t.Fatalf("cmdBotPrompt: %v", err)
	}
...
func TestCmdAgentRemove_PostsToBroker(t *testing.T) {
	...
	ctx, out := captureMessages()
	if err := cmdBotRemove(ctx, "pm-bot"); err != nil {
		t.Fatalf("cmdBotRemove: %v", err)
	}
...
func TestCmdAgentEdit_ProviderSwitch(t *testing.T) {
	...
	ctx, out := captureMessages()
	if err := cmdBotEdit(ctx, "pm-bot --provider openclaw --session-key agent:test:pm"); err != nil {
		t.Fatalf("cmdBotEdit: %v", err)
	}
