...
func ManifestPath() string {
	if path := strings.TrimSpace(os.Getenv("HIVEX_COMPANY_FILE")); path != "" {
		return path
	}

	if strings.TrimSpace(os.Getenv("HIVEX_RUNTIME_HOME")) == "" {
		if cwd, err := os.Getwd(); err == nil {
			local := filepath.Join(cwd, "hivex.company.json")
	...
	if home == "" {
		return filepath.Join(".hivex", "company.json")
	}
	return filepath.Join(home, ".hivex", "company.json")
}
...
func backfillFromConfig(manifest Manifest) Manifest {
	cfg, _ := config.Load()
	if strings.TrimSpace(manifest.Name) == "" || manifest.Name == "Your hivebot team" {
...
func launchFromScratchRequested() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("HIVEX_START_FROM_SCRATCH"))) {
	case "1", "true", "yes":
		return true
	default:
		return false
...
func normalizeManifest(manifest Manifest) Manifest {
	if strings.TrimSpace(manifest.Name) == "" {
		manifest.Name = "Your hivebot team"
	}
