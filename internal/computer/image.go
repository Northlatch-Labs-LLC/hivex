...
const (
	...
	ImageRepository   = "localhost/hivebot/cua-computer"
	ImageLayerVersion = "1"
	...
	Display        = ":1"
	CuaSocket      = "/run/user/1000/hivebot-cua.sock"
	CuaExecutable  = "/usr/local/libexec/hivebot/cua-driver"
...
func ManagedImageDockerfile() string {
	...
	sb.WriteString("      > /usr/local/bin/prepare-hivebot-workspace.sh \\\n")
	sb.WriteString("    && chmod 0755 /usr/local/bin/prepare-hivebot-workspace.sh\n")
	// Driver start script: wait for X, then serve on a private socket.
	sb.WriteString("RUN printf '%s\\n' \\\n")
	for _, line := range []string{
		"#!/bin/sh",
		"/usr/local/bin/prepare-hivebot-workspace.sh",
		"attempt=0",
	...
	sb.WriteString("      > /usr/local/bin/start-hivebot-cua-driver.sh \\\n")
	sb.WriteString("    && chmod 0755 /usr/local/bin/start-hivebot-cua-driver.sh\n")
	sb.WriteString("RUN printf '%s\\n' \\\n")
	for _, line := range []string{
		"",
		"[program:hivebot-cua-driver]",
		"command=/usr/local/bin/start-hivebot-cua-driver.sh",
		"user=cua",
...
func PrepareImage(ctx context.Context, rt Runtime, stream StreamRunner, onLine func(string)) error {
	...
	dir, err := os.MkdirTemp("", "hivebot-cua-image-")
	if err != nil {
		return err
	}
	...
	report("Building the hivebot desktop layer with Cua Driver " + CuaDriverVersion + "…")
