...
const (
	...
	RemoteCuaExecutable = "/opt/hivebot/cua-driver"
	RemoteCuaSocket     = "/opt/hivebot/run/cua.sock"
	RemoteCuaSession    = "hivebot"
	MaxCommandLength    = 4000
	PanelPath           = "/tmp/hivebot-panel.jpg"
	ShotPath            = "/tmp/hivebot-shot.jpg"
	ShotWidth           = 1280
	jpegQuality         = 70
)
...
func EnsureCuaCommand() string {
	...
	return strings.Join([]string{
		...
		"    display=${display:-:0}",
		`    nohup env HOME="$HOME" DISPLAY="$display" CUA_DRIVER_INSTALL_CHANNEL=python_package CUA_DRIVER_RS_TELEMETRY_ENABLED=0 ` + RemoteCuaExecutable + " serve --socket " + RemoteCuaSocket + " --permission-mode standard > /tmp/hivebot-cua-driver.log 2>&1 &",
...
func BootstrapCommand(botName string) string {
	...
	installer := strings.Join([]string{
		"set -eu",
		"trap 'rm -f /tmp/hivebot-cua-installing' EXIT",
		"sudo mkdir -p /opt/hivebot/run",
		`sudo chown -R "$(id -u):$(id -g)" /opt/hivebot`,
		`arch="$(uname -m)"`,
...
PY`,
		...
		"touch /opt/hivebot/cua-" + RemoteCuaVersion + "-ready",
		`rm -f "$wheel"`,
	}, "\n")
	banner := base64.StdEncoding.EncodeToString([]byte("  ▦ " + botName + "'s computer — hivebot"))
	...
	return strings.Join([]string{
		...
		"sudo mkdir -p /opt/hivebot/run",
		"[ -f /opt/hivebot/cua-" + RemoteCuaVersion + "-ready ] || [ -f /tmp/hivebot-cua-installing ] || { touch /tmp/hivebot-cua-installing; nohup bash -c " + ShellQuote(installer) + " > /tmp/hivebot-cua-install.log 2>&1 & }",
		EnsureCuaCommand(),
