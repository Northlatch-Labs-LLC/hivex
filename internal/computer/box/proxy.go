...
func MCPLaunch(binary, boxID, token string, control computer.ControlEndpoint) computer.MCPLaunch {
	launch := computer.MCPLaunch{
		...
		Env:     map[string]string{"HIVEX_BOX_API_KEY": token},
	}
	if control.URL != "" && control.Token != "" {
		launch.Env["HIVEX_COMPUTER_CONTROL_URL"] = control.URL
		launch.Env["HIVEX_COMPUTER_CONTROL_TOKEN"] = control.Token
	}
	return launch
}
...
func RunProxy(ctx context.Context, client *Client, boxID string, gate *computer.GateClient) error {
	if !client.Configured() {
		return fmt.Errorf("HIVEX_BOX_API_KEY is not set")
	}
	if boxID == "" {
		return fmt.Errorf("box id is required")
	}
	p := &Proxy{client: client, boxID: boxID, gate: gate}
	server := mcp.NewServer(&mcp.Implementation{Name: "hivebot-computer", Version: "0.1.0"}, nil)
	p.register(server)
...
func cuaOrX11(tool, argsShell, fallback string) string {
	return strings.Join([]string{
		...
		"then if env " + cuaEnv + " " + RemoteCuaExecutable + " call " + tool + " " + argsShell + " --socket " + RemoteCuaSocket + " >/dev/null 2>/tmp/hivebot-cua-call.error;",
