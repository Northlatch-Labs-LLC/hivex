// Package upgradecheck compares the running hivex version against the latest
// published on npm and (optionally) summarises the diff via the GitHub
// compare API. It is consumed by the `hivex upgrade` subcommand and by the
// in-app web banner.
//
// Scope note: the npm shim (npm/bin/hivex.js, PR #273) already self-heals
// when the installed hivex binary falls behind the latest published release,
...
// surfaces (`hivex upgrade`, the web banner).
...
const (
	NPMPackage = "hivex"
	GitHubRepo = "Northlatch-Labs-LLC/hivex"
)
...
// ("dev") hivex build. The literal sentinel set ("" / "dev") lives in
...
// published hivex version. hivex is at v0.83.x as of this change, so
...
func fetchLatestVersion(ctx context.Context, client *http.Client) (string, error) {
	...
	req.Header.Set("User-Agent", "hivex-upgradecheck/1.0")
...
// for the `hivex upgrade` subcommand. Breaking-change commits are surfaced
