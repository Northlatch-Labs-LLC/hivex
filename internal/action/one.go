...
func NewOneCLIFromEnv() *OneCLI {
	bin := strings.TrimSpace(os.Getenv("HIVEX_ONE_BIN"))
	workDir := strings.TrimSpace(os.Getenv("HIVEX_ONE_WORKDIR"))
	if workDir == "" {
...
func (o *OneCLI) executeActionViaFlow(ctx context.Context, req ExecuteRequest) (ExecuteResult, error) {
	tempRoot, err := os.MkdirTemp("", "hivex-one-action-flow-")
	if err != nil {
		return ExecuteResult{}, fmt.Errorf("create temp flow dir: %w", err)
	}
	defer func() { _ = os.RemoveAll(tempRoot) }()

	flowKey := fmt.Sprintf("hivex-auto-action-%d", time.Now().UTC().UnixNano())
...
func buildOneActionFlowDefinition(req ExecuteRequest) map[string]any {
	...
	return map[string]any{
		"key":         "hivex-auto-action",
		"name":        "hivebot Auto Action",
...
func (o *OneCLI) run(ctx context.Context, args []string) ([]byte, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		if errors.As(err, &execErr) && errors.Is(execErr.Err, exec.ErrNotFound) {
			return nil, fmt.Errorf("one CLI not found. Install One, make npx available, or set HIVEX_ONE_BIN")
		}
...
func (o *OneCLI) commandWorkDir(args []string) string {
	...
	if dir := strings.TrimSpace(os.Getenv("HIVEX_ONE_ACTION_WORKDIR")); dir != "" {
		return dir
	}
