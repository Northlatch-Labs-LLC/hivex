...
const composioWorkflowVersion = "hivex_workflow_v1"
...
func (c *ComposioREST) decodeWorkflowDefinition(definition json.RawMessage) (composioWorkflowDefinition, error) {
	...
	for i := range spec.Steps {
		...
		default:
			// Unknown step types are accepted on purpose: rejecting at decode
			// time would make an already-saved workflow (one carrying a step
			// type from a retired or newer build) unloadable and unlistable,
			// hiding it from the operator who needs to edit it. The failure
			// belongs at execution, which names the step and says what to do.
		}
	}
	return spec, nil
}
...
func (c *ComposioREST) executeWorkflowStep(ctx context.Context, step workflowStep, scope map[string]any, workflowDryRun bool) (map[string]any, error) {
	...
	default:
		return nil, fmt.Errorf(
			"workflow step %q uses the unsupported step type %q. "+
