...
//	HIVEX_EMBEDDING_MODEL     — overrides the model (default text-embedding-3-small)
//	HIVEX_EMBEDDING_DIMENSION — overrides the expected dimension (default 1536)
//	HIVEX_EMBEDDING_TIMEOUT   — Go duration string (default 30s)
...
func openAIModelFromEnv() string {
	if v := strings.TrimSpace(os.Getenv("HIVEX_EMBEDDING_MODEL")); v != "" {
		return v
	}
...
// embeddingDimensionOverride reads HIVEX_EMBEDDING_DIMENSION and returns
...
func embeddingDimensionOverride() (int, bool) {
	raw := strings.TrimSpace(os.Getenv("HIVEX_EMBEDDING_DIMENSION"))
	if raw == "" {
		return 0, false
	}
...
func openAITimeoutFromEnv() time.Duration {
	raw := strings.TrimSpace(os.Getenv("HIVEX_EMBEDDING_TIMEOUT"))
	if raw == "" {
