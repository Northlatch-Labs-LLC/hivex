...
const (
	...
	// HIVEX_EMBEDDING_DIMENSION when switching models.
...
func newVoyageProvider() Provider {
	...
	model := strings.TrimSpace(os.Getenv("HIVEX_EMBEDDING_MODEL"))
	if model == "" {
		model = defaultVoyageModel
	}

	// Voyage's voyage-3-large default is 1024 (vs OpenAI's 1536). Honour
	// an explicit HIVEX_EMBEDDING_DIMENSION when set so users on
