...
func TestEmbeddingDimensionOverride(t *testing.T) {
	t.Run("unset", func(t *testing.T) {
		t.Setenv("HIVEX_EMBEDDING_DIMENSION", "")
	...
	t.Run("explicit 1536 is honoured", func(t *testing.T) {
		t.Setenv("HIVEX_EMBEDDING_DIMENSION", "1536")
		n, ok := embeddingDimensionOverride()
		if !ok || n != 1536 {
	...
	t.Run("explicit 512 is honoured", func(t *testing.T) {
		t.Setenv("HIVEX_EMBEDDING_DIMENSION", "512")
		n, ok := embeddingDimensionOverride()
		if !ok || n != 512 {
	...
	t.Run("garbage falls back to default", func(t *testing.T) {
		t.Setenv("HIVEX_EMBEDDING_DIMENSION", "not-a-number")
...
// user on voyage-code-2 with HIVEX_EMBEDDING_DIMENSION=1536 must get a
...
func TestNewVoyageProvider_HonoursExplicitDimension(t *testing.T) {
	...
	t.Setenv("HIVEX_EMBEDDING_MODEL", "voyage-code-2")
	t.Setenv("HIVEX_EMBEDDING_DIMENSION", "1536")
