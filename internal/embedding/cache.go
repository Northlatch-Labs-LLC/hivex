...
// Two `hivex` processes pointed at the same JSONL will race the rotation
...
// other tools talk to it; do not point a second hivex at the same
// HIVEX_EMBEDDING_CACHE_PATH.
...
// running with HIVEX_EMBEDDING_CACHE=disabled).
...
// DefaultCachePath resolves $HIVEX_HOME/.hivex/cache/embeddings.jsonl.
// Honours HIVEX_EMBEDDING_CACHE_PATH for full override and
// HIVEX_EMBEDDING_CACHE=disabled to opt out entirely.
func DefaultCachePath() string {
	if strings.EqualFold(strings.TrimSpace(os.Getenv("HIVEX_EMBEDDING_CACHE")), "disabled") {
		return ""
	}
	if v := strings.TrimSpace(os.Getenv("HIVEX_EMBEDDING_CACHE_PATH")); v != "" {
		return v
	}
	...
	return filepath.Join(home, ".hivex", "cache", "embeddings.jsonl")
}
