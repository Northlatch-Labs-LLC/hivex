...
// ~/.hivex/config.json edits take effect on the next bot turn without
// requiring a restart.
//
...
var httpClientForOpenAICompat = func() *http.Client {
	...
	// for loopback; users pointing HIVEX_OLLAMA_BASE_URL at a remote box
	// (e.g. a Mac Studio over Wi-Fi) can bump via
	// HIVEX_OPENAI_COMPAT_DIAL_TIMEOUT_SECONDS.
	tr := &http.Transport{
...
func openAICompatDialTimeout() time.Duration {
	if raw := strings.TrimSpace(os.Getenv("HIVEX_OPENAI_COMPAT_DIAL_TIMEOUT_SECONDS")); raw != "" {
...
func setOpenAICompatProviderHeaders(req *http.Request, kind, botSlug string) {
	...
	// requests. Tying them to the office slug gives each hivebot member a stable
...
// OpenAICompatAPIKey returns the bearer token hivebot will use for kind's
...
// parseOpenAISSEStream consumes the SSE body and translates it into hivex's
...
func parseOpenAISSEStream(ch chan<- bot.StreamChunk, kind string, body io.Reader) {
	...
	// accumulated text and synthesize a tool_use chunk so the rest of hivex
