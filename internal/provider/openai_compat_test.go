...
func TestOpenAICompatDialTimeout_EnvOverride(t *testing.T) {
	t.Setenv("HIVEX_OPENAI_COMPAT_DIAL_TIMEOUT_SECONDS", "")
	if got := openAICompatDialTimeout(); got != 5*time.Second {
		t.Errorf("default = %v, want 5s", got)
	}
	t.Setenv("HIVEX_OPENAI_COMPAT_DIAL_TIMEOUT_SECONDS", "30")
	...
	t.Setenv("HIVEX_OPENAI_COMPAT_DIAL_TIMEOUT_SECONDS", "not-a-number")
	if got := openAICompatDialTimeout(); got != 5*time.Second {
		t.Errorf("garbage env = %v, want 5s fallback", got)
	}
	t.Setenv("HIVEX_OPENAI_COMPAT_DIAL_TIMEOUT_SECONDS", "0")
	if got := openAICompatDialTimeout(); got != 5*time.Second {
...
func TestOpenAICompatStreamFn_HTTP500SurfacesAsError(t *testing.T) {
	...
	t.Setenv("HIVEX_MLX_LM_BASE_URL", srv.URL+"/v1")
...
func TestOpenAICompatStreamFn_TextStream_EndToEnd(t *testing.T) {
	...
	t.Setenv("HIVEX_OLLAMA_BASE_URL", srv.URL+"/v1")
...
func TestOpenAICompatStreamFn_RequestBodyShape(t *testing.T) {
	...
	t.Setenv("HIVEX_EXO_BASE_URL", srv.URL+"/v1")
	t.Setenv("HIVEX_EXO_MODEL", "test-model")
...
func TestOpenAICompatStreamFn_HermesHeadersWhenAuthenticated(t *testing.T) {
	...
	t.Setenv("HIVEX_HERMES_AGENT_BASE_URL", srv.URL+"/v1")
	t.Setenv("HIVEX_HERMES_AGENT_API_KEY", "test-hermes-key")
	...
	if gotSessionID != "hivex-bot-one" {
		t.Fatalf("X-Hermes-Session-Id = %q, want hivex-bot-one", gotSessionID)
	}
...
func TestOpenAICompatStreamFn_HermesOmitsSessionHeadersWithoutAuth(t *testing.T) {
	...
	t.Setenv("HIVEX_HERMES_AGENT_BASE_URL", srv.URL+"/v1")
	t.Setenv("HIVEX_HERMES_AGENT_API_KEY", "")
...
func TestOpenAICompatStreamFn_OpenclawHTTPUsesGatewayToken(t *testing.T) {
	...
	t.Setenv("HIVEX_OPENCLAW_HTTP_BASE_URL", srv.URL+"/v1")
	t.Setenv("HIVEX_OPENCLAW_HTTP_API_KEY", "")
	t.Setenv("OPENCLAW_GATEWAY_TOKEN", "gateway-token")
	t.Setenv("HIVEX_OPENCLAW_TOKEN", "")
	...
	if !strings.Contains(gotBody, `"user":"hivex-bot-one"`) {
		t.Fatalf("request user = %s, want stable hivebot bot user", gotBody)
	}
}
