...
// is opted in with HIVEX_TEST_LIVE_MLX_LM=1 so CI / `go test ./...` stays
// hermetic.
//
// Run manually with:
//
...
//	HIVEX_TEST_LIVE_MLX_LM=1 go test -v \
//	  -run TestOpenAICompatStreamFn_LiveMLXServer \
//	  ./internal/provider/
func TestOpenAICompatStreamFn_LiveMLXServer(t *testing.T) {
	if os.Getenv("HIVEX_TEST_LIVE_MLX_LM") != "1" {
		t.Skip("set HIVEX_TEST_LIVE_MLX_LM=1 to run against a live mlx_lm.server on :8080")
	}
...
// unless HIVEX_TEST_LIVE_MLX_LM=1.
//
// Tool-calling reliability varies by model; this test asserts the hivex
...
// HIVEX_TEST_LIVE_OLLAMA=1; the model name comes from
// HIVEX_OLLAMA_MODEL (default: the registered package default, which
...
func TestOpenAICompatStreamFn_LiveOllama(t *testing.T) {
	if os.Getenv("HIVEX_TEST_LIVE_OLLAMA") != "1" {
		t.Skip("set HIVEX_TEST_LIVE_OLLAMA=1 (and ensure HIVEX_OLLAMA_MODEL is pulled) to run against a live ollama daemon on :11434")
	}
...
func TestOpenAICompatStreamFn_LiveOllama_CtxCancelAbortsHTTP(t *testing.T) {
	if os.Getenv("HIVEX_TEST_LIVE_OLLAMA") != "1" {
		t.Skip("set HIVEX_TEST_LIVE_OLLAMA=1 to run against a live ollama daemon on :11434")
	}
...
func TestOpenAICompatStreamFn_LiveExo_ConnectRefused(t *testing.T) {
	...
	t.Setenv("HIVEX_EXO_BASE_URL", "http://127.0.0.1:1/v1")
