...
func TestMigrateLegacyConfigDirCopiesForward(t *testing.T) {
	home := t.TempDir()
	legacy := filepath.Join(home, ".hive")
	newDir := filepath.Join(home, ".hivex")
...
func TestMigrateLegacyConfigDirNoLegacy(t *testing.T) {
	home := t.TempDir()
	if migrateLegacyConfigDir(filepath.Join(home, ".hivex"), filepath.Join(home, ".hive")) {
...
// ─── rename CHAIN ────────────────────────────────────────────────────────────
// Renames chain: .hive became .hivex, and .hivex becomes whatever is next. The
...
// Skipping a generation is normal: someone may have installed at .hive, never
