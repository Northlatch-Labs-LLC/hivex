...
// RuntimeCacheDir returns the cache directory under the active hivebot runtime
...
// CacheDirForRuntimeHome returns the .hivex cache directory for runtimeHome.
func CacheDirForRuntimeHome(runtimeHome string) string {
	runtimeHome = strings.TrimSpace(runtimeHome)
	if runtimeHome == "" {
		return ""
	}
	return filepath.Join(runtimeHome, ".hivex", "cache")
}
