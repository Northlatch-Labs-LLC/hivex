// Package config handles loading, saving, and resolving hivebot configuration.
...
// RuntimeHomeDir returns the home directory hivebot should use for persisted
// runtime state. Inventive runs may override this with HIVEX_RUNTIME_HOME so
// they don't inherit an existing office from the user's global ~/.hivex.
//
// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — this is the
// definition of RuntimeHomeDir itself; os.UserHomeDir() is the fallback only.
func RuntimeHomeDir() string {
	if v := strings.TrimSpace(Getenv("HIVEX_RUNTIME_HOME")); v != "" {
		return v
	}
...
// Config mirrors ~/.hivex/config.json.
type Config struct {
	...
	// HIVEX_POSTHOG_KEY env), so an unset flag is moot until analytics is
	...
	// (HIVEX_MLX_LM_BASE_URL, HIVEX_OLLAMA_MODEL, etc.) take precedence over
	// this map.
...
const (
	...
	// MemoryBackendMemanto selects the self-hosted Memanto memory agent
...
// ConfigPath returns the absolute path to ~/.hivex/config.json, with a legacy
// fallback to ~/.hive/config.json when the old file already exists.
func ConfigPath() string {
	// Env override for test harnesses that need to isolate config state from
	// the user's real ~/.hivex/config.json without remapping HOME (which
	// breaks macOS keychain-backed CLI auth).
	if p := strings.TrimSpace(Getenv("HIVEX_CONFIG_PATH")); p != "" {
		return p
	}
	home := RuntimeHomeDir()
	if home == "" {
...
//
// A retired backend name normalizes to the empty string like any other
// unrecognized value, so a stale `memory_backend` value left in an existing
...
//  2. HIVEX_MEMORY_BACKEND env var.
...
//     zero-config, git-native wiki at ~/.hivex/wiki.
//
// A retired backend name persisted in an old config file normalizes away in step 3 and lands on the step-4 default, so
...
func ResolveMemoryBackend(flagValue string) string {
	...
	if backend == "" {
		backend = NormalizeMemoryBackend(Getenv("HIVEX_MEMORY_BACKEND"))
	}
	if backend == "" {
		cfg, _ := Load()
...
func gbrainBinaryInstalled() bool {
	// An explicit HIVEX_GBRAIN_COMMAND is authoritative (mirrors
	...
	if cmd := strings.TrimSpace(Getenv("HIVEX_GBRAIN_COMMAND")); cmd != "" {
...
func ResolveLLMProvider(flagValue string) string {
	...
	if v := normalizeLLMProvider(Getenv("HIVEX_LLM_PROVIDER")); v != "" {
		return v
	}
	cfg, _ := Load()
...
// for --provider, HIVEX_LLM_PROVIDER, and the config file. claude-code and
...
// HIVEX_CODEX_MODEL/CODEX_MODEL env > nearest .codex/config.toml > ~/.codex/config.toml.
func ResolveCodexModel(cwd string) string {
	if v := strings.TrimSpace(Getenv("HIVEX_CODEX_MODEL")); v != "" {
		return v
	}
...
func codexConfigSearchPaths(cwd string) []string {
	...
	// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — codex config
	// layering reads from the user's real home, not the hivebot workspace root.
...
// run. Resolution: HIVEX_OPENCODE_MODEL env > OPENCODE_MODEL env > empty (Opencode
// picks its configured default). Unlike Codex, Opencode has no on-disk config
// file layout hivebot needs to inspect — users configure their Opencode
// ~/.config/opencode settings directly, so there is no cwd-relative search.
func ResolveOpencodeModel() string {
	if v := strings.TrimSpace(Getenv("HIVEX_OPENCODE_MODEL")); v != "" {
		return v
	}
...
// Resolution: HIVEX_ONE_SECRET env > ONE_SECRET env > config file.
func ResolveOneSecret() string {
	if v := strings.TrimSpace(Getenv("HIVEX_ONE_SECRET")); v != "" {
		return v
	}
...
// ResolveOneIdentity resolves the identity scope hivebot should use with One.
// Resolution: HIVEX_ONE_IDENTITY env > ONE_IDENTITY env > config email.
func ResolveOneIdentity() string {
	if v := strings.TrimSpace(Getenv("HIVEX_ONE_IDENTITY")); v != "" {
		return v
	}
...
// Resolution: HIVEX_ONE_IDENTITY_TYPE env > ONE_IDENTITY_TYPE env > "user".
func ResolveOneIdentityType() string {
	if v := strings.TrimSpace(Getenv("HIVEX_ONE_IDENTITY_TYPE")); v != "" {
		return v
	}
...
func OneSetupBlurb() string {
	email := ResolveOneIdentity()
	if email != "" {
		return fmt.Sprintf("hivebot uses One for integrations, scoped to %s.", email)
	}
	return "hivebot uses One for integrations. Set ONE_SECRET and ONE_IDENTITY to enable it."
}

// ResolveComposioAPIKey resolves the Composio API key.
// Resolution: HIVEX_COMPOSIO_API_KEY env > COMPOSIO_API_KEY env > config file.
func ResolveComposioAPIKey() string {
	if v := strings.TrimSpace(Getenv("HIVEX_COMPOSIO_API_KEY")); v != "" {
		return v
	}
...
// (`uak_…`). Resolution: HIVEX_COMPOSIO_USER_API_KEY env > config file.
func ResolveComposioUserAPIKey() string {
	if v := strings.TrimSpace(Getenv("HIVEX_COMPOSIO_USER_API_KEY")); v != "" {
		return v
	}
	cfg, _ := Load()
...
func ResolveComposioOrgID() string {
	if v := strings.TrimSpace(Getenv("HIVEX_COMPOSIO_ORG_ID")); v != "" {
		return v
	}
	cfg, _ := Load()
...
func ResolveComposioProjectID() string {
	if v := strings.TrimSpace(Getenv("HIVEX_COMPOSIO_PROJECT_ID")); v != "" {
		return v
	}
	cfg, _ := Load()
...
// Resolution: HIVEX_POSTHOG_KEY env > POSTHOG_KEY env.
func ResolvePostHogKey() string {
	if v := strings.TrimSpace(Getenv("HIVEX_POSTHOG_KEY")); v != "" {
		return v
	}
...
// Resolution: HIVEX_POSTHOG_HOST env > POSTHOG_HOST env.
func ResolvePostHogHost() string {
	if v := strings.TrimSpace(Getenv("HIVEX_POSTHOG_HOST")); v != "" {
		return v
	}
...
func ResolveTelegramBotToken() string {
	if v := strings.TrimSpace(Getenv("HIVEX_TELEGRAM_BOT_TOKEN")); v != "" {
		return v
	}
	cfg, _ := Load()
...
// Resolution: HIVEX_GEMINI_API_KEY env > GEMINI_API_KEY env > config file.
func ResolveGeminiAPIKey() string {
	if v := strings.TrimSpace(Getenv("HIVEX_GEMINI_API_KEY")); v != "" {
		return v
	}
...
// Resolution: HIVEX_REALTIME_MODEL env > config file > DefaultRealtimeModel.
func ResolveRealtimeModel() string {
	if v := strings.TrimSpace(Getenv("HIVEX_REALTIME_MODEL")); v != "" {
		return v
	}
...
// Resolution: HIVEX_MINIMAX_API_KEY env > MINIMAX_API_KEY env > config file.
func ResolveMinimaxAPIKey() string {
	if v := strings.TrimSpace(Getenv("HIVEX_MINIMAX_API_KEY")); v != "" {
		return v
	}
...
// ResolveComposioUserID resolves the Composio user identity hivebot should use.
// Resolution: HIVEX_COMPOSIO_USER_ID env > COMPOSIO_USER_ID env > config email >
...
func ResolveComposioUserID() string {
	if v := strings.TrimSpace(Getenv("HIVEX_COMPOSIO_USER_ID")); v != "" {
		return v
	}
...
// Resolution: HIVEX_ACTION_PROVIDER env > ACTION_PROVIDER env > config file > auto.
func ResolveActionProvider() string {
	if v := strings.TrimSpace(Getenv("HIVEX_ACTION_PROVIDER")); v != "" {
...
func ResolveInsightsPollInterval() int {
	minutes := 30
	if raw := Getenv("HIVEX_INSIGHTS_INTERVAL_MINUTES"); raw != "" {
...
// HIVEX_OPENCLAW_TOKEN wins for hivebot-specific setup; OPENCLAW_GATEWAY_TOKEN is
// accepted for compatibility with OpenClaw's own Gateway docs.
func ResolveOpenclawToken() string {
	if v := strings.TrimSpace(Getenv("HIVEX_OPENCLAW_TOKEN")); v != "" {
		return v
	}
...
func ResolveOpenclawGatewayURL() string {
	if v := strings.TrimSpace(Getenv("HIVEX_OPENCLAW_GATEWAY_URL")); v != "" {
		return v
	}
	cfg, _ := Load()
...
//
//  1. Per-kind env vars: HIVEX_<KIND>_BASE_URL and HIVEX_<KIND>_MODEL
//     (kind uppercased with '-' → '_'; e.g. mlx-lm → HIVEX_MLX_LM_BASE_URL).
...
func ResolveProviderEndpoint(kind, defaultBaseURL, defaultModel string) (string, string) {
	...
	baseURL := strings.TrimSpace(Getenv("HIVEX_" + envKind + "_BASE_URL"))
	model := strings.TrimSpace(Getenv("HIVEX_" + envKind + "_MODEL"))
	if baseURL == "" || model == "" {
		cfg, _ := Load()
...
// user-scoped 0600 file under the hivebot home.
//
// user-global; intentionally NOT under HIVEX_RUNTIME_HOME — OpenClaw identity
...
func ResolveOpenclawIdentityPath() string {
	if v := strings.TrimSpace(Getenv("HIVEX_OPENCLAW_IDENTITY_PATH")); v != "" {
		return v
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(".hivex", "openclaw", "identity.json")
	}
	return filepath.Join(home, ".hivex", "openclaw", "identity.json")
}
