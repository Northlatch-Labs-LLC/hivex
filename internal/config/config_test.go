...
func TestResolvePostHogKeyAndHostFromEnv(t *testing.T) {
	t.Setenv("HIVEX_POSTHOG_KEY", "phc_test")
	t.Setenv("POSTHOG_KEY", "ignored")
	t.Setenv("HIVEX_POSTHOG_HOST", "https://eu.i.posthog.com")
	if got := ResolvePostHogKey(); got != "phc_test" {
		t.Errorf("key: HIVEX_POSTHOG_KEY should win, got %q", got)
	}
...
func TestSaveCreatesParentDirs(t *testing.T) {
	withTempConfig(t, func(dir string) {
		...
		path := filepath.Join(dir, ".hivex", "config.json")
...
func TestSaveWritesValidJSON(t *testing.T) {
	withTempConfig(t, func(dir string) {
		...
		raw, _ := os.ReadFile(filepath.Join(dir, ".hivex", "config.json"))
...
func TestResolveMemoryBackendExplicitCommandMissingDoesNotFallBack(t *testing.T) {
	// An explicit HIVEX_GBRAIN_COMMAND that no longer resolves (e.g. a reaped
	...
	withTempConfig(t, func(_ string) {
		...
		t.Setenv("ANTHROPIC_API_KEY", "")
		fakeGBrainOnPath(t)
		t.Setenv("HIVEX_GBRAIN_COMMAND", filepath.Join(t.TempDir(), "missing-wrapper.sh"))
		if got := ResolveMemoryBackend(""); got != MemoryBackendMarkdown {
...
func TestResolveComposioUserID(t *testing.T) {
	...
	clearEnv := func(t *testing.T) {
		t.Helper()
		t.Setenv("HIVEX_COMPOSIO_USER_ID", "")
	...
	t.Run("hivebot env override wins", func(t *testing.T) {
		withTempConfig(t, func(_ string) {
			clearEnv(t)
			t.Setenv("HIVEX_COMPOSIO_USER_ID", "u_env")
...
func TestResolveGeminiAPIKeyEnvOverride(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_GEMINI_API_KEY", "hivex-gemini")
		_ = Save(Config{GeminiAPIKey: "file-gemini"})
		if got := ResolveGeminiAPIKey(); got != "hivex-gemini" {
...
func TestResolveMinimaxAPIKeyEnvOverride(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_MINIMAX_API_KEY", "hivex-minimax")
		_ = Save(Config{MinimaxAPIKey: "file-minimax"})
		if got := ResolveMinimaxAPIKey(); got != "hivex-minimax" {
			t.Fatalf("expected hivebot env override, got %q", got)
		}
	})
}
...
func TestResolveLLMProviderUsesEnvOverride(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_LLM_PROVIDER", "codex")
...
func TestResolveLLMProviderAcceptsOpencode(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_LLM_PROVIDER", "opencode")
...
func TestResolveOpencodeModelEnvOverride(t *testing.T) {
	t.Setenv("HIVEX_OPENCODE_MODEL", "qwen3.6:35b-a3b")
	if got := ResolveOpencodeModel(); got != "qwen3.6:35b-a3b" {
		t.Fatalf("expected HIVEX_OPENCODE_MODEL override, got %q", got)
	}
}

func TestResolveOpencodeModelFallsBackToOpencodeEnv(t *testing.T) {
	t.Setenv("HIVEX_OPENCODE_MODEL", "")
...
func TestResolveCodexModelUsesEnvOverride(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_CODEX_MODEL", "gpt-5.4")
...
func TestResolveOpenclawTokenEnvWins(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_OPENCLAW_TOKEN", "env-token")
...
// TestResolveMemoryBackendDegradesRetiredBackend pins the persisted-state
// half of a backend retirement: a config.json still carrying a backend name
// this build no longer ships is on real users' disks. Loading it must land
// on a working backend, not error and not leave the office with no memory.
func TestResolveMemoryBackendDegradesRetiredBackend(t *testing.T) {
	withTempConfig(t, func(_ string) {
		t.Setenv("HIVEX_MEMORY_BACKEND", "")
		clearGBrainReadiness(t)
		if err := Save(Config{MemoryBackend: "legacy-retired"}); err != nil {
			t.Fatalf("save config: %v", err)
		}
		got := ResolveMemoryBackend("")
		if got != MemoryBackendMarkdown {
			t.Fatalf("a retired backend should degrade to the markdown default, got %q", got)
		}
	})
}

// TestNormalizeMemoryBackendRejectsRetiredName documents that a retired
// backend name is no longer selectable: it normalizes away like any unknown
// value, which is what makes the degradation above fall through to the
// implicit default.
func TestNormalizeMemoryBackendRejectsRetiredName(t *testing.T) {
	if got := NormalizeMemoryBackend("legacy-retired"); got != "" {
		t.Fatalf("NormalizeMemoryBackend(\"legacy-retired\") = %q, want \"\"", got)
	}
}
