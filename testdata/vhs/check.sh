#!/usr/bin/env bash
# Visual regression check for hivex.
#
...
#
# Tapes covered:
#   help.tape     — `hivex --help` output
#   version.tape  — `hivex --version` output
#
...
#
# Version bumps in cmd/hivebot/buildinfo drift version.txt — update the golden
...
go build -o /tmp/hivex-vhs ./cmd/hivebot
...
check_one() {
  ...
  if ! normalize "$golden_txt"; then
    ...
    rm -f "$backup_txt"
    echo "hivex visual regression: ${name} normalize failed — golden restored" >&2
    return 1
  fi
  ...
  if ! diff -q "$backup_txt" "$golden_txt" >/dev/null; then
    ...
    cp "$backup_txt" "$golden_txt"
    rm -f "$backup_txt"
    echo "hivex visual regression: ${name}.txt drifted" >&2
  ...
  echo "hivex visual regression: ${name} OK"
}
