# @hivex/protocol — Agent Guidelines
...
The default `@hivex/protocol` entry targets **Node.js (and Electron's main
...
The `@hivex/protocol/browser` subpath is the curated renderer-safe surface for
...
Anything a consumer can import from `@hivex/protocol` is in
...
- Bump `schemaVersion` only via a coordinated cross-implementation release per
  decision 7 (shared `hive-receipt.schema.json` ships simultaneously to Hive
  Cloud, Hive CLI, Hive MCP).
