...
hivebot v1 protocol package: branded types, receipt schema, IPC envelopes, audit events.
...
- `@hivex/protocol` is the Node/Electron-main entry. It includes audit-chain
  hashing, `FrozenArgs`, receipt codecs, and other helpers backed by
  `node:crypto`.
- `@hivex/protocol/browser` is the sandboxed Electron renderer entry. It
...
Spec: `business-musings/hivex-greenfield-rewrite-rfc-2026-05.md` §6 (Receipt schema), §7.3 (IPC discipline).
...
## Approval request wire shape

`ApprovalRequest` is the route/projection artifact for pending approvals. Its runtime TypeScript shape is camelCase, while `approvalRequestToJsonValue` emits snake_case JSON:
