...
8. Cross-field invariants MUST be enforced in the validator: receipt co-sign token `claim.receiptId` equals the enclosing receipt ID; external-write tokens also bind `claim.frozenArgsHash` to the locally re-derived `proposedDiff` hash; write-scoped tokens bind `claim.writeId` to `ExternalWrite.writeId`; token `scope.role` matches the approval event role; `expiresAt` is strictly after `notBefore`; and write `approvedAt` must lie in the half-open window `[notBefore, expiresAt)` — at or after token `notBefore` and strictly before token `expiresAt`.
