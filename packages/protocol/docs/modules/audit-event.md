...
This module defines the deterministic audit-chain record shape, canonical hash projection, and chain verifier for hivebot audit events. It is the package's tamper-evidence boundary: if it is removed or drifts, independent writers and verifiers cannot prove that event order, payload bytes, or Merkle checkpoints are stable across runtimes.
...
1. `GENESIS_PREV_HASH` MUST be `sha256("hivex:audit:genesis:v1")`, not the older RFC sketch literal `sha256("genesis")`; code and consumer-facing specs MUST call out the divergence.
