...
This module defines the pure-data IPC contract for Hivex's broker boundary: renderer/main OS verbs, loopback HTTP envelopes, approval submission, SSE projections, WebSocket frames, and DNS-rebinding loopback checks. It belongs in `@hivex/protocol` because downstream packages need the same branded types and validators without importing I/O, Electron, HTTP servers, or keychain code. Removing it would leave bearer-token bootstrap, approval submission, and loopback security checks stringly typed across processes.
...
`apiBootstrapToJson`, `approvalSubmitRequestFromJson`,
...
Shared constants in `ipc-shared.ts`: `SIGNED_APPROVAL_TOKEN_KEYS`. It stays
outside `ipc.ts` so IPC, receipt codecs, and receipt validators share one token
key allowlist without import cycles or duplicated literal tuples.
...
4. Approval submissions MUST carry only `receiptId`, `approvalToken`, and `idempotencyKey`. They MUST NOT carry mutable proposed payloads; the broker compares the WebAuthn token and receipt-owned data.
5. `validateApprovalSubmitRequest` MUST reject non-objects, unknown request keys, accessor fields, missing fields, invalid `ReceiptId`, invalid `IdempotencyKey`, non-object tokens, unknown token/claim/scope/assertion keys, malformed WebAuthn assertion fields, and invalid token lifetimes.
6. Approval submit tokens MUST carry a `receipt_co_sign` claim; that claim and its scope MUST reject unknown keys, validate optional `writeId`, validate `frozenArgsHash`, enforce `MAX_APPROVAL_TOKEN_LIFETIME_MS`, and require request `receiptId === approvalToken.claim.receiptId`.
7. The IPC validator MUST NOT verify WebAuthn assertions, current-time expiry, credential trust, role thresholds, write/diff binding, idempotency replay, or broker policy. Those require broker state.
...
flowchart TD
  ...
  G --> H["validate claim, scope, and assertion shapes"]
  H --> I["require receipt_co_sign claim"]
  I --> J["bind scope to claim"]
  J --> K["validate notBefore/expiresAt"]
  K --> L{"expiresAt strictly after notBefore"}
  L -->|no| X
  L -->|yes| M{"lifetime within cap"}
  M -->|no| X
  M -->|yes| N{"request receiptId equals claim receiptId"}
  N -->|yes| OK["ok"]
  N -->|no| X
```
...
| Token scope without `role` | `approvalToken/scope/role: is required` | Role thresholds need signed role intent. |
| `expiresAt === notBefore` | `expiresAt must be strictly greater than notBefore` | Equal timestamps must not mint zero-lifetime capabilities. |
...
Callers provide parsed runtime objects, not raw bytes. Approval token timestamps are caller-supplied integer epoch milliseconds; raw JSON must pass through `approvalSubmitRequestFromJson` before `validateApprovalSubmitRequest`. Broker code must strip ports from remote peer addresses, run both loopback checks, perform WebAuthn verification and policy decisions, and treat SSE/WebSocket payloads as untrusted until a runtime codec or validator checks them.
