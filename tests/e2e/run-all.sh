#!/bin/bash
# E2E acceptance tests for hivebot CLI Go TUI
...
SOCKET="/tmp/hivex-go-e2e.sock"
NEX="${HIVEX_BIN:-$REPO_ROOT/hivex}"
...
cleanup() {
  pkill -f "termwright.*hivex-go-e2e" 2>/dev/null || true
  rm -f "$SOCKET"
  sleep 1
}
...
echo "=== hivebot CLI E2E Acceptance Tests ==="
echo "Binary: $NEX"
