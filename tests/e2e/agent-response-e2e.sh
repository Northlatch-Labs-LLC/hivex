...
hivebot="${HIVEX_BIN:-$REPO_ROOT/hivex}"
ARTIFACTS="${ARTIFACTS:-$REPO_ROOT/termwright-artifacts/agent-response-$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$ARTIFACTS"

SOCKET="/tmp/hivex-agent-resp-$$.sock"
DAEMON_PID=""

cleanup() {
  [ -n "$DAEMON_PID" ] && kill "$DAEMON_PID" 2>/dev/null
  tmux -L hivex kill-session -t hivex-team 2>/dev/null || true
  rm -f "$SOCKET"
  sleep 2
}
trap cleanup EXIT
...
# Start fresh
cleanup
rm -f ~/.hivex/team/broker-state.json 2>/dev/null

"$TERMWRIGHT" daemon --socket "$SOCKET" --cols 140 --rows 40 -- "$hivebot" -no-hive 2>/dev/null &
DAEMON_PID=$!
sleep 22

BROKER_TOKEN=$(cat /tmp/hivex-broker-token)
...
echo ""
echo "=== Agent Panes ==="
tmux -L hivex list-panes -t hivex-team -F "pane #{pane_index}: #{pane_current_command}" 2>/dev/null
...
" 2>/dev/null)
  ...
  # Check pane activity
  PANE1=$(tmux -L hivex capture-pane -p -J -S -3 -t hivex-team:team.1 2>/dev/null | grep -v '^$' | tail -1)
...
if [ "$RESPONDED" = true ]; then
  ...
  tmux -L hivex capture-pane -p -J -S -10 -t hivex-team:team.1 2>/dev/null
  echo ""
  echo "--- All messages ---"
...
" 2>/dev/null)
  ...
  PANE1=$(tmux -L hivex capture-pane -p -J -S -2 -t hivex-team:team.1 2>/dev/null | grep -v '^$' | tail -1)
...
if [ "$RESPONDED2" = true ]; then
  ...
  echo "--- CEO pane ---"
  tmux -L hivex capture-pane -p -J -S -15 -t hivex-team:team.1 2>/dev/null
fi
...
echo "Agent panes:"
tmux -L hivex list-panes -t hivex-team -F "  pane #{pane_index}" 2>/dev/null
...
if [ "$RESPONDED3" = true ]; then
  ...
  echo "--- FE pane (pane 3) ---"
  tmux -L hivex capture-pane -p -J -S -15 -t hivex-team:team.3 2>/dev/null
fi
