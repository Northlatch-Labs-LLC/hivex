...
start_office() {
  ...
  SOCKET="/tmp/hivex-session-office-$$.sock"
  "$TERMWRIGHT" daemon --socket "$SOCKET" --cols 120 --rows 40 -- "$hivebot" -no-hive &
  DAEMON_PID=$!
  sleep 20
  BROKER_TOKEN=$(cat /tmp/hivex-broker-token 2>/dev/null)
}

start_1o1() {
  cleanup
  rm -f ~/.hivex/team/broker-state.json 2>/dev/null
  SOCKET="/tmp/hivex-session-1o1-$$.sock"
  "$TERMWRIGHT" daemon --socket "$SOCKET" --cols 120 --rows 40 -- "$hivebot" -no-hive -1o1 &
  DAEMON_PID=$!
  sleep 20
  BROKER_TOKEN=$(cat /tmp/hivex-broker-token 2>/dev/null)
}
...
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          hivebot SESSION E2E — Full Feature Coverage          ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo "Binary: $hivebot"
...
start_office
if tmux -L hivex list-panes -t hivex-team 2>/dev/null | grep -q "." ; then
...
PANE_COUNT=$(tmux -L hivex list-panes -t hivex-team 2>/dev/null | wc -l | tr -d ' ')
...
SOCKET="/tmp/hivex-session-splash-$$.sock"
"$TERMWRIGHT" daemon --socket "$SOCKET" --cols 120 --rows 40 -- "$hivebot" -no-hive &
sleep 8
screenshot "33-splash"
# Splash should show character sprites or hivebot title
SCREEN=$(screen_text)
if echo "$SCREEN" | grep -q "CEO\|hivebot\|Channels" 2>/dev/null; then
