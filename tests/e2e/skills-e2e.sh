...
SOCKET="/tmp/hivex-skills-e2e-$$.sock"
...
cleanup() {
  pkill -f "termwright.*hivex-skills-e2e" 2>/dev/null || true
  rm -f "$SOCKET"
  sleep 1
}
...
start_daemon() {
  ...
  BROKER_TOKEN=$(cat /tmp/hivex-broker-token 2>/dev/null || true)
}
