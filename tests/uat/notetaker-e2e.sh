...
log_test "Version"
if "$BINARY" --version 2>&1 | grep -q "hivex v"; then pass; else fail "bad version"; fi

log_test "Start hivex"
...
BROKER_TOKEN=$(cat /tmp/hivex-broker-token 2>/dev/null || echo "")
if [ -z "$BROKER_TOKEN" ]; then
  echo "    WARNING: no broker token found, authenticated requests will fail"
fi

log_test "tmux session exists"
if tmux -L hivex list-sessions 2>&1 | grep -q "hivex-team"; then pass; else fail "no session"; fi

log_test "Single team window exists"
WIN_NAMES=$(tmux -L hivex list-windows -t hivex-team -F "#{window_name}" 2>&1)
...
TOTAL_PANES=$(tmux -L hivex list-panes -t hivex-team:team 2>&1 | wc -l | tr -d ' ')
...
CHAN=$(tmux -L hivex capture-pane -p -t hivex-team:team.0 2>&1)
echo "$CHAN" > "$ARTIFACTS/channel-boot.txt"
if echo "$CHAN" | grep -qi "channel\|waiting\|hivex\|team"; then pass; else fail "empty"; fi
...
sleep 3
CHAN2=$(tmux -L hivex capture-pane -p -t hivex-team:team.0 2>&1)
...
for attempt in 1 2 3 4 5; do
  BOOTED=0
  for i in 1 2 3 4; do
    PANE_CMD=$(tmux -L hivex display-message -p -t "hivex-team:team.$i" "#{pane_current_command}" 2>/dev/null || echo "")
    CONTENT=$(tmux -L hivex capture-pane -p -t "hivex-team:team.$i" 2>/dev/null || echo "")
...
sleep 3
CHAN3=$(tmux -L hivex capture-pane -p -t hivex-team:team.0 2>&1)
...
tmux -L hivex send-keys -t "hivex-team:team.0" "/reset" Enter
sleep 12
RESET_STATUS=$(tmux -L hivex list-panes -t hivex-team:team -F "#{pane_index} #{pane_dead} #{pane_current_command}" 2>/dev/null || true)
...
RESET_CHAN=$(tmux -L hivex capture-pane -p -t hivex-team:team.0 2>&1)
echo "$RESET_CHAN" > "$ARTIFACTS/channel-after-reset.txt"
if echo "$RESET_CHAN" | grep -qi "#general\|The HIVEX swarm\|Message #general"; then
  pass
else
...
if tmux -L hivex list-sessions 2>&1 | grep -qi "no server\|error"; then pass; else fail "still running"; fi
