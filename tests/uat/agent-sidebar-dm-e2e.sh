...
SOCKET="/tmp/hivex-sidebar-dm-$$.sock"
...
# --no-hive: skip API-key prompt so the model starts clean in office-only mode
# HIVEX_NO_SPLASH: skip the splash screen so channelModel starts immediately
LAUNCHER="$ARTIFACTS/launcher.sh"
cat > "$LAUNCHER" <<EOF
#!/bin/bash
export HIVEX_NO_SPLASH=1
exec "$BINARY" --channel-view --no-hive "\$@"
EOF
chmod +x "$LAUNCHER"
...
# With HIVEX_NO_SPLASH=1, the channel view starts immediately.
echo ""
