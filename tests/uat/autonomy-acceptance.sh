...
SOCKET="/tmp/hivex-acceptance-$$.sock"
mkdir -p "$ARTIFACTS"
...
capture_channel_pane() {
  local label="$1"
  tmux -L hivex capture-pane -p -t hivex-team:team.0 > "$ARTIFACTS/$label.txt"
}
...
send_channel_line() {
  local text="$1"
  tmux -L hivex send-keys -t hivex-team:team.0 C-c
  sleep 0.2
  tmux -L hivex send-keys -t hivex-team:team.0 "$text" Enter
  sleep 1
}
...
echo "=== hivebot Autonomy Acceptance ==="
...
sleep 3
tmux -L hivex capture-pane -p -t hivex-team:team.0 > "$ARTIFACTS/channel-blocking.txt"
...
sleep 3
tmux -L hivex capture-pane -p -t hivex-team:team.0 > "$ARTIFACTS/channel-human-facing.txt"
