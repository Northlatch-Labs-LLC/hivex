...
`hivexbench` is the desktop SLO probe driver for the hivebot broker and web
proxy startup path.
...
go build -o hivexbench ./cmd/hivebotbench
```

The benchmark binary looks for `./hivex` first, then `hivex` on `PATH`.
Set `HIVEX_BENCH_HIVEX_BINARY=/path/to/hivex` to benchmark a specific build.

## Usage

```bash
./hivexbench --probe cold-start
./hivexbench --probe cold-start --iters 3
./hivexbench --probe ipc-latency
./hivexbench --probe all --out markdown
...
`cold-start` launches `hivex --no-open` on
...
flags: `hivex --no-open --web-port <port> --broker-port <port>`.
