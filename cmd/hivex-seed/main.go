// hivebot-seed populates a dev broker home with the 3 Sam ICP tutorial
// scenarios so a fresh dev launch lands on a non-empty Decision Inbox.
// Honours HIVEX_RUNTIME_HOME for path isolation (mirroring the dev
// alias). Idempotent: re-running replaces tutorial tasks but leaves
// other state untouched.
//
// Usage:
//
//	HIVEX_RUNTIME_HOME="$HOME/.hivebot-dev-home" go run ./cmd/hivebot-seed
package main
...
func main() {
	home := strings.TrimSpace(os.Getenv("HIVEX_RUNTIME_HOME"))
	if home == "" {
		home = config.RuntimeHomeDir()
	}
	if home == "" {
		log.Fatal("hivebot-seed: cannot resolve runtime home")
	}
	statePath := filepath.Join(home, ".hivebot", "team", "broker-state.json")
	if err := os.MkdirAll(filepath.Dir(statePath), 0o700); err != nil {
		log.Fatalf("hivebot-seed: mkdir state dir: %v", err)
	}
	...
	for _, p := range []struct{ slug, name string }{
		...
		if err := b.EnsureBridgedMember(p.slug, p.name, "seed"); err != nil {
			log.Fatalf("hivebot-seed: ensure member %q: %v", p.slug, err)
		}
	}
...
func must(err error, where string) {
	if err != nil {
		log.Fatalf("hivebot-seed: %s: %v", where, err)
	}
}
...
func seedTutorial3(b *team.Broker, channel string) (taskA, taskB string, err error) {
	...
	if _, ok, err := b.BlockTask(taskA, "sam", "tutorial 3 demo: blocked on inbox feature ("+taskB+") shipping first", taskB); err != nil || !ok {
		log.Fatalf("hivebot-seed: tutorial 3 BlockTask: ok=%v err=%v", ok, err)
	}
