...
func TestBuildOfficeIntroLinesUsesWorkspaceState(t *testing.T) {
	...
	t.Setenv("HIVEX_MEMORY_BACKEND", "none")
	...
	if !strings.Contains(plain, "Welcome to hivebot.") {
...
func TestBuildOfficeIntroLinesShowsOfflinePreviewGuidance(t *testing.T) {
	...
	if !strings.Contains(plain, "Launch hivebot to attach the live team, or run /doctor to inspect runtime readiness.") {
