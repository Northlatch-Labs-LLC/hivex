...
func buildOfficeMessageLines(messages []channelui.BrokerMessage, expanded map[string]bool, contentWidth int, threadsDefaultExpand bool, unreadAnchorID string, unreadCount int) []channelui.RenderedLine {
	...
	if len(messages) == 0 {
		lines = append(lines,
			...
			channelui.RenderedLine{Text: mutedStyle.Render("  Welcome to hivebot. The staff is assembled, and they are all looking at you.")},
			...
			channelui.RenderedLine{Text: mutedStyle.Render("  The Chief of Staff triages first, then the right specialists pile in. This one ships.")},
		)
		return lines
	}
