...
// `hivebot workspace pause <name>` — drain & stop a workspace's broker.
//
...
func runWorkspacePause(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot workspace pause — stop a running workspace")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot workspace pause [--force] <name>")
		fmt.Fprintln(os.Stderr, "  hivebot workspace pause <name>            Graceful drain, 90s timeout")
		fmt.Fprintln(os.Stderr, "  hivebot workspace pause --force <name>    Skip drain; SIGKILL after 5s")
		fmt.Fprintln(os.Stderr, "")
	...
	_, _ = fmt.Fprintf(os.Stdout, "Paused %q. Resume with: hivebot workspace resume %s\n", name, name)
}
