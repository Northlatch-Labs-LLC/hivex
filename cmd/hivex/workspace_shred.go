...
// `hivebot workspace shred <name>` — back up and wipe a workspace.
//
// Default: confirm dialog (type the name to confirm), then the orchestrator
// splits `~/.hivex-spaces/<name>/.hivex/` into a categorized backup at
// `~/.hivex-spaces/.backups/<name>-<unix>/{wiki,skills,chats,context}/` and
...
//     today's `hivebot shred` confirm flow.
//   - We add an extra warning line about ~/.hivex/ contents because main
...
func runWorkspaceShred(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot workspace shred — burn a workspace down")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot workspace shred <name>             Back up wiki/skills/chats/context (30-day retention)")
		fmt.Fprintln(os.Stderr, "  hivebot workspace shred <name> --permanent  Skip backup (irreversible)")
		fmt.Fprintln(os.Stderr, "  hivebot workspace shred <name> --yes        Skip the confirm prompt (scripts only)")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Restorable for 30 days unless --permanent. List shredded workspaces with")
		fmt.Fprintln(os.Stderr, "`hivebot workspace list --trash` and bring one back with `hivebot workspace restore`.")
	}
	...
	_, _ = fmt.Fprintf(os.Stdout, "Shredded %q. Wiki, skills, chats, and context backed up; restorable for 30 days via `hivebot workspace restore`.\n", name)
}
...
// reader/writer.
//
// For `main`, we print an extra warning about the migrated ~/.hivex/ contents
...
func shredConfirmFromReader(in io.Reader, out io.Writer, name string, permanent bool) (bool, error) {
	if name == "main" {
		_, _ = fmt.Fprintln(out, "WARNING: shredding `main` removes your migrated ~/.hivex/ workspace.")
		_, _ = fmt.Fprintln(out, "         This is the workspace hivebot created from your pre-multi-workspace install.")
