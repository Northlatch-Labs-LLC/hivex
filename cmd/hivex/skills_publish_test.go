...
func TestBuildManifest(t *testing.T) {
	...
	manifest, err := skillpublish.BuildManifest(skillpublishFrontmatterFromTeam(fm), body, "hivex-pr4-publish", time.Date(2026, 4, 28, 0, 0, 0, 0, time.UTC))
	if err != nil {
	...
	if manifest.Source != "hivex-hivex-pr4-publish-deploy-frontend" {
...
func TestHubURL_GithubScheme(t *testing.T) {
	t.Parallel()
	got, err := skillpublish.HubURL("github:Northlatch-Labs-LLC/hivex-skills", "review-pr")
	if err != nil {
		t.Fatalf("HubURL: %v", err)
	}
	want := "https://raw.githubusercontent.com/Northlatch-Labs-LLC/hivex-skills/main/skills/review-pr/SKILL.md"
	if got != want {
...
func TestHubURL_GithubScheme_BranchOverride(t *testing.T) {
	t.Parallel()
	got, err := skillpublish.HubURL("github:Northlatch-Labs-LLC/hivex-skills@master", "review-pr")
	if err != nil {
		t.Fatalf("HubURL: %v", err)
	}
	want := "https://raw.githubusercontent.com/Northlatch-Labs-LLC/hivex-skills/master/skills/review-pr/SKILL.md"
	if got != want {
...
func TestResolveSkillPath_Slug(t *testing.T) {
	wikiHome := t.TempDir()
	t.Setenv("HIVEX_RUNTIME_HOME", wikiHome)
	skillsDir := filepath.Join(wikiHome, ".hivex", "wiki", "team", "skills")
...
func TestResolveSkillPath_Missing(t *testing.T) {
	...
	if _, err := resolveSkillPath("/tmp/hivex-test-nope.md"); err == nil {
...
func TestSanitizeHubLabel(t *testing.T) {
	...
	cases := []struct{ in, want string }{
		...
		{"github:Northlatch-Labs-LLC/hivex-skills", "github-northlatch-labs-llc-hivex-skills"},
		{"github:Northlatch-Labs-LLC/hivex-skills@master", "github-northlatch-labs-llc-hivex-skills-master"},
		{"  Lobehub  ", "lobehub"},
	}
...
func TestBuildPublishPRBody_NoCWDLeak(t *testing.T) {
	...
	manifest := skillpublish.Manifest{
		...
		Version:     "1.0.0",
		License:     "MIT",
		Source:      "hivex-team-deploy-frontend",
...
// endpoint, point HIVEX_BROKER_BASE_URL at it, and assert the request
...
func TestPostBrokerSkill_RoundTrip(t *testing.T) {
	...
	t.Setenv("HIVEX_BROKER_BASE_URL", srv.URL)
...
func TestBuildPublishPRBody(t *testing.T) {
	...
	body := buildPublishPRBody(manifest, "Cheers from team hivebot.")
	checks := []string{
		"Ship the frontend.",
		"`deploy-frontend` (v1.2.3, MIT)",
		"`hivex-team-deploy-frontend`",
		"2026-04-28T00:00:00Z",
		"Cheers from team hivebot.",
	}
