...
var channelSlashCommands = []tui.SlashCommand{
	...
	{Name: "quit", Description: "Exit hivebot — Michael would make a speech first", Category: "session"},
}
...
func newChannelModelWithApp(threadsCollapsed bool, initialApp channelui.OfficeApp) channelModel {
	...
	oneOnOneBot := ""
	if strings.EqualFold(strings.TrimSpace(os.Getenv("HIVEX_ONE_ON_ONE")), "1") || strings.EqualFold(strings.TrimSpace(os.Getenv("HIVEX_ONE_ON_ONE")), "true") {
		sessionMode = team.SessionModeOneOnOne
		oneOnOneBot = strings.TrimSpace(os.Getenv("HIVEX_ONE_ON_ONE_AGENT"))
		if oneOnOneBot == "" {
...
func (m channelModel) buildOneOnOneModePickerOptions() []tui.PickerOption {
	enableDescription := "Restart hivebot in direct mode with one selected bot and kill the rest of the Claude sessions"
	if m.isOneOnOne() {
		enableDescription = "Pick a different single bot for this direct session"
	}
	disableDescription := "Restart hivebot with the full office team"
	if !m.isOneOnOne() {
