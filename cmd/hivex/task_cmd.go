...
//
//   - `hivebot task start [intent]`      — prompt-driven intake → ready → running
//   - `hivebot task list [--filter=<f>]` — print inbox grouped by LifecycleState
//   - `hivebot task review <id>`         — open the Decision Packet view in the browser
//   - `hivebot task block <id> --on <pr-or-task-id>` — set blocked
//
...
// `hivebot` running (the broker), and the CLI POSTs intent to a future
...
// runTaskCmd dispatches `hivebot task <verb> [args]`.
func runTaskCmd(args []string) {
	...
	case "block":
		runTaskBlock(rest)
	default:
		fmt.Fprintf(os.Stderr, "hivebot task: unknown verb %q\n", verb)
		printTaskHelp()
		os.Exit(2)
	}
}
...
func printTaskHelp() {
	fmt.Fprintln(os.Stderr, "hivebot task — drive the multi-bot control loop")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Usage:")
	fmt.Fprintln(os.Stderr, "  hivebot task start [intent]                Walk an intent through intake → ready → running")
	fmt.Fprintln(os.Stderr, "  hivebot task list [--filter=<filter>]      List tasks grouped by lifecycle state")
	fmt.Fprintln(os.Stderr, "  hivebot task review <id>                   Open the Decision Packet view in your browser")
	fmt.Fprintln(os.Stderr, "  hivebot task block <id> --on <ref>         Mark task blocked on a PR or task")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Filters: needs_decision (default), running, blocked, merged_today, all.")
}

// runTaskStart implements `hivebot task start [intent]`. v1 talks to the
...
func runTaskStart(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot task start — drive an intent through intake")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot task start \"fix the cache invalidation bug\"")
		fmt.Fprintln(os.Stderr, "  hivebot task start                       # reads the intent from stdin")
	}
	_ = fs.Parse(args)
...
var taskStartHook = func(ctx context.Context) (*team.Broker, team.IntakeProvider, error) {
	return nil, nil, fmt.Errorf("task start: not yet wired to a live broker (set HIVEX_TASK_PROVIDER for tests)")
}
...
// runTaskList implements `hivebot task list`. Calls GET /tasks/inbox,
...
func runTaskList(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot task list — print the inbox grouped by lifecycle state")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot task list                            All tasks across all states")
		fmt.Fprintln(os.Stderr, "  hivebot task list --filter=needs_decision    Only the inbox-headline filter")
		fmt.Fprintln(os.Stderr, "  hivebot task list --json                     Emit the raw broker payload")
	}
	_ = fs.Parse(args)
	...
	if err != nil {
		...
		fmt.Fprintln(os.Stderr, "  (is the broker running? try `hivebot` first)")
		os.Exit(1)
	}
...
// runTaskBlock implements `hivebot task block <id> --on <ref>`. POSTs
...
func runTaskBlock(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot task block — mark a task blocked on a PR or task")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot task block <id> --on <pr-or-task-id> [--reason \"text\"]")
	}
	_ = fs.Parse(args)
	if fs.NArg() == 0 {
