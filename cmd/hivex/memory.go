...
// memory.go hosts the `hivebot memory` subcommand family. Today it owns
// one verb: `migrate`, which ports legacy GBrain memory into the markdown
// wiki at ~/.hivex/wiki/team/.
//
// Why a separate file
// ===================
//
...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/migration"
	"github.com/Northlatch-Labs-LLC/hivex/internal/team"
)

// runMemory dispatches `hivebot memory <verb>`. Called from main.go when
// args[0] == "memory".
func runMemory(args []string) {
	...
	default:
		fmt.Fprintf(os.Stderr, "hivebot memory: unknown verb %q — run `hivebot memory --help` for the list.\n", verb)
		os.Exit(1)
	}
}
...
func printMemoryHelp() {
	...
	fmt.Fprintln(os.Stderr, "  migrate    Port GBrain content into ~/.hivex/wiki/team/")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Usage:")
	fmt.Fprintln(os.Stderr, "  hivebot memory migrate --from gbrain [--dry-run] [--limit N]")
}
...
func runMemoryMigrate(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot memory migrate — import legacy memory into the team wiki")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot memory migrate --from gbrain [--dry-run] [--limit N]")
		fmt.Fprintln(os.Stderr, "")
