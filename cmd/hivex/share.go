...
const shareCookieName = "hivex_human_session"
...
type webShareController struct {
	...
	// nil (e.g. the standalone `hivebot share` subcommand has no in-process
...
func (c *webShareController) issueInviteLocked(ctx context.Context, bind string, port int, brokerURL string, brokerTokenFn func() (string, error)) (string, string, error) {
	if c.broker != nil {
		if st := c.broker.ShareTransport(); st != nil {
			...
			// with the public-tunnel controller (cmd/hivebot/tunnel.go), which
...
func webShareErrorMessage(err error) string {
	...
	if strings.Contains(msg, "no private network interface found") {
		return "No private network interface found.\n\nhivebot only creates team-member invites over a private network by default. Start Tailscale or WireGuard, then click Create invite again."
	}
...
func runShare(args []string) {
	...
	if opts.stop {
		fmt.Fprintln(os.Stderr, "error: `hivebot share --stop` is not active because share runs in the foreground. Press Ctrl+C in the share terminal.")
		os.Exit(1)
	}
...
func readBrokerToken() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("hivebot broker is not running. Start it with `hivebot`, then run `hivebot share`")
	}
...
func resolveShareBind(opts shareOptions) (net.IP, string, error) {
	...
	if want == "tailscale" {
		return nil, "", fmt.Errorf("no private network interface found\n\nhivebot only shares over a private network by default.\nFix: start Tailscale, then run `hivebot share` again.\nOverride: `hivebot share --bind 100.x.y.z`")
	}
	if want == "wireguard" {
		return nil, "", fmt.Errorf("no WireGuard interface found\n\nFix: start WireGuard, then run `hivebot share --bind wireguard` again")
	}
...
func validateShareIP(ip net.IP, opts shareOptions) error {
	if ip == nil || ip.IsLoopback() || ip.IsUnspecified() {
		return fmt.Errorf("refusing to expose hivebot on %s", ip)
	}
	...
	return fmt.Errorf("refusing to expose hivebot on a public interface\n\nDetected: %s\nFix: use a Tailscale/WireGuard address or keep hivebot local.\nUnsafe local-LAN escape hatch: `hivebot share --unsafe-lan`", ip)
}
...
// hivebot share terminal). JoinGate and RateLimiter are tunnel-only
...
func handleShareJoinSubmit(w http.ResponseWriter, r *http.Request, brokerURL, token string, onJoin func(), gate joinGateFn) {
	...
	if err != nil {
		writeShareJoinError(w, http.StatusInternalServerError, "invalid_request", "hivebot could not encode your invite submission.")
		return
	}
	req, err := http.NewRequestWithContext(r.Context(), http.MethodPost, brokerURL+"/humans/invites/accept", bytes.NewReader(payload))
	if err != nil {
		writeShareJoinError(w, http.StatusBadGateway, "broker_unreachable", "hivebot is not reachable from this invite. Ask the host to restart sharing.")
		return
	}
	req.Header.Set("Content-Type", "application/json")
	resp, err := shareHTTPClient.Do(req)
	...
	if resp.StatusCode != http.StatusOK {
		...
		case resp.StatusCode >= 500:
			writeShareJoinError(w, http.StatusBadGateway, "broker_failed", "hivebot could not accept this invite right now. Ask the host to retry sharing.")
		default:
...
func shareStaticHandler() http.Handler {
	...
	if embeddedFS, ok := hivex.WebFS(); ok {
