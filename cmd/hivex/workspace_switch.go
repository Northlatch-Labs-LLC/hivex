...
// `hivebot workspace switch <name>` — update cli_current and (optionally) open.
//
// The CLI flag --workspace=<name> on the top-level hivebot command is the
// "for this single command" override. `switch` is the persistent counterpart:
// it writes registry.cli_current so unqualified `hivebot` invocations target
...
func runWorkspaceSwitch(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot workspace switch — change the CLI default workspace")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot workspace switch <name>")
		fmt.Fprintln(os.Stderr, "  hivebot workspace switch --open <name>")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Updates registry.cli_current. Future unqualified `hivebot` runs target this workspace.")
