...
func (m onboardingModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case localRuntimeDetectedMsg:
		...
		if m.step == stepSetup && m.err == "Checking for a local LLM runtime…" {
			if msg.kind != "" {
				m.err = fmt.Sprintf("Detected %s running on %s. Exit with Ctrl+C and re-run: `hivebot --provider %s`. Or paste an Anthropic key below.", msg.kind, msg.addr, msg.kind)
			} else {
...
func (m onboardingModel) viewWelcome(w, h int) string {
	...
	lines = append(lines, "")
	lines = append(lines, accentStyle.Render("  hivebot — Let's set up your team"))
...
// the cold-start dependency graph for `hivebot` first-run, which has to feel
...
func localRuntimeProbeAPIKey(kind string) string {
	...
	case "openclaw-http":
		...
		return strings.TrimSpace(os.Getenv("HIVEX_OPENCLAW_TOKEN"))
	default:
		return ""
	}
}
