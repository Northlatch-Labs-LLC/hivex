...
func (m channelModel) handleKeyMsg(msg tea.KeyMsg) (channelModel, tea.Cmd) {
	...
	case "ctrl+c":
		...
		m.lastCtrlCAt = now
		m.setTransientNotice("Press Ctrl+C again to quit hivebot. They will wait.")
		return m, nil
	case "ctrl+b":
		if m.isOneOnOne() {
			return m, nil
	...
	if m.quickJumpTarget != quickJumpNone {
		...
		case "1", "2", "3", "4", "5", "6", "7", "8", "9":
			...
			if target == quickJumpChannels {
				...
				m.setTransientNotice("No app on that number. Try a different one — hivebot believes in you.")
			}
			return m, nil
		case "esc":
			m.quickJumpTarget = quickJumpNone
