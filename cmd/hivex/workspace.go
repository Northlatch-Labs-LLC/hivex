...
// This file owns the `hivebot workspace ...` CLI subcommand tree. The tree is a
...
// ~/.hivex-spaces/.backups/. `TrashID` is the directory name
...
// wired in cmd/hivebot/main.go once Lane B lands. Method shapes:
//
...
// runWorkspace is the entry point for `hivebot workspace ...`. Routes args[0]
...
func runWorkspace(args []string) {
	...
	default:
		fmt.Fprintf(os.Stderr, "hivebot workspace: unknown subcommand %q\n", sub)
...
// printWorkspaceHelp owns the `hivebot workspace --help` copy. Kept here rather
// than in printSubcommandHelp() so adding a new subcommand only touches one
// file.
func printWorkspaceHelp() {
	fmt.Fprintln(os.Stderr, "hivebot workspace — manage hivebot workspaces (the offices upstairs)")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Each workspace is one fully isolated hivebot instance — its own team, wiki,")
	...
	fmt.Fprintln(os.Stderr, "  hivebot workspace list                       List all workspaces")
	fmt.Fprintln(os.Stderr, "  hivebot workspace list --json                Stable JSON for scripting")
	fmt.Fprintln(os.Stderr, "  hivebot workspace list --trash               Show trashed workspaces with restore IDs")
	fmt.Fprintln(os.Stderr, "  hivebot workspace create <name>              Create a new workspace")
	...
	fmt.Fprintln(os.Stderr, "  hivebot workspace switch <name> [--open]     Switch CLI default workspace")
	fmt.Fprintln(os.Stderr, "  hivebot workspace pause <name> [--force]     Pause (graceful 90s, --force kills hard)")
	fmt.Fprintln(os.Stderr, "  hivebot workspace resume <name>              Resume a paused workspace")
	fmt.Fprintln(os.Stderr, "  hivebot workspace shred <name> [--permanent] Shred (to trash; --permanent skips trash)")
	fmt.Fprintln(os.Stderr, "  hivebot workspace restore <trash-id>         Restore from trash with a fresh port pair")
	fmt.Fprintln(os.Stderr, "  hivebot workspace doctor                     Reconcile registry; interactive fixes")
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "Per-command help: hivebot workspace <subcommand> --help")
}
...
// runtime_home and exports it as HIVEX_RUNTIME_HOME for the duration of this
...
// first lookup onward.
//
// On orchestrator-not-wired we degrade gracefully: HIVEX_RUNTIME_HOME stays
...
func applyWorkspaceOverride(name string) {
	...
	if ws.RuntimeHome == "" {
		printError("--workspace=%q: workspace has no runtime_home in registry (run `hivebot workspace doctor`)", name)
	}
	_ = os.Setenv("HIVEX_RUNTIME_HOME", ws.RuntimeHome)
	if ws.BrokerPort > 0 {
		_ = os.Setenv("HIVEX_BROKER_PORT", fmt.Sprintf("%d", ws.BrokerPort))
	}
}
