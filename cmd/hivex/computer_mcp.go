...
// runComputerMCP is the `hivebot computer-mcp <runtime> <container> <socket>`
...
func runComputerMCP(args []string) int {
	...
	if len(args) != 3 {
		fmt.Fprintln(os.Stderr, "usage: hivebot computer-mcp <runtime> <container> <socket> | hivebot computer-mcp box <box-id>")
		return 2
	}
	...
	gate := &computer.GateClient{
		URL:   os.Getenv("HIVEX_COMPUTER_CONTROL_URL"),
		Token: os.Getenv("HIVEX_COMPUTER_CONTROL_TOKEN"),
	}
...
func runBoxMCP(boxID string) int {
	ctx, cancel := signal.NotifyContext(context.Background(), syscall.SIGTERM, syscall.SIGINT)
	defer cancel()
	client := box.NewClient(os.Getenv("HIVEX_BOX_API_KEY"))
	if api := os.Getenv("HIVEX_BOX_API"); api != "" {
		client.API = api
	}
	gate := &computer.GateClient{
		URL:   os.Getenv("HIVEX_COMPUTER_CONTROL_URL"),
