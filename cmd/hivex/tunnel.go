...
// next to the hivebot executable AND not on PATH. The npm postinstall ships
// cloudflared into the same directory as the hivebot binary so this path
...
func cloudflaredMissingMessage() string {
	...
	return "cloudflared is not installed.\n\n" +
		"hivebot normally bundles cloudflared with the npm install. If you see this,\n" +
		"either the bundle download was blocked (corp proxy / offline install) or\n" +
		"you installed hivebot via `go install` and we did not stage it.\n\n" +
		"Fix: reinstall hivebot with `npm install -g hivebot@latest`, or install\n" +
...
// order is: bundled-next-to-hivex, then PATH. The bundled lookup is first
// because npm postinstall stages a SHA256-verified release into the same
// directory as the hivebot binary, and a system-installed cloudflared on
...
func findCloudflared() (string, error) {
	...
	if exe, err := os.Executable(); err == nil {
		// Resolve symlinks (e.g. brew links hivebot to opt/hivex/bin/hivex
