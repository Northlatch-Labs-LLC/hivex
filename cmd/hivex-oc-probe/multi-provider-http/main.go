...
// `hivex` binary in the background (e.g. via tmux) and point the orchestrator
// at its broker via BROKER_URL + BROKER_TOKEN env. This keeps MCP server
// resolution (os.Executable) pointing at the real hivex, so claude/codex
...
func main() {
	...
	if brokerToken == "" {
		die("BROKER_TOKEN env required (the running hivex's broker token)")
	}
