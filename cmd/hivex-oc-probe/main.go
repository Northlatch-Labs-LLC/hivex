// hivex-oc-probe is a tiny smoke test that drives the OpenClaw bridge client
// against a running local daemon. Build and run it with:
//
//	OPENCLAW_TOKEN=... go run ./cmd/hivebot-oc-probe
//
...
func main() {
	...
	identity, err := openclaw.LoadOrCreateDeviceIdentity(filepath.Join(home, ".hivex", "openclaw", "identity.json"))
	if err != nil {
		die("identity: %v", err)
	}
	...
	res, err := client.SessionsSend(ctx, sess.Key, "hello from hivex Go probe", fmt.Sprintf("probe-%d", time.Now().UnixNano()))
	if err != nil {
