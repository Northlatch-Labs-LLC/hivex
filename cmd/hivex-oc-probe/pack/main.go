// hivebot-oc-probe/pack is the "pack parity" smoke test. It proves that a
// hivebot install with TWO OpenClaw bridge bindings behaves like a real pack:
//
...
// Run with:
//
//	OPENCLAW_TOKEN=... go run ./cmd/hivebot-oc-probe/pack
//
...
func main() {
	...
	for i, m := range members {
		raw, err := conn.Call(ctx, "sessions.create", map[string]any{
			"agentId": "main",
			"label":   "hivebot-pack-" + m.slug + "-" + runID,
		})
		if err != nil {
	...
	// Seed a temporary hivebot HOME so broker state doesn't clash with a real
	// install. Point identity + token back at the paired daemon.
	tmpHome, err := os.MkdirTemp("", "hivebot-oc-pack-*")
	if err != nil {
		die("tmp home: %v", err)
	}
