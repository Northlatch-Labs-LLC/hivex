// hivebot-oc-probe/bridge is a higher-level smoke test that exercises the full
// team.OpenclawBridge against a real OpenClaw daemon. Unlike the protocol-only
// probe under cmd/hivebot-oc-probe, this one proves:
//
...
// Run with:
//
//	OPENCLAW_TOKEN=... go run ./cmd/hivebot-oc-probe/bridge
package main
...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/openclaw"
...
func main() {
	...
	// Load identity from the real hivebot path so we reuse the daemon pairing
	...
	if len(rows) == 0 {
		...
		raw, err := create.Call(ctx, "sessions.create", map[string]any{"agentId": "main", "label": "hivebot-smoke"})
		if err != nil {
	...
	// 2. Seed a temporary hivebot HOME so broker state doesn't clash with the
	// user's real install. Point identity + token back at the paired daemon.
	tmpHome, err := os.MkdirTemp("", "hivebot-oc-smoke-*")
	if err != nil {
		die("tmp home: %v", err)
	}
	...
	if err := os.MkdirAll(filepath.Join(tmpHome, ".hivebot"), 0o700); err != nil {
		die("mkdir .hivebot: %v", err)
	}
	if err := os.Setenv("HIVEX_OPENCLAW_IDENTITY_PATH", realIdentityPath); err != nil {
		die("setenv identity path: %v", err)
	}
	if err := os.Setenv("HIVEX_OPENCLAW_TOKEN", token); err != nil {
