...
// No server required — reads directly from ~/.hivex/office/tasks/.
//
// Usage:
//
//	hivebot log              — list the 20 most recent tasks
//	hivebot log <taskID>     — dump the full JSONL for a single task as pretty lines
//	hivebot log --bot eng  — list recent tasks for a specific bot
//	hivebot log --limit 50   — override the default list size
func runLogCmd(args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "  hivebot log                 List recent tasks across all bots")
		fmt.Fprintln(os.Stderr, "  hivebot log <taskID>        Dump one task's full tool-call history")
		fmt.Fprintln(os.Stderr, "  hivebot log --bot eng     Filter the list to one bot")
		fmt.Fprintln(os.Stderr, "  hivebot log --limit 50      Override default list size")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Reads from ~/.hivex/office/tasks/{taskID}/output.log.")
	}
	_ = fs.Parse(args)
...
func printTaskList(tasks []bot.TaskLogSummary, root string) {
	...
	fmt.Println("")
	fmt.Println("Dig into one with: hivebot log <taskID>")
}
