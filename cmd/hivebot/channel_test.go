...
import (
	...
	"github.com/Northlatch-Labs-LLC/hivex/internal/tui"
)
...
func TestNewBrokerRequestUsesEnvTokenAtRequestTime(t *testing.T) {
	oldEnv := os.Getenv("HIVEX_BROKER_TOKEN")
	oldPath := brokerTokenPath
	t.Cleanup(func() {
		_ = os.Setenv("HIVEX_BROKER_TOKEN", oldEnv)
		brokerTokenPath = oldPath
	})

	brokerTokenPath = "/tmp/non-existent-hive-broker-token"
	if err := os.Setenv("HIVEX_BROKER_TOKEN", "token-from-env"); err != nil {
...
func TestNewBrokerRequestFallsBackToTokenFile(t *testing.T) {
	...
	if err := os.Unsetenv("HIVEX_BROKER_TOKEN"); err != nil {
...
func TestChannelViewUsesOfficeHeaderAndComposer(t *testing.T) {
	...
	view := stripANSI(m.View())
	if !strings.Contains(view, "hivebot") || !strings.Contains(view, "Message #general") {
...
func TestChannelViewUsesOneOnOneChrome(t *testing.T) {
	...
	//
	// This used to check that the view contained no "hivebot", using the
	// wordmark as a proxy for "the shared-room chrome is hidden". The proxy
	// broke on the rename: "Launch hivebot to attach the live team" is a
...
func TestOneOnOneStatusBarShowsRuntimeSummary(t *testing.T) {
	...
	m.members = []channelui.Member{{
		Slug:         "cos",
		Name:         "Chief of Staff",
		LiveActivity: "go test ./cmd/hivebot",
	}}
...
func TestProviderSelectionSavesCodexAndRequestsRestart(t *testing.T) {
	...
	if err := config.Save(config.Config{LLMProvider: "claude-code", Blueprint: "niche-crm"}); err != nil {
	...
	if !strings.Contains(done.notice, "Claude teammate panes were stopped.") || !strings.Contains(done.notice, "Restart hivebot to launch the headless Codex office runtime.") {
...
func TestTypingSwitchShortcutOpensChannelPicker(t *testing.T) {
	t.Setenv("HIVEX_API_KEY", "test-key")
	m := newChannelModel(false)
	m.channels = []channelui.ChannelInfo{
		{Slug: "general", Name: "general", Members: []string{"cos", "pm"}},
		{Slug: "launch", Name: "launch", Description: "Release work", Members: []string{"cos", "fe"}},
...
func TestChannelViewRendersNexAutomationMessage(t *testing.T) {
	...
	if !strings.Contains(view, "automation") || !strings.Contains(view, "Context alert") || !strings.Contains(view, "Important: Acme mentioned budget pressure") {
		t.Fatalf("expected Hive automation rendering, got %q", view)
	}
}
...
func TestChannelDoctorDoneShowsDoctorCard(t *testing.T) {
	...
	next, _ := m.Update(channelDoctorDoneMsg{report: channelui.DoctorReport{
		...
		Checks: []channelui.DoctorCheck{{
			...
			Detail:   "Missing hivebot/Hive API key.",
			NextStep: "Run /init and paste your hivebot API key.",
		}},
	}})
	got := next.(channelModel)
	...
	if !strings.Contains(view, "Doctor") || !strings.Contains(view, "Hive API key") {
...
func TestCalendarViewRendersSchedulerAndActions(t *testing.T) {
	...
	m.scheduler = []channelui.SchedulerJob{{Slug: "hive-insights", Label: "Hive insights", IntervalMinutes: 15, NextRun: "2026-03-24T10:15:00Z", Status: "sleeping"}}
	m.members = []channelui.Member{{Slug: "cos", Name: "Chief of Staff"}}

	view := stripANSI(m.View())
	if !strings.Contains(view, "Calendar") || !strings.Contains(view, "Hive insights") || !strings.Contains(view, "Opened a follow-up task") {
...
func TestChannelResetDoneImmediatelyRehydratesDirectMode(t *testing.T) {
	...
	if strings.Contains(view, "Welcome to hivebot.") {
...
func TestMain(m *testing.M) {
	// Use a temp home dir so tests don't read the real ~/.hivex/config.json
	tmp, _ := os.MkdirTemp("", "hivex-test-*")
	os.Setenv("HOME", tmp)
	os.Setenv("HIVEX_API_KEY", "test-key")
	os.Exit(m.Run())
}
