...
func (m *channelModel) startTelegramConnect() tea.Cmd {
	token := os.Getenv("HIVEX_TELEGRAM_BOT_TOKEN")
	if token == "" {
	...
	return func() tea.Msg {
		...
		if liveSlug != "" {
			...
			body, _ := json.Marshal(map[string]any{
				...
				"surface": map[string]any{
					...
					"mode":          group.Type,
					"bot_token_env": "HIVEX_TELEGRAM_BOT_TOKEN",
				},
			})
			...
			mutateErr := company.UpdateManifest(func(manifest *company.Manifest) error {
				...
				manifest.Channels = append(manifest.Channels, company.ChannelSpec{
					...
					Surface: &company.ChannelSurfaceSpec{
						...
						BotTokenEnv: "HIVEX_TELEGRAM_BOT_TOKEN",
					},
				})
				return nil
			})
		...
		if group.ChatID != 0 {
			_ = team.SendTelegramMessage(token, group.ChatID,
				"Connected to hivebot. Messages here will be visible to the team.")
		}

		// Clear broker state so next restart picks up the manifest with surfaces
		_ = os.Remove(filepath.Join(config.RuntimeHomeDir(), ".hivex", "team", "broker-state.json"))
