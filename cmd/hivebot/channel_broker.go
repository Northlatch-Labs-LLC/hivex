...
func currentBrokerAuthToken() string {
	if token := strings.TrimSpace(os.Getenv("HIVEX_BROKER_TOKEN")); token != "" {
		return token
	}
...
func applyTeamSetup() tea.Cmd {
	return func() tea.Msg {
		notice := "Setup saved."
		cfg, _ := config.Load()
		if current := strings.TrimSpace(os.Getenv("HIVEX_HEADLESS_PROVIDER")); current != "" {
			return channelInitDoneMsg{notice: notice + " Setup saved. Restart hivebot to reload the " + current + " office runtime with the new configuration."}
		}
		if config.ResolveLLMProvider("") == "codex" || strings.TrimSpace(cfg.LLMProvider) == "codex" {
			return channelInitDoneMsg{notice: notice + " Codex was saved as the LLM provider. Restart hivebot to launch the headless Codex office runtime."}
		}
		if config.ResolveLLMProvider("") == "opencode" || strings.TrimSpace(cfg.LLMProvider) == "opencode" {
			return channelInitDoneMsg{notice: notice + " Opencode was saved as the LLM provider. Restart hivebot to launch the headless Opencode office runtime."}
		}
		l, err := team.NewLauncher("")
		if err != nil {
...
func applyProviderSelection(providerName string) tea.Cmd {
	return func() tea.Msg {
		...
		if current := strings.TrimSpace(os.Getenv("HIVEX_HEADLESS_PROVIDER")); current != "" {
			return channelInitDoneMsg{notice: "Provider switched to " + providerName + ". Restart hivebot to reload the team runtime with the new configuration."}
		}
		if providerName == "codex" {
			...
			return channelInitDoneMsg{notice: "Provider switched to codex. Claude teammate panes were stopped. Restart hivebot to launch the headless Codex office runtime."}
		}
		if providerName == "opencode" {
			...
			return channelInitDoneMsg{notice: "Provider switched to opencode. Claude teammate panes were stopped. Restart hivebot to launch the headless Opencode office runtime."}
		}
