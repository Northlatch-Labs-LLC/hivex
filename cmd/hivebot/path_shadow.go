...
// detectPathShadows returns other hivebot executables on pathEnv that would
...
// boot-noise bug (#942).
//
// When selfExec is NOT on pathEnv at all (e.g. running ./hivex from a build
...
// install layout, where the PATH entry symlinks at `hivex.js` (a launcher) but
// the native binary is `hivebot` in the same node_modules/hivex/bin dir. They
...
func detectPathShadows(selfExec, pathEnv string) []string {
	...
	for _, dir := range filepath.SplitList(pathEnv) {
		...
		// not a shadow. Covers the npm layout (hivebot + hivex.js in the same
	...
	// shell `hivebot` call elsewhere is a known different binary — not a surprise
...
// warnPathShadow writes a one-time warning to w when other hivebot executables
...
func warnPathShadow(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "hivex: warning: other hivebot binaries are on PATH and may shadow this one:")
	for _, s := range shadows {
		_, _ = fmt.Fprintf(w, "  %s\n", s)
	}
	_, _ = fmt.Fprintf(w, "  running: %s\n", self)
	_, _ = fmt.Fprintln(w, "  If `which hivebot` picks a different path, upgrades to this binary will have no effect until the other copy is removed.")
}
