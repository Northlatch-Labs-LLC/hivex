...
// `hivebot workspace restore <trash-id>` — bring a shredded workspace back.
//
// The trash ID is the directory name under `~/.hivex-spaces/.backups/` —
// `<name>-<unix-timestamp>`. Users get this from `hivebot workspace list --trash`.
// The orchestrator reconstructs a fresh hivexHome from the categorized backup
...
func runWorkspaceRestore(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot workspace restore — bring a trashed workspace back")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot workspace restore <trash-id>")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Find trash IDs with `hivebot workspace list --trash`. Restore allocates a fresh")
	...
	_, _ = fmt.Fprintf(os.Stdout, "Switch CLI default with: hivebot workspace switch %s\n", ws.Name)
}
