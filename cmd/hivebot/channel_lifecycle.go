...
// killTeamSession kills the entire hivex-team tmux session and all bot processes.
func killTeamSession() {
	...
	_ = exec.CommandContext(ctx, "tmux", "-L", "hivebot", "kill-session", "-t", "hivex-team").Run()
...
func runChannelView(threadsCollapsed bool, initialApp channelui.OfficeApp, skipSplash bool) {
	...
	if os.Getenv("HIVEX_SKIP_ONBOARDING") == "" {
	...
	if !skipSplash && os.Getenv("HIVEX_NO_SPLASH") == "" {
...
func reportChannelCrash(details string) {
	...
	fmt.Fprintln(os.Stderr, "hivebot channel crashed.")
	fmt.Fprintln(os.Stderr, "Log:", channelui.ChannelCrashLogPath())
	fmt.Fprintln(os.Stderr, "")
	fmt.Fprintln(os.Stderr, "The rest of the team is still running.")
	if strings.TrimSpace(os.Getenv("HIVEX_HEADLESS_PROVIDER")) != "" {
		fmt.Fprintln(os.Stderr, "Restart hivebot when ready to reconnect to the headless office runtime.")
	} else {
		fmt.Fprintln(os.Stderr, "Use `tmux -L hivebot attach -t hivex-team` to inspect panes,")
		fmt.Fprintln(os.Stderr, "then restart hivebot when ready.")
	}
	select {}
}
