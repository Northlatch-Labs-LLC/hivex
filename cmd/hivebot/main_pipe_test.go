...
// regression vector for the Windows-launch bug — hivebot was exiting silently
...
// regress the `echo "/foo" | hivebot` use case.
...
// A single line without a trailing newline (common for one-shot `hivebot
...
// `printf "\n" | hivebot` falls into the dispatch path with an empty command,
