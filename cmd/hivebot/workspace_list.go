...
// `hivebot workspace list` — table or JSON view of registered workspaces.
//
...
func runWorkspaceList(args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "hivebot workspace list — show workspaces")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Usage:")
		fmt.Fprintln(os.Stderr, "  hivebot workspace list              Human-readable table")
		fmt.Fprintln(os.Stderr, "  hivebot workspace list --json       Stable JSON for scripts")
		fmt.Fprintln(os.Stderr, "  hivebot workspace list --trash      Trashed workspaces with restore IDs")
	}
...
func renderWorkspaceTable(w io.Writer, workspaces []Workspace) error {
	if len(workspaces) == 0 {
		_, err := fmt.Fprintln(w, "No workspaces yet. Run `hivebot workspace create <name>` to start one.")
		return err
	}
	...
	if _, err := fmt.Fprintln(w, "* = active CLI workspace. Switch with `hivebot workspace switch <name>`."); err != nil {
		return err
	}
	return nil
}
...
func renderTrashTable(w io.Writer, trash []TrashEntry) error {
	...
	if _, err := fmt.Fprintln(w, "Restore with `hivebot workspace restore <trash-id>` (allocates a fresh port pair)."); err != nil {
		return err
	}
	return nil
}
