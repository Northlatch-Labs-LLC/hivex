...
// Tests for `hivebot workspace ...`. Coverage target per Lane D charter: 90%
...
func TestRenderList_TrashTable_WithEntries(t *testing.T) {
	...
	if !strings.Contains(out, "Restore with `hivebot workspace restore") {
...
func TestShredConfirm_MainWarningEmitted(t *testing.T) {
	...
	if !strings.Contains(out.String(), "migrated ~/.hivex/") {
...
// (mirrors how the user actually invokes `hivebot workspace list`). Catches a
...
func TestDoctor_DetailLineRendered(t *testing.T) {
	fake := &fakeWorkspaceOrchestrator{}
	report := DoctorReport{Issues: []DoctorIssue{
		{Kind: DoctorIssueOrphanTree, Subject: "foo", Detail: "tree at ~/.hivex-spaces/foo without registry entry", FixID: "x"},
	}}
	var out bytes.Buffer
	if err := runDoctorIssueLoop(context.Background(), fake, report, strings.NewReader(""), &out, doctorModeDryRun); err != nil {
		t.Fatalf("err: %v", err)
	}
	if !strings.Contains(out.String(), "tree at ~/.hivex-spaces/foo") {
