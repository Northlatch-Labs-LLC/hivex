...
func TestSplashRendersTitleInTitlePhase(t *testing.T) {
	...
	// The title phase shows the hivebot brand somewhere.
	if !strings.Contains(strings.ToUpper(got), "hivebot") {
		t.Logf("title view did not include 'hivebot' literal — may use ASCII art only: %q", got)
	}
}
