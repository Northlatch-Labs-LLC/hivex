...
// legacy "hive" automation sender slug. Used to populate the insights side
...
// messages: "you" (the human), the legacy "hive" automation slug, kind=="automation"
...
func CountUniqueBots(messages []BrokerMessage) int {
	...
	for _, m := range messages {
		if IsHumanSender(m.From) || m.From == "hive" || m.Kind == "automation" {
			continue
		}
