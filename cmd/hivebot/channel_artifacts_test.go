...
func TestBuildArtifactLinesShowsTaskLogsWorkflowRunsAndApprovals(t *testing.T) {
	...
	m := newChannelModel(false)
	m.tasks = []channelui.Task{{ID: "task-1", Title: "Ship launch notes", WorktreePath: "/tmp/hivex-task-1"}}
	...
	if !strings.Contains(plain, "Resume in /tmp/hivex-task-1") {
...
func writeTaskLog(t *testing.T, home, taskID, record string) {
	t.Helper()
	path := filepath.Join(home, ".hivex", "office", "tasks", taskID)
...
func writeWorkflowRun(t *testing.T, home, provider, key, record string) {
	t.Helper()
	path := filepath.Join(home, ".hivex", "workflows", provider)
	if err := os.MkdirAll(path, 0o755); err != nil {
