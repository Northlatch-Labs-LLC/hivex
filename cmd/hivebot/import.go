...
// hivebot broker state types (output format, mirrors internal/team unexported structs)
...
func runImport(args []string) {
	...
	fs.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "Import external orchestrator state into hivebot. No export step required.\n\n")
	...
	fmt.Printf("Imported %d bots, %d tasks from %s. Run hivebot to launch.\n", botCount, taskCount, source)
}
...
func importFromLegacyDB(portOverride int) (importedBrokerState, int, int, error) {
	...
	// Only reached by `hivebot import --from legacy` against a user's local
...
func importFromJSONFile(path string) (importedBrokerState, int, int, error) {
	...
	state, botCount, taskCount := convertToHivex(pc)
...
// convertToHivex transforms external orchestrator JSON state into hivebot broker state.
func convertToHivex(pc legacyState) (importedBrokerState, int, int) {
	now := time.Now().UTC().Format(time.RFC3339)
...
// hivexBrokerStatePath mirrors defaultBrokerStatePath in internal/team/broker.go.
func hivexBrokerStatePath() string {
	if home := config.RuntimeHomeDir(); home != "" {
		return filepath.Join(home, ".hivex", "team", "broker-state.json")
	}
	return filepath.Join(".hivex", "team", "broker-state.json")
}
