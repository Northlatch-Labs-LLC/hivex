...
hivebot bridge (`src/bridgeDataProvider.ts`), so every read/write flows through
...
3. Edit `src/App.tsx` (and add components) to implement the requested tool. Read
   ...
   by `bridgeDataProvider`, or the `src/hivex-bridge.ts` helpers — never `fetch`.
...
If this scaffold is not present (e.g. an `npx hivex` install with no repo
...
+ `@mantine/core` + `@refinedev/react-table`, and copy `src/hivex-bridge.ts` +
