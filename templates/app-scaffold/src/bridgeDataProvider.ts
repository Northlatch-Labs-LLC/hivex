/**
 * bridgeDataProvider — refine's DataProvider implemented over hivex-bridge.ts.
 *
 * This is what lets every refine data hook (`useList`, `useTable`, `useOne`,
 * `useShow`, `useForm`, …) read and write hivebot workspace data WITHOUT any HTTP.
 ...
 *
 * PROTECTED FILE — like hivex-bridge.ts, import and use this; do not reimplement
    ...
    if (params.resource !== "tasks") {
      throw new Error(
        `Apps can only create tasks (got create '${params.resource}'). ` +
          `All other writes are unsupported in a hivebot App.`,
      );
    }
