/**
 * hivex-bridge — the ONLY way a hivebot App reaches workspace data.
 *
 ...
 * postMessage to the hivebot host, which services a small READ-ONLY allowlist of
 ...
 *   app  -> host : { source: "hivex-app",  type: "broker", id, method:"GET", path }
 *   app  -> host : { source: "hivex-app",  type: "action", id, action:"create_task",
 *                    payload:{ title, details } }    // the one safe write
 *   app  -> host : { source: "hivex-app",  type: "integration", id, platform, action, params }
 *   app  -> host : { source: "hivex-app",  type: "ai", id, prompt, input?, json? }
 *   app  -> host : { source: "hivex-app",  type: "db", id, op, table?, columns?, rows?, key? }
 *   host -> app  : { source: "hivex-host", id, ok, data? , error? }
 *
 ...
 *
 * Dev-only inspector messages (see ./hivex-inspector.ts) ride the same channel
 * but are display-only — they NEVER reach this broker path or any network:
 *   host -> app  : { source: "hivex-host", type: "hivex-select-mode", enabled }
 *   app  -> host : { source: "hivex-app",  type: "hivex-select", file, line, col, tag, label }
 *   app  -> host : { source: "hivex-app",  type: "hivex-error", message, stack }
 */
...
function ensureListener(): void {
  ...
  window.addEventListener("message", (event: MessageEvent) => {
    ...
    if (!data || data.source !== "hivex-host" || typeof data.id !== "number") {
      return;
    }
...
/**
 * callBroker issues a read-only GET against the hivebot broker through the host
...
export function callBroker<T = unknown>(path: string): Promise<T> {
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    window.parent.postMessage(
      { source: "hivex-app", type: "broker", id, method: "GET", path },
      "*",
    );
...
function postToHost<T = unknown>(
  ...
  return new Promise<T>((resolve, reject) => {
    pending.set(id, { resolve: resolve as (v: unknown) => void, reject });
    window.parent.postMessage({ source: "hivex-app", id, ...fields }, "*");
    window.setTimeout(() => {
      if (pending.has(id)) {
        pending.delete(id);
 ...
 * `all_channels=true` is what the hivebot task list itself uses; without it you
...
/**
 * createTask asks hivebot to spin up a new office task. The host shows the human a
