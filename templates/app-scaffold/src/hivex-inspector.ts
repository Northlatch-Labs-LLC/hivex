...
/**
 * hivex-inspector — DEV-ONLY in-app runtime that powers the host's
 ...
 *   app  -> host : { source: "hivex-app", type: "hivex-select",
...
const HOST_SOURCE = "hivex-host";
const APP_SOURCE = "hivex-app";
...
// vite.config.ts stamps on every host JSX element: data-hivex-source="rel:line:col".
...
const SOURCE_ATTR = "data-hivex-source";
 ...
 * ancestor's stamped `data-hivex-source="rel:line:col"`. `closest` covers the
...
export function installInspector(): void {
  ...
  const flag = "__hivexInspectorInstalled";
    ...
    try {
      ...
      if (payload) {
        window.parent.postMessage(
          { source: APP_SOURCE, type: "hivex-select", ...payload },
