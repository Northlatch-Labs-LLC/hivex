...
// During a hivebot live preview the broker runs `bun run dev` and fronts it with a
// reverse proxy on a distinct 127.0.0.1 port (it injects the security CSP — do
// not rely on serving headers here). HIVEX_HMR_CLIENT_PORT is that proxy port:
...
const hmrClientPort = Number(process.env.HIVEX_HMR_CLIENT_PORT) || undefined;

// Dev-only Babel plugin: stamp every host (lowercase) JSX element with its
// source location as `data-hivex-source="relPath:line:col"`, so the live
...
function hivexSourceStamp({ types: t }: any) {
  const ATTR = "data-hivex-source";
  return {
    name: "hivex-source-stamp",
    visitor: {
...
function hivexInspectorInject() {
  return {
    name: "hivex-inspector-inject",
    ...
    transformIndexHtml() {
      ...
      return [
        {
          tag: "script",
          attrs: { type: "module", src: "/src/hivex-inspector.ts" },
...
// requests. That one file is what register_app publishes; hivebot renders it in a
...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    react(
      command === "serve" ? { babel: { plugins: [hivexSourceStamp] } } : {},
    ),
    hivexInspectorInject(),
    viteSingleFile(),
  ],
  server: {
    host: "127.0.0.1",
