# DESIGN — making a hivebot App look DESIGNED, not generated
...
- **Cards are the lazy answer.** A pile of identical `<Card>`s is the #1 AI tell.
  ...
  `// hivex-allow: card-pile — <why>` on the `.map` line.
...
appear and why and ends by naming the next action ("Connect Gmail from the hivebot
...
and warm, never jokey — the app belongs to the operator's company, not to hivebot,
