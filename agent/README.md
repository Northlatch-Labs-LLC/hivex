...
src/store.ts       file-backed per-agent store: tools + artifacts
                   (HIVEX_AGENT_DATA_DIR, default .hivex-agent-data/; atomic-ish writes)
...
cron via `internal/calendar/cron.go`; `HIVEX_AGENT_URL` points it at this
