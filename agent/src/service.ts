...
export interface ServerOptions {
	port?: number;
	// Interface to bind. Defaults to loopback (HIVEX_AGENT_HOST) — this service
	...
	// except /health. Defaults to HIVEX_AGENT_TOKEN. When empty the service is
...
export function createServer(opts: ServerOptions = {}) {
	...
	const hostname = (opts.hostname ?? process.env.HIVEX_AGENT_HOST ?? "127.0.0.1").trim();
	const authToken = (opts.authToken ?? process.env.HIVEX_AGENT_TOKEN ?? "").trim();
	...
	if (!isLoopbackHost(hostname) && authToken === "") {
		throw new Error("refusing to bind non-loopback host without HIVEX_AGENT_TOKEN (would be unauthenticated RCE)");
	}
	if (authToken === "") {
		console.warn("[hivex-agent] HIVEX_AGENT_TOKEN is unset — service is UNAUTHENTICATED and bound to loopback only.");
	}
	...
	return Bun.serve({
		...
		async fetch(req) {
			...
			if (req.method === "POST" && pathname === "/tools/call") {
				...
				// configured (HIVEX_BROKER_URL/TOKEN, TOOL_RUNTIME_MODEL=1), simulated
				// otherwise. TOOL_CALL_TIMEOUT_MS raises the hard-kill deadline for
				// hosts running slow capabilities (e.g. hive.browser drives real Chrome).
			...
			if (sessionMsg) {
				...
				if ("error" in parsed) return parsed.error;
				const { body, agent } = parsed;
				if ((body.from !== "you" && body.from !== "hive") || typeof body.body !== "string") {
					return json({ error: 'invalid message: from ("you"|"hive") and body (string) required' }, 400);
				}
...
if (import.meta.main) {
	...
	console.log(`hivex-agent service on ${server.url}`);
}
