...
beforeAll(() => {
	...
	tmp = mkdtempSync(join(tmpdir(), "hivex-agent-svc-"));
...
test("POST /routines/run (a broker fire) lands transcript + artifact and reports back", async () => {
	...
	expect(session.messages.map((m) => m.from)).toEqual(["you", "hive"]);
...
test("manual sessions: default Chat <n> titles, transcript append, 404s", async () => {
	...
	await post(`/sessions/${s1.session.id}/message`, { schema_version: 1, agent: "chatter", from: "hive", body: "hello" });
	const got = (await (await fetch(`${base}/sessions/${s1.session.id}?agent=chatter`)).json()) as { messages: SessionMessage[] };
	expect(got.messages.map((m) => `${m.from}:${m.body}`)).toEqual(["you:hi", "hive:hello"]);
