...
const READ_TOOL = makeTool(
	[
		...
		"  return hive.ai.summarize(records, { style: 'concise recap' });",
		"}",
	].join("\n"),
	[],
	"read_tool",
...
const GATED_TOOL = makeTool(
	[
		...
		"  const result = await hive.send(record, 'heads up');",
...
test("a read tool (data.list + summarize) runs ok with actions recorded", async () => {
	...
	expect(r.actions.some((a) => a.startsWith("hive.ai.summarize("))).toBe(true);
});

test("a gated write (hive.send) halts needs_approval by default (default deny)", async () => {
	...
	expect(r.gate.capability).toBe("hive.send");
...
test("the same gated call with approved: true completes", async () => {
	...
	expect(r.status).toBe("ok");
	if (r.status !== "ok") throw new Error("unreachable");
	expect(r.actions.some((a) => a.startsWith("hive.send("))).toBe(true);
});

test("hive.send is gated too", async () => {
	const t = makeTool('async function ping() { return hive.send("#sales", "hi"); }');
	...
	expect(denied.gate.capability).toBe("hive.send");
...
test("timeout: a never-resolving capability -> error", async () => {
	const capabilities: CapabilityTree = {
		hive: { run: () => new Promise(() => {}) },
		crm: {},
	};
	const t = makeTool("async function t(input) { return hive.run(input); }", [{ name: "input", type: "string" }]);
...
test("a settled run ABORTS in-flight capability calls (deferred finding [15])", async () => {
	...
	const t = makeTool("async function t() { return hive.run('x'); }");
...
test("hive.browser is gated: default deny with the browser-control detail", async () => {
	const t = makeTool('async function t() { return hive.browser("post the digest to the vendor portal"); }');
	const r = await runTool(t, {});
	expect(r.status).toBe("needs_approval");
	if (r.status !== "needs_approval") throw new Error("unreachable");
	expect(r.gate.capability).toBe("hive.browser");
...
test("injected capabilities are used AND recorded (and stay gated)", async () => {
	...
	const read = makeTool("async function t() { const d = await data.list('records', { since: '7d' }); return hive.ai.summarize(d); }");
	...
	// hive.send is still default-deny.
	const gated = makeTool('async function t() { return hive.send("Acme", "hi"); }');
...
beforeAll(() => {
	...
	// would otherwise make hive.ai.* fire a real, slow network call and this test
	...
	const store = new AgentStore(mkdtempSync(join(tmpdir(), "hivex-toolcall-")));
...
test("POST /tools/call halts needs_approval, then completes with approved: true", async () => {
	...
	expect(r1.gate?.capability).toBe("hive.send");
	const r2 = (await (
