...
//
// SEND-GATE (CQ1, default deny): gated capabilities (`hive.send`,
// `hive.browser`) halt the run with status="needs_approval" unless the run carries
...
/** A nested tree of capabilities: { hive: { ai: { score } }, crm: { deals } }. */
...
// AWAITING its capabilities, not just its CPU work: a tool that calls hive.ai.*
...
// default silently killed every AI-using tool at 5s the moment runtime hive.ai.*
// became real (2026-08-18) — it was only ever survivable while hive.ai.* was the
...
function gateDetail(path: string, args: unknown[]): string {
	if (path === "hive.send") return `send ${labelOf(args[1])} to ${labelOf(args[0])}`;
	if (path === "hive.browser") return `control your browser to ${labelOf(args[0])}`;
	return `run ${path}`;
}
...
export async function runTool(tool: Tool, args: Record<string, string> = {}, opts: ToolRunOptions = {}): Promise<ToolRunResult> {
	...
	if (!fnName) {
		return { status: "error", detail: "this tool has a problem in how it was built — ask Hive to rebuild it", actions: ctx.actions };
	}
