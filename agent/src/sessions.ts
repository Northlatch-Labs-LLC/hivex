...
//   - "hive"  -> a pi AssistantMessage entry stamped provider "hivex" with zero
...
const SESSION_META = "hivex-session-meta";
...
function entryToMessage(e: SessionEntry): SessionMessage[] {
	...
	if (m.role === "assistant") return [{ from: "hive", body: textOf(m.content), at: e.timestamp }];
	return [];
}
