...
function tmpSessions(): { sessions: PiSessions; dataDir: string } {
	const dir = mkdtempSync(join(tmpdir(), "hivex-agent-pisess-"));
	dirs.push(dir);
...
test("create -> list -> get round-trips title, kind, and transcript", async () => {
	...
	await sessions.append("a1", created.id, { from: "hive", body: "the recap", at: new Date().toISOString() });
...
test("ensureRoutineSession is stable per slug (post-flush) and re-labels on rename", async () => {
	...
	await sessions.append("a1", first.id, { from: "hive", body: "done", at: new Date().toISOString() });
