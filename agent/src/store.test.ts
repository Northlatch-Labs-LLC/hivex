...
function tmpStore(): { store: AgentStore; dir: string } {
	const dir = join(mkdtempSync(join(tmpdir(), "hivex-agent-store-")), "data");
	dirs.push(dir);
...
test("HIVEX_AGENT_DATA_DIR drives the default data dir", () => {
	expect(defaultDataDir({ HIVEX_AGENT_DATA_DIR: "/tmp/somewhere" })).toBe("/tmp/somewhere");
	// Unset -> the package-relative default.
	expect(defaultDataDir({})).toContain(".hivex-agent-data");
});
