...
// summarize, draft) and built on data.* + hive.ai.* so a taught workflow in ANY
...
const SHAPES: readonly Shape[] = [
	{
		...
		code: [
			...
			"    const score = await hive.ai.score(r, { rubric: rubric || 'priority' });",
	...
	},
	{
		...
		code: [
			...
			"  const summary = await hive.ai.summarize(records, { style: 'concise recap' });",
	...
	},
	{
		...
		code: [
			...
			"  const draft = await hive.ai.write('message', { context: record, tone: 'warm, brief' });",
...
export function authorTool(description: string): Tool {
	...
	return {
		...
		code: `async function ${name}(input) {\n  // Hive scripted this from: "${commentDesc}"\n  return hive.run(input);\n}`,
	};
}
...
- hive.now() -> the current time as an ISO string (the ONE reliable clock — use this for "now", SLA windows, "hours since", never Date.now())
- hive.ai.score(subject, { rubric }) -> number 0-100
- hive.ai.summarize(items, { style }) -> string
- hive.ai.write(kind, { context, tone }) -> string
...
- hive.browser(goal) -> drive the operator's browser to accomplish a goal when no integration exists (needs the operator's approval)
- hive.send(target, content) -> external send (needs the operator's approval)
- hive.run(input) -> opaque fallback for ONE genuinely un-decomposable step inside a larger flow
...
- Date.now() inside the tool body is unreliable — get the current time from await hive.now() (an ISO string) and compute "hours/days since" from the record's own timestamp fields against that.
- hive.ai.* return plain strings/numbers; do not JSON.parse them.
...
- NEVER write a tool whose body is just hive.run(input) — that is not a tool, it is a shrug. Decompose the workflow into real steps with the specific capabilities above; if the workflow genuinely cannot be decomposed, still express the parts you can (validation, shaping, summary) around the one opaque step.
- Sends are drafts first: build the content, return it in the result, and call hive.send only when the described workflow explicitly says to send. Subject lines are part of the send target/metadata, not pasted into the body.
 ...
 * ("hive.ai.score", "data.list", ...). A capability the catalog does not expose
...
const CATALOG_ROOTS = new Set(["hive", "data", "integrations"]);
...
function unknownCapabilityRef(code: string, valid: Set<string>): string {
	...
	const re = /\b(hive|data|integrations)((?:\.[a-zA-Z_$][\w$]*)+)\s*\(/g;
