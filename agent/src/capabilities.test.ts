...
test("unconfigured host: hive.browser degrades to a simulated marker", async () => {
	const tree = buildCapabilities({});
	const out = await cap(tree, "hive.browser")("open the vendor portal");
...
test("capabilityConfigFromEnv reads the broker seam + model gate", () => {
	const cfg = capabilityConfigFromEnv({
		HIVEX_BROKER_URL: "http://127.0.0.1:7893",
		HIVEX_BROKER_TOKEN: "tok",
	});
...
test("GATED_CAPABILITIES lists every mutating capability the send-gate must hold", () => {
	...
	expect(GATED_CAPABILITIES.has("hive.send")).toBe(true);
	expect(GATED_CAPABILITIES.has("hive.browser")).toBe(true);
...
test("simulated hive.run with a blank input is honest, not 'Ran on  (simulated).'", async () => {
	// QA HIGH-1: a routine ran a generic tool with no bound input, so hive.run("")
	...
	const tree = buildCapabilities({});
	const out = String(await cap(tree, "hive.run")(""));
...
test("simulated hive.run names the input it would have acted on", async () => {
	const tree = buildCapabilities({});
	const out = String(await cap(tree, "hive.run")("Summarize the open office tasks"));
...
test("labelOf-backed sims never leave a blank hole for an empty argument", async () => {
	const tree = buildCapabilities({});
	// hive.send / hive.ai.write route their arg through labelOf; a blank must render
	// the neutral marker, not a double space.
	expect(String(await cap(tree, "hive.send")(""))).not.toMatch(/\s{2,}/);
	expect(String(await cap(tree, "hive.ai.write")(""))).not.toMatch(/Drafted\s{2,}/);
});

// --- real hive.ai (stubbed complete) ---------------------------------------------

test("real hive.ai.score parses the model's integer and clamps it", async () => {
	const tree = buildCapabilities({ aiModel: MODEL, complete: fakeComplete("Score: 87") });
	expect(await cap(tree, "hive.ai.score")("Acme", { rubric: "ICP fit" })).toBe(87);
});

test("real hive.ai falls back to the simulation when the model fails", async () => {
	const throwing = (async () => {
		throw new Error("provider down");
	}) as unknown as CompleteFn;
	const tree = buildCapabilities({ aiModel: MODEL, complete: throwing });
	const score = await cap(tree, "hive.ai.score")("Acme");
	expect(typeof score).toBe("number"); // deterministic hash fallback
	const recap = await cap(tree, "hive.ai.summarize")([{ name: "Globex" }]);
	expect(String(recap)).toContain("simulated recap");
});

test("real hive.ai.summarize returns the model's text", async () => {
	const tree = buildCapabilities({ aiModel: MODEL, complete: fakeComplete("6 deals moved; Globex leads.") });
	expect(await cap(tree, "hive.ai.summarize")([1, 2, 3])).toBe("6 deals moved; Globex leads.");
});

test("hive.ai.summarize feeds the FULL input to the model, not a 57-char preview", async () => {
	let captured = "";
	...
	await cap(tree, "hive.ai.summarize")(items);
...
// --- real hive.browser (stubbed SSE) ---------------------------------------------

test("hive.browser streams the run and returns the outcome + action trace", async () => {
	const sse = [
	...
	const out = String(await cap(tree, "hive.browser")("post the digest"));
	expect(out).toContain("Posted the digest.");
	expect(out).toContain("2 browser actions");
	expect(out).toContain("Click New message");
});

test("hive.browser fails loud when the run errors", async () => {
	const sse = ['data: {"type":"error","message":"No window for Chrome"}', ""].join("\n");
	const tree = buildCapabilities({
		...BROKER,
		fetch: (async () => new Response(sse, { status: 200 })) as unknown as FetchFn,
	});
	await expect(Promise.resolve(cap(tree, "hive.browser")("x"))).rejects.toThrow("No window for Chrome");
});
