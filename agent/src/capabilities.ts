...
//
//   hive.ai.score/summarize/write  REAL via one pi-ai `complete` each when a model
...
//                                 HIVEX_BROKER_URL + HIVEX_BROKER_TOKEN are set.
...
//   hive.browser                   REAL via the broker's cua engine
...
//   hive.send                      Still simulated, and stays gated so the approval
...
export const GATED_CAPABILITIES: ReadonlySet<string> = new Set(["hive.send", "hive.browser"]);
...
export interface CapabilityConfig {
	...
	appId?: string;
	/** Model for real hive.ai.* calls; unset -> simulated. */
 ...
 * it to hive.ai.* was the reason summaries read "your data got cut off" — the
...
/** The generic fallback capability (hive.run) has no domain shape to imitate, so a
...
export function simulatedCapabilities(): CapabilityTree {
	return {
		hive: {
			...
			// time (SLA windows, "days since") calls `await hive.now()` instead of
...
// ---------------------------------------------------------------------------
// Real hive.ai.* — one structured pi-ai call each, simulated fallback on failure.
// ---------------------------------------------------------------------------
...
function realIntegrations(cfg: CapabilityConfig): CapabilityTree {
	const call: CapabilityFn = async (platform: unknown, action: unknown, params?: unknown) => {
		...
		if (body.status === "needs_approval") {
			...
			return `Held for your approval in hivebot (request ${body.request_id ?? "pending"}) — nothing was sent yet.`;
		}
...
// ---------------------------------------------------------------------------
// Real hive.browser — the broker's cua engine, SSE. Gated at the agent layer.
// ---------------------------------------------------------------------------
...
export function buildCapabilities(cfg: CapabilityConfig = {}): CapabilityTree {
	...
	const hive = tree.hive as CapabilityTree;
	if (cfg.aiModel) hive.ai = realAI(cfg);
	if (cfg.brokerUrl && cfg.brokerToken) {
		hive.browser = realBrowser(cfg);
...
export function capabilityConfigFromEnv(env: Record<string, string | undefined> = process.env): CapabilityConfig {
	const cfg: CapabilityConfig = {
		brokerUrl: env.HIVEX_BROKER_URL?.trim() || undefined,
		brokerToken: env.HIVEX_BROKER_TOKEN?.trim() || undefined,
	};
