...
//
//   - Data dir: HIVEX_AGENT_DATA_DIR, default agent/.hivex-agent-data/ relative
...
//     two service instances sharing HIVEX_AGENT_DATA_DIR) never scribble over one
...
const PACKAGE_DEFAULT_DIR = join(dirname(fileURLToPath(import.meta.url)), "..", ".hivex-agent-data");

export function defaultDataDir(env: Record<string, string | undefined> = process.env): string {
	return env.HIVEX_AGENT_DATA_DIR?.trim() || PACKAGE_DEFAULT_DIR;
}
