...
Everything runs locally on dev ports to avoid colliding with a prod `npx hivex` if you have one running:

| | Binary | Broker port | Web UI port | Isolated HOME |
|---|---|---|---|---|
| Prod | `npx hivex` | 7890 | 7891 | `~/.hivex` |
| Dev | `./hivex-dev` (this worktree) | 7899 | 7900 | any path you set via `HOME=` |
...
go build -o hivex-dev ./cmd/hivex
```

### 2. Fresh dev home

```bash
DEMO_HOME="/tmp/hivex-demo"
rm -rf "$DEMO_HOME"
mkdir -p "$DEMO_HOME"
```
...
find "$DEMO_HOME/.hivex/wiki/team" -name "*.md" | sort

# Git repo should be initialized with at least one commit
git -C "$DEMO_HOME/.hivex/wiki" log --oneline

# Index should auto-regenerate
cat "$DEMO_HOME/.hivex/wiki/index/all.md"
```
...
- git log shows `hivex: init wiki` commit plus a skeleton-materialization commit
...
**Terminal 1 — keep hivebot running** (same as the smoke test above).
...
- `git -C ~/.hivex/wiki log --oneline` after the demo shows all 9 commits with per-bot authorship
...
- [ ] Launch hivex, write one article, `kill -9` the process mid-session
- [ ] Relaunch. Check `git log` — if there was uncommitted work, a `hivex-recovery` author commit should appear
...
1. Launch hivebot + complete onboarding as per smoke test
...
6. Crop to the wiki area (exclude the rest of the hivebot chrome if it's too busy)
7. Convert to GIF:
...
6. Drop to a terminal, run `cat ~/.hivex/wiki/team/customers/customer-x.md` — real markdown on disk
7. Run `git -C ~/.hivex/wiki log --oneline` — show per-bot authorship
8. Stop recording
...
**Problem:** `./hivex-dev: command not found`
**Fix:** You haven't built yet. Run `go build -o hivex-dev ./cmd/hivex`.
...
**Fix:** Stale web bundle. Rebuild: `( cd web && npm run build ) && go build -o hivex-dev ./cmd/hivex`, then relaunch.

**Problem:** Demo script prints `broker not reachable at http://127.0.0.1:7899`
**Fix:** hivex-dev isn't running, or it bound to different ports. Check with `lsof -i :7899`. If empty, relaunch. If your broker is on a different port, set `BROKER=http://127.0.0.1:<port>`.

**Problem:** Catalog shows fake names (Sarah Chen / David Kim / Nova) I never wrote
...
**Fix:** Your dev HOME points into a read-only path or inside a git worktree with conflicting untracked files. Use a fresh `/tmp/hivex-demo` as the HOME.
